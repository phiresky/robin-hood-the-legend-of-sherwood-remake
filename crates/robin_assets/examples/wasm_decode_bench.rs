...
//!
//! With `--no-default-features`, this example instead exposes only
//! `bench_vq_chunk(alphabet, flattened_dimensions, blob)`, a raw codec benchmark
//! that does not link the simulation. Mission container exports require the
//! default `engine-adapters` feature.

#![cfg(target_arch = "wasm32")]

/// Decode one raw VQ chunk, independently of shipping missions or simulation.
/// Supply the same alphabet and grid dimensions used by encode_grids. Returns
/// JSON elapsed milliseconds, decoded value count, and a deterministic checksum.
#[wasm_bindgen::prelude::wasm_bindgen]
pub fn bench_vq_chunk(
    alphabet: u16,
    dimensions: &[u16],
    blob: &[u8],
) -> Result<String, wasm_bindgen::JsError> {
    if dimensions.len() % 2 != 0 {
        return Err(wasm_bindgen::JsError::new(
            "dimensions must contain column/row pairs",
        ));
    }
    let dims: Vec<_> = dimensions.chunks_exact(2).map(|p| (p[0], p[1])).collect();
    let started = js_sys::Date::now();
    let grids = robin_assets::sprite_codec::decode_grids(alphabet, &dims, None, blob)
        .map_err(|error| wasm_bindgen::JsError::new(&format!("{error:#}")))?;
    let decode_ms = js_sys::Date::now() - started;
    let mut checksum = 0xcbf2_9ce4_8422_2325_u64;
    let mut values = 0_usize;
    for grid in &grids {
        values += grid.len();
        for byte in bytemuck::cast_slice::<u16, u8>(grid) {
    ...
    Ok(serde_json::json!({
        "decode_ms": decode_ms,
        "values": values,
        "grids_fnv": format!("{checksum:016x}"),
    })
    .to_string())
}
...
mod mission {
    ...
    pub fn bench_mission(parts: js_sys::Array) -> Result<String, JsError> {
        ...
        if let Some(bank) = merged.payload.sprite_bank.as_mut() {
            bank.materialize_vq_chunks(&merged.payload.rhs_files)
                .map_err(js_err)?;
        }
