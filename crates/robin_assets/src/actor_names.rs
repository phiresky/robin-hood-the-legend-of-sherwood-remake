...
/// Build actor names with explicit prepared files and optional shipping values.
/// The caller must install the requested shipping mission before calling.
    ...
    let loaded = if let Some(shipping) = shipping.as_ref() {
        shipping.loaded_level(mission_filename).ok_or_else(|| {
            LoadError::Level(format!(
                "shipping mission '{mission_filename}' has no prepared level; install it before building actor names"
            ))
        })?
    } else {
    ...
    if let Some(dd) = shipping.as_ref() {
        ...
        for locale in candidates {
            let path = datadir
                ...
                .join("Text")
                .join("Level.res");
            match files.try_exists(&path.to_string_lossy()) {
                Ok(false) => continue,
                Ok(true) => {}
                Err(error) => {
                    tracing::warn!("inspect {}: error {error}", path.display());
                    continue;
                }
            }
...
fn load_profile_manager(datadir: &Path, files: &SbFileSystem) -> Result<ProfileManager, LoadError> {
    let data_dir = datadir.join("Data");
    let json_path = data_dir.join("Configuration").join("profile.cpf.json");
    if files
        .try_exists(&json_path.to_string_lossy())
