...
/// Night fog color in RGB565 — packed RGB (0, 30, 60).
...
pub struct PackedSprite {
    /// Cached validation is populated on first pixel access, preserving mmap laziness.
    #[serde(skip)]
    validation: std::sync::OnceLock<Result<(), String>>,
    pub width: u16,
    pub height: u16,
...
impl FrameDictionary {
    ...
    pub fn apply_fog_effect(&mut self, level: u16, fog_color: u16) -> bool {
        ...
        apply_fog_effect_viewport(&mut self.values, level, fog_color, self.shadow_color);
...
impl FrameHolder {
    ...
    fn sprite_packed_slice(&self, sprite_index: usize) -> Option<&[u16]> {
        self.try_packed_data(sprite_index as u32)
            .unwrap_or_else(|error| panic!("invalid sprite {sprite_index}: {error:#}"))
    }

    /// Validate each immutable packed stream once, when its mmap pages or
    /// late grid are first consumed. Missing late data is not cached.
    pub fn try_packed_data(&self, sprite_index: u32) -> Result<Option<&[u16]>> {
        let Some(sprite) = self.sprites.get(sprite_index as usize) else {
            return Err(anyhow!("sprite {sprite_index} does not exist"));
        };
        let words = if let Some(owned) = sprite.packed_data.as_deref() {
            owned.as_slice()
        } else if let Some(cell) = sprite.late_grid.as_ref() {
            let Some(grid) = cell.get() else {
                return Ok(None);
            };
            grid.as_slice()
        } else if let Some(span) = sprite.bank_span {
            let bank = self
                .bank
                .as_ref()
                .ok_or_else(|| anyhow!("sprite bank storage missing"))?;
            let range = checked_range(
                span.offset_words as usize,
                span.len_words as usize,
                bank.words().len(),
                "sprite bank span",
            )?;
            &bank.words()[range]
        ...
        let validation = sprite.validation.get_or_init(|| {
            let width = usize::from(sprite.width);
            let height = usize::from(sprite.height);
            let result = if sprite.dictionary_index == UNMAPPED_DICT {
                crate::packed_sprite::validate_rle(words, width, height).map(|_| ())
            } else {
                self.dictionaries
                    .get(usize::from(sprite.dictionary_index))
                    .ok_or_else(|| {
                        anyhow!("sprite dictionary {} is missing", sprite.dictionary_index)
                    })
                    .and_then(|dictionary| {
                        crate::packed_sprite::validate_vq(
                            ...
                            width,
                            height,
                            usize::from(dictionary.num_entries()),
                        )
                    })
            };
            result.map_err(|error| error.to_string())
        });
        validation.as_ref().map_err(|error| anyhow!("{error}"))?;
        Ok(Some(words))
    }
    ...
    pub fn initialize_sprite_bank_with_files(
        &mut self,
        data_dir: &str,
        files: &SbFileSystem,
    ) -> Result<()> {
...
impl FrameHolder {
    ...
    fn load_from_shipping(
        ...
        if bank.sprite_count as usize > SPRITE_COUNT_LIMIT {
            return Err(anyhow!(
                "shipping sprite count {} exceeds {SPRITE_COUNT_LIMIT}",
                bank.sprite_count
            ));
        }
        for (index, sprite) in &bank.sprites {
            ...
            .with_context(|| format!("shipping sprite {index}"))?;
            ...
            if sprite.dictionary_index != UNMAPPED_DICT
                && usize::from(sprite.dictionary_index) >= dictionaries.len()
            {
                return Err(anyhow!(
                    "shipping sprite {index} dictionary {} is missing",
                    sprite.dictionary_index
                ));
            }
        }
        for (index, dictionary) in dictionaries.iter().enumerate() {
            if dictionary.values.len() != usize::from(dictionary.num_entries) * 4 {
                return Err(anyhow!(
                    "shipping dictionary {index} has inconsistent entry count"
                ));
            }
        }
    ...
    pub fn initialize_sprite_bank_with_progress(
        ...
        // Legacy tool compatibility boundary. Production preparation supplies
        // its reader explicitly with the `_and_files` entry point.
        self.initialize_sprite_bank_with_progress_and_files(
            data_dir,
            progress,
            shipping,
            &SbFile::snapshot_legacy_file_system(),
    ...
    pub fn initialize_legacy_sprite_bank_with_progress_and_files(
        ...
        // `.` is the ordinary process-cache spelling for the active datadir.
        // Keep the logical asset name normalized: the ranked verifier's
        // confined resolver deliberately rejects `CurDir` components rather
        // than weakening its path boundary for this loader convenience.
        let (bks_path, dic_path) = if data_dir.is_empty() || data_dir == "." {
            (
                "Data/robinhood.bks".to_owned(),
                "Data/robinhood.dic".to_owned(),
            )
        } else {
            (
                format!("{data_dir}/Data/robinhood.bks"),
                format!("{data_dir}/Data/robinhood.dic"),
            )
        };

        let storage = open_bank_storage(&bks_path, files)?;
    ...
    fn load_sprite_index_bytes(
        ...
        // The original game reads the signature,
    ...
    pub fn from_data_dir(data_dir: &str) -> Result<Self> {
        Self::from_data_dir_with_files(data_dir, &SbFile::snapshot_legacy_file_system())
    }

    /// Explicit-reader counterpart to the legacy convenience constructor.
    pub fn from_data_dir_with_files(data_dir: &str, files: &SbFileSystem) -> Result<Self> {
        let mut holder = Self::new();
        holder
            .initialize_sprite_bank_with_files(data_dir, files)
...
impl robin_content::PixelOpacityLookup for FrameHolder {
    ...
    fn simulation_opacity_sha256(&self, sorted_bank_ids: &[u32]) -> [u8; 32] {
        ...
        for &bank_id in sorted_bank_ids {
            let sprite_index = bank_id as usize;
            let sprite = self.sprites.get(sprite_index).unwrap_or_else(|| {
            ...
            let pixel_count = width
                .checked_mul(height)
                .expect("sprite opacity dimensions overflow");
            let byte_count = pixel_count.div_ceil(8);
            let mut ordinary_opacity = vec![0_u8; byte_count];
            let mut blipped_opacity = vec![0_u8; byte_count];
            let shadow_color = if sprite.dictionary_index == UNMAPPED_DICT {
                SHADOW_KEY
            } else {
                self.dictionaries[sprite.dictionary_index as usize].shadow_color()
            };

            let mut record_pixel = |index: usize, pixel: u16| {
                if pixel == TRANSPARENT_COLOR_16 {
                    return;
                }
                let bit = 1 << (index % 8);
                blipped_opacity[index / 8] |= bit;
                if pixel != shadow_color {
                    ordinary_opacity[index / 8] |= bit;
                }
            };
            if let Some(raster) = sprite.raster.as_ref() {
                for y in 0..height {
                    for x in 0..width {
                        if let Some(pixel) = raster.pixel(x, y) {
                            record_pixel(y * width + x, pixel);
                        }
                    }
                }
            } else if let Some(packed) = self.sprite_packed_slice(sprite_index) {
                let mut pixels = vec![TRANSPARENT_COLOR_16; pixel_count];
                if sprite.dictionary_index == UNMAPPED_DICT {
                    decompress_rle_arno_law(
                        packed,
                        &mut pixels,
                    ...
                    decompress_vector(
                        ...
                        width,
                        height,
                        &self.dictionaries[sprite.dictionary_index as usize],
                    );
                }
                for (index, pixel) in pixels.into_iter().enumerate() {
                    record_pixel(index, pixel);
                }
            }

            hash.update([0]);
            hash.update(&ordinary_opacity);
            hash.update([1]);
            hash.update(&blipped_opacity);
        }
        hash.finalize().into()
    }
}
...
pub use crate::sprite_pixels::{
    apply_fog_effect_viewport, convert_decompressed_to_15bit, convert_decompressed_to_16bit,
    decompress_rle_arno_law, decompress_vector,
};
use crate::sprite_pixels::{dict_pixel_at, rle_pixel_at};
use crc32fast::hash as crc32_hash;
use robin_util::color::rgb565 as pack_rgb8_to_rgb565;
pub use robin_util::color::{pack_rgb565, unpack_rgb565};
...
mod tests {
    ...
    fn checked_stream_matches_raster_and_opacity_with_padding() {
        let packed = vec![1, 2, 0x1234, SHADOW_KEY, 0xFFFF, 0xFFFF, 0, 0, 0x4321];
        let mut holder = FrameHolder::new();
        holder.sprites.push(PackedSprite {
            width: 4,
            height: 3,
            packed_data: Some(Arc::new(packed.clone())),
            ..Default::default()
        });
        let checked = holder.try_packed_data(0).unwrap().unwrap();
        let (canvas, used) = crate::rle_jxl::decode_rle_canvas(4, 3, checked).unwrap();
        assert_eq!(used, packed.len());
        let mut decoded = vec![0x7777; 18];
        decompress_rle_arno_law(
            checked,
            &mut decoded,
            6,
        ...
        for y in 0..3 {
            assert_eq!(&decoded[y * 6..y * 6 + 4], &canvas[y * 4..y * 4 + 4]);
            assert_eq!(&decoded[y * 6 + 4..y * 6 + 6], &[0x7777; 2]);
            for x in 0..4 {
                assert_eq!(
                    holder.is_pixel_opaque(0, x, y as u16, true),
                    canvas[y * 4 + usize::from(x)] != TRANSPARENT_COLOR_16
                );
            }
        }
        assert!(holder.sprites[0].validation.get().unwrap().is_ok());
    }

    #[test]
    fn corrupt_packed_stream_is_a_contextual_error() {
        let mut holder = FrameHolder::new();
        holder.sprites.push(PackedSprite {
            width: 4,
            height: 1,
            packed_data: Some(Arc::new(vec![0, 3, 1])),
            ..Default::default()
        });
        assert!(
            holder
        ...
        assert!(holder.sprites[0].validation.get().unwrap().is_err());
        assert!(holder.try_packed_data(9).is_err());
    }
    ...
    #[test]
    fn independent_readers_keep_bank_and_dictionary_authority_together() {
        fn reader(signature: u32, word: u16) -> SbFileSystem {
            let assets = Arc::new(robin_util::asset_fs::AssetVfs::new());
            assets
                .install_preloaded_asset("Data/robinhood.bks", word.to_le_bytes().to_vec())
                .unwrap();
            let mut index = signature.to_le_bytes().to_vec();
            index.extend_from_slice(&0u16.to_le_bytes());
            index.extend_from_slice(&0u32.to_le_bytes());
            assets
                .install_preloaded_asset("Data/robinhood.dic", index)
                .unwrap();
            SbFileSystem::new(assets).snapshot()
        }
        let first = reader(123, 456);
        let second = reader(789, 987);
        let a = FrameHolder::from_data_dir_with_files(".", &first).unwrap();
        let b = FrameHolder::from_data_dir_with_files(".", &second).unwrap();
        assert_eq!(a.signature(), 123);
        assert_eq!(b.signature(), 789);
        assert_eq!(a.bank.as_ref().unwrap().words(), &[456]);
        assert_eq!(b.bank.as_ref().unwrap().words(), &[987]);
    }
    ...
    fn test_initialize_sprite_bank_from_game_data() {
        let dir = original_data::data_directory(".");
        let holder =
            FrameHolder::from_data_dir(&dir.to_string_lossy()).expect("failed to load sprite bank");
    ...
    fn test_sprite_bank_packed_data_present() {
        let dir = original_data::data_directory(".");
        let holder = FrameHolder::from_data_dir(&dir.to_string_lossy()).unwrap();
    ...
    #[test]
    #[ignore = "requires Leicester demo data via ROBINHOOD_DATA_DIR; see docs/TESTING.md"]
    fn test_validate_all_sprite_bank_streams() {
        let dir = original_data::data_directory(".");
        let holder = FrameHolder::from_data_dir(&dir.to_string_lossy()).unwrap();
        let mut populated = 0;
        for index in 0..holder.num_sprites() {
            if holder
                .try_packed_data(index as u32)
                .unwrap_or_else(|error| panic!("original sprite {index}: {error:#}"))
                .is_some()
            {
                populated += 1;
            }
        ...
        let estimated = std::mem::size_of_val(sprites)
            + holder
                .dictionaries
                .iter()
                .map(|dictionary| dictionary.resident_bytes() * 3)
                .sum::<usize>()
            + sprites
                .iter()
                .map(|sprite| {
                    let pixels = usize::from(sprite.width) * usize::from(sprite.height);
                    let decoded = if sprite.dictionary_index == UNMAPPED_DICT {
                        pixels * 2
                    } else {
                        pixels / 2
                    };
                    (sprite.packed_size as usize).max(decoded)
                })
                .sum::<usize>();
        eprintln!("estimated shipping resident bank bytes={estimated}");
        eprintln!(
            "validated sprite corpus: slots={}, populated={}, max_pixels={}, max_packed_bytes={}, total_packed_bytes={}",
            sprites.len(),
            populated,
            sprites
                .iter()
                .map(|sprite| u64::from(sprite.width) * u64::from(sprite.height))
                .max()
                .unwrap(),
            sprites
                .iter()
                .map(|sprite| sprite.packed_size)
                .max()
                .unwrap(),
            sprites
                .iter()
                .map(|sprite| u64::from(sprite.packed_size))
                .sum::<u64>()
        );
    }
    ...
    fn optimized_opacity_hash_matches_pixel_lookup_contract() {
        ...
        impl robin_content::PixelOpacityLookup for DefaultOpacity<'_> {
            ...
            fn is_pixel_opaque(
                ...
                self.0.is_pixel_opaque(bank_id, x, y, blue_pixels_are_in)
            }
        }
        ...
        holder.sprites.push(PackedSprite {
            ...
            width: 4,
            height: 2,
            packed_size: (rle.len() * 2) as u32,
            packed_data: Some(Arc::new(rle)),
            ..PackedSprite::default()
        });

        let dictionary =
            FrameDictionary::from_raw(1, vec![TRANSPARENT_COLOR_16, SHADOW_KEY, 0x2468, 0x1357]);
        let dictionary_index = holder.add_dictionary(dictionary);
        holder.sprites.push(PackedSprite {
            validation: Default::default(),
            width: 4,
            height: 1,
