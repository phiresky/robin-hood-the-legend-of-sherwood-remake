...
pub fn decode_rle_canvas(width: usize, height: usize, packed: &[u16]) -> Result<(Vec<u16>, usize)> {
    let count = crate::packed_sprite::pixel_count(width, height)?;
    crate::packed_sprite::validate_rle(packed, width, height)?;
    let mut pixels = vec![TRANSPARENT_COLOR_16; count];
    let mut p = 0usize;
    for y in 0..height {
        let row = crate::packed_sprite::read_rle_row(packed, &mut p, width)?;
        let start = y * width + row.first;
        pixels[start..start + row.literals.len()].copy_from_slice(&packed[row.literals]);
    }
    Ok((pixels, p))
}
...
fn decode_jxl_rgba8_impl(bytes: &[u8], parallel: bool) -> Result<(usize, usize, Vec<u8>)> {
    ...
    let pixel_count =
        crate::packed_sprite::pixel_count(w, h).context("JXL sprite atlas dimensions")?;
    let output_bytes = pixel_count
        .checked_mul(4)
        .ok_or_else(|| anyhow!("JXL sprite atlas byte count overflow"))?;
    ...
    let mut runner = crate::picture::rayon_jxl_runner(parallel);
    let runner_ref = runner
        .as_mut()
        .map(|runner| runner as &mut dyn jxl::api::JxlParallelRunner);
    match dec_with_frame.process(&mut input, &mut bufs, runner_ref) {
