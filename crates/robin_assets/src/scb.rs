...
pub fn parse_bytes(bytes: &[u8]) -> Result<ScbFile, Error> {
    ...
    // The original game writes a little-endian 32-bit script-type count followed by
    // exactly that many class-information records.
...
pub fn parse_file<P: AsRef<Path>>(path: P) -> Result<ScbFile, Error> {
    let p = path.as_ref();
    let resolved =
        robin_data_io::sbfile::resolve_case_insensitive(p).unwrap_or_else(|| p.to_path_buf());
...
fn take_len_prefixed_string(r: &mut Reader<'_>, context: String) -> Result<String, Error> {
    ...
    // TODO(parity): shipped SCBs are ASCII/UTF-8 in practice. If an original-game
