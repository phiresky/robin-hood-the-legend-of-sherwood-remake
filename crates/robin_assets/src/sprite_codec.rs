...
impl See {
    ...
    fn key(level: usize, sum: u32, distinct: u32, top: u32) -> SeeKey {
        ...
        // Only the saturated quartile is needed. Threshold comparisons are
        // exactly floor(4*top/sum).min(3), without a variable 64-bit divide
        // at every visited context (including single-symbol contexts).
        let scaled_top = top as u64 * 4;
        let sum = sum.max(1) as u64;
        let skew = usize::from(scaled_top >= sum)
            + usize::from(scaled_top >= sum * 2)
            + usize::from(scaled_top >= sum * 3);
...
// Keep the containing-block search separate so LLVM can vectorize the
// preceding block sum instead of reusing a scalar chain of partial sums.
#[inline(never)]
fn find_in_symbol_block(syms: &[SymbolCount], target: u32, mut cum: u32) -> (usize, u16, u32, u32) {
    for (i, &SymbolCount(symbol, count)) in syms.iter().enumerate() {
        if target < cum + count as u32 {
            return (i, symbol, cum, count as u32);
        }
        cum += count as u32;
    }
    unreachable!("target beyond symbol block");
}
...
impl Ctx {
    ...
    fn top(&self) -> u32 {
        match self {
            Ctx::Small { syms, .. } => syms.first().map(|&SymbolCount(_, c)| c as u32).unwrap_or(0),
            Ctx::Big { .. } => 0,
        }
    }
    ...
    fn find_by_target(&self, target: u32) -> (usize, u16, u32, u32) {
        match self {
            Ctx::Small { syms, .. } => {
                let (&SymbolCount(first, count), tail) =
                    syms.split_first().expect("empty coding context");
                if target < count as u32 {
                    return (0, first, 0, count as u32);
                }
                // Keep the common first-symbol hit cheap. In the tail,
                ...
                for (block_index, block) in blocks.by_ref().enumerate() {
                    ...
                    if target < end {
                        ...
                        return (1 + block_index * 16 + offset, symbol, start, count);
                    }
                    cum = end;
                }
    ...
    fn find_by_target_excl(&self, excl: &Excl, target: u32) -> (usize, u16, u32, u32) {
        ...
        match self {
            Ctx::Small { syms, .. } => {
                for (i, &SymbolCount(s, c)) in syms.iter().enumerate() {
                    if excl.contains(s) {
                        continue;
                    }
                    let c = c as u32;
    ...
    fn bump(&mut self, x: u16, alphabet: u32) {
        let small_pos = match self {
            Ctx::Small { syms, .. } => Some(syms.iter().position(|&SymbolCount(s, _)| s == x)),
            Ctx::Big { .. } => None,
        };
        match small_pos {
    ...
    fn bump_at(&mut self, i: usize, x: u16, alphabet: u32) {
        ...
        match self {
            Ctx::Small { syms, sum, dense } => {
                ...
                let dest = syms[..i].partition_point(|&SymbolCount(_, pc)| pc >= c);
                if dest < i {
    ...
    fn push_new(&mut self, x: u16, alphabet: u32) {
        match self {
            Ctx::Small { syms, sum, dense } => {
                debug_assert!(syms.iter().all(|&SymbolCount(s, _)| s != x));
                syms.push(SymbolCount(x, BUMP));
                if let Some(dense) = dense {
...
fn small_settle(syms: &mut [SymbolCount], sum: &mut u32, dense: &mut Option<Box<[u16]>>) {
    *sum += BUMP as u32;
    if *sum + escape_weight(syms.len() as u32) >= CTX_HALVE_LIMIT {
        *sum = 0;
        for SymbolCount(_, c) in syms.iter_mut() {
            *c = (*c / 2).max(1);
            *sum += *c as u32;
        }
        if let Some(dense) = dense {
...
#[inline]
fn aux_tile_at(aux: &Option<AuxRef>, col: usize, row: usize) -> u16 {
    let Some(r) = aux else {
        return EDGE;
    };
...
mod tests {
    ...
    fn grouped_symbol_search_preserves_every_frequency_interval() {
        for len in (1..=33).chain([63, 64, 65, 255, 4096]) {
            for skewed in [false, true] {
                let syms: SmallVec<[_; 4]> = (0..len)
                    .map(|i| {
                        let count = if skewed && i < 4 {
                            40 - i as u16 * 9
                        } else {
                            1
                        };
                        SymbolCount(((i * 4051) % 4096) as u16, count)
                    })
                    .collect();
                let sum = syms
                    .iter()
                    .map(|&SymbolCount(_, count)| u32::from(count))
                    .sum();
                let ctx = Ctx::Small {
                    syms: syms.clone(),
                    sum,
                ...
                let mut start = 0;
                for (index, &SymbolCount(symbol, count)) in syms.iter().enumerate() {
                    for target in start..start + u32::from(count) {
                        assert_eq!(
                            ctx.find_by_target(target),
                            (index, symbol, start, u32::from(count)),
    ...
    fn see_quartiles_match_wire_formula_at_every_context_boundary() {
        ...
        for sum in (0..=CTX_HALVE_LIMIT).chain([u32::MAX]) {
            let denominator = u64::from(sum.max(1));
            for boundary in 0..=4u64 {
                let top = (denominator * boundary).div_ceil(4);
                for candidate in [top.saturating_sub(1), top, top + 1] {
                    let top = candidate.min(u64::from(u32::MAX)) as u32;
                    let expected = ((u64::from(top) * 4) / denominator).min(3) as usize;
