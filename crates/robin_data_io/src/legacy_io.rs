...
impl<'a> LegacyReader<'a> {
    ...
    /// The original format stores booleans in one byte. As in legacy file handling and
    ...
    /// fingerprints emitted by stream validation.
    ...
    /// Read a narrow string: a little-endian 16-bit byte length followed by bytes.
    ...
    /// Read a legacy unsigned 32-bit container count, rejecting it before any
    ...
    /// Read an original-game wide string: a 16-bit code-unit count followed by
    /// little-endian UTF-16. Unlike the original game's loader, malformed surrogate
