...
/// Exact serialized path state retained while no hiking
/// path is attached. Original-game path initialization only clears its reference/index; it
...
impl PatrolPath {
    ...
    /// (60% / 30% / 0% sideways) when the straight-movement check
    ...
    pub fn compute_patrol_positions(
        ...
        for pos_idx in 0..patrol_size {
            ...
            // the straight-movement check rejects. Without a grid
...
pub struct PreparedForecastDestination {
    ...
    /// Whether AI destination forecasting assigns its 16-bit direction output
    /// output on this deterministic branch. A one-gate building enters the
    /// building branch, but the multiple-gates body does not run, so
...
impl PreparedForecastDestination {
    ...
    pub fn resolve(&self, sim: &crate::sim_rng::SimulationContext) -> ForecastedDestination {
        ...
        loop {
            ...
            // When there is already no door, the original game compares the selected
            // gate against no value and accepts every real gate on the first draw.
...
pub struct ForecastInput {
    ...
    pub sector: u16,
    /// Exact live sector identity carried by the actor's position. `None`
    ...
    /// Live door pointer and its direction, when non-null.
    pub door_pass: Option<(crate::gate::DoorIndex, bool)>,
    /// The original game's independent direct-door-passing latch. It remains true
    /// after the first PassingDoor callback clears the door pointer and still
...
/// Opt-in provenance for AI destination forecasts that run
...
pub fn prepare_forecast_destination_for_ia(
    ...
    if let Some(gs) = grid_sector {
        if gs.sector_type.is_lift() {
            ...
            // through a random gate. If the door pointer is still live, reject that
            // entry gate; after door passage clears it, the empty comparison in
            // Original accepts whichever real gate the first draw selects.
            // Direction uses exit point minus entry point.
            ...
            if building_gates.len() <= 1 {
                ...
                entry_gate = None;
                // The original game has already selected building handling here,
                // so it does not fall through to the actor's current direction.
                // With no alternate exit, the output
...
///
/// A lift sector resolves its high sector, layer, entry point, and exit direction
/// through the highest door and the low equivalents through the lowest door
/// in the original game. Those two doors are chosen at level
/// load by extreme exit-point Y among the lift's own doors, *not* by
/// door type — the original game keeps
/// the disabled `DOOR_LIFT_HIGH` assertion
...
///
/// For lifts: the aspect-corrected direction from midpoint to exit.
/// For building gates: the aspect-corrected direction from entrance to exit.
...
pub enum MacroOpcode {
    ...
    Wait = 4,
    /// `CMD_CHECK_4` — start friend-check behavior against a friendly NPC
    /// (2B LE16 friend id + 2B LE16 frames).
    Check4 = 5,
    /// `CMD_CHECK_4_SYNC` — friend check with synchronization index
    /// (2B friend id + 2B frames + 2B sync index).
    Check4Sync = 6,
    /// `CMD_STAY_HERE` — drop the patrol path,
    /// then recurse.
    StayHere = 7,
    ...
    Walk = 10,
    /// `CMD_LOOK_LEFT` — look left, then wait for DONE.
    LookLeft = 11,
    /// `CMD_LOOK_RIGHT` — look right, then wait for DONE.
    LookRight = 12,
    /// `CMD_BEND` — look down, launch macro timer
