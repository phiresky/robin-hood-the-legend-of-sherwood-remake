...
fn ordinary_goto_from_sword_state_carries_ordered_quit_prefix() {
    ...
    assert!(
        ...
        "movement teardown belongs to the movement sequence, not the standalone relationship effect"
    );
}
...
fn ordinary_goto_from_menacing_carries_serialized_stop_prefix() {
    ...
    assert!(
        ...
        "movement StopMenace belongs to the movement sequence, not a standalone launch"
    );
...
fn goto_replayed_near_flags_finish_inside_stored_tolerance() {
    let ctx = AiContext {
        ...
        self_layer: 1,
        // The original game's nearby-movement gate is not restricted to idle
        // animations, unlike the ordinary five-pixel movement shortcut.
...
fn trailing_stop_all_closes_prior_goto_before_queuing_halt() {
    ...
    let AiOwnerWork::ActorEffects(prefix) = &ai.outbox.reentrant.owner_work[0] else {
        panic!("movement prefix must precede the trailing action stop");
    };
...
/// Original-game AI facing subtracts the
/// actor's raw 3D point — the location's 3D position minus the actor position
/// — while the explicit-elevation arm
/// subtracts the actor's body point. Those origins diverge while
/// the AI position is overridden to a door's committed gate side, so the 3D
...
fn face_position_3d_measures_from_the_body_not_the_door_side_position() {
    ...
    assert_eq!(
        ...
        "the faced vector starts at the body point, not the overridden position"
    );
}
...
/// The noise elevation still carries the height that the original game recovers via
/// the origin sector during full-position facing.
#[test]
...
/// Interactive session 002-s0007 frame 9157: a purse-impact noise retained
/// its public sector number, but the compatibility trace had no exact arena
/// identity for the original game's origin sector. A public number alone is not
/// a sector pointer (numbers can be duplicated), so use the separately
/// recorded noise elevation just like a fully sector-normalized origin.
#[test]
fn face_number_only_noise_origin_uses_recorded_elevation() {
    let mut ctx = face_to_ctx(crate::element::ActionState::Moving);
    ctx.position = Position {
        x: 1023.0087,
        y: 1618.9951,
    ...
    assert_eq!(orders.len(), 1);
    assert_eq!(orders[0].explicit_direction, Some(6));
}

/// Seed3 QuickSave replay-027 frame 36857/36866: an impact noise carried an
/// actually missing original-game position (no sector, layer `0xffff`).
/// `Face(noise.posOrigin)` therefore takes the ground-level branch in
/// its position-to-3D conversion; the floating noise elevation is
...
fn owner_fifo_preserves_and_hashes_say_setstate_both_orders() {
    ...
    assert_ne!(
        ...
        "speech/state-change statement order must participate in deterministic state"
    );
}
