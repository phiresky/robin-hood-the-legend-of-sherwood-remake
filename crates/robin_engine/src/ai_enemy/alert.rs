...
///
/// Soldier alerts and officer attack commands both use
/// 16-way vector-sector conversion with aspect ratio here. Keep the aspect
...
///
/// Alerting soldiers increments the direction without
/// wrapping and masks only the argument passed to
/// formation placement checks. After a successful attempt it stores the
/// raw cursor (XORed with 8) in each soldier's gather direction. Values
/// such as 29 therefore deliberately survive even though actor direction 29
/// projects to sector 13. Preserving those high bits matters to facing commands'
...
///
/// Original-game vector normalization divides unconditionally in shipping
...
/// the target's 3D position minus the officer's world position.
...
/// The original game stores the stretched squared norm in an unsigned 32-bit value before every
/// comparison in tower-guard alerts. Preserve that truncation here: two
...
/// The original game's officer alert admits officers through combat eligibility, not the
/// distinct ability-to-help predicate used by several officer-coordination
...
/// Substates for which the original game's officer-alert scan treats a soldier as
...
/// the officer's farthest-first alert list. Squared distance reads literal 3D
...
/// active-and-outside-building status is true. Inactive soldiers therefore use
...
///
/// The original game first requires the maximum-norm distance below the alert radius, then requires
/// squared norm of delta < `ALERT_RADIUS * ALERT_RADIUS`. Expressing either test
...
/// consumers. The fallback bug in the original game's tower-guard alert, however,
...
impl EnemyAi {
    ...
    // -----------------------------------------------------------------------
    // Officer orders nearby soldiers to attack
    // Officer attack broadcast.
    // -----------------------------------------------------------------------
    ...
    pub(crate) fn command_soldiers_to_attack(
        ...
        // combat-readiness gates, over every camp's NPCs in registry order;
        // the recipient's live Think handles script/AI locks and state gates.
        for cs in &tick.alert_soldier_candidates {
            // The original game checks whether the friend detects this actor in all directions here,
    ...
    pub(super) fn finish_command_soldiers_to_attack(
        ...
        if alerted_count > 0 {
            let alerted_positions: Vec<Position> = self
                ...
                .iter()
                .map(|handle| {
                    // Officer attack commands alert rank soldiers of any
                    // camp, so the recipients are not all in the same-camp
                    // roster; the original game reads the friend's point from the actor.
            ...
            // (`center`) is further than 150 units (maximum norm) from the
            ...
            // the 3D seek position minus the actor position.
            // `center` and `my_pos` are projected map positions, so their raw
            // Y delta is only suitable for the nearby-enemy maximum-norm gate
            ...
            if enemy_max_norm > 150.0 {
                ...
                // plain sequence-element construction here).
    ...
    pub fn alert_soldiers(
        ...
        for cs in &tick.camp_soldiers {
            ...
            // After the drunk branch, the original game's question response checks
            // active-and-outside-building status. Inactive soldiers are absent
            ...
            // Soldier-call eligibility — reject soldiers whose
            ...
            // Maximum-norm and squared-norm radius gates. Evaluate both
            // positions through AI position. During a
    ...
    pub(super) fn finish_alert_soldiers(
        ...
        let mut alerted: Vec<(HumanHandle, f32, usize)> = accepted_handles
            ...
            .collect();
        // The inherited comment claims increasing distance, but the insertion loop
        ...
        if !officer_in_building {
            ...
            // substate transition.
            // After the original game's first exclusive-or, the direction is the raw successful
    ...
    // -----------------------------------------------------------------------
    // Officer processes a soldier's report
    // Soldier-report merge flow.
    // -----------------------------------------------------------------------
    ...
    pub(super) fn get_report_from_soldier(
        ...
        // per-body detectable removal and per-charly
        // missed-friend detectable additions go through the
        ...
        // Share our now-updated report back to the soldier. The original game
    ...
    // -----------------------------------------------------------------------
    // Soldier alerts a nearby officer
    ...
    pub fn alert_officer(
        ...
        if nearest_officer.is_none() {
            ...
            // friend's visibility query before the scan reaches this actor.
            let owner_index =
            ...
            for registry_index in 0..=tick.camp_soldiers.len() {
                ...
                match cs.rank {
                    ProfileRank::Officer => {
                        ...
                        // The original game stores this maximum-norm distance in an unsigned 32-bit value:
        ...
        // Forecast the nearest officer's AI destination.
        ...
        let officer_target_pos = officer
            ...
            .unwrap_or_else(|| {
                panic!(
                    "officer alerting selected soldier {} without a required destination forecast",
                    officer.handle
                )
            })
            .resolve(sim)
        ...
        // Original does this after changing state and before approaching. `go_near`
        ...
        //
        // The original game checks detectable uniqueness only in debug builds,
    ...
    /// Whether officer alerting will take its REPORT_OFFICER_AFTER instructed-
    /// group early return. That source branch returns immediately after
    /// approach and deliberately skips the ordinary final route-failure test.
    ...
    // -----------------------------------------------------------------------
    // Find alertable soldiers, select the nearest fighter, and seek a soldier
    // -----------------------------------------------------------------------
    ...
    /// `rank` and either default state or body observation, as required by
    /// officer searches. `camp_soldiers` only holds soldier NPCs
    ...
    fn get_nearest_fighter_default_or_looking_body(
        ...
        // nearest-fighter selection
        // seeds its running minimum
        ...
        // fighter order. Both the distance and the bound are unsigned 32-bit values there —
        ...
        for cs in &tick.camp_soldiers {
            // Nearest-fighter selection's general gates are distinct from the global
            // camp registry used by alertable-soldier collection: the
            // registry includes inactive and unconscious soldiers, while this
            // scan rejects inactive plus dead/unconscious candidates.
            // TODO(parity): the original game's gate is exactly
            // alive, conscious, and active; `is_able_to_help`
            // additionally folds in combat readiness and an AI-state whitelist.
            // Given the condition flag below already restricts the state, the
            // surviving difference is combat readiness's extra rejections
            ...
            // AI squared distance scales the position delta's y component by
            // the inverse aspect ratio, then takes its 3D squared norm, so
    ...
    pub fn officer_look_for_soldier(
        ...
        if soldier.is_none() {
            // Find the nearest soldier-ranked fighter within 200 units who
            // is in the default state or looking at a body.
    ...
    pub fn tower_guard_call_alert(
        ...
        // are set by tower-guard-alert decision dispatch before the
        // state-change and alert-call flow.
        ...
        // Soldier enumeration walks the camp registry in stable order. Do
        ...
        for soldier in &tick.camp_soldiers {
            ...
            // Squared distance reads world position, whose horizontal Y is map
            ...
            if sq_dist < sqr_radius {
                ...
                // the game, reusing the exact squared-distance result.
        ...
        if let Some((officer, _)) = nearest_officer {
            ...
            // investigate via the tower-guard call response.
            self.base
                .outbox
                .reentrant
                .cross_npc_actions
        ...
        // reads each camp soldier, and each candidate is compared
    ...
    // (maximum norm of the door-exit delta divided by
    // the reservist count) is minimal, with a +1000 malus for layer
    ...
    pub fn run_and_alert_soldiers(
        ...
        for door in &global.door_seek_infos {
            ...
            // direct door authorization for this actor, without forcing entry.
            if !door.npc_villain_authorized_direct {
                continue;
            }

            // Reservists behind the door: count same-camp
            ...
            // the maximum norm of the door-exit delta.
