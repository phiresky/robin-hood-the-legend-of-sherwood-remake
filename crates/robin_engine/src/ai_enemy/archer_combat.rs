...
impl EnemyAi {
    /// Original-game archer tactics use the AI position for
    ...
    pub fn propose_shot_target(
        ...
        // fighter (excluding self). The original game scans the ally list rebuilt
        // by battle planning, not every same-camp fighter in the broader
        ...
        for &enemy_handle in &self.list_them {
            ...
            // the enemy point has the same planning-position semantics as the friend
    ...
    /// Sherwood Forest level. Returns `false` if
    /// Panics if the required primary target is absent from the live fighter
    /// registry, matching the original game's unconditional reference access.
    pub fn archer_is_too_near_to_enemy(
        ...
        // logic applies via forest retreats.
...
mod tests {
    ...
    fn archer_distance_uses_door_committed_ai_position() {
        ...
        // both archer/enemy proximity checks and step-back goal selection.
        let target = 346;
    ...
    fn shot_target_range_uses_door_committed_ai_position() {
        ...
        // the enemy's AI planning position when proposing a shot target.
        let owner = 127;
        // Entity arena slot zero is live (normally Robin) and must not be
        // mistaken for shot-target selection's no-candidate result.
        let target = 0;
        let committed = Position {
            x: 399.0,
            ..Position::default()
