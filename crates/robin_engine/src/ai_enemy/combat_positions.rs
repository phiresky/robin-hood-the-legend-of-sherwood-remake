...
///
/// The original game reads the raw element position here. This deliberately
...
fn nearest_phalanx_enemy_index(distances: impl IntoIterator<Item = (usize, f32)>) -> Option<usize> {
    // The original game narrows each maximum-norm result to 16 bits before comparing it.
    // Sub-unit differences are therefore ties and preserve append order.
...
/// The forward and rightward steps used by the original game's phalanx reconsideration.
///
/// Both operations use aspect-aware 2D vector math. In
...
/// position-plus-vector result. `Position` copies its `SectorHandle`, whose
/// optional arena companion is the live analogue of the original game's sector reference.
...
fn live_swordfight_target_position(primary_target: HumanHandle, ctx: &AiContext) -> Position {
    ctx.expect_entity_view(primary_target, "swordfight-reconsideration step-in target")
        .position
}

fn literal_swordfight_target_position(primary_target: HumanHandle, ctx: &AiContext) -> Position {
    let view = ctx.expect_entity_view(primary_target, "swordfight-reconsideration facing target");
...
impl EnemyAi {
    /// The original game's swordfight-observation reconsideration uses panic when its
    ...
    fn panic_after_failed_observation_step_back(
        ...
        // Panic is synchronous in the original game, but Rust drains this actor work
        // after the AI borrow is released. Hold it until the rest of
        // Swordfight observation reconsideration has had a chance to override the
        // panic with enemy-attack/observation work from later statements.
        self.base
            .outbox
            .actor
            .begin_panic
            .take()
            .expect("panic_from_position must stage panic startup")
    }
    ...
    fn propose_left_and_right_neighbour(
        ...
        ctx: &AiContext,
        tick: &AiPerTickData,
    ) -> (Option<AiEntityHandle>, Option<AiEntityHandle>) {
        let me_pos = ctx.position;

        let pick_neighbour =
            |cached: Option<AiEntityHandle>, side_left: bool| -> Option<AiEntityHandle> {
                ...
                for handle in &self.base.list_us {
                    ...
                    // AI squared-distance receives the element and reads its
                    // raw position. Keep
                    // that literal body point for ranking neighbours; `position`
                    // is the door-aware AI Position() and may already be snapped
                    // to a gate endpoint.
                    // The original game stores squared distance in an unsigned 32-bit value before the
    ...
    fn propose_line_positions_there(
        ...
        for enemy_handle in &self.list_them {
            ...
            let cp = CombatPosition {
                attacker: Some(AiEntityHandle::new(self.base.me)),
    ...
    /// Propose combat positions to the left.
    ...
    /// Propose combat positions to the right.
    ...
    /// Propose combat positions between neighbours.
    ...
    /// Propose 16 positions ringed around
    ...
    fn propose_combat_positions_around(
        ...
        if enemy.has_as_opponent(self.base.me) {
            if Some(AiEntityHandle::new(enemy_handle)) != self.base.primary_target {
    ...
    /// Find a phalanx position. Tries to find a free slot beside an existing
            ...
            (true, true) => {
                ...
                if sq_left < sq_right {
                    Some((
                        ...
                        left_dir,
                        None,
                        Some(AiEntityHandle::new(left_guy)),
                        left_crosses_identity,
                    ))
                } else {
                    Some((
                        pos_right,
                        right_dir,
                        Some(AiEntityHandle::new(right_guy)),
                        None,
                        right_crosses_identity,
                    ))
                }
            }
            (true, false) => Some((
    ...
    /// Count nearby archers needing protection. Scans nearby friendly
    ...
    fn number_of_nearby_archers_who_need_protection(
        ...
        // Squared distance compares the soldier's position against
        // the acting entity's position,
        ...
        for f in &tick.fighter_registry {
            ...
            // decision tick, but the original game's archer and shield-bearer updates
            // modify both objects synchronously.
            ...
            match f.ai_state {
                AiState::Seeking | AiState::Wondering | AiState::Attacking => {
                    if f.is_archer_unit
                        && effective_shield_bearer_before_me.is_none()
                        && !f.is_tower_guard
                    {
                        ...
                        && effective_archer_behind_me.is_none()
    ...
    /// Check whether enemies are attacking
    ...
    /// Check archer protection by walking the right-neighbour chain
    ...
    fn phalanx_is_protecting_archers(&self, tick: &AiPerTickData) -> bool {
        ...
        for _ in 0..16 {
            ...
            let Some(snap) = self.find_fighter(current_handle, tick) else {
                return false;
            };
            if snap.archer_behind_me.is_some() {
                return true;
            }
    ...
    fn break_phalanx(
        ...
        // member's battle planning while unwinding, then does the same on the
        ...
        if self.left_combat_neighbour.is_some() {
            let mut current = self.left_combat_neighbour;
            for _ in 0..16 {
                let Some(current_handle) = current else {
                    break;
                };
                left.push(current_handle);
                let Some(snap) = self.find_fighter(current_handle, tick) else {
                    break;
                };
        ...
        if !has_left_neighbour {
            // We are already the leftmost member. The original game refreshes the
            // shared list before recursing to the right.
            self.phalanx_reinitialize_them_list(&ctx.position, ctx, tick, carried_them_list);
        }

        let mut right = Vec::new();
        if self.right_combat_neighbour.is_some() {
            let mut current = self.right_combat_neighbour;
            for _ in 0..16 {
                let Some(current_handle) = current else {
                    break;
                };
                right.push(current_handle);
                let Some(snap) = self.find_fighter(current_handle, tick) else {
                    break;
                };
                let next = snap.right_combat_neighbour;
    ...
    /// Run the leftmost member's synchronous shared-list refresh before the
    /// recursive `BreakPhalanx` unwind enters any member's decision tail.
    pub(crate) fn refresh_phalanx_from_neighbour(&mut self, ctx: &AiContext, tick: &AiPerTickData) {
    ...
    /// `BreakPhalanx` call.
    ///
    /// Original-game phalanx breaking walks the neighbour
    /// chain recursively. Every visited member clears both links, marks the
    /// formation abandoned, and runs battle planning before returning.
    ...
    fn phalanx_reinitialize_them_list(
        ...
        // (1..3) Replay the original game's recursion over the up-front member
        ...
        if !self.list_them.is_empty() {
            ...
            let rightmost_pos = {
                ...
                for _ in 0..16 {
                    let Some(cur_handle) = cur else {
                        break;
                    };
                    if let Some(snap) = self.find_fighter(cur_handle, tick) {
                        pos = snap.position;
                        let next = snap.right_combat_neighbour;
            ...
            let nearest_index = nearest_phalanx_enemy_index(
                self.list_them.iter().enumerate().filter_map(|(index, &h)| {
                    self.find_fighter(h, tick).map(|snap| {
                        ...
                        // Aspect-scaled maximum norm — Chebyshev (L∞) — not
                        // Euclidean. The original game then narrows that norm to 16 bits
            ...
            self.base.primary_target = Some(AiEntityHandle::new(best_handle));
        } else {
    ...
    /// Reassess the formation. Called by the leftmost phalanx member on timer
    ...
    pub(super) fn reconsider_phalanx(
        ...
        if let Some(nearest) = nearest
            && let Some(target) = ctx.entity_view(nearest)
        {
            let atk_dist = archer::PHALANX_ATTACK_DISTANCE as f32;
            // squared distance to the nearest enemy reads both actors' literal
            // raw element positions. Do not substitute the
            // AI-facing map positions here: those can already be snapped to
            // opposite sides of a door while either body is still crossing.
            let sq = ai_square_distance_world(
        ...
        // loop starts with the current actor, so the substate-check /
        // primary-target-assignment side effect apply to self too. If we
        // ourselves aren't still in AttackingPhalanx, bail like the
        // original game does for any non-positioned member.
    ...
    /// Arrow-protection refresh / shield-bearer attack consideration. The decision
    ...
    pub fn refresh_arrow_protection(
        ...
        tracing::trace!(
            ...
            "arrow-protection refresh: shield bearer passed substate gate"
        );
        ...
        // Both range gates below use squared distance. That helper
        // reads each actor's raw element position, not the
        // door-aware AI `Position()` stored in fighter snapshots.
        let square_distance_to = |handle: HumanHandle| -> f32 {
            let target = ctx
                .expect_entity_view(handle, "arrow-protection squared-distance target")
        ...
        for &handle in &tick.seen_last_frame_enemies {
            let Some(view) = ctx.entity_view(handle) else {
                tracing::warn!(
                    ...
                    me = self.base.me,
                    handle,
                    "arrow-protection refresh: enemy detectable has no entity view"
                );
                continue;
            };
            ...
            if view.action_state.is_bow() {
                dangerous_enemy = Some(AiEntityHandle::new(handle));
                break;
            }
        }
        tracing::trace!(
            ...
            candidates = ?tick
                ...
                .collect::<Vec<_>>(),
            dangerous_enemy = ?dangerous_enemy,
            "arrow-protection refresh: dangerous-enemy scan"
        );

        if dangerous_enemy.is_none() {
            ...
            tracing::trace!(
                ...
                me = self.base.me,
                protectable,
                "arrow-protection refresh: no dangerous archer"
            );
            if protectable <= 0 {
                if debug {
                    eprintln!(
            ...
            self.base.primary_target = Some(nearest_enemy);
        } else {
            self.base.primary_target = dangerous_enemy;
        }

        // Focus the primary target. Original-game focus updates the NPC's view target;
        // it does not turn the actor's body or overwrite direction_goal.
        // Arrow-protection refresh stores the target's raw element position,
        // not its AI position
        ...
        .unwrap_or_else(|| {
            tracing::warn!(
                ...
                "arrow-protection refresh: primary target has no position snapshot"
            );
        ...
        )) = self.find_phalanx_place(ctx, tick, grid)
        {
            ...
            // cleanup at drain time (left and right combat-neighbor
            // updates), which includes clearing my
            ...
            if inherited_sector_identity_differs {
                let order = self.base.outbox.actor.orders.last_mut().unwrap_or_else(|| {
                    panic!(
                        "phalanx placement for {} did not emit its required movement intent",
                        self.base.me
                    )
                });
            ...
            tracing::trace!(
                ...
                dangerous_enemy = ?dangerous_enemy,
                substate = ?self.base.current_substate,
                "arrow-protection refresh: raising shield in place"
            );
            self.base.stop_all();
    ...
    /// Clean up combat positions. The 0th entry is the current
    ...
    fn clean_up_list_of_combat_positions(
        ...
        let lock_to_target = self
            .find_fighter(self.base.primary_target, tick)
            .filter(|f| f.number_of_opponents <= 1)
            .map(|f| AiEntityHandle::new(f.handle));
        ...
        while i < list.len() {
            ...
            // an equivalent sweep.
            let candidate_target = list[i].target.unwrap_or_else(|| {
                panic!("combat position marked as an adversary change without a target")
            });
            let illegal_target = list[i].change_adversary
                && (!self.is_allowed_to_attack(candidate_target.get(), ctx, tick)
                    || self
            ...
            if cant_leave_target || too_far || illegal_target {
                ...
                for enemy_handle in &self.list_them {
                    ...
                    if Some(AiEntityHandle::new(*enemy_handle)) != list[i].target
                        && dist < enemy.sword_range_maximal as f32
                    {
                        list[i].bonus = list[i]
                            .bonus
            ...
            if clean_me {
                if i == 0 {
                    ...
                    // Original-game list deletion preserves proposal order.
    ...
    pub fn reconsider_swordfight(
        ...
        let detects_primary = self.is_detecting_360_degrees(
            self.base
                .primary_target
                .expect("swordfight detection requires a primary target")
                .get(),
            ctx,
        );
        if reconsider_debug {
        ...
        // the primary target changes immediately above, because the tick's
        ...
        // fighter registry using the original game's 3D-world maximum-norm distance.
        for f in &tick.reconsider_swordfight_friends {
            ...
            if f.number_of_opponents > 1 && dist < nearest_friend_solo_dist {
                nearest_friend_solo = Some(AiEntityHandle::new(f.handle));
        ...
        if primary_outnumbered && nearest_friend_solo.is_some() {
            ...
            if let Some(nearest_enemy_of_solo) = nearest_enemy_of_solo {
                let nearest_to_that_enemy = calculate_opponent_nearest_to_rene(
                    ...
                    self.base
                        .primary_target
                        .expect("swordfight rebalance requires a primary target")
                        .get(),
        ...
        // approach instead of reaching strike selection.
        ...
        //
        // This gate uses the 3D world distance to the primary target with
        // the Y delta stretched by INVERSE_ASPECT_RATIO. That differs from
        // the flat map-space distance used by the "too far to adversary"
        // test further down. Screen-
        ...
        //
        // The comparison is also plain single precision: the maximum range
        // promotes to float, so there is no 16-bit truncation here (unlike
        ...
        if do_reposition {
            ...
            if new_combat_position.change_adversary {
                ...
                if new_combat_position.change_position {
                    ...
                    // principal-opponent replacement.
                    self.set_state(AiState::Attacking, Substate::AttackingSwordfight);
                    debug_assert!(
                        self.is_allowed_to_attack(
                            self.base
                                .primary_target
                                .expect("new combat adversary requires a target")
                                .get(),
                            ctx,
                            tick
                        )
                    );
                    self.base.outbox.actor.set_principal = self.base.primary_target;
                    self.base.launch_timer(20, ctx.frame);
                    return;
                }
        ...
        if dist_to_target > my_max_range as u16
            ...
            && !self.combat_trainer
        {
            // The original game passes the principal opponent's position directly to
            // approach movement here. The fighter
            // snapshot carries combat scalars, but its position can be a
            // number-only compatibility snapshot.  Routing needs the live
            // full-position analogue so duplicate public sector numbers retain
            // their exact arena identity.
            let target_pos = live_swordfight_target_position(
                self.base
                    .primary_target
                    .expect("swordfight step-in requires a primary target")
                    .get(),
                ctx,
            );
            self.set_state(AiState::Attacking, Substate::AttackingMovingAroundOldEnemy);
        ...
        // Honour: don't hit a downed enemy. Original performs these checks
        // synchronously during swordfight reconsideration, before calling
        // strike selection. Preserve that boundary instead of relying
        // only on the later engine-side proposal drain: a recovery animation
        // can terminate between the AI Think and that deferred drain.
        if !Self::sword_strike_honour_allows_proposal(&primary) {
            return;
        }
        ...
        // the original game's single event-driven swordfight reconsideration.
        self.pending_sword_strike_consideration = true;
    }

    fn sword_strike_honour_allows_proposal(target: &FighterSnapshot) -> bool {
        !target.is_in_recovery_animation && target.in_sword_action_state
    }
    ...
    // -----------------------------------------------------------------------
    // Choose a step direction while observing combat
    // -----------------------------------------------------------------------
    ...
    fn propose_step_direction_while_observing_combat(
        ...
        // Element direction-vector lookup builds its vector with
        ...
        // Swordfight observation reconsideration rebuilds list_us with the
        // combat-readiness + MAX_SWORDFIGHT_CONSIDERATION_RADIUS gates.
    ...
    pub(super) fn reconsider_swordfight_observation(
        ...
        //     selector-local view and the original game's owner-ordered shared 16-bit
        //     scratch. The original game's count is restricted to fighters within
        ...
        let me_world = tick
            ...
            .unwrap_or_else(|| {
                panic!(
                    "swordfight-observation reconsideration owner {} is absent from its fighter registry",
                    self.base.me
                )
            })
            .raw_world_position;
        ...
        for f in &tick.reconsider_swordfight_observation_fighters {
            ...
            // The original game truncates maximum-norm distance to 16 bits on the allied list
        ...
        tracing::trace!(
            ...
            "swordfight-observation reconsideration rebuilt the ally/enemy lists"
        );
        ...
        //     stop, observe, and launch a 20-tick timer. The original game
        //     direction assignment, which changes the goal only; the actor's normal
        ...
        // Panic is synchronous in Original.  Its recursive reach-point Think
        // must construct the panic movement before the later attack test can
        // replace the AI state and launch a second movement sequence.
            ...
            if attack_opportunity {
                ...
                for handle in &self.base.list_us {
                    ...
                    if friend.primary_target != new_primary {
                        // The original game gates this on the friend's primary target,
    ...
    // Repositioning logic from swordfight observation reconsideration.
    // -----------------------------------------------------------------------
    ...
    fn propose_good_combat_position_inner(
        ...
        for handle in &self.list_them {
            ...
            let mut cp = CombatPosition {
                attacker: Some(AiEntityHandle::new(*handle)),
            ...
            {
                cp.target = Some(AiEntityHandle::new(opp.handle));
        ...
        for handle in &self.base.list_us {
            ...
            //   - soldier friends → combat position (intended
        ...
        // ask table-swordfight eligibility whether crossing a table/jump
        ...
        // The best combat position stores the table-swordfight jump line for
        // the best combat position's target.
        // in the original game. The two differ
        // whenever the winner is eligible to change adversary, which is
        // exactly the case that installs a cross-sector line jump: asking
        // about the same-sector primary target returns no value and leaves
        // the actor's jump-line reference unset for every later frame, so
        // Surrounding combat-position generation then produces the 16-direction
        // surround ring the Original replaces with its jump-line branches.
        // `target_position` is the candidate's `posTargetPosition`, i.e.
        // target planning position captured in this same call.
        ...
        // time (left and right combat-neighbor updates),
...
mod tests {
    ...
    fn reconsider_phalanx_attack_gate_uses_literal_body_positions() {
        ...
        target_view.detection_position_world =
            crate::coordinates::WorldPoint3D::new(120.0, 0.0, 0.0);
