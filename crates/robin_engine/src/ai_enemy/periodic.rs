...
impl EnemyAi {
    ...
    // Original-game hostile AI sixteenth-frame behavior.
    // -----------------------------------------------------------------------

    /// Every-16-frame update.
    ...
    pub(crate) fn the_16th_frame_before_stuck(
        ...
        // exit, retry the forest retreat to find a different door.
    ...
    pub(crate) fn the_16th_frame_after_refresh(
        ...
        // a sequence transition with no incoming element,
        // re-issue the last movement request or fire EventCouldntReachPoint.
        ...
        // Arrow protection took an action. In particular, a shield
        ...
        if in_reachpoint_arm && stuck_command_active {
            ...
            if sequence_null_about_to_launch {
                ...
                // Relaunch the last movement request, or escape to
                ...
                if dest.sector.is_some() {
                    // The original game starts movement again here. This must pass through
                    // the normal already-on-point test: directly manufacturing
                    // a move order delays the synchronous EVENT_REACHPOINT /
                    // completion-event chain and also skips its boredom-timing RNG draw.
