...
impl EnemyAi {
    ...
    // Shooting state:
    // CallCoordinate stops actions before battle decisions; EventDone
    // rebuilds the enemy list before battle decisions (no direct state
    // transition; battle decisions determine whether to reload,
    // observe, etc.).
    ...
    // shoot an arrow; else clear enemy_seen_below +
    // Battle decisions.
    ...
    // (LeaningOut rebuilds the enemy list; otherwise stop actions),
    // then make battle decisions.
    ...
    fn attacking_bow_running_behind_shield_bearer(
        ...
        match stimulus_type {
            StimulusType::EventReachPoint => {
                ...
                if self.base.primary_target.is_some() {
                    // The original game faces the primary target. The element
    ...
    pub(super) fn attacking_protecting_with_shield(
        ...
        if stimulus_type == StimulusType::EventTimer {
            ...
            } else if self.left_combat_neighbour.is_some() || self.right_combat_neighbour.is_some()
            {
                ...
                // The original game focuses this actor on the primary target here.
    ...
    fn attacking_advancing_with_shield(
        ...
        match stimulus_type {
            StimulusType::EventDone => {
                ...
                // The original game passes the primary target position here. That
                // copies the target's live position, including its sector reference
        ...
        if stimulus_type == StimulusType::EventReachPoint {
            ...
            for _ in 0..max_iters {
                ...
                if !point.is_shooting_point {
                    // Run to the next entry waypoint without stopping first.
                    self.base.go_to(
                        point.position,
                ...
                if owner.is_none() || owner == Some(me) {
                    // Occupy the point, transition state, and move.
                    self.set_state(
                        AiState::Attacking,
                        Substate::AttackingArcherRunOnShootingPathFinalSprint,
                    );
                    // Shooting-point assignment clears the prior point's
    ...
    fn attacking_archer_run_on_shooting_path_final_sprint(
        ...
        if stimulus_type == StimulusType::EventReachPoint {
            ...
            // Face the shooting point's direction.
    ...
    // Done turning on shooting point: battle decisions if high, else
    // overview.
