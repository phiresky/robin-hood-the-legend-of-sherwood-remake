...
fn officer_group_instruction_retries_location_first_after_refusal() {
    ...
    let take_instruction = |ai: &mut EnemyAi| {
        ...
        assert!(
            ...
            "the original game updates the next member only after this AI result"
        );
...
fn avenger_roof_timeout_seeks_from_live_owner_position() {
    // The original game seeks an area from the actor's position, not from the cached
...
fn avenger_roof_timeout_refaces_detected_target_and_rearms_thirty_ticks() {
    ...
    // The original game faces the primary target's position. That path
...
fn approaching_new_enemy_close_gate_stretches_world_y() {
    ...
    // 65+10 range, but the original game's squared-distance calculation stretches Y by the inverse
...
fn approaching_new_enemy_close_gate_uses_literal_positions() {
    ...
    tick.fighter_registry
        .push(crate::ai_enemy::FighterSnapshot {
            ...
            // destination for the approach branch, but it is not world position.
            position: Position {
                x: 500.0,
                y: 0.0,
                ..Position::default()
...
fn bow_running_behind_shield_faces_target_with_its_elevation() {
    ...
    // The original-game element-facing path adds `signed_16(151.12384) - 45 == 106`
...
fn shield_reestablish_uses_raw_door_passing_target_position() {
    ...
    // The element position still reports its raw actor position.
    // The shield-danger point uses the latter.
...
fn door_fight_wait_timer_starts_battle_overview() {
    // Savegame_033 replay-024 frame 24033: entering battle planning
...
fn approaching_sleeping_enemy_uses_stretched_world_distance() {
    ...
    let [movement] = ai.base.outbox.actor.orders.as_slice() else {
        panic!("distant sleeping target should author exactly one approach movement")
    };
...
fn advancing_shield_uses_live_target_sector_for_indexed_route() {
    ...
    // the live element position for this approach destination.
    ai.think_expected_event(
        &sim,
        &Stimulus::new(StimulusType::EventDone),
        &mut AiGlobalState::default(),
...
fn advancing_shield_requires_live_primary_target_view() {
    ...
    // A combat-registry snapshot cannot substitute for the live element
    // position used by the original game's primary-target positioning.
    let mut tick = AiPerTickData::stub();
    tick.fighter_registry
        .push(crate::ai_enemy::FighterSnapshot {
            handle: 282,
