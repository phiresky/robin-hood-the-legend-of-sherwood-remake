...
fn patrol_path_view_fields(
    ...
    if let Some(path) = base.patrol_path.as_ref() {
        ...
        // Alert-path initialization immediately leaves a usable path
...
pub struct AiEntityView {
    /// Original-game element creation order. Runtime entity slots are not
    ...
    /// Direct actor geometry such as detection-point calculation uses this.
    pub detection_position: MapPoint,
    /// The same raw element position in stored 3D world coordinates.
    /// Detection-point and eye-point calculations build on this point, so
    ...
    /// Used by [`crate::ai_enemy::EnemyAi`]'s tower-guard alert to
    ...
    /// Raw `ElementData::active`. Normal human detection uses this
    ...
    /// passes it to view-radius calculation for the obstacle top-plane slice.
    ...
    /// soldier alerting to skip script-locked soldier candidates so a
    /// scripted informative-wait guard isn't dragged off-script by
    /// an unrelated civilian alert.  `false` for entities without an
    /// AI brain.
    pub script_locked: bool,
    /// RNG-free alternatives for AI destination forecasts. Building-exit
    /// selection is deliberately deferred until the exact AI routine where
    /// The original game performs the same check.
    ...
    /// friendly random speech and the shield bearer's arrow-launched
    ...
    /// by the enemy-elevation check to pick the bow-down posture when an
    ...
    /// object-sighting processing to branch between purse /
    /// coin / ale reactions.
    ...
    /// body-sighting processing to push NPC corpses onto the
    ...
    /// enemy-sighting processing's Royalist-camp guards to skip
    ...
    /// an AI brain. Used by enemy-sighting processing's Royalist
    ...
    /// waypoint `ActorData::has_path`. Used by missing-PC alerts
    /// and missing-PC searches to pick between patrol-radius and
    ...
    /// of the mutable original-game NPC initial position.
    ...
    /// dispatch (e.g. PC-sighting processing) that needs
    ...
    /// money brawl. Sourced from `AiController::looted_after_money_fight` so
    ...
    /// PC-sighting processing to detect whether the
    ...
    /// the friend-check synchronization matrix to compare the partner's
    /// traversal direction against ours.
    pub path_forward_movement: bool,
    /// The hiking-path index this NPC is patrolling.  `None` when the
    /// target has no patrol path. Used by missing-PC searches to walk the
...
pub enum EntityKind {
    ...
    Scroll,
    /// Projectile object ([`Entity::Projectile`]); thrown coins use
    /// this kind while remaining valid money objects.
    Projectile,
...
impl AiEntityView {
    ...
    ///
    /// The original game stores AI entity references as element-array slots. A
    ...
    /// Whether a human is dead, unconscious, netted, tied, carried, or a
    /// player character in a coma.
    ///
    /// This is *not* `!is_able_to_fight`: combat readiness also excludes
    /// civilians and soldiers who are sleeping, menacing, fleeing, or
    /// staggering. This predicate asks only whether the human is a "body".
    /// Queues of other bodies to examine are pruned with this predicate, so
