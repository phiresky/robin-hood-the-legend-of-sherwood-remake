...
/// One surface-owned view-radius result, corresponding to the original game's
...
/// frame therefore overwrites the prior entry, exactly as original-game sight obstacles
...
/// π/8.
// The original game's active definition is PI/8. The PI/16 value immediately
...
pub struct VisibilityQuery<'a> {
    ...
    /// active obstacles outside building sectors for the
...
/// calculation. The original game's human-visibility check computes the view radius
...
pub fn compute_visibility_with_effective_radius(
    ...
    // The original game's human-visibility check subtracts the world-space eye and
    // detection points, then stretches world Y. Projected map Y is reserved
    // for the later reachability check because elevation changes it.
    ...
    // Cone + LOS test.
    // Detection uses the stretched 3D norm to choose its close-range
    ...
    // The original game's 3D visibility calculation chooses its close-distance
...
pub struct ObjectVisibilityQuery<'a> {
    ...
    pub target_los: MapPoint,
    /// Original-game world-space object point, with Z increased by one.
    pub target_world: WorldPoint3D,
    /// Sight obstacle list plus FastFindGrid for the LOS check.
    pub sight_obstacles: crate::sight_obstacle::ObstacleList<'a>,
    pub fast_grid: &'a crate::fast_find_grid::FastFindGrid,
...
pub fn compute_object_visibility(q: &ObjectVisibilityQuery<'_>) -> f32 {
    ...
    // Original-game object visibility uses the complete
    // stretched 3D vector for the outer range and detection checks.
    // vector visibility selects its close-distance return
...
fn is_detecting_cone_and_los(
    ...
    if sqr_distance > SQR_HALFCIRCLE_VIEW_RADIUS {
        ...
        // matching aspect-corrected direction-sector selection for the view vector.
...
fn is_detecting_180_degrees(
    ...
    if sqr_distance < 50.0 * 50.0 {
        // The original game uses the direction vector, not the independently mutable
        // view-refresh direction. The facing vector's map-space Y is already
    ...
    // Keep this lazy: the original game does not compute view radius for a target
...
pub fn distance_sharpness(sqr_distance: f32, view_radius: f32) -> f32 {
    ...
    // does. `f32::max(0.0)` selects 0 for NaN, but the original game only clamps
    // when `fDistanceSharpness < 0`; an unordered result therefore survives
    // as the detectable object's last visibility. This is observable when an actor's
...
/// Reproduce the original game's shadow-sector query ordering.
///
...
fn night_fog_shadow_sector_indices(
    ...
    // coordinates in the original game's spatial-grid query. On the shipped Linux
...
/// eye-point calculation. Obstacle planes and shadow-sector barycentres use
...
pub fn compute_view_radius(
    ...
    // The original game does not select lights by barycentre distance. Its sector query
    ...
    for sector_idx in shadow_sectors {
        ...
        // Original-game shadow-sector initialization (`level.shadow_data` populated
...
/// vector. View-radius calculation samples exactly those three differently scaled
...
/// stretch).
///
/// Original-game direction quantization reads the literal
/// constant sector-coordinate tables rather than evaluating trigonometry, and
...
///
/// The original game's vector rotation uses single-precision trigonometry and
/// products. Runtime inspection confirms every stage stays single precision.
///
/// The last ULP is observable. The original game's view refresh feeds the rotated
...
/// un-normalised, ~90-units-long — stare vector. View-radius calculation
/// then places `ptReference` thousands of units off the map, the ±400
/// light box clips to nothing, and the night-light
/// eye-to-3D-barycentre reachability rays are never issued.
...
fn refresh_view_stare(npc: &mut AiActorData, vdx: f32, vdy: f32, own_position: &GroundPoint) {
    ...
    // Zero-length guard: fall back to the actor's direction vector. The original game's
    ...
    // The original game determines the side from a positive determinant.
    ...
    // The original game treats a nonnegative dot product as forward-facing.
    ...
    if dot_forward >= 0.0 {
        ...
        if vector_right {
            if npc.view_angle < half_range {
                ...
                // snap to it, matching the original game.
...
/// The original game's 2D dot product.
///
/// Runtime inspection shows every product and the sum round to `f32`
/// individually, with no extended precision to emulate.
#[inline]
fn geo_dot(ax: f32, ay: f32, bx: f32, by: f32) -> f32 {
    ax * bx + ay * by
}

/// The original game's 2D determinant with the default aspect ratio.
///
...
///
/// The original game's vector-angle calculation keeps the dot, determinant,
/// and quotient in `f32`; the
/// build emits `divss` for `fDet / fDot` — and only then widens for the
/// arctangent: a single-precision quotient is promoted for the arctangent and then narrowed
...
mod tests {
    ...
    fn night_fog_light_lookup_uses_touched_blocks_not_barycentre_radius() {
        ...
        // reference. The original game's sector query still includes it.
    ...
    /// The original game's vector rotation uses single-precision trigonometry
    /// and products; runtime inspection confirms every arithmetic stage.
    ///
    ...
    /// turns the stare-vector determinant from exactly zero into a positive value and
    /// makes the view refresh snap the view direction onto the raw
    /// ~148-unit-long stare vector. View-radius calculation would then place
    /// the reference point far off the map and drop three night-light
    /// reachability rays the original game does issue.
    #[test]
    #[allow(clippy::approx_constant)]
    fn rotate_unit_uses_original_single_precision_trig() {
        ...
        // ASPECT_RATIO, exactly as the view refresh builds it.
        ...
        assert_eq!(
            ...
            0.0,
            "single-precision rotation must leave the overshoot determinant at zero"
        );
    ...
    /// The vector-angle calculation forms the dot and
    /// determinant in single precision, divides in single precision, but promotes
    /// the quotient for the arctangent before rounding back to single precision.
    #[test]
    #[allow(clippy::approx_constant)]
    ...
    fn cross_elevation_visibility_uses_world_horizontal_distance() {
        // Original-game position points share world Y here, while their
    ...
    /// Original-game vector visibility clamps with an explicit
    ...
    fn forest_180_degree_view_uses_body_not_mutable_view_direction() {
        let mut q = query(pt(0.0, 0.0), 4, pt(100.0, 0.0), &[]);
        q.forest_180_degree_view = true;
        // The body faces east while the independently mutable view cone
        // faces west. The original game's forward-half-plane detection uses the direction vector,
    ...
    fn forest_180_degree_view_computes_effective_radius_at_original_boundary() {
        ...
        assert_eq!(
            calls.get(),
            0,
            "behind targets skip view-radius calculation"
        );
    ...
    fn object_no_posture_multiplier() {
        ...
        // the human path, with no running, walking, bow, or sword state
