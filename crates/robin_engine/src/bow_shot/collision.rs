...
/// Rebuild the runtime collision view of a Human's serialized shield.
///
/// The original game serializes the embedded sight obstacle itself and
/// Shield-hit lookup keeps consulting that exact object even when its
/// holder is inactive. Loaded actors therefore cannot wait for a later
/// shield update to publish runtime geometry: inactive shield bearers
/// never receive one.
pub(crate) fn shield_obstacle_from_serialized_state(
    shield: &crate::element::HumanShieldState,
    ...
    obstacle.obstacle_points = shield
        .points
        .iter()
        .map(|point| ObstaclePoint {
            x: point.obstacle[0],
            y: point.obstacle[1],
            z_top: point.obstacle[2],
            z_bottom: point.obstacle[3],
        })
        .collect();
    let point = |point: crate::coordinates::WorldPoint3D| [point.x, point.y, point.z];
    obstacle.top_plane_points = [
        point(shield.top_plane.origin),
        point(shield.top_plane.a),
        point(shield.top_plane.b),
    ];
    obstacle.bottom_plane_points = [
        point(shield.bottom_plane.origin),
        point(shield.bottom_plane.a),
        point(shield.bottom_plane.b),
    ];
    obstacle.rebuild_geometry();
    // Preserve the serialized flag rather than silently deriving a different
    // collision contract from malformed or historical point storage.
    obstacle.on_ground = shield.on_ground;
    obstacle
}
...
pub fn compute_shield_obstacle(
    ...
    // Facing-vector calculation constructs the caller's direction with
    // the isometric aspect ratio. The caller uses that vector for its
    // first offset, then box updating normalizes the already-compressed vector
    ...
    // Original-game sight-obstacle updates normalize compressed input in ordinary
...
/// Refresh retained shield geometry at an original-game shield-update point.
