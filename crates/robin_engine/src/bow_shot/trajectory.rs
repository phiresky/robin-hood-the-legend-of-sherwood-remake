...
/// terminal segment.
///
/// Original-game projectile trajectory calculation keeps the
/// sight obstacle returned by impact reachability and uses that exact
...
pub(super) fn compute_trajectory_ballistic_bounce(
    ...
    // no unused flag until those original-game lifecycles are verified.
...
pub(super) fn compute_trajectory_ballistic_impl(
    ...
    for _ in 0..50 {
        ...
        if let Some(check) = obstacle_check {
            ...
            // The original game's projectile trajectory checks
            ...
            // segment. The original game's projectile trajectory integration
            // tests reachability over sight obstacles;
            // it does not collide with pathfinding/motion-line
            // geometry.  Keep this path sight-obstacle-only.
            // Original-game projectile trajectory calculation uses the ratio of
...
pub fn compute_bow_point(
    ...
    match shoot_mode {
        ...
        ShootMode::Down => {
            ...
            // along the original-game direction vector, whose Y component is
...
pub fn roll_hit_and_compute_bias(
    ...
    // The original game uses the raw unsigned 32-bit capacity here. Normal experience gain caps
    // it at 100, but capacity assignment does not, so preserve negative factors for
    // explicitly authored/scripted capacities above 100.
    let skill_factor = bow_miss_skill_factor(bow_skill_capacity);
    // The original game constructs a 3D vector from three random draws.
