...
pub struct CloakObservation {
    ...
    /// data when a shipped or modded profile supplies evidence for a special
    /// sense. The Original has no animal runtime and no generic special-sense
    /// flag, so callers currently pass `false` rather than inventing classes.
