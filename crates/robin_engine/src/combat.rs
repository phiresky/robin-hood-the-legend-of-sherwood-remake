...
mod tests {
    ...
    fn damage_event_constructors() {
        let sword = DamageEvent::sword(EntityId::from_raw(1), SwordStrike::A);
        assert_eq!(sword.kind, DamageKind::Sword);
        assert_eq!(sword.origin, Some(EntityId::from_raw(1)));
        assert_eq!(sword.sword_strike, Some(SwordStrike::A));

        let arrow = DamageEvent::arrow(EntityId::from_raw(2), 30);
        ...
        let hit = DamageEvent::hit(EntityId::from_raw(3), 40, true);
        assert_eq!(hit.kind, DamageKind::Hit);
        assert!(hit.is_harder_hit);

        let net = DamageEvent::net(EntityId::from_raw(4));
