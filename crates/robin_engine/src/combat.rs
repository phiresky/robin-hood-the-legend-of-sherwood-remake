...
pub fn set_life_points(
    ...
    let new_value = if invulnerable {
        // Human life-point updates use the literal 100 here,
    ...
    if !is_parrying {
        ...
        if let Some(def_profile) = defender_profile {
            ...
            let protection = get_sword_protection(
                ...
                // Sword-damage handling asks the defender's sword for both
            ...
            if roll > protection {
                ...
                if cutting > 0 {
                    ...
                    if !concussion_ctx.scroll_attached {
                        // Player-character wounding ignores a lethal hit
...
pub fn receive_piercing_damage(
    ...
    // Player-character wounding ignores an otherwise-lethal hit while
    // the PC is already in an amulet coma. Piercing damage still
    // proceeds to concussion handling, and sublethal wounds still use
...
pub fn receive_hit_damage(
    ...
    // hitting damage element; the receive-hit-damage command consumes the
...
/// Apply a completed strike's energy to the original game's serialized
/// unsigned 16-bit value
/// tiredness accumulator.
///
/// Tiredness accumulation performs the addition after integer
...
/// Rust-side mirror of the original game's tiredness diagnostics.
///
/// `PARITY_DEBUG_TIREDNESS` enables it; the optional
/// `PARITY_DEBUG_TIREDNESS_CREATION_ORDER` narrows it to one actor, matched on
/// the original game's creation-order value so the two trails line up
...
/// Original-game vector rotation by PI / 2. That operation receives a floating-point angle but
...
pub struct NearbyVictim<'a> {
    ...
    /// collector bypasses sword-strike victim eligibility checks and can therefore
    /// retain an inactive principal opponent, while the lateral collector
    /// explicitly requires active humans.
    pub is_active: bool,
    /// Whether the original game accepts this human as a sword-strike victim.
...
fn is_victim_in_strike_arc(
    ...
    match thrust.kind {
        ...
        WeaponThrustKind::Lateral => {
            ...
            // The original game enters the collector only when the maximum-norm distance is below 150,
        ...
        //
        // The original game's direction-vector calculation starts from the literal 16-sector
        ...
        WeaponThrustKind::PushAside => {
            ...
            // The original game stores this in an unsigned 32-bit value and divides before comparing
        ...
        WeaponThrustKind::TrueHalfCircle | WeaponThrustKind::FalseHalfCircle => {
            // Half-circle sword-strike victim collection uses the same
            // positive maximum-norm/range guards as the lateral collector, with
        ...
        WeaponThrustKind::TrueCircle | WeaponThrustKind::FalseCircle => {
            // Apply a strict maximum norm < 150 quick reject, then
...
fn degrees_to_radians(degrees: u16) -> f32 {
    ...
    // narrowed only by the single-precision return value.
...
/// Direction used by sword protection during strike estimation.
///
...
/// separate calculation in the original game: protection subtracts the actors'
/// unprojected ground positions and selects an aspect-corrected
/// direction sector. Reconstruct world Y from map Y + elevation
...
///
/// The original game's strike estimation builds its candidate victim list
/// through its possible-victim collector. The lateral and half-circle collectors are not
/// side-effect free: both overwrite the human-owned
/// Initial, final, and current strike angles
...
/// geometry. Circle sword-strike victim collection
/// on failure, the straight collector and the push collector write nothing,
...
pub(crate) fn propose_good_sword_strike_with_debug(
    ...
    if !only_parade {
        ...
        for (i, &strike) in NORMAL_STRIKES.iter().enumerate() {
            ...
            // Sword-strike damage estimation always builds its victim list
...
mod tests {
    ...
    fn lateral_and_half_circle_collectors_reject_nan_positions() {
        ...
        // maximum norm < 150 followed by positive inclusive range checks.
    ...
    fn estimated_protection_uses_ground_space_isometric_direction() {
        ...
        // Sword protection reconstructs ground/world Y and applies the
    ...
    fn push_strike_width_uses_original_integer_half_width() {
        ...
        // of 5 therefore has the original game's integer half-width 2, not 2.5.
        let victim = NearbyVictim {
            is_active: true,
            eligible_for_regular_strikes: true,
            dx: 2.25,
    ...
    fn strike_energy() {
        ...
        // Sword strike energy returns the authored value directly for a
