...
//!
//! The original game uses one point representation for several incompatible spaces.
...
coord2!(
    ...
    ///
    /// This is a floored draw origin. Convert to map space with
...
coord2!(
    ...
    ///
    /// Offset from [`SpriteTopLeft`] to the entity's
    /// projected [`MapPoint`].
    SpriteAnchor,
    sprite_anchor
);
...
coord2!(
    ...
    ///
    /// The original game uses the movement box's bottom-right corner.
    MoveBoxHalfDiagonal,
    move_box_half_diagonal
);

// Share the storage operations, not the coordinate domains. MoveBox retains
// its distinct translation-to-MapBBox API below.
macro_rules! bbox2 {
...
bbox2!(ScreenBBox, ScreenPoint);
bbox2!(MapBBox, MapPoint);
bbox2!(GroundBBox, GroundPoint);
bbox2!(MoveBox, MapVec);
...
///
/// The original game uses this zero-centered box in
/// character-position coordinates. Translate it by a [`MapPoint`] to get the
/// absolute map-space [`MapBBox`] stored as the movement map bounds.
#[derive(
    Debug,
    Clone,
    Copy,
...
impl MapPoint {
    ...
    ///
    /// This is the original game's projection rule: map `(x, y)` is
    /// world `(x, y - z)`.
    #[inline]
