...
/// Encode the authoritative relationship matrix as typed, ordered entries.
///
/// JSON object keys can only be strings, so directly deriving `Serialize` for
/// `BTreeMap<(u16, u16), Relationship>` fails as soon as a mission has one
/// non-default relationship. A pair list keeps both allegiance IDs typed and
/// is also the canonical on-disk order because the runtime map is a
/// `BTreeMap`.
mod relationship_entries {
    use std::collections::BTreeMap;

    use serde::{Deserialize, Deserializer, Serialize, Serializer, de::Error as _};

    use super::{DiplomacyRule, Relationship};
        ...
        S: Serializer,
    {
        relationships
            .iter()
            .map(|(&(first, second), &relationship)| DiplomacyRule {
                first,
                second,
                relationship,
            })
    ...
    pub(super) fn deserialize<'de, D>(
        deserializer: D,
    ) -> Result<BTreeMap<(u16, u16), Relationship>, D::Error>
    where
        D: Deserializer<'de>,
    {
        let entries = Vec::<DiplomacyRule>::deserialize(deserializer)?;
        let mut relationships = BTreeMap::new();
        for entry in entries {
            if entry.first >= entry.second {
                return Err(D::Error::custom(format!(
                    "diplomacy relationship entry must use canonical distinct allegiance order, got ({}, {})",
                    entry.first, entry.second
                )));
            }
            if entry.relationship == Relationship::Hostile {
                return Err(D::Error::custom(format!(
                    "diplomacy relationship entry ({}, {}) redundantly stores the default hostile relationship",
                    entry.first, entry.second
                )));
            }
            if relationships
                .insert((entry.first, entry.second), entry.relationship)
                .is_some()
            {
                return Err(D::Error::custom(format!(
                    "duplicate diplomacy relationship entry for allegiances {} and {}",
                    entry.first, entry.second
                )));
            }
        }
...
pub struct DiplomacyState {
    ...
    #[serde(with = "relationship_entries")]
    relationships: BTreeMap<(u16, u16), Relationship>,
    revision: u64,
}
...
mod tests {
    ...
    #[test]
    #[should_panic(expected = "diplomacy query requires a valid allegiance, got Error")]
    fn invalid_allegiance_is_rejected_even_when_diplomacy_is_disabled() {
        let _ = DiplomacyState::default().relationship_to_player(Camp::Error);
    }
    ...
    #[test]
    fn populated_relationship_matrix_is_json_roundtrippable() {
        let state = DiplomacyState::from_definition(
            true,
            false,
            Some(&DiplomacyDefinition {
        ...
        let encoded = serde_json::to_string(&state).expect("serialize populated diplomacy state");
        ...
        let decoded: DiplomacyState =
            serde_json::from_str(&encoded).expect("deserialize populated diplomacy state");

        assert_eq!(decoded, state);
    }

    #[test]
    fn relationship_matrix_json_rejects_noncanonical_entries() {
        let state = DiplomacyState::from_definition(
            true,
            true,
            Some(&DiplomacyDefinition {
        ...
        for (relationships, expected) in [
            (
                serde_json::json!([
                    {"first": 2, "second": 7, "relationship": "neutral"},
                    {"first": 2, "second": 7, "relationship": "allied"}
                ]),
                "duplicate diplomacy relationship entry",
            ),
            (
                serde_json::json!([
                    {"first": 7, "second": 2, "relationship": "neutral"}
                ]),
                "canonical distinct allegiance order",
            ),
            (
                serde_json::json!([
                    {"first": 2, "second": 7, "relationship": "hostile"}
                ]),
                "redundantly stores the default hostile relationship",
            ),
        ] {
            let mut document = baseline.clone();
            document["relationships"] = relationships;
            let error = serde_json::from_value::<DiplomacyState>(document).unwrap_err();
