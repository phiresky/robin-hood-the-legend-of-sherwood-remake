...
pub fn direction_vector_16(sector: i16) -> (f32, f32) {
    // The original game's direction quantization indexes two literal tables.
...
pub use robin_engine_types::camp::Camp;
...
pub enum GameMaterial {
    ...
    Ice,
    Hole,
    /// Original-game material-count sentinel stored on ordinary
...
impl GameMaterial {
    ...
    pub fn from_u32(value: u32) -> Self {
        Self::try_from_u32(value).unwrap_or_else(|| panic!("invalid live material value: {value}"))
    }
...
pub enum Command {
    ...
    GetKilledAtBottom,
    WakeUp,
    /// Unused command slot retained for save compatibility.
    Roll,
    CrouchDown,
    CrouchUp,
    ActionAvailable,
    ...
    BeggarShowFace,
    EnterLeisure,
    /// Compatibility placeholder for the original game's misspelled
    /// the misspelled animal-afraid command.
    ///
...
///
/// The original game constructs both message entry points with the same three
/// integer properties and delivers that exact triple to the
...
mod tests {
    ...
    #[test]
    fn set_moving_matches_game_classification() {
