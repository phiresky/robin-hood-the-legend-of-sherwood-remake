//! Priority rules used by the sequence system to decide which
//! commands can interrupt which.
//!
//! ## Rule hierarchy
//!
//! Actor-specific rules fall back to more general rules:
//!
//! ```text
//! Actor
//! └── Human
...
//! ```
//!
//! An explicitly assigned priority takes precedence over these rules.
//!
//! This module resolves the rules in `determine_priority` keyed on
...
/// Resolve a sequence element's priority using actor-specific rules.
///
/// Returns an explicitly assigned priority before consulting any rule table.
...
fn pc_branch(ctx: ActorPriorityContext, elem: &SequenceElement) -> SequencePriority {
    match elem.command {
        ...
        // crouching up at preference priority and appends the cape or
        // tree transition order to that same element during the posture change.
        ...
        // in NPC priority selection. Custom missions can
...
fn actor_branch(elem: &SequenceElement) -> SequencePriority {
    match elem.command {
        ...
        // the unset-priority sentinel. The original game's base priority selection falls
        // through to no priority for this command in release builds, and
        // Stopping the sequence element immediately promotes that result to Normal.
        ...
        other => {
            debug_assert!(
                false,
                "priority evaluation: unhandled command {other:?} reached actor_branch default",
            );
            SequencePriority::None
        }
    }
...
mod tests {
    ...
    fn make_elem(command: Command) -> SequenceElement {
        SequenceElement::new(1, command, Some(EntityId::Pc(crate::entity_id::PcId(1))))
    }
