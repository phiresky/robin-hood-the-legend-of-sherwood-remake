...
fn missed_pc_search_reads_current_forecast_instead_of_old_seek_position() {
    ...
    engine.place(target, WorldPoint3D::new(750.0, 200.0, 0.0));
    ...
    engine
        ...
        .execute_live_battle_without_visible_enemies(Vec::new());
...
fn directed_cassos_reads_the_live_target_and_completes_panic() {
    let (mut engine, assets, owner, target) = fixture(false);
    engine.place(target, WorldPoint3D::new(725.0, 175.0, 0.0));
    ...
    let ai = &engine.enemy(owner).base;
    assert_eq!(
...
fn repeated_cassos_without_a_target_remains_undirected() {
    ...
    let result = engine
        .ai_ctx(&crate::sim_rng::test_context(), &assets, owner)
        .execute_live_battle_decision(Decision::Cassos, Substate::FleeingPanic, 0, false);
    assert_eq!(result, Some(Decision::Cassos));
    let ai = &engine.enemy(owner).base;
    assert!(!ai.directed_panic);
...
fn help_decision_finishes_real_officer_route_before_fallback_and_logs_once() {
    for disconnected in [false, true] {
        ...
        let entity = engine.ent_mut(officer);
        ...
        let officer_ai = engine.enemy_mut(officer);
...
fn archer_step_back_without_target_completes_shoot_to_observation_fallback() {
    ...
    engine.enemy_mut(owner).list_them.clear();
    engine
        .ent_mut(owner)
        .ai_actor_data_mut()
        .unwrap()
