...
impl EngineInner {
    ...
    /// heard-noise-bounds rebuild. The inactive/building and breath arms
    ...
    /// Refresh the produced-noise state at one PC's live human-update
    /// boundary. The original game's produced-noise refresh follows the base actor slice,
    ...
    /// Refresh produced noise using the actor-order animation visible to the
    /// Original-game human update tail.
    ///
    ...
    /// order is newer than the original game's latched actor order; the fused owner walk
    ...
    /// save pointer fixup.
    ///
    /// Engine serialization walks every human here, but produced-noise refresh
    /// immediately returns for
    ...
    /// remark-completion notification inline during local-AI deserialization.
    ...
    /// creation-order update boundary.
    ///
    /// The original-game NPC update clears the flag and calls
    /// friend-detection broadcast immediately before that same NPC's
    ...
    /// Original-game detectable-object restoration, executed inline by the waking
    /// soldier before resurrection fan-out and any state-change callback.
    ...
    /// of the simulation-tick deferred-effects phase where matured
    /// exclamations are collected.
    ///
    /// NPC sound completion
    /// in the original game converts the
    ...
    fn auto_forbid_remark(
        ...
        match remark {
            ...
            // Standard speaker-type list from the original game's speech selection.
