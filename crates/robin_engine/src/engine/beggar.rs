...
/// Maximum-norm proximity window (map units) within which a civilian will
/// notice a beggar in front of them.
const BEGGAR_PROXIMITY: f32 = 70.0;

/// Radius (maximum norm, map units) for the near-coins toggle sweep.
const NEAR_COINS_RADIUS: f32 = 100.0;

/// The original game's minimum beggar-handling intelligence.
...
/// Original-game beggar assignment for seeking soldiers: the camp
/// registry is scanned in order, difficulty-modified intelligence is compared
/// against `CHECK_BEGGAR_MIN_IQ`, and `_ANY_SEEK_AREA_SUBSTATE_` is the only
/// admitted state family. Detection registration appends to the Beggar list. The original game
/// has a debug-only uniqueness assertion immediately before that append, but
/// the shipping build still appends a duplicate when a repeated disguise
...
pub(super) fn can_give_money_to_beggar(
    ...
    // civilian's heading. The maximum norm of this vector must be ≤ 70 and
...
fn give_money_to_beggar(
    ...
    // The original game assigns beggar ownership after adding the element.
...
impl EngineInner {
    ...
    ///
    /// Run the beggar-simulation animation bid belonging to one selected
    ...
    pub(crate) fn tick_beggar_bid_for(
        ...
        let Some(entity) = self.get_entity(pc_id) else {
            panic!("beggar owner {pc_id:?} disappeared in its actor-update slot");
        };
        ...
        // Fast turning precedes action processing in the original game and is not a sprite
