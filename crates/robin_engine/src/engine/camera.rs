...
impl EngineInner {
    ...
    pub(crate) fn set_level_size(&mut self, width: f32, height: f32) {
        ...
        assert!(
            ...
            "level dimensions must be finite and either both zero or both positive, got {width}x{height}"
        );
        if self.control.sim_config.fog_of_war && width > 0.0 && height > 0.0 {
            self.players
                .fog_of_war
                .initialize(MapSize::new(width, height));
        }
    }
    ...
    /// during zoom drawing — both are intentional unconditional writes
