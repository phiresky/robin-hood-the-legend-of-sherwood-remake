...
impl EngineInner {
    ...
    pub fn selected_view_cone_params(
        ...
        let tint = entity.ai_controller().map(|ai| {
            ...
            let max_suspect = npc
                ...
                .unwrap_or(0);
            // `view_alert_status` already carries the forced-attention
    ...
    pub fn all_npc_view_cone_params(&self) -> Vec<ViewConeParams> {
        ...
        for (_, entity) in self.world.entities.npcs() {
            ...
            let tint = entity.ai_controller().map(|ai| {
                ...
                // `view_alert_status` carries the forced-attention
    ...
    pub fn compute_display_order(&self) -> DrawOrder {
        ...
        for (id, entity) in self.world.entities.occupied() {
            // Original-game element insertion puts elevation-zero FX-base elements only
            // in the background-animation collection. They render once before the
    ...
    pub fn minimap_dot_info(
        ...
        let custom_dot = CustomDot::from_u16(elem.custom_minimap_dot);

        // Original-game minimap refresh only queries camp in the
        // ordinary, active, non-blipped, non-VIP soldier branch. Most engine
        // elements have no allegiance at all, so querying diplomacy while
        // merely gathering their dot inputs violates the Camp::Error
        // sentinel invariant.
        let camp_required = entity.is_active()
            && custom_dot == CustomDot::NotCustomized
            && entity.is_soldier()
            && !elem.blipped
            && !is_soldier_vip;
        let (camp, legacy_camp) = if camp_required {
            let entity_camp = entity.camp();
            let camp = match self.relationship_to_player(entity_camp) {
            ...
            let legacy_camp = if entity_camp == crate::element::Camp::Royalists {
                MmCamp::Allied
            } else {
                MmCamp::Hostile
            };
            (Some(camp), Some(legacy_camp))
        } else {
            (None, None)
        };
...
///
/// Original-game element ordering compares the raw display-order floats and uses
/// creation order only when `==`. A NaN therefore compares neither before nor
/// after any finite value. That violates the original game's ordering contract and
...
mod display_order_tests {
    ...
    fn minimap_dot_info_does_not_query_allegiance_for_fx() {
        ...
        let info = engine
            .minimap_dot_info(fx, &LevelAssets::new())
            .expect("live FX should provide minimap classification inputs");

        assert_eq!(info.camp, None);
        assert_eq!(info.legacy_camp, None);
        assert_eq!(crate::minimap::classify_element_dot(&info), None);
    }
