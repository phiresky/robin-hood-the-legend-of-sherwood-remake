...
//!    original game's sentinel event value `-2`.
...
/// matching the original game's default event code.
#[test]
fn filter_runs_for_unmapped_stimulus_type() {
    let sim_context = crate::sim_rng::test_context();
    let sim = &sim_context;
    let (mut engine, _, sensitive_handle, _) = build_engine();
    // The enemy-near event exists in the original game but has no public AI event mapping
...
fn reentrant_return_to_duty_uses_absent_live_order_not_stale_sprite_animation() {
    ...
    assert_eq!(
        ...
        "animation query must read NonanimationEnd and take movement's already-at-post path"
    );
    assert_eq!(
        engine
            .world
...
fn remove_all_subordinates_force_returns_script_locked_civilian_to_duty() {
    ...
    assert_eq!(
        ...
        "forced return to duty bypasses the script-lock gate on return-to-duty events"
    );
    assert!(
    ...
    assert!(
        on_post_ai.stimulus_queue.is_empty(),
...
fn prototype_filter_event_preserves_the_outer_this_actor() {
    // The original game's prototype event filter deliberately leaves
    // the script receiver unchanged so the prototype knows the actual event receiver.
...
/// base filter result: one (allow). Script handling forwards the call
/// directly to the NPC, and the shipped ActorScript
...
#[test]
fn sequence_launch_dispatches_immediate_engine_elements() {
    use crate::element::Command;
    ...
    // execute before the NPC update returns.
...
fn animation_execution_gates_do_not_skip_action_change() {
    ...
    for skip in [
        ...
        assert_eq!(
            ...
            "{skip} actors still own the base-update ActionChange boundary"
        );
        // The engine update runs every element's update regardless of
...
fn combat_injury_think_finishes_before_same_slot_action_change() {
    ...
    let order_id = order.order_id;
    install_test_order_queue(&mut engine, &assets, actor, [order]);
    {
    ...
    engine
        .world
        .entities
        .expect_ai_controller_mut(actor, format_args!("combat injury retained stimulus"))
        .stimulus_queue
        .push(crate::ai::Stimulus::new(
    ...
    assert!(
        ...
        "StandingUpSword must run combat AI before completion/next-action processing and ActionChange: {phases:?}"
    );
    assert_eq!(
        engine
            .world
...
fn live_actor_walk_visits_callback_spawned_later_slot_and_skips_holes() {
    ...
    let removed_during_callback = engine.add_test_entity(make_pc(false));
    ...
    assert!(
        !engine
            .world
            .actor_registry_ids
            .contains(&removed_during_callback)
    );
    assert!(
        !engine
            .world
...
fn earlier_owner_callback_installs_invalid_later_pc_init_order_rejected_same_frame() {
    ...
    engine.tick_actor_owner_envelopes_with_test_owner_hook(
        ...
        |engine, completed_owner| {
            ...
            // No antagonist: the original game's pickup initialization must abort it.
...
fn terminating_animation_promotes_next_order_before_same_actor_action_change() {
    ...
    install_test_order_queue(&mut engine, &assets, actor, [first, second]);
    {
        let sprite = &mut engine
            .world
            .entities
    ...
    assert_eq!(
        ...
        "order advancement must rewrite serialized motion state when promoting a successor"
    );
}
...
fn wait_timer_zero_completes_after_execute_and_before_action_change() {
    ...
    assert_eq!(
        ...
        "zero WAIT_TIMER must execute its current order, complete/advance, then expose the null order to ActionChange"
    );
    assert_eq!(
        engine
            .world
        ...
        "actor updates must retain the post-WAIT_TIMER execution result in the serialized motion state"
    );
    assert!(
        engine
            .orders
...
fn turning_selects_sprite_row_after_the_direction_step() {
    ...
    assert_eq!(
        ...
        15,
        "original-game turning advances direction 0->15 before action processing selects the directional row"
    );
}
...
fn wait_timer_termination_replaces_forwarded_completion_exactly_once() {
    ...
    assert_eq!(
        element.orders.len(),
        1,
        "raw termination plus wait-timer termination must not advance the order twice"
    );
    assert_eq!(
        element
            .orders
...
fn frozen_all_consumes_actor_initialisation_once_without_sprite_identity() {
    ...
    assert_eq!(
        engine
            ...
            .state,
        crate::sequence::SequenceState::InProgress,
        "DrinkingAle's inactive-antagonist initialization gate must not repeat on a later frozen tick"
    );
}
...
fn civilian_random_speech_closes_its_owner_boundary_before_the_lock_gate() {
    ...
    // frame-1381 original-game event shape.
...
fn actor_execute_arm_ledger_has_unique_linked_routes() {
    ...
    assert_eq!(ACTOR_EXECUTE_CATALOG.len(), 308);
    let mut expected = std::collections::HashSet::new();
    for &(override_kind, order, owner) in ACTOR_EXECUTE_CATALOG {
        assert!(
    ...
    for actor in [EntityId::Pc(PcId(0)), EntityId::Soldier(SoldierId(0))] {
        for order in [
            ...
            assert_eq!(
                ...
                "smalltalk strike {order:?} must retain its specialized human execution behavior"
            );
        }
    }
}
...
fn earlier_opponent_prune_synchronously_quits_both_combatants() {
    ...
    for pruner_before_mutated in [true, false] {
        ...
        // manager; the manager's own update — which runs after every
        ...
        for actor in [pruner, mutated] {
            ...
            // the update instructs it this frame, its lower-sword transition
...
fn waking_up_creation_order_engine(
    ...
    install_test_order_queue(&mut engine, &assets, rescuer, [waking]);
    ...
    install_test_order_queue(&mut engine, &assets, target, [unconscious]);
    {
        let sprite = &mut engine
            .world
            .entities
...
fn waking_up_done_does_not_force_awake_a_script_locked_npc() {
    ...
    assert!(
        target_entity
            ...
            .expect("wake target is human")
            .unconscious,
        "Original concussion handling clamps a script-locked NPC at the wake threshold"
    );
    assert!(
        !target_entity
            .ai_controller()
            ...
            .any(|line| line.line_type == crate::ai::LogLineType::Event
                && line.info == crate::ai::StimulusType::EventFitAgain as u16),
...
fn state_change_filter_class(
    ...
    if mutate_state {
        // Reserve slot 3 for proof that the decision-entry NO_EVENT callback
        // received no source. Store source+1 only for code -2;
    ...
    // strongest regression sentinel because decision entry would reject it.
    quads.extend([
...
fn install_unrelated_multi_exit_building_actor(
    ...
    // sector reference, and a nonempty arena cannot validly resolve a sector that
...
fn select_unrelated_pass_door_fixture(
    engine: &mut EngineInner,
    assets: &LevelAssets,
    door_actor: EntityId,
) {
    ...
    // Original-game door-passing detection uses selected-command state, not merely the
...
fn destination_forecast_requires_a_live_door() {
    ...
    assert_eq!(
        super::ai::extract_forecast_input(&actor, true)
            ...
            .door_pass,
        None,
        "The original game falls back when the forecast destination has no door"
    );
    assert!(
        !super::ai::extract_forecast_input(&actor, true)
            .expect("actor has forecast state")
...
fn destination_forecast_retains_direct_passage_after_the_live_door_clears() {
    ...
    assert!(
        ...
        "Original keeps direct-passage state after door traversal clears the door pointer"
    );
}
...
fn script_native_state_effects_stabilize_before_adjacent_instruction() {
    ...
    // Area search's causal Move is only registered with the sequence manager;
    // normal-priority launches are dispatched by the manager's update
    // later in the frame, so an adjacent GetCurrentAction still reads the
    // orderless no-animation sentinel.
    assert_eq!(
        seeking_values[3],
        crate::order::OrderType::NonanimationEnd as i32,
        "area search's Move stays registered-not-instructed at the adjacent instruction"
    );
    assert!(
        engine
            .orders
        ...
        "the causal area-search Move is queued for the manager update"
    );
    ...
    // deterministic acceptance draw rejects that point. The original game's area seeking
    // appends personal point 2222 at its center when the list is empty, so
    // movement targets the actor's current position and decision completion synchronously
    // reaches FilterAIEvent(EVENT_REACHPOINT). The zero return refuses that
    // recursive decision tick after the earlier seeking state-change callback.
    assert_eq!(
        &zero_values[0..3],
        &[3, 3, 3],
    ...
    assert_eq!(
        ...
        "personal point 2222 is exactly the actor-centered area-search fallback"
    );
    assert!(
        !engine
            .orders
    ...
    assert_eq!(
        &at_point_values[4..8],
        &[-2, 103, 3, 103],
        "decision completion dispatches AI_EVENT_REACHPOINT before the recursive next-search-point state callback"
    );
    assert_eq!(
    ...
    // movement takes the already-at-post gate (REACHPOINT) before the map-bounds
    // check can fail; the resulting already-facing turn completes immediately
    // and the completion-event decision tick synchronously reaches the enemy state change
    // tail. The original game reports the out-of-range AI state after that
...
fn pre_existing_same_owner_moves_are_stopped_without_being_dispatched_as_causal_move() {
    ...
    *destination = deferred_destination;
    ...
    // The causal area-search Move is only registered at launch; the manager's
    // the update instructs it later in the frame, so the adjacent
...
fn set_ai_state_seeking_and_fleeing_do_not_draw_unrelated_building_exit_gate_rng() {
    ...
    select_unrelated_pass_door_fixture(&mut seeking_engine, &seeking_assets, door_actor);
    {
    ...
    assert!(
        ...
        "Seeking, including at-point decision-completion recursion, must remain owner-local and forecast-free"
    );
    assert_eq!(
    ...
    select_unrelated_pass_door_fixture(&mut fleeing_engine, &fleeing_assets, fleeing_door_actor);
...
fn fleeing_panic_classification_occurs_after_no_event_callback_mutation() {
        ...
        .with_arena_index(crate::fast_find_grid::SectorIndex::new(sector_index).unwrap());
    {
        ...
        // recursion fallback and returning to duty instead of isolating the
        // post-filter state-change commit.
...
fn set_ai_state_ignores_start_think_freeze_script_lock_and_ai_lock_results() {
    for (class_name, refusal, configure) in [
        ...
        assert_eq!(
            values[4], -2,
            "{class_name} must run the empty-source/no-event filter before its post-filter gate"
        );
        let ai = engine
            .world
            .entities
...
fn enemy_state_change_callback_is_owner_local_observes_outgoing_and_ignores_zero() {
    ...
    assert_eq!(
        &values[4..7],
        &[103, -2, 106],
        "outer callback, synchronous empty-source/-2 prelude, then panic-state callback"
    );
    assert_eq!(
        values[7], 1,
    ...
    assert_eq!(values[3], 1, "no-event callback source was absent");
...
fn initialization_binds_scripts_before_draining_init_one_ai_state_callbacks() {
    ...
    assert_eq!(
        ...
        101,
        "the bound actor VM must receive Default's AI_STATE_CHANGE callback during per-NPC initialization"
    );
}
...
fn initialization_caches_the_aspect_adjusted_initial_view_direction() {
    ...
    assert_eq!(
        ...
        1,
        "initial-position setup uses aspect 1 for the vector, but facing bins that \
...
#[test]
fn civilian_blink_clears_detection_edges_inline() {
    let mut engine = EngineInner::new();
    let civilian = engine.add_test_entity(make_scripted_civilian(""));
    let enemy = engine.add_test_entity(make_scripted_soldier(""));
    let npc = engine
        .world
        .entities
        .expect_ai_actor_data_mut(civilian, format_args!("test civilian"));
    ...
    engine.execute_ai_blink_all_enemies(civilian);
    let npc = engine
        .world
        .entities
        .expect_ai_actor_data(civilian, format_args!("test civilian"));
    let entry = &npc.detectable_lists[crate::element::DetectableType::Enemy as usize][0];
    assert!(!entry.seen_now);
    assert!(!entry.seen_last_frame);
    assert_eq!(entry.element, Some(enemy));
}
...
fn ai_focus_accepts_live_object_element() {
    ...
    engine.execute_ai_focus(observer, Some(crate::ai::AiEntityHandle::new(ale.index())));
...
fn fit_again_engine_calls_surround_state_callback_in_original_order() {
    ...
    fn observe(friendly: bool) {
        ...
        assert_eq!(
            ...
            "resurrection notification must finish before state change"
        );
        if friendly {
            assert_eq!(
                observation.eye_status,
                EyeStatus::LookForward,
                "friendly view-status assignment precedes return to duty/state change"
            );
        } else {
            assert_eq!(
                observation.eye_status,
                EyeStatus::Closed,
                "enemy view-status assignment follows state change"
            );
            assert!(
                !observation.timer_is_running,
                "enemy timer launch follows state change"
            );
        }
