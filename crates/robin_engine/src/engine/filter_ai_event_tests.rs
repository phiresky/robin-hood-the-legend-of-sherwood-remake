...
fn waiting_sword_execute_faces_world_xy_not_projected_map_xy() {
    ...
    let (_, _, execute_result) = engine.tick_actor_animation_for(
        &crate::sim_rng::test_context(),
        &LevelAssets::new(),
        actor,
    );
    ...
    for civilian_type in [CivilianType::Man, CivilianType::Beggar] {
        for animation in [OrderType::WaitingUpright, OrderType::Weeping] {
            ...
            reference.with_simulation_context(|reference, sim| {
                let scratch = reference.build_owner_context_scratch_without_forecast(&assets);
                ...
                reference.drain_direct_ai_owner_boundary_without_forecast(sim, id, &assets);
            });
            engine.with_simulation_context(|engine, sim| {
...
fn direct_ai_drain_consumes_civilian_blink_enemy_request() {
    ...
    assert!(
        !engine
            ...
            .actor
            .blink_all_enemies,
        "clearing an NPC's blinking enemy must settle for civilians as well as soldiers"
    );
}
