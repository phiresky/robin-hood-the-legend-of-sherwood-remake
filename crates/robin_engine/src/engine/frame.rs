...
//! the engine tick, then performs widgets, rendering, and sound
//! afterwards. This module names that
//! boundary without changing the established hourglass implementation or its
//! phase ordering.
//!
//! This is the authoritative engine portion of an original-game host-loop frame.
//! It can represent the original game skipping an actor update at a gate, preserves
...
/// simulation frame.
///
/// The structure encodes the only legal original-game phase order: every director
/// completion produced during refresh, followed by zero or one sound boundary and any recorded
...
pub enum ExternalAction {
    ...
    /// simulation paused. The enclosing frame journals this action
...
/// not represent a host iteration where simulation was skipped
...
pub struct SimulationFrameInput {
    ...
    /// Commands admitted before the simulation tick, in dispatch order.
    ...
    /// Commands admitted after the simulation tick, in dispatch order.
    ///
    ...
    /// Whether this host iteration crosses the engine tick.
...
pub enum SimulationCommandPhase {
    PreHourglass,
    PostHourglass,
}
...
pub enum FrameAdvanceError {
    #[error("ranked policy rejected {phase:?} settings command {index}: {field} is immutable")]
    RankedSimulationSettingCommandRejected {
        phase: SimulationCommandPhase,
        index: usize,
        field: super::RankedSimulationConfigField,
    },
    #[error("ranked simulation config differs at {field}")]
    RankedSimulationConfigViolation {
        field: super::RankedSimulationConfigField,
    },
    #[error("Spellforge mission aborted: {failure}")]
    SpellforgeMissionAborted {
        failure: crate::spellforge::SpellforgeGuestError,
    },
