...
fn bow_ground_y(point: crate::coordinates::WorldPoint3D) -> f32 {
    // The original game's ground position uses world X and Y.
...
fn bow_aim_height_command(
    ...
    // The original game's bow validation does not assign a shot type
    // when it returns OUT_OF_RANGE (and cannot provide an authoritative mode
    // for INVALID_TARGET either). Bow aiming must therefore leave the bow
...
/// orientation processing, after its action-state, animation, and shoot-list
...
impl EngineInner {
    ...
    pub fn is_point_on_sprite(
        ...
        // Box from the original game's sprite position, offset, and size.
    ...
    pub fn is_entity_focusable(
        ...
        if !self.fog_entity_visible(entity_id) {
            return false;
        }
    ...
    fn npc_focusable(
        ...
        match focus {
            ...
            // the sword-targeting branch returns directly without falling through
    ...
    ///      enabled or disabled pay cursor depending on remaining
    ///      ransom
    ///   1. Scroll-attached: talk cursor
    ///   2. Dead/unconscious (or tied with Untie enabled) + SEARCH →
    ///      search cursor
    ///   3. Unconscious + execute + lying: finish-him cursor
    ///   4. Unconscious + tie: tie cursor
    ///   5. Unconscious + resuscitate + same camp: wake-up cursor
    ///   6. Dead/unconscious + carry + not carried →
    ///      enabled or disabled pickup cursor
    ...
    ///   contextual action: short-leg cursor for climbing on shoulders
    /// - Otherwise: default cursor; the regular arrow stays put;
    ...
    ///     → enabled pickup cursor for macro recording
    /// else if (!is_takable(pc))
    ///     → disabled pickup cursor
    /// else if (is_unique)
    ///     → enabled pickup cursor
    /// else switch (quantity)
    ///     1..5 → numbered enabled pickup cursors
    ///     otherwise → plain enabled pickup hand
    /// ```
    ///
    /// Returns enabled, disabled, or numbered pickup cursor resource ids.
    ...
    pub fn can_shoot_with_bow_at(
        ...
        if target.is_human() {
            ...
            if status == BowTarget::Valid {
                if shoot == ShootMode::Long {
                    ...
                    if let Some(eyes) = target.compute_eyes_point(None) {
                        ...
                        return resolve_leaning_target((status, shoot), Some(head));
                    }
                }
                return (status, shoot);
            }
            ...
            return resolve_leaning_target((status, shoot), None);
        }

        // Non-human target — animals/objects. The original game checks range/LOS
        // against the target position; the later shot trajectory
    ...
    pub fn can_shoot_with_bow_at_point(
        ...
        // Get the shooter's bow profile. The original game uses the actor's
        // bow validation, so scripted NPC
        ...
        // for Down), matching the original game's bow-point calculation; the current hand point
    ...
    fn compute_trajectory_preview_to_point_from(
        ...
        // The original game's preview computes the source hand point using the
        // direction from the selected PC to the hovered destination for
        // every projectile action. Bow uses the long-shot bow point;
        ...
        // The original game's bow hover handling checks trajectory validity in
        // the cursor branch before the shoot-bow command is dispatched.
    ...
    /// Find any patch related to the given door, mirroring the original game.
    /// Checks both directions of
    ...
    fn turn_pc_throw(&mut self, pc_id: EntityId, ground_pt: GroundPoint) {
        ...
        // The original game uses the ground position here, i.e. the world-space
        // world-position X/Y pair. Projected map Y is
...
fn resolve_leaning_target(
    ...
    // Preserve the game's targeting quirk: a successful normal head shot
    // does not replace the valid long belt shot. A non-valid head status
    // with normal shooting mode does replace it.
