...
fn retain_legacy_grid_topology(
    ...
    // Serialized line references index the combined line array of a layer.
    ...
    for &chunk in &loaded.proto.grid_chunk_order {
        ...
        match chunk {
            ...
            ProtoGridChunk::Motion => {
                ...
                // Proto-stream motion-obstacle initialization constructs and
            ...
            ProtoGridChunk::Lift => {
                for lift in &loaded.proto.lifts {
                    ...
                    // number, but is not registered in either sector collection.
            ...
            ProtoGridChunk::Building => {
                for entry in &loaded.proto.buildings {
                    ...
                    if let Some(building_number) = building_number {
                        // A building sector is constructed before its doors
                        // and only added to the sector collection afterwards. Its
                        // public sector number is therefore this sparse
            ...
            ProtoGridChunk::Jump => {
                let jump_sector_numbers = loaded
                    ...
                    .map(|_| sectors.construct())
                    .collect::<Vec<_>>();
                topology.jump_sector_numbers = jump_sector_numbers
                    .iter()
                    .map(|&number| {
                        u16::try_from(number).expect("jump-sector creation number exceeds u16")
                    })
                    .collect();
...
/// Finish the identity that the original game establishes in
/// building-sector initialization and sector insertion.
///
...
/// sector slot. Every door owned by the building is then rewired to
...
fn validate_legacy_position_sector_bijection(
    ...
    // no saved-position reference role and therefore no sparse position binding.
...
/// the current mission's sparse sector topology has been retained.
/// Move recording passes this exact sector identity through to sequence
/// construction; reducing it to the public number makes
...
fn retain_script_location_sector_identities(
    ...
    let handles = script_objects
        ...
        // numeric sector index, not a sector reference resolved through
        // the script-sector array.
...
/// Resolve each serialized waypoint's sparse sector slot to the exact live
/// sector identity retained by the original game's save encoding.
...
fn append_patch_sector_constructors(
    ...
    // Patch initialization constructs old mouse, old masking, new mouse, new masking,
    ...
    for patch in patches {
        ...
        // The no-apply sector is constructed but never registered.
        sectors.construct();
    }
}
...
impl MissionLevelBuilder {
    ...
    /// `Bind("StartUp")` is required only when
...
/// anchor used by the position interface.
///
/// Loading animation position information stores the
/// serialized pair as the sprite's top-left and adds the sprite center when
...
///
/// Loading animation position information treats the
...
/// action initialization / waiting while constructing actors, before gameplay can
/// render them.
///
/// Keeping `last_action` initialized is also important for blipped NPCs:
/// Revealing a blip switches from the silhouette profile to the character profile
...
fn prime_mission_start_sprite(
    ...
    // action when it exists in the primary profile so blip reveal can select
...
pub fn load_mission_for_campaign(
    ...
    load_mission_for_campaign_with_files(
        campaign,
        profiles,
        level_directory,
        progress,
        &crate::sbfile::SbFile::snapshot_legacy_file_system(),
    )
}

/// Load every mission input, including optional authored patches, through one
/// explicitly supplied preparation authority.
pub fn load_mission_for_campaign_with_files(
    campaign: &crate::campaign::Campaign,
    profiles: &crate::profiles::ProfileManager,
    level_directory: &str,
    progress: &mut dyn FnMut(f32),
    ...
    if files.try_exists(&path).map_err(|status| {
        EngineError::Io(std::io::Error::other(format!(
            "probe {path}: file error {status}"
        )))
    })? {
        tracing::info!("Loading hackable level descriptor: {path}");
        let bytes = files.read_all(&path).map_err(|error| {
            EngineError::Io(std::io::Error::other(format!(
        ...
        progress(1.0);
        progress(1.0);
        return apply_loaded_level_patch(level, mission_filename, files);
    }
    ...
    let level = crate::level_data::load_level_with_files(
        mission_filename,
    ...
    apply_loaded_level_patch(level, mission_filename, files)
}

/// Apply authored JSON patches after decoding a raw or shipping level.
pub fn apply_loaded_level_patch(
    level: crate::level_data::LoadedLevel,
    mission_filename: &str,
    files: &crate::sbfile::SbFileSystem,
) -> Result<crate::level_data::LoadedLevel, EngineError> {
    for (old_suffix, new_suffix) in [
        ("characters.patch.json", "level.patch.json"),
        ("text.patch.json", "descriptors.patch.json"),
    ] {
        crate::content_patch::reject_legacy(
            files,
            &format!("Data/Levels/{mission_filename}.{old_suffix}"),
            &format!("Data/Levels/{mission_filename}.{new_suffix}"),
        )
        .map_err(|reason| EngineError::MissionLevelStage {
            stage: "JSON level patch",
            reason,
        })?;
    }
    let path = format!("Data/Levels/{mission_filename}.level.patch.json");
    let patched =
        crate::content_patch::apply_with_files(&level, files, &path).map_err(|reason| {
            EngineError::MissionLevelStage {
                stage: "JSON level patch",
                reason,
            }
        })?;
    Ok(patched.unwrap_or(level))
}
...
fn spawn_patch_fx_entities(
    ...
    for (local_patch_idx, raw) in patches.iter().enumerate() {
        ...
        let mut sprite = crate::sprite::Sprite::default();
        // Patch initialization always allocates and owns an
        // effect element, even when sprite loading reads an empty frame
...
impl EngineInner {
    ...
    pub(crate) fn attach_accessory_sprite(
        ...
        // The accessory master supplies immutable script/profile data, but
        // The original game's per-instance sprite setup owns this runtime
        // sentinel. Do not replace it with the level-load prototype's value.
        let last_processed_order_id = entity.element_data().sprite.last_processed_order_id;
    ...
    pub(crate) fn preload_campaign_peasant_sprites(
        ...
        for (profile_index, filename, profile_name, alternate_profile) in profiles {
            ...
            // Player-actor initialization loads the alternative track
            ...
            // a serialized alternate-profile flag selects an empty Rust
    ...
    pub(crate) fn initialize_from_mission(
        ...
        // install the original-game sector identity immediately.
        retain_legacy_grid_topology(assets, &loaded, config.script_enabled)?;
        let legendary_soldiers =
            self.apply_legendary_reinforcements(assets, &mut loaded, mission_name, config)?;
        ...
        if !legendary_soldiers.is_empty() {
            let authored_soldier_count = loaded.mission.soldiers.len();
            self.spawn_soldier_slice(
                assets,
                &legendary_soldiers,
                loaded.mission.civilians.len(),
                authored_soldier_count,
                config,
            )?;
            // Keep the loaded mission's roster consistent for later
            // diagnostics and mission-domain setup. Spawning happens only
            // after every authored entity so their legacy handles remain
            // unchanged.
        ...
        // Jump overlays are non-position sectors. Restore their sparse object
        // identity only after the position-sector lookup is rebuilt so legacy
        // group-move translation continues to treat them as overlays.
        self.restore_jump_overlay_sector_numbers(assets);
        ...
        for (layer_idx, layer_areas) in motion_data.layers.iter().enumerate() {
            ...
            for area in layer_areas {
                ...
                for current in 0..poly.points.len() {
                    // The original game seeds the previous point from the final
                    // polygon vertex, so the closing edge is inserted first,
                    // followed by 0→1, 1→2, ... . Position authorization
                ...
                // mapping used by motion-sector line activation so a
    ...
    fn register_motion_sectors(
        ...
        // AREA). This enables map-space and screen-space sector queries.
        {
            ...
            //
            // The sector word in the lift chunk is a sector number, not
            ...
            // GridSector. The original game reads the lift's associated click sector
            // during lift-sector initialization, but the two
            // associated-sector additions are disabled,
            ...
            // The original game constructs one real motion-area sector for the
            ...
            //
            // TODO(parity): retain its sparse sector slot through
    ...
    pub(crate) fn load_jump_lines_from_proto(&mut self, staging: &mut LevelLoadStaging) {
        ...
        // jump-line initialization adding the line to the jump-sector line
        // map. The line's motion-sector
        // association is assigned separately below from the paired
        // zone, as during fast-grid motion initialization.
        ...
        for pair in line_pairs {
            ...
            let jl2_helper_needed = jl2.helper_needed;
            {
                ...
                // nearest-jump-line selection can iterate without a global scan.
    ...
    /// Restore the sparse sector identities of runtime jump overlays.
    ///
    /// Motion registration precedes construction of the complete retained
    /// topology, so jump sectors are initially installed with the sentinel
    /// public number `-1`. Original constructs the overlays in proto-zone
    /// order; preserve that alignment once the sparse constructor numbers are
    /// available.
    fn restore_jump_overlay_sector_numbers(&mut self, assets: &LevelAssets) {
        ...
        let jump_indices = self
            ...
            .iter()
            .enumerate()
            .filter_map(|(index, sector)| sector.sector_type.is_jump().then_some(index))
            .collect::<Vec<_>>();
        assert_eq!(
            jump_indices.len(),
            jump_sector_numbers.len(),
            "retained jump-sector identities disagree with runtime jump overlays"
        );

        let level = self.world.fast_grid_mut().level_mut();
        for (&index, &number) in jump_indices.iter().zip(jump_sector_numbers) {
            let number = crate::sector::SectorNumber::new(
                i16::try_from(number)
                    .expect("Original jump-sector number exceeds signed-word domain"),
            );
            level.sectors[index].sector_number = number;
        }
    }
    ...
    fn cache_door_ai_metadata(&mut self) {
        self.ai.global.door_seek_infos = self
            ...
            .map(|(idx, door)| {
                ...
                // authorization used to find a door an enemy could be behind.
            ...
            .collect();
        tracing::debug!(
            "Cached {} door seek infos for concealed-enemy searches",
            self.ai.global.door_seek_infos.len(),
        );

        // Populate reinforcement door info for forest retreats.
        self.ai.global.reinforcement_doors = self
            ...
            .collect();
        tracing::debug!(
            "Cached {} reinforcement doors for forest retreats",
    ...
    /// Resolve an original-game sector slot to the paired public sector
    /// number and exact live arena object. Door stream endpoints use this
    /// sparse pointer space used during gate initialization.
    ...
    /// Resolve an original-game sparse sector slot into the exact saved position
            ...
            for raw in &lift.doors {
                ...
                self.script_domains
                    ...
                    .push(crate::gate::Door {
                        ...
                        sector_out_index: Some(sector_out_index),
                        sector_in_index: Some(sector_in_index),
                        // The lift sector owns every door embedded in this
                        // CHUNK_LIFT record. The owning sector is the chunk's
                        // the sector field (`motion_area_index` here), not
    ...
    fn build_door_lift_attachment_stage(&mut self) -> Result<(), MissionLevelBuildError> {
        ...
        // so map-space and screen-space sector queries can find them during click hit-testing.
        {
            use crate::sector::SectorType;
        ...
        // Original-game lift sectors expose high/low exit points for
        // movement-animation selection. Populate the Rust cache after the
    ...
    fn build_patch_stage(
            ...
            .enumerate()
        {
            ...
            // Mirrors the original game's patch registration
            // — the late layer read at the end of patch initialization
            ...
            //     non-empty
            // Both get registered as sectors with sector lines
            ...
            self.script_domains
                ...
                .push(crate::patch::Patch {
                    ...
                    // index may itself be zero (`(1 - 1) >> 1`), while the original-game
                    // loader still enables changing obstacles in that case.
                    ...
                    // and the selected patch's layer.
        ...
        // points to itself when door-triggered, or as the
        // patch→door swap-rights list when it triggers a door.
    ...
    /// building's first gate, matching building occupant initialization.
    fn attach_mission_level_stage(
        ...
        for building in &stages.buildings.attachments {
            ...
            // Building occupant initialization resolves the first gate after
            // door-point adaptation, so attachment must use this canonical
            ...
            for &element_index in &building.tenant_element_indices {
                ...
                // Human actor layer-and-sector changes copy the
                // building pointer and lift layer to a PC's carried actor.
                // Occupant initialization then changes only the tenant's position.
    ...
    /// finalize amounts, spawn MAKE_* bonuses at production
    ...
    pub(crate) fn apply_production_sector_data(
        ...
        // Resolve last-mission info — drives amount, experience, and life-point updates.
        ...
        // Resolve per-sector specialist flags and update amounts (adds
