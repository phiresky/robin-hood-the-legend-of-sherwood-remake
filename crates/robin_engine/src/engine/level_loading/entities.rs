...
impl EngineInner {
    pub(super) fn spawn_proto_entities_stage(
        ...
        //   4. BONU bonuses
        //   5. PARC scrolls
        //   6. GUYS tenants  (not represented as entities — see note below;
        //                      occupant initialization consumes GUYS already)
        //   7. PCs from beam-mes (one slot per beam-me, empty if unfilled)
        //
    ...
    pub(super) fn spawn_mission_patch_entities_stage(
        &mut self,
        assets: &mut LevelAssets,
        loaded: &crate::level_data::LoadedLevel,
    ) {
        // Mission PATCH_2 precedes ELEMENT in shipped mission streams. The
        // patch animations therefore enter the flat script-element array
        // between prototype effects and mission actors (the mission chunk
        // loop initializes patches immediately from their prototypes). They must
        // also exist before legacy grid topology retains each patch's
        // serialized effect-element identity.
    ...
    pub(super) fn spawn_civilians_and_rescue_pcs_stage(
        ...
        for raw in &loaded.mission.pcs_to_rescue {
            ...
            // character profile right after frame-metadata loading, the same
            ...
            // dynamic status block referenced by the original-game PC.
                    ...
                    _ => {
                        ...
                        if profile.vip {
                            ...
                            for _ in 0..MAX_ATTEMPTS {
                                // Original-game name generation draws randomness once
    ...
    pub(super) fn spawn_soldier_slice(
        &mut self,
        assets: &mut LevelAssets,
        soldiers: &[crate::level_data::RawSoldier],
        civilian_count: usize,
        soldier_index_offset: usize,
        config: SimConfig,
    ) -> Result<(), EngineError> {
        let profiles = assets.profile_manager.clone();
        let char_base_dir = "Data/Characters";
        ...
        for (local_soldier_index, raw) in soldiers.iter().enumerate() {
            let soldier_index = soldier_index_offset
                .checked_add(local_soldier_index)
                .expect("soldier load index exceeds usize");
            let npc_register_number = civilian_count
            ...
            // Set the sprite's move box + pathfinder index from the
            // soldier profile right after frame-metadata loading.
    ...
    pub(super) fn spawn_targets_stage(
        ...
        // `Data/Animations/<ambiance>/`), then the action and direction are
        // applied. Accessory targets reuse the object-master
        ...
        for raw in &loaded.mission.targets {
            ...
            match sprite.load_frame_info(
                ...
                Ok(()) => {
                    // Apply the action and direction to the animation.
    ...
    pub(super) fn spawn_scrolls_stage(
        ...
        // (forced visibility sets visible status). Collect the handles here
        ...
        for raw in &loaded.mission.scrolls {
            ...
            // activate when `is_to_be_replaced_by_amulet`
            // (Easy + presence[Easy]==false, so the scroll spawns in
            // place to later morph into an amulet), else
            // activate according to difficulty-specific presence. Skipping this left every
        ...
        // building occupants; occupant initialization runs after
