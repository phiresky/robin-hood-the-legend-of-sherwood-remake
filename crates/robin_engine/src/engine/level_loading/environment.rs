...
/// Match original-game sight-obstacle initialization: it builds both
...
impl EngineInner {
    ...
    pub(super) fn load_environment_stage(
        ...
        // references and are invisible to sound-sector queries
        ...
        //
        // Original-game spatial-grid initialization registers the default
        // sight obstacle's material list at layer 0
        // and then loads every obstacle,
        ...
        if let Some(ref tactic) = loaded.mission.tactic_data {
            ...
            // Archery sectors are populated during AI initialization from tactic data.
        ...
        let static_obstacles: Vec<crate::sight_obstacle::SightObstacle> = loaded
            ...
            .map(|(idx, raw)| {
                ...
                // reads (e.g. water/hole material lookup)
    ...
    pub(super) fn install_tactic_seek_points_stage(
        ...
        for raw in &tactic.seek_points {
            // The tactic stream stores an original-game sector slot,
            // and original-game directional seek-point initialization
            // resolves it through the stored sector index. Retain that exact
    ...
    /// sector slot, and original-game ambush-point initialization
    /// resolves it as a stored sector index rather than treating it as the
    ...
    pub(super) fn install_tactic_ambush_points_stage(
        ...
        for raw in &tactic.ambush_points {
            self.ai.global.ambush_points.push(crate::ai::AmbushPoint {
                ...
                direction: 0,
                // AI initialization lifts the point onto its projection plane and
        ...
        // source manager, filtering by the current ambiance bitmask.
        {
            ...
            for raw in &loaded.proto.sound_sources {
                if !runtime_switchable && raw.ambience_filter & ambiance_mask == 0 {
                    // Preserve original-game handle alignment for ordinary
    ...
    /// Register every original-game sound edge after the fast grid has
    ...
    /// retains its owning material sector.
