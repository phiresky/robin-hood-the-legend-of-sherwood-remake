...
impl EngineInner {
    pub(super) fn spawn_mobile_elements_stage(
        ...
        // master is simulation-only, matching the original game: only its child
        // Masked effect elements enter the general element/render array.
        for (mobile_index, raw_mobile) in loaded.mission.mobile_elements.iter().enumerate() {
            if raw_mobile.sprites.is_empty() {
                return Err(EngineError::MissionLevelStage {
                    ...
                    reason: format!(
                        "mobile {mobile_index} has no first masked child for its update owner boundary"
                    ),
                });
            }
                ...
                let entity = Entity::Fx(crate::element::ElementFx {
                    ...
                    fx: crate::element::FxData {
                        ...
                        display_polyline: raw
                            .display_polyline
                            .iter()
                            // Effect elements read this as absolute map geometry.
    ...
    pub(super) fn load_mission_script_stage(
        ...
        // Original-game engine initialization binds StartUp only when scripting is enabled.
        ...
        // Flush `force_visible` scroll visibility, assigning status
    ...
    pub(super) fn install_reinforcement_doors_stage(
            ...
            && !tactic.reinforcement_points.is_empty()
        {
            ...
            // The original game retains one real motion-area sector reference for the
            ...
            for raw in &tactic.reinforcement_points {
                // The sector field in the reinforcement chunk is an original-game sparse
                // sector slot, not the displayed sector number.
                // The spatial grid resolves it from the sector index and
                // stores that exact sector reference on the door.
                ...
                // Original-game reinforcement-point initialization
                // narrows both computed points through signed 16-bit values before passing
                // them to the door's midpoint and exit points. Diagonal exits normally
                // have fractional intersections/steps, so retaining those
                // fractions shifts every later exit-point movement goal.
                ...
                // Point adaptation is a no-op for `Reinforcement` doors
