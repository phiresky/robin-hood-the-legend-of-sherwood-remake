...
/// Beam-me assignment state flowing between the Phase A assignment steps of
/// [`EngineInner::spawn_beam_me_pcs_stage`]. No serde:
/// `SherwoodPlacementRoll` (engine/teleport.rs) has no serde derive, and
/// this is transient per-call state that is never persisted.
struct BeamMeAssignment {
    /// Mission team list: (char_idx, profile_idx).
    team: Vec<(usize, crate::profiles::CharacterProfileIdx)>,
    team_remembered_sherwood_slot: Vec<i16>,
    assignments: Vec<Option<usize>>,
    instanced: Vec<bool>,
    sherwood_placement_rolls: Vec<Option<super::teleport::SherwoodPlacementRoll>>,
}

/// One Phase A spawn-plan entry:
/// (char_idx, profile_idx, beam_me_idx, pre-shuffle Sherwood placement roll).
type BeamMePcSpawnPlanEntry = (
    usize,
...
/// Sprite and PC attributes produced by
/// [`EngineInner::build_beam_me_pc_sprite`] and consumed by
/// [`EngineInner::add_beam_me_pc_entity`]. No serde: `Sprite` has no serde
/// derive, and this is transient per-call state that is never persisted.
struct BeamMePcSpawn {
    sprite: crate::sprite::Sprite,
    kind: Option<crate::character_kind::CharacterKind>,
    is_robin: bool,
    has_lockpick: bool,
    has_climb: bool,
    has_jump: bool,
    initial_posture: crate::element::Posture,
    initial_action_state: crate::element::ActionState,
    list_index: u8,
}
...
impl EngineInner {
    pub(super) fn spawn_beam_me_pcs_stage(
        ...
        let is_sherwood;
        {
            ...
            let mut assignment = Self::seed_beam_me_assignment(campaign, loaded)?;

            // Phase 1: Handle required characters.
            Self::assign_required_beam_me_pcs(&profiles, loaded, &mut assignment);
            ...
            // actually spawned.
            if is_sherwood {
                Self::seat_sherwood_returners_and_shuffle(sim, loaded, &mut assignment);
            }

            Self::fill_remaining_beam_me_slots(
                campaign,
                &profiles,
                loaded,
                is_sherwood,
            ...
            pc_spawn_plan = Self::collect_beam_me_spawn_plan(campaign, loaded, &assignment);
        }
        ...
        for (bm_idx, beam_me) in loaded.mission.beam_mes.iter().enumerate() {
            ...
            if let Some(&(char_idx, profile_idx, _, sherwood_placement_roll)) = plan_entry {
                ...
                let spawned_eid =
                    self.add_beam_me_pc_entity(profile, profile_idx, beam_me, char_idx, spawn)?;
                pc_count += 1;

                self.record_beam_me_pc_sherwood_slot(
                    beam_me,
                    char_idx,
                    is_sherwood,
                    sherwood_placement_roll,
                    spawned_eid,
                );
            } else {
    ...
    /// snapshot remembered Sherwood slots and allocate the assignment state.
    fn seed_beam_me_assignment(
        campaign: &mut crate::campaign::Campaign,
        loaded: &crate::level_data::LoadedLevel,
    ) -> Result<BeamMeAssignment, EngineError> {
        ...
        let team: Vec<(usize, crate::profiles::CharacterProfileIdx)> = campaign
            ...
            .map(|&char_idx| {
                ...
                let profile_idx = description.character_profile_idx.ok_or_else(|| {
                    EngineError::MissionLevelStage {
                        stage: "beam-me assignment",
                        reason: format!("campaign character {char_idx} has no character profile"),
                    }
                })?;
        ...
        let assignments: Vec<Option<usize>> = vec![None; loaded.mission.beam_mes.len()];
        let instanced = vec![false; team.len()];
        // Pre-rolled placement values are kept by team member until
        // Phase B can create and mutate the corresponding entity.
        let sherwood_placement_rolls = vec![None; team.len()];

        Ok(BeamMeAssignment {
            team,
            team_remembered_sherwood_slot,
            assignments,
            instanced,
            sherwood_placement_rolls,
        })
    }

    /// Phase 1: seat the team members named by each beam-me's `required_pc`.
    fn assign_required_beam_me_pcs(
        profiles: &crate::profiles::ProfileManager,
        loaded: &crate::level_data::LoadedLevel,
        assignment: &mut BeamMeAssignment,
    ) {
        let BeamMeAssignment {
            team,
            assignments,
            instanced,
            ..
    ...
    /// Sherwood branch body: seat remembered Sherwood returners and shuffle
    /// the beam-me slots (see the comment at the call site).
    fn seat_sherwood_returners_and_shuffle(
        sim: &crate::sim_rng::SimulationContext,
        loaded: &mut crate::level_data::LoadedLevel,
        assignment: &mut BeamMeAssignment,
    ) {
        ...
        for ti in 0..team.len() {
            ...
            sherwood_placement_rolls[ti] = Some(super::teleport::roll_sherwood_placement(sim));
        }
    ...
    /// Phase 2: fill the remaining beam-me slots, honoring the per-beam-me
    /// action requirements.
    fn fill_remaining_beam_me_slots(
        ...
        let slot_valid_for = |beam_me: &crate::level_data::BeamMe,
         ...
         -> bool {
            // The original game's slot-validity check receives an index
            // into mMissionTeam, but (despite that contract) indexes
            // the gang array with it. Preserve this observable bug: it controls
        ...
        while assignments.iter().any(|a| a.is_none()) && !available.is_empty() {
            ...
            for (bm_idx, beam_me) in loaded.mission.beam_mes.iter().enumerate() {
                ...
                let candidates: Vec<usize> = available
                    ...
                    .collect();
                let pick = if candidates.len() == 1 || (force_decision && !candidates.is_empty()) {
                    Some(candidates[0])
                } else {
                    None
                };
    ...
    /// Collect the spawn plan and mark the planned characters instanced in
    /// the campaign.
    fn collect_beam_me_spawn_plan(
        campaign: &mut crate::campaign::Campaign,
        loaded: &crate::level_data::LoadedLevel,
        assignment: &BeamMeAssignment,
    ) -> Vec<BeamMePcSpawnPlanEntry> {
        let BeamMeAssignment {
            team,
            assignments,
            sherwood_placement_rolls,
    ...
    /// Phase B profile resolution: beam-me profile override, Robin
    /// forest/town swap, profile lookup and override write-back.
    fn resolve_beam_me_pc_profile<'p>(
        &mut self,
        profiles: &'p crate::profiles::ProfileManager,
        ...
        (
            crate::profiles::CharacterProfileIdx,
            &'p crate::profiles::CharacterProfile,
        ),
        EngineError,
    > {
        ...
        // stored `character_profile_idx`.
        {
                ...
                && is_town != want_town
            {
                ...
                if let Some(new_idx) = profiles.characters.iter().position(|p| {
                    CharacterKind::from_profile(&p.filename, &p.profile_name) == Some(target_kind)
                }) {
                    let new_profile_idx = crate::profiles::CharacterProfileIdx(new_idx as u32);
    ...
    fn build_beam_me_pc_sprite(
        ...
        char_base_dir: &str,
        bank_signature: u32,
    ) -> Result<BeamMePcSpawn, EngineError> {
        ...
        let list_index = u8::try_from(char_idx).map_err(|_| EngineError::MissionLevelStage {
            stage: "beam-me PC spawn",
            reason: format!(
        ...
        // character profile right after frame-metadata loading so
    ...
    /// Phase B entity creation: disabled action slots, status mirror and
    /// the `ActorPc` entity itself. Returns the spawned entity id.
    fn add_beam_me_pc_entity(
        &mut self,
        profile: &crate::profiles::CharacterProfile,
        profile_idx: crate::profiles::CharacterProfileIdx,
        beam_me: &crate::level_data::BeamMe,
        ...
        let entity = Entity::Pc(crate::element::ActorPc {
            ...
            actor: crate::element::ActorData {
                ...
                // Action state from action initialization.
    ...
    /// Phase B post-spawn: Sherwood slot write-back and Sherwood-returner
    /// position randomisation.
    fn record_beam_me_pc_sherwood_slot(
        &mut self,
        beam_me: &crate::level_data::BeamMe,
        char_idx: usize,
        is_sherwood: bool,
        sherwood_placement_roll: Option<super::teleport::SherwoodPlacementRoll>,
        spawned_eid: EntityId,
    ) {
