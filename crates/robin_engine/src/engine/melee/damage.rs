...
/// Direction-sector conversion consequently leaves every comparison bit
...
impl EngineInner {
    ...
    /// translators. Generic, arrow, sword, hit, and push damage translation
    /// explicitly disables direction computation on direct
    ...
    /// separate roll-translation helper deliberately retain the default `true`.
    ...
    /// only registered here: the original game's sequence-element launch leaves it
    /// for sequence-manager processing after every actor has performed
    ...
    pub(crate) fn queue_scaled_sword_damage(
        ...
        // Resolve the value normally determined by instruction handling, but bypass the
    ...
    pub(super) fn apply_sword_damage(
        ...
        // Human life updates invoke death handling synchronously, before
        // Sword-damage reception returns to sword-damage translation. In
        // particular, Kill clears an unconscious victim before the later
        // hurt-speech call, so a lethal hit says DIES rather than taking
        // the unconscious early return. Push strikes already close
        ...
        // records bad sword-strike experience from the damage origin's command.
        // This is before the sound/parry return in
        // human-actor instruction, so even a fully parried hit updates
        ...
        // strikes.
        {
            ...
            if result.contains(combat::SwordDamageResult::NO_DAMAGE_PARRIED) {
                ...
                // strikes therefore still run push-damage translation (including
                // its fall/flight and hurt speech) when parried. Only an ordinary
        ...
        // lethal hit saved by an amulet is different: player wound handling
        // establishes the coma and sets lying posture inside that
        // dispatch boundary, so the later PC posture switch takes its grounded
        ...
        // Sword-damage reception returns to push-damage translation. Apply that
        ...
        // the one-shot death cascade; push-damage translation may still author
        ...
        // Sword-damage translation terminates immediately for a human who is
        ...
        // still the selected actor-instruction pointer at this point, so its
        // synchronous condolence makes instruction handling return before the ordinary
        ...
        if grounded_translation_terminates {
            ...
            // callback resumes a postponed successor before the strike
            // translator continues (for example a PC's interrupted Move,
            // whose lying-posture transition becomes StandingUpSword).
        ...
        // their life-point-update speech edge was handled above.
        if !coma_saved
            && !pushed
            && !result.is_empty()
        ...
        // remark.
        //
        // Push strikes never enter sword-damage translation. Its posture switch
        ...
        let informer_reachable = {
            ...
            // shoulder-damage translation. It therefore never reaches the base
        ...
        // Life-point updates refuse to apply death handling twice, but
        // Sword-damage translation still runs after that early return. Until
        ...
        // actor sequence-element and order fields after it interrupts the first one.
        ...
        if !pushed && !result.is_empty() {
            if victim_went_unconscious {
                ...
                // reaches sword-damage translation: upright victims leave their
    ...
    pub(crate) fn force_drop_carried_corpse_instant(&mut self, carrier_id: EntityId) {
        ...
        if let Some(carried) = self.get_entity_mut(carried_id) {
            ...
            // before clearing the
            // back-reference. Direction assignment writes only the *goal*
    ...
    pub(super) fn apply_generic_damage(
        ...
        // Civilian actors override only wound handling and
        // concussion handling; human translation still runs
        // the command-specific reaction after those no-op primitives return
        // for the command-specific reaction.
    ...
    pub(super) fn apply_piercing_damage(
        ...
        // Attached-scroll civilian immunity belongs to the wound and
        // concussion primitives, not around piercing damage's later
        ...
        // Piercing damage is applied before translating the hit,
        ...
        // piercing-damage reception
        // The original game wounds the victim
        // before arrow-damage translation runs, and human life updating
        // invokes the kill chain
        // synchronously inside that call. `Kill`
        // quits the swordfight and snaps the
        // AI to SLEEPING_FOREVER regardless of the victim's posture, while
        // arrow-damage handling merely
        ...
        let life_points_after = self
            ...
            .map(get_life_points)
            .unwrap_or(raw_life_points_after);
        // Updating player life points speaks
        // HERO_DIE / HERO_HURT off the stored life-point edge, inside the same
        // wounding call. The human pain-reaction speech
        ...
        // Human arrow-damage translation opens with the pain reaction before its
        // posture switch. In particular,
        ...
        // this call: player arrow-damage translation may take its shoulder override,
        // and human pain reaction is a no-op for every player path anyway.
        if !self
        ...
        // Player actors override arrow and generic damage translation:
        // these three PC postures never enter the Human posture switch below,
        // but instead dispatch shoulder-damage translation. Keep this actor-specific
        ...
        // Posture::is_lying here: this classification also includes Flying,
        ...
        // owner still receives Dying*; human life updating does nothing for the
        ...
        //
        // A newly lethal element gets its dying order from the death-processing
        ...
        if still_on_ground {
            let translated_anim = if life_points_before <= 0 && life_points_after <= 0 {
                ...
                // tests only death: a surviving victim always extracts
                // the arrow, even while unconscious. Only the generic
                // generic damage handling has
                ...
                // generic-damage translation's backward-fall animation plus swordfight exit
                // currently gives it no order
    ...
    pub(super) fn apply_hit_damage(
        ...
        // The receive-hit-damage command unconditionally continues from
        // concussion accumulation into hit-damage translation. A civilian with
        // an attached scroll suppresses the concussion primitive, but still
        // authors the falling-hit order.
        if let Some(attacker_id) = attacker_id {
        ...
        // Concussion handling performs the knockout transition inline:
        // Leave swordfight (including its synchronous AI callback), add the
        ...
        if went_unconscious {
            ...
            // RECEIVE_HIT_DAMAGE explicitly leaves swordfight once more
            // after concussion handling. The relationship list is
        ...
        // Original-game hit-damage translation calls
        // hurt speech synchronously before it appends the FALLING_HIT order.
        ...
        // that pending exclamation before sound ticking resolves the
        ...
        // completed synchronously above, and hit-damage translation's
    ...
    /// Hit-damage translation selects and appends the `FALLING_HIT_*` order.
    ///
    /// This deliberately does not initialize non-hard flight state.
    /// The original game's hit-damage translation only stores the antagonist and an
    /// order with direction computation disabled; falling-hit execution samples
    /// the live positions, changes facing, and prepares takeoff when
    ...
    pub(super) fn dispatch_hit_fall_animation(
        ...
        let anim = match select_hit_fall_animation(victim_posture, victim_action, is_harder_hit) {
            Some(a) => a,
            None => {
                // Hit-damage translation terminates these
    ...
    ///
    /// This is the original game's initialization branch
    /// hit-induced falling: it samples live actor positions, sets the
    ...
    pub(crate) fn initialize_hit_flight(
        ...
        let (
            ...
            (
                ...
                // The original game copies the victim's exact position
                // sector reference into takeoff preparation. Legacy saves can retain
                // only its public number, so recover the omitted arena
                // identity from the live point/layer before projecting the
                // landing plane. Public sector 0 is authored and valid; it is
                // not a null-pointer sentinel.
        ...
        let chosen = chosen.filter(|point| {
            victim_sector
                .and_then(|sector| {
                    crate::engine::movement::grid_sector_for_position_handle(
        ...
        // Takeoff preparation retains its already-cached 3D takeoff point while
    ...
    pub(super) fn handle_post_damage(
        ...
        // Human life updating returns immediately for a dead actor, so damage
        // delivered to an already-dead actor may still perform its authored
        // protection rolls but cannot invoke the death-processing cascade again.
    ...
    /// scroll. The civilian's wound / concussion primitives are
    ...
    ///
    /// Original-game arrow and stone impacts
    /// launch sequence elements from the projectile's entity tick
    /// slot. That only appends the damage element to
    /// pending sequence-element queue; sequence-manager processing instructs
    ...
    /// actor's next update installs its ordinary `Wait`, whose
    ...
    /// separate from death handling because life-point updates complete the
    ...
    pub(super) fn apply_knockout_side_effects(
        ...
        if let Some(npc) = victim.ai_actor_data_mut() {
            ...
            // swordfight exit, titbit, and healing work and before AI processing.
            npc.clear_all_suspects();
        }

        // NPC concussion handling invokes AI inline
        ...
        if let Some(npc) = victim.ai_actor_data_mut() {
            // The original game assigns this only after the loss-of-consciousness event
            // returns.
