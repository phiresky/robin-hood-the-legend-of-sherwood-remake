...
///
/// Human falling-pushed execution accepts exactly these three
...
fn push_flight_vector(
    ...
    match kind {
        ...
        WeaponThrustKind::TrueCircle | WeaponThrustKind::FalseCircle => {
            // Original-game vector norm first rounds the squared sum
...
/// Facing selected during pushed falling after computing its flight.
///
/// The original game computes the directional sector unconditionally. A coincident circular
...
/// Select the animation queued for push damage.
///
/// Dead riders take precedence over other posture rules, so even a
...
/// Immediate posture change performed during push-damage translation.
///
...
/// The original game's movement-box query currently ignores its posture argument and
/// returns the actor's live primary movement box. Roll destination selection translates
...
impl EngineInner {
    ...
    pub(super) fn translate_shoulder_damage(
        ...
        // Shoulder translation requests hurt speech. NPCs handle it differently;
        // PCs inherit the human actor's no-op and receive any hurt/death
        // speech from their life-point updates at damage-apply time.
        if matches!(
    ...
    pub(super) fn apply_push_effect(
        ...
        // Push damage triggers pain speech for NPCs, but not PCs.
        if matches!(
            self.get_entity(victim_id),
            Some(Entity::Soldier(_) | Entity::Civilian(_))
        ) {
        ...
        if let Some(anims) = push_anims {
            ...
            // Pushed falling owns the flying state on motion start and the
            ...
            if is_dead && !death_cascade_already_applied {
                ...
                // including when the victim was already
    ...
    pub(crate) fn initialize_push_flight(
        ...
        let (victim_pos, victim_z, layer, sector, move_box, rider, frames) = {
            ...
            (
                ...
                // only the public number, so reconstruct that omitted pointer
                // from the victim's live point and layer before looking up
                // the landing projection. Public
                // sector 0 is authored data, not a null-pointer sentinel.
                crate::engine::ai::ai_view_position_sector(self, victim.element_data()),
                // The original game's lying-posture movement box currently returns the
                // primary move box regardless of posture.
        ...
        // Sprite takeoff preparation updates the position obstacle,
        // not obstacle/material assignment: install the goal plane immediately but
        ...
        let position = self
            ...
            .expect("falling-pushed victim vanished")
            .position_iface_mut();
        // The original game captures the actor's 3D position before
        // installing the landing obstacle. That assignment only invalidates the
        // lazy 3D cache; the following increment and position updates
    ...
    pub(super) fn obstacle_top_normal(obstacle: &crate::sight_obstacle::SightObstacle) -> [f32; 3] {
        ...
        // The plane-normal calculation divides each
        ...
        // destination calculation, and from there the normalized
    ...
    pub(super) fn find_roll_point(
        ...
        // The original game's lying-posture movement box currently returns the common movement box
    ...
    pub(super) fn try_queue_roll(
        ...
        // Its execution arm drives ordinary motion processing directly.
        self.push_translated_roll_order(damage_element, dest);
    }

    /// Append the positional order produced by the original game's roll translation. Unlike
    /// the direct damage-reaction animations, this order retains the default
    /// default direction computation enabled.
    ...
    pub(super) fn execute_multi_target_strike(
        ...
        let mut victims = match kind {
            ...
            WeaponThrustKind::PushAside => {
                // Element direction-vector lookup builds the literal 16-sector
                // vector with ASPECT_RATIO before push sword-strike execution
        ...
        // engine actors in lookup order
        // (using the same actor walk
...
mod tests {
    ...
    fn ready_for_takeoff_uses_vector_reciprocal_multiplication_rounding() {
        ...
        // while the original game's 3D vector operation produces c0636d01. The
