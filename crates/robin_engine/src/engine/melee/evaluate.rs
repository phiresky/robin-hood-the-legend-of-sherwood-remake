...
fn is_within_smalltalk_strike_range(maximal_range: u16, squared_distance: f32) -> bool {
    ...
    // The original game widens the unsigned 16-bit range to 32 bits, squares it, and narrows the
    // floating-point squared norm to an unsigned 32-bit value before comparing.
...
pub(super) fn opponent_sword_strike_time_limit(
    ...
    match action_done_timing() {
        ...
        // This state has no valid completion frame. The original game
        // produced -16230 at S075 frame 731; use a
...
#[derive(Clone, Copy)]
enum UncapturedImpossibleDeadline {
    Strict,
    Unavailable,
}
...
impl EngineInner {
    ...
    /// Actor animation selection exposes the live selected order, while the
    /// frames-until-action-done query still reads the sprite's
    /// current row even when that order has not executed yet
    /// during opponent maintenance.
    ///
    ...
    /// running signed 16-bit value. That allocator residue is not reproducible from game
    ...
    pub(super) fn opponent_sword_strike_time_limit_for_actor(
        ...
        self.opponent_sword_strike_time_limit_for_actor_with_fallback(
            proposer,
            opponent,
            UncapturedImpossibleDeadline::Strict,
        )
    }

    /// The old schema-12/15 traces predate captured strike deadlines.  At an
    /// impossible action point the shipping executable's unchecked delay-row
    /// read can therefore only be recovered from the recorded proposal
    /// outcome. Swordfight reconsideration in those traces observed the legacy
    /// `-1` (unavailable) result, while reactive parade's S075 control observed
    /// a negative wrapped deadline and must remain strict.
    pub(super) fn enemy_reconsider_sword_strike_time_limit_for_actor(
        &mut self,
        proposer: EntityId,
        opponent: EntityId,
    ) -> Option<i16> {
        self.opponent_sword_strike_time_limit_for_actor_with_fallback(
            proposer,
            opponent,
            UncapturedImpossibleDeadline::Unavailable,
        )
    }

    fn opponent_sword_strike_time_limit_for_actor_with_fallback(
        &mut self,
        proposer: EntityId,
        opponent: EntityId,
        uncaptured: UncapturedImpossibleDeadline,
    ) -> Option<i16> {
        ...
        let timing = if timing == crate::sprite::ActionDoneTiming::Impossible {
            let key = (
            ...
            match uncaptured {
                UncapturedImpossibleDeadline::Strict => {
                    resolve_stale_impossible_action_done(captured)
                }
                UncapturedImpossibleDeadline::Unavailable => captured.map_or(
...
impl EngineInner {
    ...
    pub(super) fn take_smalltalk_initiative(&mut self, entity_id: EntityId) {
        ...
        // The original game's swordfighting test is exactly
        // a nonempty opponent list; it does not inspect the actor action
    ...
    /// unit away (maximum norm) and straight-reachable.
    ///
    /// Distance branch:
    ...
    pub(super) fn update_swordfight_distance(
        ...
        let snapshot = {
            ...
            let principal = human.opponents.first().copied().unwrap_or_else(|| {
                panic!("swordfight evaluation distance owner {entity_id:?} has no principal")
            });
            ...
            let sector = entity
                ...
                .map(i16::from)
                .unwrap_or_else(|| {
                    panic!("swordfight evaluation distance owner {entity_id:?} has no sector")
                });
        ...
        let opp_sector = opp
            ...
            .unwrap_or_else(|| {
                panic!(
                    "swordfight evaluation distance principal {principal_id:?} for {entity_id:?} has no sector"
                )
            });
    ...
    /// WaitingSword enters swordfight evaluation.
    ///
        ...
        match current {
            Some((seq_id, elem_idx, command, order_id, order_type)) => {
                tracing::debug!(
                    ...
                    "WaitingSword swordfight evaluation entry"
                );
            }
            None => {
                tracing::debug!(
                    ...
                    frame = self.control.frame_counter,
                    owner = ?entity_id,
                    "WaitingSword swordfight evaluation entry has no current order"
                );
            }
        }
    }
    ...
    fn evaluate_swordfight_for(
        ...
        let (self_pos, self_sector, self_uber, tiredness, is_pc, num_opponents) = {
            ...
            (
                ...
                entity
                    ...
                    .map(i16::from)
                    .unwrap_or_else(|| {
                        panic!("swordfight evaluation owner {entity_id:?} has no sector")
                    }),
                uber,
                human.tiredness,
                entity.is_pc(),
        ...
        let (principal_pos, principal_sector, principal_uber) = {
            ...
            (
                ...
                principal
                    ...
                    .unwrap_or_else(|| {
                        panic!(
                            "swordfight evaluation principal {first_principal:?} for owner {entity_id:?} has no sector"
                        )
                    }),
                uber,
            )
        ...
        // The original game's swordfight evaluation tears down the relationship and returns
        ...
        if !range_or_los_prune {
            let self_eye = self
                ...
                .unwrap_or_else(|| {
                    panic!("swordfight evaluation owner {entity_id:?} has no upright eye point")
                });
            let principal_eye = self
                ...
                .unwrap_or_else(|| {
                    panic!(
                        "swordfight evaluation principal {first_principal:?} for owner {entity_id:?} has no upright eye point"
                    )
                });
        ...
        if mutual {
            let (has_initiative, received, relative_ability) = self
                ...
                .unwrap_or_else(|| {
                    panic!("swordfight evaluation owner {entity_id:?} lost human state")
                });
            if has_initiative {
                if received {
        ...
        if is_pc && !selected_pc {
            // Human-actor swordfight evaluation.
            // proposes a strike for
    ...
    pub(super) fn pc_propose_and_launch_strike(
        ...
        let character = assets
            ...
            .unwrap_or_else(|| {
                panic!(
                    "swordfight evaluation strike proposal PC {pc_id:?} references missing character profile {:?}",
        ...
        let attacker_profile = assets
            ...
            .unwrap_or_else(|| {
                panic!(
                    "swordfight evaluation strike proposal PC {pc_id:?} references missing hand-to-hand weapon {weapon_id}"
                )
            });
        ...
        // Strike selection mutates the live boredom counters while
    ...
    pub(super) fn is_step_back_needed(
        ...
        let opponents: Vec<EntityId> = entity
            .human_data()
            .unwrap_or_else(|| {
                panic!("swordfight evaluation step-back owner {entity_id:?} is not human")
            })
            .opponents
            .ids();
        if opponents.is_empty() {
        ...
        // Element direction-vector lookup uses 16-way sector conversion with
    ...
    pub(super) fn can_he_kill_me_but_me_not(
        ...
        let my_max_range = required_hth_weapon_profile(
            ...
            me_id,
            &assets.profile_manager,
            "swordfight evaluation initiative range",
        )
        .distance[crate::weapons::WeaponDistance::Maximal as usize]
            as f32;
        let opp_max_range = required_hth_weapon_profile(
            ...
            opponent_id,
            &assets.profile_manager,
            "swordfight evaluation opponent initiative range",
        )
        .distance[crate::weapons::WeaponDistance::Maximal as usize]
            as f32;
    ...
    pub(super) fn warn_for_strike(
        ...
        for &victim_id in victims {
            ...
            // if in a swordfight substate. Strike warnings on soldiers
            ...
            //
            // The original game's swordfighting test is relationship-based:
            // a nonempty opponent list. Merely retaining a sword action
            ...
            let (
                ...
                // Player-character strike warning launches a counter-strike
                // against the principal opponent, i.e. the first entry in
            ...
            match proposed {
                Some(crate::combat::ProposedCombatAction::Parry) => {
                    // The original game's sword-attack consideration runs from the current
                    // sword-waiting execution callback. Sequence-element launch
                    // only registers ParrySword here; its instruction/priority
                    // arbitration happens in the sequence-manager tick after
    ...
    pub(crate) fn consider_to_begin_parade(
        ...
        let (
            ...
            // Sword-strike selection reads the actor-specific fighting ability.
            // Soldier actors override it to apply the active
        ...
        // Strike selection always times the defender's principal
        ...
        if dispatch_to_all && is_circular {
            ...
            let friend_ids: Vec<EntityId> = self
                ...
                .filter(|&id| {
                    ...
                    // Skill check. The original game performs the specialized
                    // fighting-ability calculation, which applies the active
                    ...
                    // on the learner's map position but tests the
                    // friend's ground position (world x/y, i.e. map y
                    // plus elevation).
