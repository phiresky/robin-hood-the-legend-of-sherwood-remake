...
//!
//! Covers instruction/execution paths for sword commands, hit detection
...
//!
//! - **Straight strikes**: Distance-based hit detection
...
/// without checking sword-strike victim eligibility, even while that opponent is
...
/// combat, so scaling Y by the inverse swordfight aspect ratio is a no-op.
...
/// Map-unit radius used by the domino effect to scan for nearby
...
impl EngineInner {
    ...
    pub(crate) fn apply_scripted_life_points(
        ...
        // Game scripts pass an integer to the sword selector, so preserve the original
    ...
    /// Run the PC/NPC/soldier/human death-processing chain used by
    /// human life-point updates, without synthesizing a damage
    ...
    pub(in crate::engine) fn apply_scripted_virtual_kill(
        ...
        // Script/native life-point setters reach this death-processing boundary
        ...
        let human = self
            .get_entity_mut(entity_id)
            .expect("script-killed human vanished during death handling")
            .human_data_mut()
            .expect("script-killed human lost HumanData during death handling");
...
/// concussion handling forces the value to
...
pub(crate) fn concussion_ctx_full(
    ...
    let is_in_coma = match entity {
        Entity::Pc(pc) => match campaign {
            ...
            None => false,
            Some(campaign) => {
                // The list index is independent actor/UI storage. The original game
                // resolves this through the serialized description reference
    ...
    ConcussionContext {
        ...
        // Sherwood detection uses the current campaign mission's location,
...
/// conversion used during actor initialization.
...
/// The original game measures a straight sword strike at its completed
/// marker with distance to the antagonist. Those
...
fn can_enter_swordfight_with(
    ...
    // Swordfight-entry eligibility asks the current sector directly whether it is
...
fn sword_strike_distance_is_in_range(distance: f32, min_distance: f32, max_distance: f32) -> bool {
    ...
    // position is NaN: both comparisons reject the actor in the original game, while the
...
///
/// Circle-strike victim selection divides by the sword's strike rotation
/// angle, whose profile degrees are converted to
/// radians before this formula is evaluated, then narrows the extension to
/// 16 bits before adding it to the strike's maximal distance.
...
///
/// The original game's warning-time push-strike victim search works only
/// in map space and has no elevation gate. The DONE-time
/// Push sword-strike victim collection uses ground space and first truncates
/// the absolute elevation difference to an unsigned 32-bit value before comparing it with
...
/// Warning-time push collection has a maximum-norm `< 150` shortcut;
...
/// Original-game sword-strike selection from animation.
///
...
/// Sword strike-angle calculation keeps its unsuffixed-literal expression in
/// double precision and narrows only at the single-precision return boundary.
fn strike_profile_angle(degrees: u16) -> f32 {
    ((f64::from(degrees) / 360.0) * 2.0 * f64::from(std::f32::consts::PI)) as f32
}

/// Push collectors halve an authored unsigned 16-bit width with integer division
