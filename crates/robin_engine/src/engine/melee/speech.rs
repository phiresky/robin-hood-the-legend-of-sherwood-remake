...
impl EngineInner {
    ...
    ///
    /// This is separate from hurt speech: PCs use the shared human
    ...
    pub(crate) fn say_ouch(
        ...
        // NPC hurt speech routes through the ordinary speech filters with EMERGENCY,
        // exactly like NPC pain-reaction speech. Profile lookup happens only
        ...
        let (is_vip, is_civilian) = match owner {
            OuchOwner::Soldier(profile_index) => (
                assets
                    ...
                    .unwrap_or_else(|| {
                        panic!(
                            "hurt-speech owner {} requires missing soldier profile {}",
                            entity_id.index(),
                            profile_index
                        )
                    })
            ...
            OuchOwner::Civilian(profile_index) => (
                assets
                    ...
                    .unwrap_or_else(|| {
                        panic!(
                            "hurt-speech owner {} requires missing civilian profile {}",
                            entity_id.index(),
                            profile_index
                        )
                    })
    ...
    /// - Hero speech eligibility (chorus timer + forbidden expression list)
    ...
    /// Scripted speech-recording path. The original game calls
    /// scripted hero speech with a forced variant from
    /// immediate player-character execution.
    ...
    fn hero_speaking_ex_with_variant(
        ...
        // hero speech eligibility (chorus + forbidden-list) checks below, NOT
    ...
    ///
    /// When neither swordfighting nor moving, tiredness falls by one tenth of endurance.
    ...
    pub(crate) fn tick_tiredness_for(&mut self, id: EntityId, assets: &LevelAssets) {
        ...
        // The original game staggers this tail by element creation order, not by
        ...
        // The original game's moving-state query forwards to
        // the position-interface movement check
        // which is the 3D
