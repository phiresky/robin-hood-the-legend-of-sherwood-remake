...
/// Run synchronous swordfight-entry preparation unless this exact
/// actor/opponent pair is already being prepared higher in the callback stack.
///
/// Swordfight preparation and
/// Mirrors the original game's swordfight preparation scope.
...
impl EngineInner {
    ...
    pub(crate) fn add_opponent(
        ...
        // The original game receives typed human references. Preserve that
        ...
        entities
            .get(opponent_id)
            .unwrap_or_else(|| panic!("opponent insertion target {opponent_id:?} is missing"))
            .human_data()
            .unwrap_or_else(|| panic!("opponent insertion target {opponent_id:?} is not human"));

        let human = entities
            .get_mut(entity_id)
            .unwrap_or_else(|| panic!("opponent insertion owner {entity_id:?} is missing"))
            .human_data_mut()
            .unwrap_or_else(|| panic!("opponent insertion owner {entity_id:?} is not human"));
    ...
    pub(super) fn remove_opponent(
        ...
        entities
            .get_mut(entity_id)
            .unwrap_or_else(|| panic!("opponent removal owner {entity_id:?} is missing"))
            .human_data_mut()
            .unwrap_or_else(|| panic!("opponent removal owner {entity_id:?} is not human"))
            .opponents
            .remove(opponent_id)
    }

    /// Remove one opponent with the original game's authoritative side effects.
    ///
    /// In particular, deleting the final entry recursively reaches
    /// swordfight exit, which synchronously sends `EVENT_QUIT_SWORDFIGHT` to
    ...
    pub(crate) fn evaluate_opponents(
        ...
        if count == 0 {
            // Original-game opponent evaluation changes an
            ...
            // detection refresh can emit VIEW / OUTOFVIEW. Deferring the
    ...
    /// ordinary upright form. The shipped game
    /// maps `WalkingWithSword` to `WalkingUpright` and every other
    ...
    pub(super) fn rewrite_sword_movement_for_fight_exit(
        ...
        *action = match *action {
            ...
            // actor still owns an opponent. Preserve the shipped game's
            // behavior for that inconsistent state.
    ...
    /// Direct human swordfight entry used by both
    /// Swordfight reconsideration and the already-swordfighting `EVENT_GOTHIT`
    /// arm. Unlike the ENTER_SWORDFIGHT command's `Translate` path, the
    /// direct original-game path does not first invoke
    ...
    fn enter_swordfight_impl(
        ...
        // The original game's swordfight-entry check deliberately has no camp or
        // diplomacy clause. Admission sites decide
        // who may start a fight; once a scripted/brawl/dynamic-allegiance
        // request exists, even two currently same-camp actors must be able to
        ...
        // so it must bypass this opponent stop/decision boundary.
        ...
        if !already_opponent {
            ...
            {
                // The original game's swordfight entry compares
                // distance to the opponent. Use
            ...
            // between upright eye points, matching the original game.
            {
                let eye_a = self
        ...
        if !opponent_is_swordfighting {
            ...
            //
            // The original game's swordfight entry stores the associated jump
            // line — the far side of the
            // table — in the reciprocal element's
            // jump-line destination, not a null line. That property
        ...
        // Swordfight entry mutates the relationship and initiator PC UI.
        ...
        // The original game first forwards a targeted no-action selection message.
        // Action selection clears the target PC's current action even
    ...
    ///
    /// This mirrors the original game's swordfight exit: despite
    ...
    pub(crate) fn quit_swordfight_with_far_opponents(
        ...
        for opp_id in opponents {
            ...
            if dist > uber_range && dist > opp_uber {
                // The original game's distant-opponent swordfight exit deliberately
                // edits the owner's list directly, so its cached relative
                // fighting ability is retained.  The reciprocal removal does
                // go through opponent removal, including its strength,
    ...
    /// Complete the PC wounded/coma response at a damage-apply site.
    ///
    /// The wounding entry points dispatch actor-specific wound handling, so a VIP
    ...
    /// and before shared hurt-speech processing classifies the result. The shared
    ...
    pub(in crate::engine) fn close_pc_wounded_coma_boundary(
        ...
        _life_points_after: i16,
    ) -> bool {
        let (victim_is_pc, concussion_before, unconscious_before) = self
            .get_entity(victim_id)
            .map(|victim| {
                let human = victim.human_data();
                (
                    victim.kind().is_pc(),
                    human.map_or(0, |human| human.concussion_of_the_brain),
                    human.is_some_and(|human| human.unconscious),
                )
            })
        ...
        let coma_saved = victim_is_pc
            ...
            && damage > 0
            // Player wound handling selects the VIP/amulet branch from the attempted
            // damage before updating life points. In Sherwood that
            // setter rejects the reduction, but the remainder of the coma
        ...
        if coma_saved && sherwood_immunity {
            ...
            // concussion handling. They do not guard the surrounding
            // player wound-handling coma branch, so retain the pre-hit values while
            // preserving try_pc_coma_save's campaign/posture side effects.
            let pc = self
                .get_entity_mut(victim_id)
                .and_then(Entity::pc_data_mut)
                .expect("Sherwood coma-save victim stopped being a PC");
            pc.life_points = life_points_before;
            let human = self
                .get_entity_mut(victim_id)
                .and_then(Entity::human_data_mut)
                .expect("Sherwood coma-save PC lost required human state");
            human.concussion_of_the_brain = concussion_before;
            human.unconscious = unconscious_before;
        }
        ...
        // The shared hurt-speech path skips unconscious actors, so preserve that
        // life-point-setter callback explicitly at this boundary.
        let stored_life_points = self
            .get_entity(victim_id)
