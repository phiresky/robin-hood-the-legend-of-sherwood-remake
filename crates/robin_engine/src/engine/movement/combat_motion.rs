...
pub(super) fn is_sword_motion_context(
    ...
    // Human opponent-facing owns only its literal sword-movement
    // execution paths. Movement animation selection performs the state-dependent
    // WALKING_UPRIGHT -> WALKING_WITH_SWORD rewrite while translating an
    // ordinary movement element. A
...
/// Whether the selected logical action enters PC's
/// walking-with-shield execution path.
///
/// The actor's action state is stamped to `MOVING_SHIELD` only after that
/// arm has faced the danger point and processed motion, so it cannot be used
/// to recognize the first frame of a shield walk.
pub(super) fn executes_shield_movement_action(
    door_pass_action: Option<OrderType>,
    order_action: OrderType,
) -> bool {
    let action =
...
/// Match original-game movement-animation selection: these are logical
/// dispatch tokens consumed by the Human Execute override, not sprite rows.
/// Opponent-facing chooses the concrete forward/backward/strafe sword animation
...
/// as original-game opponent-facing measures it, normalized to
/// `[0, 2π)`.
///
...
pub(super) fn combat_directional_animation(
    ...
    match action_state {
        crate::element::ActionState::MovingShield => {
            // Danger-facing's four comparisons all reject NaN, then its
        ...
        _ => {
            // Opponent-facing likewise falls through to
