...
/// Local sequence assembly state, carried between the gate and goal phases.
/// Keeping the command level explicit preserves the original branch-specific increments.
#[derive(serde::Serialize, serde::Deserialize)]
struct GateRouteEmission {
    seq: crate::sequence::Sequence,
    level: u16,
    prev_sector: Option<crate::position_interface::SectorHandle>,
    ended_early: bool,
}

impl GateRouteRequest {
    fn entity_goal(&self) -> Option<(EntityId, f32, bool)> {
        match self.goal {
            GoalShape::Seek {
                target, tolerance, ..
...
// Public sector numbers are not unique; retained arena identity must win.
fn is_building_sector(
    engine: &EngineInner,
    sector: crate::position_interface::SectorHandle,
) -> bool {
    route_sector_by_exact_handle(engine, sector)
        .map(|gs| gs.sector_type.is_building())
        .unwrap_or(false)
}

fn is_ladder_lift(engine: &EngineInner, sector: crate::position_interface::SectorHandle) -> bool {
    route_sector_by_exact_handle(engine, sector)
...
impl EngineInner {
    /// Launch a route for callers that do not retain the resulting sequence ID.
    ...
    pub(crate) fn launch_gate_movement_sequence(
        ...
        let entity_id = request.entity_id;
        let source_sector = request.source_sector;
        let base_action = request.base_action;
        let speed_factor = request.speed_factor;
        ...
        // the borrow checker.
        let gate_shots = self.snapshot_gate_route(&request.gate_path, base_action)?;
        ...
        for mut elem in request.prefix_elements.drain(..) {
        ...
        // The original game carries the possibly door-adapted source sector
        // through both the leading validity check and previous-sector tracking.
        ...
        if let Some(source_sector) = source_sector {
            ...
            leading_ap.data = SequenceElementData::Movement {
                ...
                // The leading cross-sector position check uses zero
                // tolerance. Gate-entry and post-pass checks use 10.
                tolerance: 0.0,
                direction: 0,
                action: base_action,
                speed_factor,
        ...
        let emission = GateRouteEmission {
            seq,
            level,
            prev_sector,
            ended_early: false,
        ...
        // arrived" line once the destination is reached.
        // The sequence receives the incremented count left by
        // movement construction, so speech is the next command level,
        // after the final movement has completed. The PC's instruction handling
        // terminates it on dispatch and queues
        // `HeroDoneCommand` via `arbitrate_instruct`.
        if request.append_arrival_speech && !seq.is_empty() {
            let speak_level = seq
                .last()
        ...
        tracing::trace!(
            entity = ?entity_id,
            ?seq_id,
            gates = request.gate_path.len(),
            early = ended_early,
            goal = ?request.goal,
            move_after_last_door = request.move_after_last_door,
    ...
    fn append_gate_elements(
        ...
        let speed_factor = request.speed_factor;
        let gate_path = &request.gate_path;
        let entity_goal = request.entity_goal();
        let GateRouteEmission {
            mut seq,
            mut level,
            mut prev_sector,
        ...
        for (gate_idx, shot) in gate_shots.iter().enumerate() {
            ...
            // original game's movement construction. The door-vs-jump split
            // happens after this approach.
            // Pathfinding retains exact sector identities on every gate. The
            // first retained gate's agreeing source side is therefore the
            // exact identity of the original game's initial previous sector. This remains
            ...
            // still proves which sector the search used. Never recover
            // through a disagreeing public number, and after the first gate
            // keep using the previous gate's exact new-sector equivalent.
            ...
            if old_is_building {
                ...
                // Original-game movement-sequence construction in
                // The original game sums two masked random values
            ...
            if shot.is_locked_pc_unlockable && has_lockpick {
                // The original game chooses the inside or outside point from direction, which is
            ...
            pass.data = SequenceElementData::Movement {
                ...
                // and only attaches the gate through gate assignment.
                flags: MoveFlags::empty(),
                tolerance: 0.0,
                // The original-game gate carries a path-local direction flag
                // while constructing door traversal. AI positioning then reads the
    ...
    /// Append the goal-specific movement only when gate traversal did not stop for lockpicking.
    fn append_route_tail(
        &mut self,
        sim: &crate::sim_rng::SimulationContext,
        request: &GateRouteRequest,
        gate_shots: &[GateShot],
        ...
        let entity_id = request.entity_id;
        let goal = request.goal;
        let goal_layer = request.goal_layer;
        let base_action = request.base_action;
        let initial_flags = request.initial_flags;
        let speed_factor = request.speed_factor;
        let move_after_last_door = request.move_after_last_door;
        let entity_goal = request.entity_goal();
            ...
            match goal {
                ...
                GoalShape::Line {
                    ...
                    if !last_into_building {
                        ...
                        seq.append_element(final_move);
                        // Queuing movement to a line advances the count after
                ...
                GoalShape::Door {
                    ...
                    if !move_after_last_door {
                        ...
                        final_move.data = SequenceElementData::Movement {
                            ...
                            layer: far_side_layer,
                            sector: None,
                            // The original game's door-movement queuing
                            // trailing goal move is a plain MOVE to
                            // the goal point; it does not assign a gate.
                            gate_id: None,
                            line_id: None,
                            element: None,
                            flags: trailing_flags,
