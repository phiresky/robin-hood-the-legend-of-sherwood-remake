...
impl EngineInner {
    ...
    pub(in crate::engine) fn find_plane_obstacle_split(
        ...
        // The original game narrows each block coordinate to a signed 16-bit value before the
        ...
        assets
            ...
            .filter(|(_, obs)| {
                ...
                obs.projection_area_ref()
                    ...
                    // Check polygon-vs-block registration
                    // before applying elevation-line point predicates.
    ...
    /// Elevation-bond crossing for a shipped mobile master. The original-game mobile
    ...
    /// original-game owner movement instruction.
    ///
    /// Seek movement follows that same branch in the original game, so this
    /// must run before seek refresh resolves an entity target or constructs a
    ...
    pub(in crate::engine) fn cross_elevation_line(
        ...
        if let Some(crossed) = Self::crossed_elevation_obstacle(current, left, right) {
            // The original game compares obstacle identities here, so absence is a
    ...
    /// Close the original game's post-execution actor update
    /// line-crossing-check boundary for an actor whose selected execution branch
    /// moved it without going through the movement owner.
    ///
    /// The original game collects every line-crossing candidate once. With exactly one
    ...
    pub(in crate::engine) fn dispatch_actor_post_execute_line_crossing(
        ...
        // The original game obtains one unique line list for LINE_CROSS, then
        ...
        // this update's branch or dispatch set.
        ...
        let elevation_indices = crossing_indices
            ...
            .collect::<Vec<_>>();
        // In the original game's single-line branch every flag on that line is
        ...
        if crossed_elevation || crossing_count > 1 {
                ...
                && let Some(entity) = self.world.entities.get_mut(entity_id)
            {
                ...
                // The original game computes all movement increments here without forcibly
    ...
    /// ```text
    ///   if patch is active:
    ///       if patch.apply_sector contains the map position:
    ///           patch.Enter(actor)
    ...
    pub(in crate::engine) fn dispatch_patch_line_crossing(
        ...
        // PCs, matching line-crossing checks.
...
mod tests {
    ...
    fn split_elevation_emergency_excludes_planes_absent_from_the_probe_block() {
        ...
        // never intersects the probe's x=64..128/y=0..64 grid block.
        // Sector registration therefore does not add this later plane there.
