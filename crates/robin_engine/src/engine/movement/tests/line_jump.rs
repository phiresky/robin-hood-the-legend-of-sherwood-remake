...
mod suite {
    ...
    fn running_with_sword_uses_distance_motion() {
        ...
        assert!(
            order_uses_distance_motion(OrderType::WalkingWithShield),
            "player shield walking dispatches its directional shield row through ordinary motion processing"
        );
    ...
    fn run_running_stairs_outer_crossing_fixture(
        ...
        // changes the facing selected by increment computation.
    ...
    fn running_stairs_crossing_uses_outer_pre_position() {
        ...
        assert_eq!(
            ...
            vec![line_index],
            "the bond must be crossed by the first motion step"
        );
        assert!(
            engine
                .world
        ...
        assert_eq!(
            ...
            "post-crossing increment computation must refresh direction_goal"
        );
    }
    ...
    fn first_fast_climb_commit_keeps_lift_forecast_when_second_call_is_stationary() {
        ...
        assert_eq!(
            ...
            "an upward first motion step must publish positive lift movement"
        );
        ...
        assert_eq!(
            sprite.position_iface.get_forecasted_movement(),
            WorldVec3D::new(0.5, -1.0, -1.5),
            "a downward first motion step must publish negative lift movement"
        );
    }
    ...
    #[test]
    fn loaded_shield_danger_point_uses_original_ground_coordinates() {
        let mut engine = EngineInner::new();
        let mut shield_pc = extraction_test_pc(Posture::Upright);
        shield_pc
            .element_data_mut()
            .set_position(crate::coordinates::WorldPoint3D::new(100.0, 100.0, 40.0));
        shield_pc
            .pc_data_mut()
        ...
        assert_eq!(
            ...
            (Some(crate::coordinates::MapPoint::new(140.0, 100.0)), true),
            "loaded shield danger point retains raw ground X/Y and uses ground position"
        );
    }
    ...
    fn face_opponent_direction_uses_isometric_map_aspect() {
        ...
        assert_ne!(
            bare, isometric,
            "raw map-space binning must not replace aspect-corrected direction-sector selection"
        );
    }
}
