...
mod suite {
    ...
    fn perform_seek_registers_enter_swordfight_before_outer_terminal_provoke() {
        ...
        // post-seek sequence. The original game starts it inside
        // seeking, so swordfight entry reaches the manager FIFO before the
    ...
    fn crenel_transition_does_not_inherit_stale_sword_facing_context() {
        ...
        assert!(
            ...
            "the base actor crenel transition must retain its lift-facing goal instead of entering human opponent-facing"
        );
    ...
    fn door_distance_successor_dispatches_its_literal_non_sword_action() {
        ...
        assert!(
            ...
            "PassDoor's concrete WalkingUpright order enters base actor execution"
        );
        assert!(
    ...
    fn exhausted_stop_transition_clears_goal_before_queued_point_goto_promotion() {
        ...
        // once stopping all work has built its live movement-stop transition, keep that
        // transition authoritative while ordinary point movement is instructed.
        // The terminal order advancement below must therefore release the movement
    ...
    fn exact_terminal_pop_controls_stopped_route_manager_fifo_handoff() {
            ...
            [(false, false), (true, false), (true, true)]
        {
            ...
            let obstacle_points = vec![
                MapPoint::new(60.0, 0.0),
                MapPoint::new(70.0, 0.0),
                MapPoint::new(70.0, 100.0),
                MapPoint::new(60.0, 100.0),
            ];
            let mut obstacle_box = crate::coordinates::MapBBox::new();
            for &point in &obstacle_points {
                obstacle_box.expand_point(point);
            }
            fast_grid.add_sector(
                crate::fast_find_grid::GridSector {
                    points: obstacle_points,
                    bounding_box: obstacle_box,
                    sector_type: crate::sector::SectorType::MOTION,
                    layer: 0,
            ...
            // A completed door route can retain the historical shape searched by
            // the stopped-player fallback: a terminated movement followed by an
            // interrupted continuation. It is not the current actor-update
            // stack and cannot advance a later independently queued move.
            let mut stale_movement = SequenceElement::new_movement(
                1,
                Command::MoveOk,
                Some(owner),
                OrderType::WalkingUpright,
            ...
            let mut stopped_following = SequenceElement::new(3, Command::CrouchDown, Some(owner));
            stopped_following.state = SequenceState::Impossible;
            engine
                .orders
                .sequence_manager
                .get_sequence_mut(stale_sequence)
                .unwrap()
                .elements
                .extend([completed_callback, stopped_following]);

            let mut incoming = SequenceElement::new_movement(
                1,
                Command::Move,
                Some(owner),
                OrderType::WalkingUpright,
    ...
    fn new_terminal_pc_stop_transition_replaces_stale_direction_goal() {
        ...
        assert_eq!(
            i16::from(entity.position_iface().get_direction_goal()),
            7,
            "new-order increment computation must replace rather than restore the stale goal"
        );
    }
    ...
    fn terminal_door_pass_goal_clear_follows_crossing_recompute() {
        ...
        let terminal_position = MapPoint::new(1446.2887, 620.40155);
        {
            ...
            // Crossing an elevation line invalidates the cached trajectory before
            // Line-crossing detection rebuilds it against the live order's
            // destination.
    ...
    fn install_terminal_interaction_seek(
        ...
        } = &mut movement.data
        {
            *flags = MoveFlags::SEEK;
            ...
            // element target, while seeking still owns seek_target on
            // ActorData.
            *element = None;
    ...
    fn looped_seek_stop_transition_rechecks_final_order_after_deleting_followers() {
        ...
        *seek_target = Some(target);
        ...
        assert_eq!(
            ...
            "seeking must observe that motion through the last frame deleted all followers and launch the attached post-seek"
        );
    }
    ...
    fn looped_seek_start_transition_refreshes_before_copied_stop_transition() {
        ...
        *tolerance = 17.0;
        ...
        assert_ne!(
            ...
            "seeking must refresh before executing the copied stale stop transition"
        );
    }
    ...
    fn terminal_pc_tie_seek_outside_init_range_publishes_tie_before_execute_validation() {
        ...
        assert_eq!(
            i16::from(entity.position_iface().get_direction_goal()),
            7,
            "Tie translation itself must not run the next update's initialization turn"
        );
    }
    ...
    fn translated_point_seek_terminal_in_matching_sector_launches_drop_ale() {
        ...
        *sector = crate::position_interface::SectorHandle::new(2);
        ...
        // Point-target seeking keys the terminal handoff on the absence
