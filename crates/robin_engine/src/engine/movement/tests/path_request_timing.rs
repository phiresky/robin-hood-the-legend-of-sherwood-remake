...
mod suite {
    ...
    #[test]
    fn terminal_door_topology_does_not_substitute_for_thick_reachability() {
        use crate::sequence::MoveFlags;

        assert!(!movement_path_dispatch_is_direct(
            MoveFlags::empty(),
            true,
            true,
            false,
    ...
    fn original_two_request_special_case_keeps_first_request_first() {
        ...
        // Path-request insertion appends unconditionally while fewer than two entries
