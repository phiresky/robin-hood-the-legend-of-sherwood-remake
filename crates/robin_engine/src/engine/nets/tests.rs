...
fn net_body_publication_includes_self_and_nearby_money_fighters() {
    ...
    let mut engine = make_engine();
    let mut assets = assets_with_profiles();
    let victim = add_soldier(&mut engine, WorldPoint3D::new(100.0, 100.0, 0.0), 0, false);
    let friend = add_soldier(&mut engine, WorldPoint3D::new(105.0, 100.0, 0.0), 0, false);
