...
//!
//! 4. **Purse update**: the burst purse element stays alive
...
impl EngineInner {
    ...
    /// Execute the original game's complete creation and per-frame registration
    ...
    pub(super) fn publish_new_purse(
        ...
        // Purse-specific tick animation runs after every base branch,
        // including synchronous obstacle-impact child publication.
        purse
            .element
            .sprite
    ...
    fn hydrate_unpublished_projectile(
        ...
        let prototype = assets
            ...
            .unwrap_or_else(|| {
                panic!(
                    "missing {:?} accessory master during pre-publication update",
    ...
    /// impact point. Trajectory calculation has already bound the terminal
    ...
    pub(super) fn publish_primed_coin(
        ...
        if exhausted && shield.is_none() {
            ...
            if material == crate::element::GameMaterial::Water || coin.projectile.dive {
                ...
                let goal_sector = coin.projectile.purse.sector_goal;
                coin.element.set_sector_topology(
                    goal_sector,
                    goal_sector.and_then(|sector| sector.arena_index()),
                );
    ...
    ///   broadcast.
    /// * **Purse update** post-burst — prunes dead/taken child handles
    ...
    pub(super) fn tick_purse_or_coin(
        ...
        // ── Phase 3: Purse update — prune dead children ──────────
        //
        ...
        // Purse/coin animation follows the base projectile update. The
        // animation uses the flying state at update entry, so a
        // projectile that lands during the base update still receives the flying
        ...
        match object_type {
            // Coin elements ignore their projectile base result, and both
            // branches return true.
            ObjectType::Coin => true,
            // Grounded purse elements skip projectile updates entirely;
    ...
    fn burst_purse(
        ...
        // the shooter here, so a missing thrower is corrupted state.
        ...
        // Trajectory calculation authored the terminal material in the waypoint
        // runtime and projectile ticking installed it before dispatching
        // obstacle impact. Do not perform a second spatial/material query here:
        ...
        for _ in 0..NUMBER_OF_COINS_IN_PURSE {
            ...
            for _ in 0..bow_shot::COIN_SCATTER_ATTEMPTS {
                // Original-game purse bursting in
        ...
        // Coin effects follow every child spawn and the purse bookkeeping
        // in the original game, but still precedes deactivation.
        self.feedback
            .pending_side_effects
            .sounds
            .push(super::SoundCommand::Fx {
        ...
        // Projectile ticking plays impact sound only after the actor-specific
        // purse obstacle-impact processing returns (and therefore after deactivation).
        self.feedback
            .pending_side_effects
            .sounds
            .push(super::SoundCommand::Fx {
...
mod tests {
    ...
    fn purse_burst_spawns_child_coins_and_marks_purse() {
        ...
        // ObjectBursting is selected on the next grounded update.
    ...
    fn live_purse_pass_reaches_newly_appended_coins_in_the_same_frame() {
        ...
        for child_id in child_ids {
            ...
            assert_ne!(
                same_frame, primed,
                "coin {child_id:?} must receive its element-array update after the explicit pre-insertion primer"
            );
        }
    }
    ...
    fn purse_hourglass_prunes_dead_children_but_keeps_purse_alive() {
        ...
        // Run a tick — the update drain should prune the child list…
        let assets = purse_test_assets();
        tick_purses(&mut engine, &assets);
        let purse = engine
            .get_entity(purse_id)
        ...
        assert!(
            !p.element.active,
            "obstacle impact must leave the retained purse inactive post-drain"
        );
        assert!(
            p.projectile.purse.burst,
