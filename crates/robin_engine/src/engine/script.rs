...
fn spellforge_event_name(key: ScriptVmKey, fn_name: &str) -> &str {
    match (key, fn_name) {
        ...
        (ScriptVmKey::Zone(_), "EnterZone") => "Enter",
        (ScriptVmKey::Zone(_), "ExitZone") => "Exit",
        (ScriptVmKey::Actor(_), "FilterAIEvent") => "FilterAiEvent",
        _ => fn_name,
    }
}
...
#[cfg(test)]
#[test]
fn spellforge_event_names_cover_global_and_per_entity_contracts() {
    let path = crate::ai::PathId::new(0).expect("path zero is valid");
    let cases = [
        (ScriptVmKey::Global, "Hourglass", "Timer"),
        (
            ScriptVmKey::Global,
            "CheckVictoryCondition",
            "CheckVictoryCondition",
        ),
        (ScriptVmKey::Global, "Finalize", "Finalize"),
        (ScriptVmKey::Global, "KeyPressed", "KeyPressed"),
        (ScriptVmKey::Actor(1), "FilterAIEvent", "FilterAiEvent"),
        (ScriptVmKey::Actor(1), "ActionChange", "ActionChange"),
        (
            ScriptVmKey::Target(2),
            "ActivatedBySword",
            "ActivatedBySword",
        ),
        (ScriptVmKey::Scroll(3), "Hourglass", "Timer"),
        (ScriptVmKey::Scroll(3), "IsTaken", "Taken"),
        (ScriptVmKey::Zone(4), "EnterZone", "Enter"),
        (ScriptVmKey::Zone(4), "ExitZone", "Exit"),
        (ScriptVmKey::Waypoint(path, 5), "ReachPoint", "ReachPoint"),
    ];
    for (key, engine_name, spellforge_name) in cases {
        assert_eq!(spellforge_event_name(key, engine_name), spellforge_name);
    }
}
...
impl ScriptDriverError {
    ...
    fn spellforge(error: crate::spellforge::SpellforgeGuestError) -> Self {
        Self {
            detail: error.to_string(),
            spellforge: Some(error),
            sequence_element_failed: false,
        }
    }
}
...
impl From<crate::spellforge::SpellforgeGuestError> for ScriptDriverError {
...
impl EngineInner {
    ...
    pub(super) fn call_script_vm(
        ...
        match self.call_script_vm_inner(tcx, key, fn_name, params, frame, &mut Vec::new()) {
            Ok(result) => Ok(result),
            Err(error) => {
                if let Some(failure) = error.spellforge {
                    self.scripts.spellforge.fail(failure);
                }
                Err(error.detail)
            }
        }
    ...
    pub(super) fn call_script_vm_inner(
        ...
        let Some(binding) = binding else {
            return self.call_script_vm_inner_unwrapped(tcx, key, fn_name, params, frame, active);
        };
        ...
        let before: Vec<_> = self
            ...
            .collect();
        let result =
            self.call_script_vm_inner_unwrapped(tcx, key, fn_name, params, frame, active)?;
    ...
    fn call_script_vm_inner_unwrapped(
        ...
        let Some(runtime) = tcx.assets.attachments.spellforge_runtime.as_ref().cloned() else {
            return self.call_scb_script_vm_inner(tcx, key, fn_name, params, frame, active);
        };
        let invocation = self.spellforge_invocation(tcx.assets, key, fn_name, params, frame)?;
        let has_lua = runtime
            .has_handler(&invocation, &self.scripts.spellforge)
            .map_err(|mut failure| {
                if failure.invocation.is_none() {
                    failure.invocation = Some(invocation.clone());
                }
                failure.message = format!("Spellforge handler lookup failed: {}", failure.message);
                ScriptDriverError::spellforge(failure)
            })?;
        let mode = runtime.package().script_mode;
        let diagnostic_invocation = invocation.clone();
        let has_scb_vm = self
            .scripts
            .mission
            .as_ref()
            .is_some_and(|script| script.has_scb_script_vm(key));

        let result = match mode {
            crate::spellforge::SpellforgeScriptMode::Replace => {
                if has_lua {
                    self.call_spellforge_vm_inner(
                        tcx,
                        runtime.as_ref(),
                        invocation,
                        key,
                    ...
                    Ok(if fn_name == "FilterAIEvent" { 1 } else { 0 })
                }
            }
            crate::spellforge::SpellforgeScriptMode::AugmentBefore => {
                let lua_result = if has_lua {
                    Some(self.call_spellforge_vm_inner(
                        tcx,
                        runtime.as_ref(),
                        invocation,
                        key,
                ...
                if has_scb_vm {
                    self.call_scb_script_vm_inner(tcx, key, fn_name, params, frame, active)
                } else {
                    Ok(lua_result.unwrap_or(if fn_name == "FilterAIEvent" { 1 } else { 0 }))
                }
            }
            crate::spellforge::SpellforgeScriptMode::AugmentAfter => {
                let scb_result = if has_scb_vm {
                    self.call_scb_script_vm_inner(tcx, key, fn_name, params, frame, active)?
                } else if fn_name == "FilterAIEvent" {
                    1
                } else {
                    0
                };
        ...
        result.map_err(|mut error| {
            let context = format!(
                "fatal Spellforge event {}.{fn_name}: {}",
                self.scripts
                    .mission
                    .as_ref()
            ...
            if let Some(failure) = error.spellforge.as_mut() {
                if failure.invocation.is_none() {
                    failure.invocation = Some(diagnostic_invocation.clone());
                }
                failure.message = context.clone();
            } else {
                error.spellforge = Some(
                    crate::spellforge::SpellforgeGuestError::new(
                        crate::spellforge::SpellforgeGuestErrorKind::HostNative,
                        context.clone(),
                    )
                    .with_invocation(diagnostic_invocation),
                );
            }
    ...
    fn call_scb_script_vm_inner(
        ...
        if !script.has_scb_script_vm(key) {
        ...
        let mut activation = self
            .with_script_session_in_driver(tcx, |script, _, _| {
        ...
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            self.drive_started_script_vm(tcx, key, fn_name, frame, &mut activation, active)
        }));
    ...
    fn spellforge_invocation(
        ...
        params: &[i32],
        frame: crate::natives::ScriptCallFrame,
    ) -> Result<crate::spellforge::SpellforgeInvocation, ScriptDriverError> {
        use crate::element::Entity;
        use crate::spellforge::{SpellforgeInvocation, SpellforgeTarget};

        let target = match key {
            ScriptVmKey::Global => SpellforgeTarget::Global,
            ScriptVmKey::Actor(handle) => {
                let id = self.entity_id_for_actor_handle(handle).ok_or_else(|| {
                    ScriptDriverError::new(format!(
                        "Spellforge actor handle {handle} has no live entity"
                    ))
                })?;
                let class = self
                    .world
                    ...
                    .map(|actor| actor.script_class.clone())
                    .filter(|class| !class.is_empty())
                    .ok_or_else(|| {
                        ScriptDriverError::new(format!(
                            "Spellforge actor handle {handle} has no script class"
                        ))
                    })?;
                SpellforgeTarget::Actor { class, handle }
            }
            ScriptVmKey::Target(handle) => {
                let id = self.entity_id_for_actor_handle(handle).ok_or_else(|| {
                    ScriptDriverError::new(format!(
                        "Spellforge target handle {handle} has no live entity"
                    ))
                })?;
                let class = match self.world.entities.get(id) {
                    Some(Entity::Target(target)) if !target.target.script_class.is_empty() => {
                        target.target.script_class.clone()
                    }
                    _ => {
                        return Err(ScriptDriverError::new(format!(
                            "Spellforge target handle {handle} has no target script class"
                        )));
                    }
                };
                SpellforgeTarget::Target { class, handle }
            }
            ScriptVmKey::Scroll(handle) => {
                let id = self.entity_id_for_actor_handle(handle).ok_or_else(|| {
                    ScriptDriverError::new(format!(
                        "Spellforge scroll handle {handle} has no live entity"
                    ))
                })?;
                let class = match self.world.entities.get(id) {
                    Some(Entity::Scroll(scroll)) if !scroll.script_class.is_empty() => {
                        scroll.script_class.clone()
                    }
                    _ => {
                        return Err(ScriptDriverError::new(format!(
                            "Spellforge scroll handle {handle} has no scroll script class"
                        )));
                    }
                };
                SpellforgeTarget::Scroll { class, handle }
            }
            ScriptVmKey::Zone(index) => {
                let class = self
                    .script_domains
                    .zones
                    .scripts
                    .get(index)
                    .and_then(|zone| zone.script_class_name.clone())
                    .filter(|class| !class.is_empty())
                    .ok_or_else(|| {
                        ScriptDriverError::new(format!(
                            "Spellforge zone {index} has no script class"
                        ))
                    })?;
                SpellforgeTarget::Zone {
                    class,
                    index: u32::try_from(index).map_err(|_| {
                        ScriptDriverError::new(format!("Spellforge zone index {index} exceeds u32"))
                    })?,
                }
            }
            ScriptVmKey::Waypoint(path, waypoint) => {
                let class = assets
                    ...
                    .and_then(|waypoint| match &waypoint.command {
                        crate::level_data::WaypointCommand::Script(class) => Some(class.clone()),
                        _ => None,
                    })
                    .filter(|class| !class.is_empty())
                    .ok_or_else(|| {
                        ScriptDriverError::new(format!(
                            "Spellforge waypoint {path}/{waypoint} has no script class"
                        ))
                    })?;
                SpellforgeTarget::Waypoint {
                    class,
                    path: path.get(),
                    waypoint,
                }
        ...
        let event = spellforge_event_name(key, fn_name).to_owned();
        Ok(SpellforgeInvocation {
            target,
            event,
            args: params.to_vec(),
            script_this: frame.script_this(),
            current_scroll: frame.current_scroll(),
        })
    }

    fn call_spellforge_vm_inner(
        &mut self,
        tcx: TickCtx<'_>,
        runtime: &dyn crate::spellforge::SpellforgeRuntime,
        invocation: crate::spellforge::SpellforgeInvocation,
        key: ScriptVmKey,
        frame: crate::natives::ScriptCallFrame,
        active: &mut Vec<ActiveScriptCall>,
    ) -> Result<i32, ScriptDriverError> {
        ...
        let mut step = runtime
            .begin(invocation.clone(), &self.scripts.spellforge)
            .map_err(ScriptDriverError::spellforge)?;
        let activation = match step {
            crate::spellforge::SpellforgeStep::Native { activation, .. } => activation,
            crate::spellforge::SpellforgeStep::Complete { activation, result } => {
                runtime
                    .commit(activation, result, &mut self.scripts.spellforge)
                    .map_err(ScriptDriverError::spellforge)?;
                return Ok(result);
            }
        };
        ...
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            loop {
                let crate::spellforge::SpellforgeStep::Native {
                    activation: yielded_activation,
                    native_index,
                    arguments,
                } = step
                else {
                    let crate::spellforge::SpellforgeStep::Complete {
                        activation: completed_activation,
                        result: value,
                    } = step
                    else {
                        unreachable!()
                    };
                    if completed_activation != activation {
                        break Err(ScriptDriverError::new(format!(
                            "Spellforge completion changed activation from {activation} to {completed_activation}"
                        )));
                    }
                    runtime
                        .commit(activation, value, &mut self.scripts.spellforge)
                        .map_err(ScriptDriverError::spellforge)?;
                    break Ok(value);
                };
                if yielded_activation != activation {
                    break Err(ScriptDriverError::new(format!(
                        "Spellforge activation changed from {activation} to {yielded_activation}"
                    )));
                }
                let outcome = if native_index == u32::MAX {
                    let value = match arguments.as_slice() {
                        [] => crate::sim_rng::f32(tcx.sim, crate::sim_rng::RngSite::LuaMathRandom)
                            .to_bits() as i32,
                        [upper] if *upper >= 1 => crate::sim_rng::i32(
                            tcx.sim,
                            crate::sim_rng::RngSite::LuaMathRandom,
                            1..=*upper,
                        ),
                        [lower, upper] if lower <= upper => crate::sim_rng::i32(
                            tcx.sim,
                            crate::sim_rng::RngSite::LuaMathRandom,
                            *lower..=*upper,
                        ),
                        [_] | [_, _] => {
                            break Err(ScriptDriverError::new(format!(
                                "invalid Spellforge math.random arguments {arguments:?}"
                            )));
                        }
                        _ => {
                            break Err(ScriptDriverError::new(format!(
                                "Spellforge math.random expected 0..=2 arguments, got {}",
                                arguments.len()
                            )));
                        }
                    };
                    crate::interp::NativeCallOutcome::Return(value)
                } else {
                    self.with_script_session_in_driver(tcx, |script, domains, capabilities| {
                        let mut stack = crate::interp::NativeStack::default();
                        ...
                        crate::interp::HostFunctions::call(&mut context, native_index, &mut stack)
                    })
                    .ok_or_else(|| {
                ...
                let return_word = match outcome {
                    ...
                    crate::interp::NativeCallOutcome::Yield(request) => {
                        let operation_result =
                            self.execute_native_operation(tcx, frame, request.operation, active)?;
                ...
                step = runtime
                    .resume(activation, return_word, &self.scripts.spellforge)
                    .map_err(ScriptDriverError::spellforge)?;
            }
        }));
        let popped = active.pop();
        ...
        self.scripts
            .mission
            .as_mut()
            .expect("Spellforge mission script vanished while restoring callback frame")
            .pop_active_driver_frame(frame);
        match result {
            Ok(result) => {
                if result.is_err() {
                    runtime.abort(activation);
                }
                result
            }
            Err(payload) => {
    ...
    fn drive_started_script_vm(
        ...
        loop {
            let stop = self
                .with_script_session_in_driver(tcx, |script, script_domains, capabilities| {
    ...
    fn execute_native_operation(
        ...
        match operation {
            ...
            crate::interp::NativeOperation::LaunchSequence(sequence_id) => {
                self.start_sequence_inline(tcx, active, sequence_id)?;
                Ok(0)
            }
            crate::interp::NativeOperation::EngineAction(action) => {
                self.execute_synchronous_script_request(tcx, action, active)
            }
            crate::interp::NativeOperation::Command(command) => {
                self.execute_native_command(tcx, command)
            }
        }
    }
    ...
    fn execute_synchronous_script_request(
        ...
        match request {
            ...
            crate::interp::SynchronousScriptRequest::ApplyAiStateNative {
                ...
                // Scripted AI state selection wraps SEEKING and
                // FLEEING in no-event decision-tick admission/completion. Admission runs
                // The original game filters the global event before its later freeze/lock gates;
                        ...
                        match self.call_script_vm_inner(
                            ...
                            Err(error) => {
                                tracing::warn!(
                                    actor_handle = actor,
                                    %error.detail,
                                    "SetAIState decision-entry NO_EVENT FilterAIEvent callback failed — allowing"
                                );
                                true
                            }
                        }
                ...
                if matches!(
                    ...
                    self.end_script_ai_native_think(tcx, owner);
                }
                Ok(0)
            }
            crate::interp::SynchronousScriptRequest::SetPersistentLifePoints {
                ...
                self.apply_scripted_life_points(tcx, actor, amount);
                Ok(0)
            }
            crate::interp::SynchronousScriptRequest::SetPersistentConcussion {
                ...
                // The original game first narrows to 16 bits, then tests the stored value as
                // signed 16-bit value. Values with bit 15 set become negative and normalize
                ...
                self.apply_scripted_concussion(tcx, actor, value, true);

                Ok(0)
            }
            crate::interp::SynchronousScriptRequest::SetActorPosture { actor, posture, .. } => {
                self.apply_script_actor_posture(tcx, actor, posture, active)?;
                Ok(0)
            }
            crate::interp::SynchronousScriptRequest::SetActorLocation {
                actor, location, ..
            } => Ok(self.apply_script_actor_location(tcx, actor, location)?),
            crate::interp::SynchronousScriptRequest::SetActorActionState {
                ...
                self.launch_element_inline(tcx, active, wait)?;
                Ok(0)
            }
            crate::interp::SynchronousScriptRequest::StareActor { actor, target, .. } => {
                ...
                // The shipped implementation deliberately leaves all three
                // NPC view-target selection overloads as ROBINME
                // stubs. Script handling still performs its argument validation,
                // then the call has no gaze, direction, or Halt side effect.
                Ok(0)
            }
            crate::interp::SynchronousScriptRequest::StareLocation { actor, .. } => {
                let owner = self.entity_id_for_actor_handle(actor).ok_or_else(|| {
                    format!(
                ...
                // The NPC position view-target form is the
                // same shipped ROBINME stub as the actor-target overload.
                Ok(0)
            }
            crate::interp::SynchronousScriptRequest::LockAi {
                ...
                self.execute_ai_script_lock_in_driver(tcx, owner, remember_events, active)?;
                Ok(0)
            }
            ...
            crate::interp::SynchronousScriptRequest::AssignPath { actor, way, .. } => {
                ...
                self.execute_ai_assign_patrol_path(tcx, owner, assignment, true);
            ...
            crate::interp::SynchronousScriptRequest::SwitchToAlertPath { actor, .. } => {
                ...
                if let Some(alert_path_id) = alert_path_id {
                    ...
                    ai.patrol_path = crate::ai::PatrolPath::new(
                        alert_path_id,
            ...
            crate::interp::SynchronousScriptRequest::SetPathWalkingStyle { actor, .. } => {
                ...
                // Setting AI path-walking flags requests movement
                ...
                self.relaunch_path_at_new_speed(tcx, owner);
                Ok(0)
            }
            crate::interp::SynchronousScriptRequest::RemoveAllSubordinates { actor, .. } => {
                ...
                self.script_remove_all_subordinates(tcx, chief);
                Ok(0)
            }
    ...
    pub(crate) fn execute_ai_script_lock(
        ...
        self.execute_ai_script_lock_in_driver(tcx, owner, remember_events, &mut Vec::new())
    ...
    fn apply_script_actor_location(
        ...
        if location != 0 {
            ...
            // The script command writes map position/layer/sector before
        ...
        let is_human = self
            ...
            .is_human();
        if is_human {
            self.quit_swordfight(tcx, actor);
            let still_unconscious = self
                .get_entity(actor)
                .expect("SetActorLocation human vanished after leaving swordfight")
                .human_data()
        ...
        if lock_npc {
            self.execute_ai_script_lock(tcx, actor, false);
        }
        Ok(1)
    }

    /// Scripted actor posture translated at the engine boundary so every
    /// stop/posture/wait/concussion/death step completes in the required order
    ...
    fn apply_script_actor_posture(
        ...
        let wait = |engine: &mut Self, active: &mut Vec<ActiveScriptCall>| {
            ...
            engine
                .launch_element_inline(tcx, active, element)
                .map(|_| ())
        };
        let clear_concussion = |engine: &mut Self| {
            engine.apply_scripted_concussion(tcx, actor, 0, true);
        };
        ...
        match posture_id {
            ...
            15 => {
                self.apply_scripted_life_points(tcx, actor, 0);
            ...
            17 => {
                ...
                self.set_entity_posture(actor, Posture::Lying);
                self.apply_scripted_concussion(tcx, actor, crate::combat::CONCUSSION_MAX, true);
            ...
            100 => {
                ...
                self.get_entity_mut(actor)
                    ...
                    .action_state = ActionState::Waiting;
                self.add_hidden_titbit_for_script_actor(tcx.assets, actor_handle, actor);
                wait(self, active)?;
            }
    ...
    pub(super) fn with_script_session<R>(
        ...
        let result = self.with_script_session_in_driver(tcx, callback);
        if let Some(script) = self.scripts.mission.as_ref() {
            script.assert_no_active_call_frames();
        }
        result
    ...
    fn execute_native_command(
        ...
        match command {
                ...
                match cmd {
                    ...
                    crate::natives::SoundCommand::ResumeAll => {
                        ...
                        schedule_source_finishes_for_all_active(
                            ...
                            &tcx.assets.audio.source_durations,
            ...
            crate::natives::NativeCommand::World(cmd) => {
                match cmd {
                    crate::natives::WorldNativeCommand::AddAsSubordinateInitialize {
                        ...
                        self.initialize_patrol_for_npc_prefix(tcx.assets, chief_id, member_count);
                    }
                    crate::natives::WorldNativeCommand::SelectPC { actor, select } => {
                        ...
                        if actor == 0 {
                            ...
                            if select {
                                ...
                                self.select_pc(tcx, 0, id, false, false);
                            } else {
                    ...
                    crate::natives::WorldNativeCommand::SetPlayable { actor, playable } => {
                        ...
                        if playable {
                            ...
                            let hide_interface = !self.is_sherwood(&tcx.assets.profile_manager);
                            let pc = self
                                .world
                                .entities
                                .get_mut(pc_id)
                    ...
                    crate::natives::WorldNativeCommand::ApplyPatch { patch_index, reset } => {
                        if reset {
                            self.reset_patch(tcx, patch_index);
                        } else {
            ...
            crate::natives::NativeCommand::Engine(cmd) => {
                self.apply_host_commands(tcx, std::iter::once(cmd));
            }
        }
        Ok(0)
    }
    ...
    pub(crate) fn load_mission_script(
        &mut self,
        assets: &LevelAssets,
        scb_path: &std::path::Path,
    ) -> Result<(), String> {
        ...
        if program.is_none()
            && !crate::sprite_script::original_mission_program_required(assets, true)
        {
            tracing::info!(
                "Mission {} uses its prepared replacement package without an original SCB",
                scb_path.display()
            );
            return Ok(());
        }
        ...
        match result {
            ...
            Err(e) => Err(format!(
                "Could not load required mission script {}: {e}",
                scb_path.display()
            )),
        }
    }
    ...
    pub(crate) fn initialize_mission_script_with(
        ...
        self.attach_script_bindings(tcx.assets);
        if tcx.assets.attachments.spellforge_runtime.is_some() {
            self.scripts
                .mission
                .as_mut()
        ...
        // during mission-stream initialization.
        ...
        // mission-stream initialization and walk the list calling
        // the scroll script's `Initialize()` callback.
        ...
        let bound = self.with_script_session(tcx, |script, _, _| {
        ...
        for (handle, _) in &per_scroll_scripts {
            ...
            if let Err(error) =
                self.call_script_vm(tcx, ScriptVmKey::Scroll(*handle), "Initialize", &[], frame)
            {
        ...
        // Engine initialization populates script-sector occupant lists before
        // IEngineScript::Initialize(0). Mission startup scripts query those
        // lists synchronously (for example H07's indoor NPC reveal), so the
        // scan cannot wait until after StartUp::Initialize returns.
        self.initialize_zone_occupants(tcx.assets);
        #[cfg(test)]
        ...
        self.initialize_zone_scripts(tcx);
    }

    /// Finalize the mission script (called on mission end).
    /// `abandoned` is true if the player quit/interrupted.
    pub(crate) fn finalize_mission_script(&mut self, tcx: TickCtx<'_>, abandoned: bool) {
        if let Err(error) = self.call_script_vm(
            tcx,
            ScriptVmKey::Global,
            "Finalize",
    ...
    pub(super) fn dispatch_actor_action_change_for(
        ...
        // Match actor updating by rereading animation only after
    ...
    #[cfg(test)]
    pub(crate) fn dispatch_actor_action_changes(&mut self, tcx: TickCtx<'_>) {
    ...
    pub(super) fn dispatch_scroll_hourglass_for(&mut self, tcx: TickCtx<'_>, scroll_id: EntityId) {
        ...
        let has_script = self
            ...
            .mission
            .as_ref()
            .is_some_and(|mission| mission.has_script_vm(ScriptVmKey::Scroll(handle)));
        if !has_script {
            return;
        }
        let timeout = {
        ...
        let frame = crate::natives::ScriptCallFrame::default()
            ...
            .with_current_scroll(handle);
        if let Err(error) =
            self.call_script_vm(tcx, ScriptVmKey::Scroll(handle), "Hourglass", &[0], frame)
        {
    ...
    /// Dispatch the script's `IsTaken(pc)` callback for a scroll being picked up.
    ///
    ...
    /// `Action::TakeScroll` dispatch) is not yet implemented; this helper
    ...
    pub(crate) fn scroll_is_taken(
        ...
        let has_script = self
            ...
            .mission
            .as_ref()
            .is_some_and(|ms| ms.has_script_vm(ScriptVmKey::Scroll(handle)));
        if !has_script {
            return false;
        }

        // Step 3 — dispatch within the scroll-executing-script scope.
    ...
    pub(crate) fn initialize_zone_scripts(&mut self, tcx: TickCtx<'_>) {
        ...
        let bound: Vec<usize> = self
            .with_script_session(tcx, |script, _, _| {
                ...
                for (zone_idx, class_name) in &classes {
                    ...
                    } else if tcx.assets.attachments.spellforge_runtime.is_some() {
    ...
    ///
    /// Implements the candidate-scan and exact-containment half of zone
    ...
    /// occupant-list insertion semantics that never trigger scripts.
    ...
    /// script-sector objects and clears their occupant lists — no
    ...
    ///
    /// Original-game script-sector initialization only appends occupants to the list. Zone
    ...
    pub(super) fn dispatch_script_line_crossing(
        ...
        let Some(zone_idx) = self.world.fast_grid.level.lines[usize::from(line_index)]
            ...
            // Script-line crossing is intentionally empty in the original game.
            return;
        };
        ...
        self.dispatch_script_zone_crossing(tcx, zone_idx, entity_id, inside);
    }
    ...
    fn dispatch_script_zone_crossing(
        ...
        if let Some(carried) = carried {
            self.dispatch_script_zone_crossing(tcx, zone_idx, carried, entering);
        }
    ...
    ///
    /// Flights do not traverse ordinary movement lines. The original game handles
    /// this exceptional relocation after flight, checking layer, owning motion-sector
    ...
    pub(super) fn update_script_sectors_after_flight(
        ...
        for (zone_idx, &grid_idx) in tcx.assets.scripts.zone_grid_indices.iter().enumerate() {
            if self.script_domains.zones.scripts[zone_idx].transformed_to_apex {
                continue;
            }
            ...
            if was_inside != is_inside {
                self.dispatch_script_zone_crossing(tcx, zone_idx, entity_id, is_inside);
            }
        }
    }
    ...
    /// script blocked the stimulus. Temporarily set the script receiver to
    /// this actor, evaluate `FilterAIEvent(stimulus_actor, event_code)`, and
    /// restore the previous receiver. A zero result logs rejection and
    /// prevents the decision tick.
    ///
    ...
    /// a script actor handle; other info variants become zero (originally empty).
    pub(crate) fn filter_stimulus(
        ...
        self.call_ai_event_filter(tcx, handle, source, code)
    }
    ...
    /// allows it. Returns `true` when a typed decision is filtered out,
    /// after its completion runs. Returns `false` when the entity
    ...
    ///
    /// Nested decisions enter this same boundary on the engine call stack.
    pub(crate) fn dispatch_filtered_stimulus(
        ...
        self.dispatch_filtered_stimulus_inner(tcx, entity_id, stimulus)
    }
    ...
    pub(super) fn dispatch_filtered_stimulus_inner(
        ...
        let entered_think = self.begin_ai_think_before_filter(tcx.assets, entity_id, stimulus);
        debug_snapshot(self, "before_filter");
        let handle = crate::natives::ScriptHandleCodec::actor_handle(entity_id);
        let filter_allowed = self.filter_stimulus(tcx, handle, stimulus);
        debug_snapshot(
            self,
            if filter_allowed {
        ...
        if !filter_allowed {
            if entered_think {
                self.execute_ai_end_think(tcx, entity_id);
            }
            return entered_think;
        }
        if !entered_think {
            return false;
        }
        ...
        let handled = if route_arrival {
            self.think_patrol_arrival(tcx, entity_id, stimulus)
        } else {
            self.execute_ai_think(tcx, entity_id, stimulus)
        };
        debug_snapshot(
            self,
            if handled {
    ...
    pub(in crate::engine) fn execute_ai_consider_to_begin_parade(
        ...
        self.consider_to_begin_parade(tcx, owner, attacker, Some(command_strike), animation_strike);
    }
    ...
    pub(crate) fn initialize_from_campaign(
        ...
        // Miscellaneous level initialization owns the forest-level flag.
            ...
            match cmd {
                ...
                EngineCommand::DefineFlatTrajectoryZone {
                    ...
                    if loc_idx < points_count
                        || loc_idx >= tcx.assets.scripts.location_positions.len()
                    {
                        tracing::warn!(
                        ...
                        if let Some(zone) = self.script_domains.zones.scripts.get_mut(zone_idx) {
                            if zone.script_associated {
                                ...
                                if let Some(&grid_idx) =
                                    tcx.assets.scripts.zone_grid_indices.get(zone_idx)
                                {
                ...
                EngineCommand::SetActorLocation {
                    ...
                    // Set map position, then layer/sector, then the projection
                    // obstacle; finally recompute all positions.
                    ...
                    // spawn, layer, and sector without assigning an obstacle,
                    ...
                    {
                        let new_obstacle =
                            self.get_projection_area_index(tcx.assets, sector, layer, pt);
                    ...
                    {
                        let elev = self
                            .position_to_point_3d(tcx.assets, Some(sector), layer, probe_x, probe_y)
                            .z;
                        if let Some(entity) = self.world.entities.get_mut(id) {
                ...
                EngineCommand::ScriptMakePCCrouched { actor_handle } => {
                    ...
                    self.actor_make_crouched(tcx, eid);
                }
                ...
                EngineCommand::HeroSpeak { pc_id, expression } => {
                    self.hero_speaking(tcx.assets, pc_id, expression);
                }
                EngineCommand::MakeNoise {
                    ...
                    // NPC noise handling resolves the
                    // script point through world-point conversion before the
                    ...
                    let source = self.position_to_point_3d(tcx.assets, Some(sector), layer, x, y);
                    tracing::debug!(
    ...
    /// Deactivate, mark hidden-in-building, move to the
    ...
    fn put_actor_in_building(&mut self, actor: i32, building: i32) {
        ...
        // The original game stores the building-sector reference itself on the actor, while
        ...
        if let Some(entity) = self.world.entities.get_mut(actor_id) {
            ...
            // After setting map position to the gate's point-in, re-derive
        ...
        if is_pc {
            ...
            // occupants list and activate humans that are
...
impl EngineInner {
    ...
    pub(crate) fn call_external_native(
        ...
        self.call_external_native_with_this(tcx, native_name, args, None)
    }
