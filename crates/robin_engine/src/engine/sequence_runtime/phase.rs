                            ...
                            match abilities::begin_strangle(
                                ...
                                AbilityBeginResult::Started => {
                                    ...
                                    let active = self
                                        .get_entity(owner)
                                        .unwrap_or_else(|| {
                                            panic!("strangler {owner:?} vanished during EventStop Think")
                                        })
                                        .actor_data()
                                        .unwrap_or_else(|| {
                                            panic!("strangler {owner:?} lost actor state during EventStop Think")
                                        })
                                        .active_ability
                                        .clone();
                                    assert_eq!(
                                        active.kind,
                                        Some(crate::movement::AbilityKind::Strangle)
                                    );
                                    assert_eq!(active.sequence_id, Some(seq_id));
                                    assert_eq!(active.element_index, elem_idx);
                                    ...
                                    let order_id = self
                                        ...
                                        .unwrap_or_else(|| {
                                            panic!(
                                                "strangle owner identity {seq_id:?}/{elem_idx} lost its order during EventStop Think"
                                            )
                                        })
                                        .order_id;
                                    assert_eq!(active.order_id, Some(order_id));

                                    let attacker_pos = self
                                        .get_entity(owner)
                                        .expect(
                                            "validated strangler vanished before initialization",
                                        )
                                        .element_data()
                                        .position_map();
                                    let victim_pos = self
                                        .get_entity(target)
                                        .expect("validated strangle victim vanished before initialization")
                                        .element_data()
                                        .position_map();
                                    let facing =
                                    ...
                                    self.get_entity_mut(owner)
                                        .expect(
                                            "validated strangler vanished before direction init",
                                        )
                                        .element_data_mut()
                                        .set_direction_goal(facing);
                                    self.get_entity_mut(target)
                                        .expect("validated victim vanished before direction init")
                                        .element_data_mut()
                                        .set_direction_goal(facing);
                                }
                            }
