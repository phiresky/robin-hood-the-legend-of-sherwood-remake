...
impl EngineInner {
    ...
    pub(in crate::engine) fn dispatch_script_synchronous_action(
        ...
        match action {
            SequenceAction::InstructOwner {
                ...
                {
                    // Original-game instruction returns after generating a transition if
                    // the transition cascade already made the element
                    // terminal. It never resolves priority for that
                    // retained element.
                    return Ok(());
                }
                ...
                {
                    // Actor instruction handling resolves priority after transition
                ...
                let barrier = if command == Command::Move {
                    ...
                    // Termination performs readiness processing and then
                    // postponed-element startup on the same original-game stack.
                    ...
                    // Movement issued from a sword action state is one
                    ...
                    // Releasing, launching, and instructing elements is re-entrant in the
                    // Original. A terminal owner card may therefore promote
                    // an EnterSwordfight successor inside the same actor
                    // update slot; use the ordinary command dispatcher,
                    // but do not defer it to the sequence-manager tick.
                    let opponent = self
                        .orders
                        .sequence_manager
                        .get_element(sequence_id, element_index)
                    ...
                    // Release/launch/instruction resumes it synchronously when the
                    ...
                    // postponed Face Turn through release → launch → instruction.
                    ...
                    // Releasing postponed sequence elements is re-entrant
                    // in the original game. A damage element released by the
                ...
                // Accepted actor instruction handling assigns the order after translation
                ...
                {
                    self.publish_instructed_order_as_installed(owner, sequence_id, element_index);
                }
    ...
    fn dispatch_synchronous_move_instruct(
        ...
        // Keep the synchronous native entry point on the same instruction
