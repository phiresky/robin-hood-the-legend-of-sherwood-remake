...
//! instruction sites to reject elements that were valid when queued but
...
impl EngineInner {
    ...
    pub fn check_sequence_element_validity(
        ...
        match element.command {
            ...
            //    All five require command eligibility, allowing buildings
            ...
            // ── EnterHelpingClimb ───────────────────────────────
            // Require outdoor command eligibility and the HelpToClimb action.
            ...
            // ── EnterBeggar ─────────────────────────────────────
            // Require outdoor command eligibility and the Beggar action.
            Command::EnterBeggar => {
                if actor.is_pc() && !self.can_pc_execute_commands(actor_id, false) {
                    return false;
                }
            ...
            // ── UseLever ───────────────────────────────────────
            // Original-game player sequence-validity checking
            // treats the interaction antagonist as the owning
            // For mobile elements, the cart must still be stopped and, on the
            ...
            // ── DropAmmo ────────────────────────────────────────
            // Require outdoor command eligibility without a position check.
            Command::DropAmmo => {
                if !actor.is_pc() {
                    return true;
                }
            ...
            // ── EnterListen ─────────────────────────────────────
            // Require command eligibility, allowing buildings without a position check.
            Command::EnterListen => {
                if !actor.is_pc() {
                    return true;
                }
            ...
            // ── Heal ────────────────────────────────────────────
            // Execution eligibility + positive healing supplies; for human
            ...
            // ── ThrowApple ──────────────────────────────────────
            // Execution eligibility + positive apple supplies + (non-human OR
            ...
            // ── ThrowPurse ──────────────────────────────────────
            // Execution eligibility + positive purse supplies + ransom budget covers a
    ...
    ///
    /// Init phase is detected via the actor update's selected-order identity,
    /// independently of sprite processing (FrozenAll consumes initialization
    /// even though sprite handling returns before stamping its own order ID).
    ///
    /// Human arms covered:
    ...
    pub(super) fn pre_tick_human_execute_validity_for(
        ...
        // The actor update establishes this for the exact execution call before
        // entering human action execution. Do not reconstruct it from a stale or
...
/// original-game downward-strike sequence validation.
///
/// `Execute(STRIKING_DOWN_SWORD)` repeats this check with
/// position checking disabled immediately after performing the action, so keep it usable
...
///
/// Distinguishes the `Aborted` (marked impossible) vs
/// terminated-result early exits into order advancement, plus the special
...
pub(super) enum ValidityArmTerminal {
    /// Maps an aborted motion to `element_impossible`. Note:
    ...
    Aborted,
    /// Maps a terminated motion to `element_terminated`.
    Terminated,
...
/// Human action execution owns the ordinary/high bow-release branches for players,
/// soldiers, and civilians. Player execution delegates those orders to it, while
...
mod tests {
    ...
    #[test]
    fn pc_hit_rejects_victim_in_door_transit() {
        let mut engine = EngineInner::new();
        let assets = LevelAssets::new();
        let actor = add_pc(&mut engine);
        let victim = add_soldier(&mut engine);
        engine
        ...
        engine
            ...
            .expect("test soldier NPC data")
            .life_points = 100;
        let Entity::Soldier(victim_entity) = engine.get_entity_mut(victim).expect("test soldier")
        else {
            panic!("test victim changed entity kind");
        };
        victim_entity.soldier.cached_camp = crate::element::Camp::Lacklandists;
        let element =
            SequenceElement::new_interaction(1, Command::HitCmd, Some(actor), Some(victim));
        ...
        engine
            ...
            .position_iface_mut()
            .set_door_for_test(
                crate::position_interface::DoorHandle::new(0).expect("valid test door"),
            );
