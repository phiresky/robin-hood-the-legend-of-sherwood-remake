...
impl EngineInner {
    ...
    fn send_condolation_card(
        ...
        let stimulus = match command {
            ...
            // reachability failure. Nothing for Interrupted (motion
    ...
    ///   Unlocking and view updates have no effect on corpses but are kept for
    ...
    fn send_condolation_card_pc(
        ...
        match command {
            ...
            Command::TakeCorpse => {
                ...
                // launch/instruction work created by the drop. It does *not*
                // run the body's update: starting the new move and the first
    ...
    /// Orchestrate a door-battle after an indoor enemy alert: pick the
    ...
    pub(crate) fn init_battle_before_door(
        ...
        // The original game copies the door's outside position into the battle center.
        // The ensuing position/vector arithmetic retains its sector reference
    ...
    pub(crate) fn send_before_door_to_fight(
        ...
        match kind {
            ...
            Some((_, true)) => {
                ...
                // The original game's send-before-door-to-fight behavior calls
    ...
    fn send_before_door_to_fight_pc(
        ...
        {
            // Original-game movement sequence construction adapts an actor
            // whose door pointer is non-null onto the committed far side
...
/// nearly aligned with the goal) return `true` (very slow turning);
/// every other delta returns `false` (slow turning). `direction`
...
mod tests {
    ...
    fn door_fight_route_adapts_selected_pass_door_before_building_exit_rng() {
        ...
        assert_eq!(
            ...
            "after door traversal clears the door pointer, the same raw building source must build one exit"
        );
