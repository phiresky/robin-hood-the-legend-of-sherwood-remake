...
impl ScriptRuntime {
    pub(crate) fn new() -> Self {
        Self {
            globals: Vec::new(),
            mission: None,
            spellforge: crate::spellforge::SpellforgeTape::default(),
            native_attachments_ready: false,
        }
    }
    ...
    pub(crate) fn preflight_level_assets(&self, assets: &LevelAssets) -> Result<(), String> {
        ...
        match (
            ...
            (Some(package), Some(runtime)) if runtime.package() != package => {
                return Err(format!(
                    "decoded Spellforge package {} does not exactly match attached package {}",
                    crate::spellforge::hex_hash(&package.sha256),
                    crate::spellforge::hex_hash(&runtime.package().sha256),
                ));
            }
            (Some(package), None) => {
                return Err(format!(
                    "decoded snapshot requires Spellforge package {}, but no Lua runtime is attached",
                    crate::spellforge::hex_hash(&package.sha256)
                ));
            }
            (None, Some(runtime)) => {
                return Err(format!(
                    "attached Spellforge package {} is absent from the decoded snapshot",
                    crate::spellforge::hex_hash(&runtime.package().sha256)
                ));
            }
            _ => {}
    ...
    pub(crate) fn initialize_spellforge_package(
        &mut self,
        assets: &LevelAssets,
    ) -> Result<(), String> {
