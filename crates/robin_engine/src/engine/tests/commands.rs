...
fn group_formation_uses_original_pair_distance_bounds_and_integer_radius() {
    ...
    assert_eq!(three_candidates[2].y.to_bits(), 0x4137_fffe);

    let mercenary = [
        crate::coordinates::map_pt(0.0, 0.0),
        crate::coordinates::map_pt(50.0, 0.0),
        crate::coordinates::map_pt(25.0, 43.0),
    ];
    assert!(uses_mercenary_group_formation(&mercenary));
    assert!(!uses_mercenary_group_formation(&[
        crate::coordinates::map_pt(0.0, 0.0),
        crate::coordinates::map_pt(70.0, 0.0),
        crate::coordinates::map_pt(35.0, 60.621_777),
    ]));
    assert!(!uses_mercenary_group_formation(&[
        crate::coordinates::map_pt(0.0, 0.0),
        crate::coordinates::map_pt(10.0, 0.0),
        crate::coordinates::map_pt(5.0, 8.660_254),
    ]));
    // The junctions cannot be reused, so an otherwise compact chain is not
    // enough: the final actor has no unused neighbour inside the limit.
    assert!(!uses_mercenary_group_formation(&[
        crate::coordinates::map_pt(0.0, 0.0),
        crate::coordinates::map_pt(50.0, 0.0),
        crate::coordinates::map_pt(100.0, 0.0),
    ...
    let click = crate::coordinates::map_pt(2097.0, 1679.0);
    let spread = [
        crate::coordinates::map_pt(1888.6392, 1660.1849),
...
fn ground_mark_hourglass_advances_and_retires_on_screen_marks() {
    let mut display = HostDisplayState::default();
    // The per-mark animation advance is gated on screen visibility and
...
fn set_campaign_value_ransom_emits_jingle_only_when_growing() {
    ...
    let jingle_count = engine
        ...
        .count();
    assert_eq!(jingle_count, 1);
    // Value assignment does NOT credit collected_money — only addition does.
    assert_eq!(engine.mission_domain.mission_stat.collected_money, 0);
}
...
fn timer_started_by_sequence_dispatch_ticks_on_its_launch_frame() {
    ...
    // DisplayPopupText is deferred through the sequence-manager tick. Its
...
#[test]
#[should_panic(expected = "swordfight evaluation extended range")]
...
#[test]
#[should_panic(expected = "smalltalk-hint evaluation owner")]
