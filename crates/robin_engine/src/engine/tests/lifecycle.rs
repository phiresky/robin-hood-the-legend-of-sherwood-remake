...
fn earlier_projectile_runs_before_later_bow_release_and_spawned_arrow_runs_again() {
    ...
    let expected_frame = ((spawned_arrow.projectile.last_orientation_azimuth as f32
        * 0.066_666_67_f32
        + 0.5_f32) as i32
        + 4) as u16;
    assert_eq!(
        (
            spawned_arrow.element.sprite.current_row,
            spawned_arrow.element.sprite.current_frame
        ),
        (
            spawned_arrow.projectile.last_orientation_sector,
            expected_frame
        ),
        "the released arrow must cross its normal Refresh publication boundary before this owner-pass snapshot"
    );
    assert_ne!(
        (
            spawned_arrow.element.sprite.current_row,
        ...
        (0, 0),
        "the integration fixture must remain causal for the missing newborn Refresh"
    );
}
...
#[test]
fn newborn_arrow_refresh_publishes_quicksave_flight_orientation_before_snapshot() {
    use crate::coordinates::WorldPoint3D;
    use crate::element::{
        Animation, ElementData, ElementKind, ElementProjectile, ObjectData, ObjectType,
    ...
    // QuickSave Projectile651 at frame-after 35731 entered Original Refresh
    // at this exact position with sprite 0/0/FFFF. A horizontal next segment
    // in sector 6 produces the captured Refresh result 6/4/FFFF.
    let current = WorldPoint3D::new(281.5, 3230.5, 20.25);
    let old = WorldPoint3D::new(257.25, 3213.75, 30.125);
    let [dx, dy] = crate::position_interface::sector_to_vector_iso(6);
    let next = WorldPoint3D::new(current.x + dx * 25.0, current.y + dy * 25.0, current.z);
    let mut element = ElementData {
        kind: ElementKind::ObjectProjectile,
        active: true,
        ..Default::default()
    };
    element.set_position(current);
    element.sprite.position_iface.set_old_position(old);
    let mut engine = EngineInner::new();
    let arrow = engine.add_entity(Entity::Projectile(ElementProjectile {
        element,
        object: ObjectData {
    ...
    let Entity::Projectile(before) = engine.get_entity(arrow).unwrap() else {
        unreachable!()
    };
    assert_eq!(
        (
            before.element.sprite.current_row,
            before.element.sprite.current_frame,
            before.element.sprite.frame_count
        ),
        (0, 0, u16::MAX)
    );

    let (_, draws) = crate::sim_rng::with_draw_trace(|| {
        engine.apply_newborn_arrow_refresh(&crate::sim_rng::test_context(), &[arrow]);
    });

    let Entity::Projectile(after) = engine.get_entity(arrow).unwrap() else {
        unreachable!()
    };
    assert_eq!(
        (
            after.element.sprite.current_row,
            after.element.sprite.current_frame,
            after.element.sprite.frame_count
        ),
        (6, 4, u16::MAX)
    );
    assert!(after.element.active && after.projectile.flying && !after.projectile.falling);
    assert!(draws.is_empty());
}

#[test]
fn newborn_falling_arrow_refresh_uses_fx_merged_display_order() {
    ...
    fn falling_arrow(position: WorldPoint3D, direction: u16) -> Entity {
        let mut element = ElementData {
            kind: ElementKind::ObjectProjectile,
            active: true,
            ..Default::default()
    ...
    let mut engine = EngineInner::new();
    let shallower = engine.add_entity(falling_arrow(WorldPoint3D::new(0.0, 10.0, 0.0), 2));
    let deeper = engine.add_entity(falling_arrow(WorldPoint3D::new(100.0, 20.0, 0.0), 8));
    ...
    let (_, draws) = crate::sim_rng::with_draw_trace(|| {
        // Deliberately pass shooter/release order, the inverse of display
        // order. The batch must still consume the deeper arrow's draw first.
        engine
            .apply_newborn_arrow_refresh(&SimulationContext::with_seed(seed), &[shallower, deeper]);
    });
    assert_eq!(draws, vec![RngSite::ArrowFallingFrame; 2]);
    ...
    assert_eq!(deeper_arrow.element.sprite.current_row, 8);
    assert_eq!(deeper_arrow.element.sprite.current_frame, deeper_frame);
    assert_eq!(deeper_arrow.projectile.falling_direction, 6);
    assert_eq!(shallower_arrow.element.sprite.current_row, 2);
    assert_eq!(
        shallower_arrow.element.sprite.current_frame,
        shallower_frame
    );
    assert_eq!(shallower_arrow.projectile.falling_direction, 0);
}

#[test]
fn newborn_arrow_refresh_uses_normal_retirement_path_without_touching_older_arrow() {
    use crate::element::{
        Animation, ElementData, ElementKind, ElementProjectile, ObjectData, ObjectType,
        ProjectileData,
    };

    fn settled_arrow() -> Entity {
        Entity::Projectile(ElementProjectile {
            element: ElementData {
                kind: ElementKind::ObjectProjectile,
                active: true,
    ...
    let mut engine = EngineInner::new();
    let older = engine.add_entity(settled_arrow());
    let newborn = engine.add_entity(settled_arrow());

    engine.apply_newborn_arrow_refresh(&crate::sim_rng::test_context(), &[newborn]);

    assert!(engine.get_entity(older).unwrap().element_data().active);
    assert!(!engine.get_entity(newborn).unwrap().element_data().active);
}
