...
fn tick_production_owner_coordinator(engine: &mut EngineInner, tcx: TickCtx<'_>) {
    engine.tick_actor_owner_envelopes(tcx);
}

/// Run movement and the following sequence phase to observe the completed frame.
fn tick_movement_and_sequences(engine: &mut EngineInner, tcx: TickCtx<'_>) {
    engine.tick_actor_owner_envelopes(tcx);
...
fn exact_building_source_draws_exit_wait(
    ...
    assert_eq!(
        ...
        "the original game consumes both random terms while constructing the exit wait"
    );
    let sequence = engine
        .orders
        .sequence_manager
...
fn number_only_building_source_draws_exit_wait(
    ...
    assert_eq!(
        ...
        "the first route gate retains the original game's exact building-side identity even when the compatibility source handle has only a duplicate public number"
    );
...
fn gate_builder_retains_pass_direction_and_faces_locked_gate_exit() {
    ...
    let capture = |direct| {
        ...
        assert_eq!(
            ...
            "Original movement appends arrival speech at the sequence count after movement-sequence construction increments it for the final movement"
        );
...
/// Original-game path processing never looks at the requesting
/// sequence element in the original game: a
...
/// because setting the successful-move command then writes to an element the
/// actor no longer executes.
#[test]
fn dead_path_request_still_consumes_its_scheduling_slot() {
    ...
    // with the move command; its already-queued request stays in the list.
    let dead = engine
        .orders
        .sequence_manager
...
fn expired_failed_path_dispatches_owner_card_at_paths_barrier() {
    ...
    assert!(
        ...
        "path-request processing must synchronously deliver the failed MOVE card before actor update slots"
    );
    ...
    engine.tick_ai_normal_timer_for_npc(TickCtx::new(&sim, &assets), earlier_timer_owner);
    let earlier_ai = engine.ai_ctrl(earlier_timer_owner);
    assert!(
        earlier_ai.ai_log.iter().any(|line| {
            line.line_type == LogLineType::Event && line.info == StimulusType::EventTimer as u16
...
fn make_fast_does_not_postprocess_an_unrelated_live_movement() {
    ...
    assert_eq!(
        ...
        OrderType::WalkingUpright,
        "original-game tests only selected the sequence element before fast-path processing"
    );
}
...
fn galopp_execute_callback_rejects_non_rider_owner() {
    ...
    engine.dispatch_galopp_loop_event(
        TickCtx::new(&crate::sim_rng::test_context(), &LevelAssets::new()),
        owner,
    );
}
...
fn production_owner_final_arrival_delivers_reachpoint_callback_exactly_once() {
    ...
    let (_, trace) = capture_condolation_stimuli(|| {
        tick_production_owner_coordinator(&mut engine, TickCtx::new(&sim, &assets))
    });
...
fn rider_charge_frozen_all_real_victim_is_damaged_once_across_multiple_ticks() {
    ...
    let ((), observations) = capture_sword_damage_observations(|| {
        tick_movement_and_sequences(&mut engine, TickCtx::new(&sim, &assets));
        tick_movement_and_sequences(&mut engine, TickCtx::new(&sim, &assets));
        tick_movement_and_sequences(&mut engine, TickCtx::new(&sim, &assets));
    });
...
fn rider_charge_frozen_then_unfrozen_initializes_sprite_motion_on_first_live_tick() {
    ...
    assert_eq!(
        ...
        "the first unfrozen motion step reports Start"
    );
}
...
fn rider_charge_first_execute_turns_before_initializing_new_motion_goal() {
    ...
    assert_eq!(
        ...
        4,
        "motion processing computes the new eastward goal after turning"
    );
    assert_eq!(
...
fn rider_charge_perform_motion_uses_live_anti_collision_diversion() {
    ...
    // Original-game sprite motion deviates from that physical commit.
...
fn rider_charge_retains_unhit_candidates_in_shared_human_sword_list() {
    ...
    // this candidate in the actor's sword-strike victim list after
...
fn current_movement_bootstraps_from_waiting_with_destination_state() {
    ...
    assert_eq!(
        entity.actor_data().unwrap().action_state,
        ActionState::Moving,
        "the first motion tick must enter the walking state"
    );
    assert_eq!(
        entity.element_data().position_map(),
        MapPoint::new(112.0, 100.0),
        "the initial invocation still commits its nonzero motion frame"
    );
    assert_eq!(
        entity
            .element_data()
...
fn deviated_blocked_post_step_arrival_pops_intermediate_waypoint_without_snapping() {
    ...
    // Keep this fixture focused on motion-processing order: the actor starts
    ...
    *sector = SectorHandle::new(1);
    ...
    assert_eq!(
        ...
        final_goal,
        "post-step goal detection must pop exactly the intermediate waypoint"
    );
}
...
fn concussion_wakeup_pc_applies_specific_blink_inline_to_opposite_camp_npcs() {
    ...
    assert_eq!(
        engine.apply_scripted_concussion(TickCtx::new(sim, &assets), waker, 0, true),
