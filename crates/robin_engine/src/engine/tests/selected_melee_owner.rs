...
fn frozen_all_bound_melee_animation_leaves_sprite_strike_and_order_untouched() {
    ...
    assert_eq!(
        ...
        7,
        "FrozenAll preserves goal-direction assignment plus exactly one turn before the sprite boundary"
    );
    assert_eq!(
...
fn eligible_principal_is_warned_once_on_start_and_not_again_in_progress() {
    ...
    assert!(
        ...
        "strike warnings must not repeat after MotionState::Start: {in_progress_warnings:?}"
    );
}
...
fn lateral_start_warns_in_original_actor_creation_order_before_rng() {
    ...
    assert_eq!(
        ...
        "strike-warning callbacks must follow actor insertion order, not PC slot order"
    );
    assert_eq!(
        rng_trace
            .iter()
...
fn straight_done_hits_principal_without_common_victim_filter() {
    ...
    assert!(
        ...
        "straight sword-strike victim collection must use principal + distance only for the completed hit as well as its warning"
    );
}
...
fn smalltalk_done_uses_ground_positions_for_back_hit_gate() {
    ...
    assert!(
        ...
        "smalltalk back-hit must use Original ground-position coordinates"
    );
}
...
/// The original game's bad-strike experience dispatch filters friends with the
/// actor-specific fighting-ability calculation, which applies the difficulty modifier for
