...
#[test]
fn accepted_empty_building_move_keeps_finalized_goal_after_terminal_card() {
    use crate::element::{Command, Posture};
    use crate::order::OrderType;
    ...
    let destination = MapPoint::new(1137.9464, 490.93048);
    {
        let entity = engine.get_entity_mut(owner).unwrap();
        entity.element_data_mut().set_position_map(start);
        entity.element_data_mut().set_sector(SectorHandle::new(42));
        // Save047 enters this replacement with the outgoing MoveOk goal
        // already finalized to the same building destination. The accepted
        // empty Move must not let its own terminal card erase that goal.
    ...
    let movement = engine
        ...
        .sequence_manager
        .get_element(sequence, 0)
        .expect("accepted building Move remains inspectable");
    assert_eq!(movement.state, SequenceState::Terminated);
    assert!(
        movement.orders.is_empty(),
        "ordinary interior Move finalizes position without producing an order"
    );
    let entity = engine.get_entity(owner).unwrap();
    assert_eq!(entity.element_data().position_map(), destination);
    assert_eq!(
        entity.position_iface().map_goal(),
        destination,
        "empty-order terminal condolence must observe the detached selection and preserve the finalized goal"
    );
    assert_eq!(
        entity.actor_data().unwrap().continuation.motion_state,
        MotionState::InProgress,
        "Actor::Instruct publishes IN_PROGRESS before terminating an accepted empty element"
    );
}
