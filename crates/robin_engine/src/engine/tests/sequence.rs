...
fn retained_shot_prelude_captures_replacement_for_aborted_execution() {
    ...
    let outgoing = engine.launch_element_for_owner(TickCtx::new(&sim, &assets), outgoing);
    complete_test_runtime_fixture(&mut engine, &mut assets);
    engine.mission_domain.campaign.characters[0]
        .status
        .set_ammo(crate::profiles::Action::Bow, 1);
    engine.elem_mut(owner).sprite.last_action = OrderType::TransitionLoadingBow;
    let incoming =
        SequenceElement::new_interaction(1, Command::ShootBow, Some(owner), Some(target));
    let incoming = engine.launch_element_for_owner(TickCtx::new(&sim, &assets), incoming);
    assert_eq!(
...
fn instruction_publication_follows_actor_selection_before_progress_promotion() {
    ...
    engine.actor_mut(owner).selected_sequence_element = Some(
        crate::sequence::SequenceElementRef::new(incoming_sequence, 0),
    );
...
fn waiting_alerted_execute_does_not_duplicate_a_leave_already_waiting_to_launch() {
    ...
    engine.execute_waiting_alerted(
        TickCtx::new(&crate::sim_rng::test_context(), &LevelAssets::new()),
        owner,
    );
...
fn manager_instruct_rejects_transition_terminated_element_before_priority_and_arbitration() {
    ...
    {
        let soldier = engine.ent_mut(owner);
    ...
    let live_sequence = engine.t_launch_in_progress(&assets, live);
    ...
    // stamped Crouched posture, transition generation follows the game's silent
    ...
    assert_eq!(
        ...
        "actor instruction returns before priority resolution"
    );
...
fn accepted_zero_order_damage_preserves_in_progress_motion_edge() {
    ...
    let mut engine = EngineInner::new();
    let soldier = engine.add_test_entity(make_test_soldier(Posture::Upright));
    engine.actor_mut(soldier).continuation.motion_state = MotionState::Done;

    // A malformed damage element is still accepted by actor instruction handling, then
...
fn manager_redundant_stop_parry_skips_instruct_motion_epilogue() {
    ...
    let soldier = engine.add_test_entity(make_test_soldier(Posture::Upright));
    {
        let actor = engine.actor_mut(soldier);
...
fn manager_redundant_enter_attentive_skips_instruct_motion_epilogue() {
    ...
    let soldier = engine.add_test_entity(make_test_soldier(Posture::Upright));
    {
        let entity = engine.ent_mut(soldier);
...
fn assert_position_translation_preserves_terminal_motion_edge() {
    ...
    engine.actor_mut(civilian).continuation.motion_state = MotionState::Terminated;
...
fn synchronous_accepted_wait_stamps_in_progress_motion_and_publishes_order() {
    ...
    let actor = engine.actor(soldier);
...
fn synchronous_accepted_zero_order_damage_stamps_in_progress_motion() {
    ...
    // no reaction order. Actor instruction handling detaches that accepted empty element
...
fn synchronous_accepted_zero_order_shoot_bow_stamps_in_progress_motion() {
    ...
    let target = engine.add_test_entity(make_test_pc(Posture::Upright));
    {
        ...
        // deliberately has no belt hotspot, so bow-target validation rejects the
    ...
    let sequence = engine.launch_element_for_owner(TickCtx::new(&sim, &assets), shot);
    ...
    let actor = engine.actor(shooter);
    assert_eq!(
        actor.continuation.motion_state,
        MotionState::InProgress,
        "accepted empty ShootBow must retain the actor instruction's motion edge"
    );
    assert!(
...
fn synchronous_terminal_enter_swordfight_skips_instruct_epilogue() {
    ...
    let sequence = engine.t_launch_element(&assets, enter);
    ...
    assert_eq!(
        visibility_queries.len(),
        1,
        "direct admission performs its visibility check once before terminal translation returns"
    );
}
...
fn manager_redundant_parry_skips_instruct_epilogue_after_generated_transition() {
    ...
    // Transition generation first appends the parry-to-waiting prefix. Translate
...
fn manager_redundant_quit_swordfight_skips_instruct_epilogue() {
    ...
    assert_eq!(
        actor.continuation.motion_state,
        MotionState::Terminated,
        "redundant QuitSwordfight returns before the actor instruction's motion epilogue"
    );
}
...
fn synchronous_self_seek_skips_instruct_epilogue() {
    ...
    let mut engine = EngineInner::new();
    let soldier = engine.add_test_entity(make_test_soldier(Posture::Upright));
    engine.actor_mut(soldier).continuation.motion_state = MotionState::Terminated;
...
fn entity_phase_completion_resumes_postponed_work_in_same_manager_drain() {
    ...
    let blocker_sequence = engine.orders.sequence_manager.insert_element(blocker);
    engine
        .orders
        .sequence_manager
    ...
    let successor_sequence = engine.orders.sequence_manager.insert_element(successor);
    engine
        .orders
        .sequence_manager
        .start_sequence_level(successor_sequence);
    engine.t_postpone_element(&assets, successor_sequence, 0);
    engine
        .orders
        .sequence_manager
    ...
    engine.t_element_terminated(&assets, blocker_sequence, 0);
...
fn deep_postpone_chain_preserves_unrelated_work_and_reaches_weak_tail() {
    ...
    let unrelated = engine.t_launch_sequence(&assets, unrelated);
...
fn interrupted_postponed_successor_is_replaced_after_its_condolation() {
    ...
    let blocker_sequence = engine.t_launch_in_progress(&assets, blocker);
...
fn post_seek_handoff_clears_selected_movement_goal() {
    ...
    engine.actor_mut(owner).post_seek_sequence = Some(post_seek.into_post_seek());

    let seek =
        SequenceElement::new_movement(1, Command::Seek, Some(owner), OrderType::WalkingUpright);
    let seek_sequence = engine.t_launch_in_progress(&assets, seek);
...
fn post_seek_handoff_registers_parent_successor_before_post_seek_tail() {
    ...
    let parent_id = engine.orders.sequence_manager.insert_sequence(parent);
    engine
        .orders
        .sequence_manager
    ...
    assert_eq!(
        ...
        "completion registers the parent successor before post-seek launch starts its tail"
    );
}
...
fn initial_seek_dispatch_clears_outgoing_movement_goal_until_first_execute() {
    ...
    let outgoing_sequence = engine.orders.sequence_manager.insert_element(outgoing);
    engine
        .orders
        .sequence_manager
...
fn same_building_entity_seek_keeps_replaced_movement_goal_when_translation_is_empty() {
    ...
    for entity in [owner, target] {
        engine.elem_mut(entity).set_sector(SectorHandle::new(42));
    }
...
fn same_sector_seek_waiting_for_pass_door_installs_generated_transition() {
    ...
    for entity in [owner, target] {
        engine.elem_mut(entity).set_sector(SectorHandle::new(24));
    }

    // f4987 starts from the player's selected bored Wait. Actor instruction handling
    // generates this exit transition before it interrupts that Wait
    // in the original game. Install the same
    ...
    let old_sequence = engine.orders.sequence_manager.insert_element(old_wait);
    engine
        .orders
        .sequence_manager
...
fn different_building_rewritten_seek_keeps_its_refresh_order() {
    ...
    let owner = engine.add_test_entity(make_test_civilian(Posture::Upright));
    let target = engine.add_test_entity(make_test_pc(Posture::Upright));
    engine.elem_mut(owner).set_sector(SectorHandle::new(42));
    engine.elem_mut(target).set_sector(SectorHandle::new(43));
...
fn assert_refreshing_seek_owner_envelope_ignores_stale_sprite_motion(
    ...
    let target = engine.add_test_entity(make_test_pc(Posture::Upright));
    {
        let element = engine.elem_mut(owner);
    ...
    // retaining seek movement. In a building, the original game does not teleport or
    // terminate this final element: it installs the seek-refresh hold.
    ...
    engine.elem_mut(owner).sprite.last_motion_state = Some(stale_sprite_motion);
...
fn point_refreshing_seek_with_successor_projects_back_to_in_progress() {
    ...
    let sequence = engine.t_launch_in_progress(&assets, refreshing);
    engine.select_sequence_element(owner, Some((sequence, 0)));
...
fn parry_sword_queues_transition_and_hold_orders() {
    ...
    let assets = LevelAssets::new();
    let soldier = engine.add_test_entity(make_test_soldier(Posture::Upright));
    engine.set_action_state_of(soldier, ActionState::WaitingSword);

    let seq_id = engine.t_launch_element(
        &assets,
        crate::sequence::SequenceElement::new(1, Command::ParrySword, Some(soldier)),
    );
    engine.t_instruct_owner(&assets, soldier, seq_id, 0);
...
fn waiting_parry_survives_normal_movement_successor_replacement() {
    ...
    let existing_sequence = engine.orders.sequence_manager.insert_element(existing);
    engine
        .orders
        .sequence_manager
        .start_sequence_level(existing_sequence);
    engine.t_postpone_element(&assets, existing_sequence, 0);

    let mut parry = SequenceElement::new(1, Command::ParrySword, Some(owner));
    parry.priority = SequencePriority::Preference;
    parry.posture_after_transition = Posture::Upright;
    let parry_sequence = engine.orders.sequence_manager.insert_element(parry);
    engine
        .orders
        .sequence_manager
        .start_sequence_level(parry_sequence);
    engine
        .orders
        .sequence_manager
    ...
    let incoming_sequence = engine.orders.sequence_manager.insert_element(incoming);
    engine
        .orders
        .sequence_manager
    ...
    let (accepted, cards) = crate::engine::soldier_helpers::capture_condolation_cards(|| {
        EngineInner::with_condolation_callback(
            ...
            || engine.t_instruct_owner(&assets, owner, parry_sequence, 0),
        )
    });
    assert!(
        accepted,
...
fn parry_sword_terminates_when_either_parry_is_already_active() {
    ...
    for (action_state, command, low) in [
        ...
        let assets = LevelAssets::new();
        let soldier = engine.add_test_entity(make_test_soldier(Posture::Upright));
        engine.set_action_state_of(soldier, action_state);
...
fn stop_parry_sword_queues_exit_transition() {
    ...
    let assets = LevelAssets::new();
    let soldier = engine.add_test_entity(make_test_soldier(Posture::Upright));
    engine.set_action_state_of(soldier, ActionState::ParryingSword);
...
fn fresh_wait_replaces_pre_init_upright_idle_with_authored_sitting_idle() {
    ...
    // wait before AI state initialization evaluates its authored initial animation.
    engine.actor_wait(
        TickCtx::new(&crate::sim_rng::test_context(), &assets),
        owner,
    );
...
fn idle_wait_runs_while_future_owner_action_is_behind_ownerless_timer() {
    ...
    // The original actor update sees no current order and launches its
    ...
    let scripted_sequence = {
        let launch_assets = engine.test_runtime_assets();
        engine.t_launch_sequence(&launch_assets, sequence)
    };
...

    {
        ...
        engine.ensure_wait_element(
            TickCtx::new(&crate::sim_rng::test_context(), &launch_assets),
            owner,
        )
    };
    let mut display = HostDisplayState::default();
...
fn play_anim_uses_custom_wrapper_instead_of_requested_animation_semantics() {
    ...
    for (command, wrapper) in [
        ...
        assert_eq!(
            actor.continuation.motion_state,
            crate::sprite::MotionState::InProgress,
            "accepted {command:?} must project the actor instruction's IN_PROGRESS edge"
        );
    }
}
...
fn soldier_enter_attentive_mode_queues_transition_anim() {
    ...
    {
        let launch_assets = engine.test_runtime_assets();
        engine.t_launch_element(&launch_assets, elem)
    };
    {
        let launch_assets = engine.test_runtime_assets();
        engine.ensure_wait_element(
...
fn attentive_then_forget_preserves_launch_but_clears_local_flags() {
    ...
    for forget_after in [false, true] {
        ...
        if forget_after {
            let enemy = engine.enemy_mut(soldier_id);
            enemy.attentive = false;
            enemy.will_be_attentive = false;
        }
        ...
        assert!(
            ...
            "the state change's earlier attentive-mode request must still launch"
        );
        let enemy = engine.enemy(soldier_id);
...
fn set_soldier_attentive_mode_plays_transition_while_movement_is_postponed() {
    ...
    // Instruction handling (and the movement postponement it causes) runs at the next
...
fn leave_attentive_translation_keeps_transition_after_attentive_was_already_cleared() {
    ...
    engine.ent_mut(soldier_id).set_posture(Posture::Upright);
    let sequence = engine.t_launch_element(
        &assets,
    ...
    let handled = engine.t_instruct_owner(&assets, soldier_id, sequence, 0);
    ...
    let soldier_id = engine.add_test_entity(make_test_ai_soldier(Camp::Lacklandists));
    engine.ent_mut(soldier_id).set_posture(Posture::Crouched);
    ...
    let handled = engine.t_instruct_owner(&assets, soldier_id, non_upright, 0);
    assert!(handled);
    let element = engine
        .orders
        .sequence_manager
...
fn enter_attentive_translation_still_suppresses_an_already_satisfied_enter() {
    ...
    let soldier_id = engine.add_test_entity(make_test_ai_soldier(Camp::Lacklandists));
    engine.ent_mut(soldier_id).set_posture(Posture::Upright);
    engine.enemy_mut(soldier_id).attentive = true;
    let sequence = engine.t_launch_element(
        &assets,
...
fn arbitration_ignores_serialized_order_ai_lock_like_original() {
    ...
    let current_seq = engine.t_launch_in_progress(&assets, current);
...
#[test]
fn injury_postpones_nonterminating_lift_wait_despite_done_sprite_cycle() {
    use crate::element::{Command, Posture};
    use crate::order::{Order, OrderType};
    use crate::sequence::{SequenceElement, SequencePriority, SequenceState};
    ...
    fn arbitrate_done_current(command: Command, order_type: OrderType) -> SequenceState {
        ...
        if command == Command::WaitFreeLift
            && matches!(
                order_type,
                OrderType::WaitingSword
                    | OrderType::WaitingUprightBored
    ...
    assert_eq!(
        ...
        SequenceState::Postponed,
        "WaitingSword execution returns InProgress, so injury postpones the blocked lift waiter itself"
    );
    assert_eq!(
        arbitrate_done_current(Command::WaitFreeLift, OrderType::WaitingUprightBored),
        SequenceState::Postponed,
        "WaitingUprightBored execution returns InProgress, so injury postpones the blocked lift waiter itself"
    );
    assert_eq!(
        arbitrate_done_current(Command::WaitFreeLift, OrderType::WaitingUprightBoredRandom,),
        SequenceState::Postponed,
        "WaitingUprightBoredRandom execution returns InProgress, so injury postpones the blocked lift waiter itself"
    );
    assert_eq!(
...
fn reentrant_lethal_interrupt_supersedes_injury_before_postponing_its_wait() {
    ...
    let outgoing_sequence = engine.t_launch_in_progress(&assets, outgoing);
    engine.select_sequence_element(owner, Some((outgoing_sequence, 0)));
...
fn pc_shoot_bow_waits_through_load_and_wait_then_retries_only_while_aiming() {
    ...
    let mut engine = EngineInner::new();
    let pc = engine.add_test_entity(make_test_pc(Posture::Upright));
    engine.elem_mut(pc).sprite.last_action = OrderType::TransitionLoadingBow;

    // A missing antagonist makes the eventual Translate deterministic and
    // side-effect free; this regression is about human instruction admission,
    // not projectile construction.
    let incoming = SequenceElement::new_interaction(1, Command::ShootBow, Some(pc), None);
    let incoming_seq = engine.launch_element_for_owner(TickCtx::new(&sim, &assets), incoming);
    ...
    engine.process_shoot_list_for(TickCtx::new(&sim, &assets), pc);
    ...
    // Only the bow-aiming idle admits the retained element. Instruction handling then
    ...
    assert_eq!(
        ...
        "the retained shot must resolve actor priority when readmitted"
    );
}

#[test]
fn postponed_held_pc_shot_leaves_human_fifo_owned_by_sequence_manager() {
    use crate::element::{Command, Posture};
    use crate::order::OrderType;
    use crate::sequence::{SequenceElement, SequencePriority, SequenceState};
    ...
    blocker.priority = SequencePriority::PostponeEverythingButInjuries;
    let blocker_seq = engine.t_launch_in_progress(&assets, blocker);
    ...
    let held_seq = engine.launch_element_for_owner(TickCtx::new(&sim, &assets), held);
    assert_eq!(
        engine.human(pc).pending_shoots,
    ...
    assert_eq!(
        engine
            .orders
            .sequence_manager
            .get_element(held_seq, 0)
            .unwrap()
            .state,
        SequenceState::Postponed,
        "the postponed element remains owned by the sequence manager"
    );
}
...
fn pc_shoot_list_readmits_retained_terminated_element() {
    ...
    let incoming = SequenceElement::new_interaction(1, Command::ShootBow, Some(pc), None);
    let incoming_seq = engine.launch_element_for_owner(TickCtx::new(&sim, &assets), incoming);
    engine.t_element_terminated(&assets, incoming_seq, 0);
    assert_eq!(
        engine
            .orders
            .sequence_manager
    ...
    let wait_seq = engine.t_launch_in_progress(&assets, wait);
    bind_test_action_point(
        &mut engine,
        pc,
        OrderType::AimingWithBow,
    ...
    // The shipped game re-enters instruction handling, but
    // its post-transition terminal-state check returns false. The
    ...
    let result = engine.tick_actor_animation_for(TickCtx::new(&sim, &assets), pc);
...
fn started_pass_door_rejects_new_move() {
    ...
    let pass_seq = engine.t_launch_in_progress(&assets, current_pass);
    engine.select_sequence_element(owner, Some((pass_seq, 0)));
    engine.actor_mut(owner).sequence_element_started = true;
...
fn parity_pass_door_snapshot_reads_selected_movement_without_runtime_latch() {
    ...
    *direction = -1;
    let sequence = engine.t_launch_in_progress(&assets, pass);
    engine.select_sequence_element(owner, Some((sequence, 0)));
...
fn executing_pass_door_postpones_new_move() {
    ...
    engine.actor_mut(owner).sequence_element_started = false;
...
/// the soldier command only fires when transition generation couldn't promote
/// posture to Upright (e.g. on a ladder).  That arm
/// isn't reachable from Crouched once transition generation is wired in.
#[test]
...
fn initialize_mission_script_binds_waypoint_classes() {
    ...
    let assets = crate::engine::LevelAssets::new();
    engine.initialize_mission_script_with(TickCtx::new(sim, &assets), 0, &paths);
...
fn mission_startup_runs_after_script_sector_occupants_are_initialized() {
    ...
    let (_, phases) = capture_mission_initialization_phases(|| {
        engine.initialize_mission_script_with(TickCtx::new(&sim, &assets), 0, &[]);
    });
    ...
    assert_eq!(
        ...
        "original-game engine initialization populates sector occupants before initializing the mission script"
    );
}
...
/// and visible state — instruction handling runs synchronously inside
/// sequence-element launch and ends with state `InProgress` after the
...
fn waking_up_done_clears_target_concussion_and_waits() {
    ...
    // The original game makes the target wait, replacing this equal-priority element,
    // rather than merely ensuring that some Wait exists.
    let stale_wait = engine.actor_wait(
        TickCtx::new(&crate::sim_rng::test_context(), &assets),
        target,
    );
    ...
    engine.execute_waking_up_done(TickCtx::new(sim, &assets), (rescuer, target));
...
fn get_killed_at_bottom_kills_lying_victim_immediately() {
    ...
    assert!(
        ...
        "a PC execution must preserve the NPC death's killer notification"
    );
    assert_eq!(
        engine.mission_domain.campaign.values[CampaignValue::Score],
        score_before + 50,
        "soldier wound handling awards the fight score after a Lacklandist dies"
    );
}
...
fn get_killed_at_bottom_uses_vip_pc_amulet_coma_save_and_preserves_existing_coma() {
    ...
    // against the already-comatose player. Original-game wound handling does nothing
