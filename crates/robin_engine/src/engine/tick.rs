...
impl EngineInner {
    ...
    pub(super) fn drain_script_synchronous_actions(
        ...
        while let Some(action) = self.orders.sequence_manager.pop_pending_immediate_action() {
            ...
            let dispatch_result =
                self.dispatch_script_synchronous_action(assets, action, active_scripts);
            let result = match dispatch_result {
                Ok(()) => self.drain_script_synchronous_actions(assets, active_scripts),
                Err(mut error) => {
    ...
    pub(super) fn drive_detached_sequence_operation(
        ...
        let dispatch_result =
            self.dispatch_script_synchronous_action(assets, operation.action, active_scripts);
        let result = match dispatch_result {
            Ok(()) => self.drain_script_synchronous_actions(assets, active_scripts),
            Err(mut error) => {
                if !error.sequence_element_failed {
        ...
        result?;
        self.drain_script_synchronous_actions(assets, active_scripts)
    }
