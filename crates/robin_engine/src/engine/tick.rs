...
impl EngineInner {
    ...
    fn hourglass_phase_entity_systems(
        ...
        let (arrived_entities, galopp_entities) = self.tick_entity_movement(sim, assets);
        ...
        // Only check entities actively moving in sword state.
        {
            ...
            for eid in ids_to_check {
                self.quit_swordfight_with_far_opponents(sim, assets, eid);
            }
        }
        ...
        if !arrived_entities.is_empty() {
            self.dispatch_reach_point_events(sim, assets, &arrived_entities);
        }
        ...
        if !galopp_entities.is_empty() {
            self.dispatch_galopp_loop_events(sim, assets, &galopp_entities);
        }
        ...
        // Moving actors are animated inside tick_entity_movement(sim, ).
    ...
    pub(super) fn drain_script_synchronous_actions(
        &mut self,
        sim: &crate::sim_rng::SimulationContext,
        assets: &LevelAssets,
        active_scripts: &mut Vec<super::script::ActiveScriptCall>,
    ) -> Result<(), super::script::ScriptDriverError> {
    ...
    pub(super) fn drive_detached_sequence_operation(
        &mut self,
        sim: &crate::sim_rng::SimulationContext,
        assets: &LevelAssets,
    ...
    pub(super) fn dispatch_script_synchronous_action(
        &mut self,
        sim: &crate::sim_rng::SimulationContext,
        assets: &LevelAssets,
        action: crate::sequence::SequenceAction,
    ...
    fn apply_script_user_lock(&mut self, assets: &LevelAssets, command: Command) {
        match command {
            Command::LockUser => {
                self.players.user_locked = true;
                self.feedback
