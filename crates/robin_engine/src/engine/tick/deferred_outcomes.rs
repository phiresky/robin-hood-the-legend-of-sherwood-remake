...
//!
//! These handlers preserve the original-game callback boundary: actor animation
...
impl EngineInner {
    ...
    pub(super) fn drain_weak_stunned_start(
        ...
        for (entity_id, anim_type, action_state_before_perform) in weak_stunned_start {
            // Human-actor execution notifies every adversary before
            // processing the action. Rust has to defer the cross-entity decision tick
