...
//! - Smoke belongs to mobile elements, but every chariot record in shipped
...
impl EngineInner {
        ...
        for t in self.feedback.titbit_manager.titbits_mut().iter_mut() {
            ...
            match t.kind {
                ...
                TitbitKind::QuickAction | TitbitKind::QuickActionRun => {
                    // The original game uses raw 3D position only for FX targets
                    // (world-position assignment plus map projection). Normal
    ...
    fn sync_emoticon_titbits(&mut self) {
        ...
        for npc_id in all_ids {
            ...
            // ── Active, outside a building, and not blipped ──
