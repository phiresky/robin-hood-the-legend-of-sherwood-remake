...
pub struct GameplayConfig {
    ...
    /// intelligence. This extension defaults off, including for profiles that
...
mod tests {
    ...
    #[test]
    fn profiles_without_a_gameplay_object_do_not_gain_opt_in_features() {
        let migrated = GameplayConfig::migrated();
        assert!(!migrated.sherwood_trading);
        assert!(!migrated.show_achievement_badges);
        assert!(!migrated.show_achievement_debrief);

        let fresh = GameplayConfig::default();
        assert!(fresh.sherwood_trading);
        assert!(fresh.show_achievement_badges);
        assert!(fresh.show_achievement_debrief);
    }
    ...
    #[test]
    fn fresh_and_migrated_profiles_disable_fog_of_war() {
        assert!(!GameplayConfig::default().fog_of_war);
        assert!(!GameplayConfig::migrated().fog_of_war);
    }
}
