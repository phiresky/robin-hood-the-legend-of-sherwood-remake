...
pub struct Door {
    ...
    ///
    /// The original game compares sector references when linking and traversing
    ...
    /// Lift-sector lowest/highest-door candidates are only the
    ...
    /// Index into the canonical mission patch table. Mirrors the original
    /// game's door-to-patch link, populated only for `door_triggered`
...
impl Door {
    ...
    pub fn click_polygon_contains(&self, x: f32, y: f32) -> bool {
        ...
        // Original-game polygon containment counts polygon boundaries
...
impl Door {
    ...
    /// the building's capacity check. For all other door types, pass `true`.
    ///
...
/// successful gate chain and an observed gate-path search failure instead of
...
/// Add reachable gates to the open-gate list.
///
...
pub fn find_path_into_door_with_sector_index(
    ...
    // The original game returns the level-authored midpoint, deserialized as
    // a point of its own between `point_out` and `point_in`. It is NOT the average of
    ...
    // in door-entry and door-node path searches measures against the
    // authored midpoint.
    let goal_mid = goal_door.point_mid;

    // No "the goal door already borders the source sector" shortcut:
    // the original game always seeds every gate of the source sector and runs
    // the A*, and the
    // goal test only fires when the goal door is *popped*
    // when the goal door is popped. A
    // cheaper multi-gate route can therefore relax the goal door first —
    // overwriting its direction, source distance, and
    // previous link, so
...
///
/// The original game's lift initialization owns only the doors whose inside
/// sector is this lift and picks the lowest/highest doors purely by
/// exit-point Y — the largest screen Y is the lowest door, the smallest
/// screen Y the highest. Door *type* plays no part:
/// a lift's high endpoint is frequently not a
/// `DOOR_LIFT_HIGH`. Comparisons are strict, so the first door wins a tie.
///
/// Every lift-sector endpoint pair is derived from these two doors.
/// The returned endpoints include the outside point, sector, layer, and exit direction.
...
mod tests {
    ...
    fn exact_gate_graph_links_alias_number_only_when_arena_identity_matches() {
        ...
        let mut doors = vec![
            ...
            Door {
                ...
                // identity is the original-game identity authority.
    ...
    fn avenger_wait_position_blocked_by_civilian_lock() {
        ...
        // Wait position = exit position = (100, 100) in sector 1.
    ...
    /// capacity check as original-game door authorization. The avenger can leave a
