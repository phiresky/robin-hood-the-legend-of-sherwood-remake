...
pub struct GraphicConfig {
    ...
    /// Relative path under `vendor/slang-shaders` for the selected
...
mod tests {
    ...
    fn default_values() {
        ...
        assert!(cfg.quick_action_cursor_pulse);
    }
    ...
    fn serde_roundtrip() {
        ...
        cfg.quick_action_cursor_pulse = false;
        ...
        assert!(!restored.quick_action_cursor_pulse);
    }
    ...
    #[test]
    fn old_profiles_enable_quick_action_cursor_pulse() {
        let mut json = serde_json::to_value(GraphicConfig::default()).unwrap();
        json.as_object_mut()
            .expect("graphics config serializes as an object")
            .remove("quick_action_cursor_pulse");
