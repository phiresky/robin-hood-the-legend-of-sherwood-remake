...
pub enum SynchronousScriptRequest {
    ...
    /// Clear the AI patrol before the VM invokes
    /// its next native. Clearing a patrol synchronously calls
    /// a forced return to duty for each default-state member, which requires the
    /// engine-owned AI and sequence machinery.
    RemoveAllSubordinates {
        actor: i32,
        native_return: i32,
    },
    /// Stop the actor before the VM invokes its next native.
...
/// One accepted AI-state operation. The native validates its
/// public arguments, then yields this typed operation without mutating the AI.
/// The engine can therefore run the SEEKING/FLEEING decision-entry NO_EVENT
