...
pub fn take_object(
    ...
    // The original game computes this subtraction from two unsigned 16-bit values and stores
    // the result in an unsigned integer during object pickup. Integer
    // promotion therefore makes an over-capacity legacy status negative
    // before conversion to unsigned 32 bits wraps it to a large positive capacity.
    // That quirk consumes the whole world pickup, while the later
    // The ammunition-increase range check rejects the inventory addition.
...
mod tests {
    ...
    fn pickup_over_capacity_consumes_world_object_without_adding_ammo() {
        // The original game's unsigned 16-bit subtraction is assigned to 32 bits. A save can
        // retain six units after Hard difficulty lowers this capacity to
        // four; the wrapped capacity removes the pickup, while
        // Ammunition increase rejects 6 + 3 > 4.
        let mut status = PcStatus::default();
