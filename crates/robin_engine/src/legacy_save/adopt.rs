...
//!
//! Original-game element references use creation-order IDs, while AI-local element
//! pointers use indices into the serialized element collection. Rust entity IDs
...
///
/// Original-game load fixup restores
/// the plane is set to the obstacle's top plane immediately after resolving the
...
/// Mission-created arrays used by original-game position-identity fixups.
///
/// Projection-area and sight-obstacle indices are separate Original spaces.
/// layer `0xffff` selects sight obstacles; every other layer selects
/// projection areas in the original game.
#[derive(Clone, Debug, PartialEq)]
pub struct LegacyPositionTopology {
    /// Original-game sparse sector slot to Rust's compact runtime sector.
    ...
    /// Original-game sparse sector slots whose object is a door sector.
...
/// Complete Rust counterpart of one non-null original-game sector reference.
///
...
/// Original-game sparse sector slot. They are intentionally independent.
...
/// Exact original-game jump-line identity space.
///
/// The original game serializes a line identity as its layer plus its ordinal inside
/// that layer's combined line array. Rust stores jump lines separately,
...
impl LegacyLineTopology {
    ...
    /// Resolve a shifted retail AI jump-line reference from
    /// the same owner/primary-target geometry that authored it.
    ///
    /// Table-swordfight detection finds a line in the primary target's sector
...
pub struct LegacyEntityFixups {
    ...
    /// `None` is an original-game mobile master, whose runtime identity lives in
...
impl LegacyEntityFixups {
    ...
    /// Resolve the AI serializer's 16-bit element index.
...
///
/// Required original-game behavior:
///
/// - Original-game sight-obstacle initialization inserts a
///   synthetic flat ground into the projection-area collection before every authored
///   projection obstacle.
/// - Sight-obstacle references store the obstacle ID minus one.
...
/// - Gate references index the complete gate collection; Rust retains
///   that same array in `interactables.doors`.
/// - sector references store their sparse sector slot directly.
...
pub fn derive_position_topology(
    ...
    // Compatibility surface until every saved-position consumer accepts the
...
pub(crate) fn preflight_v48_position(
    ...
    // Legacy enum validation copies the complete four-byte storage without
    // validating it. Projectile updates can copy a trajectory sentinel or
    // indeterminate trajectory material into the position interface before a
...
mod tests {
    ...
    fn position_topology_keeps_original_ground_and_obstacle_index_spaces() {
        ...
        // the retained authored ID minus one, while Rust handles still
