...
//!
//! Original-game element serialization first removes live dynamic elements,
...
//! global element creation counter, before replacing that provisional identity
//! with the identity stored in the save. Constructing a bonus element
//! additionally selects a random sprite frame, consuming one global RNG draw
            ...
            let entity_id = match planned {
                ...
                PlannedElement::Construct(entity) => {
                    if entity.kind().is_bonus() {
                        // Bonus initialization always invokes
                        // Random sprite-frame selection during the phase-one load
