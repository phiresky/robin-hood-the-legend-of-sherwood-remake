...
/// Preflighted common element/sprite state for an element embedded
/// outside the main element stream (currently a patch effect).
///
    ...
    pub(crate) fn apply(self, engine: &mut EngineInner) {
        ...
        for converted in self.records {
            ...
            if let Some(saved) = converted.actor {
                ...
                // The original game serializes one overloaded wait timer for both
...
fn apply_element_base(element: &mut crate::element::ElementData, converted: ConvertedElementBase) {
    ...
    // Original-game sprites store their own fields before delegating to
    // position serialization. On read, a changed alternate-profile
    // flag immediately switches the alternate profile, so the row is
...
fn restore_position_and_gameplay_posture(
    ...
    // The original game has only one posture source: element posture forwards
    // directly to the position interface embedded in its sprite, and that
            ...
            bucket
                ...
                .map(|detectable| {
                    Ok(Detectable {
                        ...
                        // Detectable serialization omits this member. On load
                        // the just-constructed detectable contributes false.
...
fn convert_local_ai_common(
    ...
    let mut converted = AiController {
        ...
        // remark-completion notification. The dedicated post-load plan owns
...
fn motion_state(value: u32, creation_order: u32) -> Result<MotionState, LegacyAdoptError> {
    match value {
        ...
        value => {
            // The actor update overwrites motion state with the
...
///
/// Unlike the neighboring enums, this value can contain arbitrary data before
/// the first AI decision. Preserve its complete four-byte storage word.
fn preserve_old_ai_state(raw: i32) -> i32 {
    raw
}

/// Preserve the indeterminate hostile-AI previous-state pair.
///
/// The original game initializes neither value. The pair is
/// assigned atomically before the only branch that consumes it, so arbitrary
/// saved words are inert unless that branch is live.
fn preserve_previous_enemy_state_word<T>(raw: i32) -> crate::ai::StoredEnumWord<T> {
    crate::ai::StoredEnumWord::from_raw(raw)
}
...
fn clear_ambush_points_on_enemy_load(ai_global: &mut AiGlobalState) {
    ...
    // Enemy AI serialization clears ambush points
    // (not just clearing its contents) for every enemy AI it reads.
...
fn convert_seek_point(
    ...
    Ok(crate::ai::SeekPoint {
        ...
        // The save format repeats these members at the end;
...
fn convert_waypoint_macro_cursor(
    ...
    // Breaking an AI macro only clears the in-progress flag; it
    // deliberately leaves both the byte counter and macro-command buffer alone.
    // The save format also stores the relative cursor whenever a patrol
    ...
    // on the macro-in-progress flag and never dereferences those dormant fields.
...
fn convert_stimulus(
    ...
    let (actual_info_type, actual_name, info) = match &saved.info {
        ...
        LegacyStimulusInfo::Noise {
            ...
            StimulusInfo::Noise(Noise {
                ...
                volume: *volume,
                elevation: *elevation,
                // The original game does not serialize this value in noise information.
                element_id: 0,
            }),
        ),
    ...
    let (stimulus_type, info) = match stimulus_type(
        ...
        Err(_) if is_uninitialized_default_stimulus(saved) => {
            // Stimulus construction initialized exactly these four
...
mod tests {
    ...
    fn sprite_adoption_preserves_original_profile_switch_before_position_restore() {
        ...
        assert_eq!(
            element.sprite.current_frame, 0,
            "Original's eager profile switch resets the frame before restoring position"
        );
    }
    ...
    fn linux_v48_previous_enemy_state_preserves_indeterminate_storage_bits() {
        assert_eq!(
            (
                preserve_previous_enemy_state_word::<crate::ai::AiState>(72).raw(),
                preserve_previous_enemy_state_word::<crate::ai::Substate>(0x5a3b_010e).raw()
            ),
            (72, 0x5a3b_010e),
        );
    }
