...
/// The original game reconstructs the player-character collection while loading every PC element,
...
/// constructor order.
///
/// Original-game beam-me setup can assign the selected team to beam-me points
/// in a different construction order after a campaign reload. Both
/// construction order and beam-me index can consequently differ, while
/// the character profile remains the exact sprite/behavior identity. Exact
/// description identity is preferred where the initialized campaign
...
fn remap_saved_beam_pc_identities(
    ...
    // The original game sets the static-element creation count immediately before
    // The original game's beam-me setup means every saved team PC
    // lies on the dynamic side of the load boundary. Element deserialization
    // reconstructs each one, consuming a provisional creation counter
