//! Validated adoption of original-game Linux-v48 spatial-grid runtime state.
//!
...
impl LegacyFastFindGridAdoptionPlan {
    ...
    pub(crate) fn apply(self, engine: &mut EngineInner) -> LegacyGridHostState {
        ...
        // cache as well. The original game's enemy-door search reads the live
        // door on every query; retaining the proto-time bit could let a
...
fn preflight_patch_fx(
    ...
    // Original-game patch restoration writes this common base after the ordinary element
...
fn preflight_static_repulsive_points(
    ...
    state
        ...
        .map(|(index, saved)| {
            ...
            Ok(RepulsivePoint {
                // Script handles are signed 32-bit values but the original game stores an unsigned value;
