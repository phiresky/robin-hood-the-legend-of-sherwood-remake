...
//!
//! [`super::adopt_elements`] owns the common element/sprite base.
...
///
/// Repulsive-object lookup rebuilds the point position and force from the
...
struct PlannedProjectile {
    ...
    trajectory_origin_layer: Option<crate::position_interface::Layer>,
...
fn preflight_object_item(
    ...
    let (object, projectile, saved_kind, predicate): (
        ...
        // Rust stores plain object-element leaves in the
...
///
/// The original game does not serialize damage on arrow elements: human hits read
/// it from the serialized bow profile and flat-shot flag at impact
/// in the original game. Rust carries the value on
...
fn saved_arrow_damage(
    ...
    // Bow serialization writes the profile index, which is
...
fn preflight_projectile(
    ...
    let trajectory = saved
        ...
        .map(|saved| {
            Ok(TrajectoryPoint {
                position: point(saved.position, "projectile trajectory point position")?,
                time: saved.time,
            })
        })
        .collect::<Result<Vec<_>, LegacyObjectLeafAdoptError>>()?;
    ...
    Ok(PlannedProjectile {
        ...
        // The dive flag is the water/hole landing state.
        // Arrow ricochet state is the derived falling
        ...
        trajectory_origin_layer: crate::position_interface::Layer::new(
            saved.trajectory_origin_level,
        ),
        flight_direction: saved.flight_direction,
        start: point(saved.start, "projectile start")?,
        end: point(saved.end, "projectile end")?,
...
pub(crate) fn preflight_vm(
    ...
    {
        // Loading an already initialized actor overwrites its script class
        ...
        // Member-variable serialization writes no bytes for a zero-member class,
    ...
    // Actor serialization always reads the script class, but only calls
    // Bind when script initialization is incomplete. Mission-created actors have
    // already been bound during level loading, so a loaded beam-me
