...
//! mutates or rebuilds them while finishing engine restoration: active NPC
//! remarks are completed during each local-AI read, the process-wide RNG is
//! reseeded during AI deserialization, its global forbidden-remark list is cleared,
...
/// Whether a load should reproduce the original game's ordinary saved-seed initialization or
...
pub struct LegacyPostLoadHostOutput {
    ...
    /// The original game's invalid-engine state: invalidate host-rendered background
    ...
    /// The original game resets sight obstacles and every projection area's
