...
pub struct LegacySequenceTopology {
    /// Complete original-game gate order mapped to Rust door/gate IDs.
...
impl LegacySequenceTopology {
    ...
    ///
    /// Original-game gate order includes both stateful doors and
    /// stateless jump gates. Rust represents both in its door table,
...
impl LegacySequenceAdoptionPlan {
    ...
    /// Resolve a tail-owned original-game sequence-element reference against the
    ...
    /// Resolve an original-game order reference in the manager-owned identity space.
    ///
...
pub(crate) fn preflight_v48_sequence_manager(
    ...
    // in the original game's sequence serialization. Preserve those otherwise
    ...
    Ok(LegacySequenceAdoptionPlan {
        ...
        // Original-game order ID zero is valid, while Rust uses `NonZeroU32`.
...
fn convert_element(
    ...
    // Original-game sequence elements do not initialize either
    // transition result. Actor instruction handling stamps both from the actor's current
    // state before transition generation or command translation can read them.
    // Only an actor-owned INPROGRESS element can expose the stored results to
    // execution. TODO and POSTPONED elements pass through instruction handling (and are
    ...
    // A missing actor command terminates before the stamp or any transition use.
...
/// Resolve the two links written together by the original game's jump-command construction.
///
/// Original stores the approached line and then its reciprocal associated
/// line during jump-command construction. Some retail data editions add
...
/// The original game's ordinary, fast, and element-facing turn translation writes the
/// actor's direction goal once, then appends a `TURNING` order. The serialized
/// The order's compute-direction flag remains at its default `true`, but the original game
...
fn convert_transition_result<T>(
    ...
    // Actor instruction overwrites both fields from the actor's live
    ...
    // Rust instruction boundary performs that same refresh.
    if dormant {
...
fn convert_movement_action(
    ...
    // Four movement-sequence paths cannot consume the action:
    //
    ...
    // * TELEPORT does initialize it, but immediate execution never reads it.
    //
    // Terminal elements are likewise never dispatched again: sequence startup
...
mod tests {
    ...
    fn preflights_and_atomically_restores_manager_identity_and_fifo_order() {
        ...
        assert_eq!(
            ...
            5,
            "three deserialized orders advance the original game's restored shared counter"
        );
