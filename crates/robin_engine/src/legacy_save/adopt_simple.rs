...
//!
//! These sections surround the sequence-manager data in engine saves.
//! Conversion resolves every original-game reference before mutating the initialized
...
impl LegacySimpleAdoptionPlan {
    pub fn preflight(
        ...
        // The ground mark serializes the stored sprite position directly. On
        ...
        for (index, saved) in titbits.titbits.iter().enumerate() {
            ...
            converted_titbits.push(TitbitInfo {
                ...
                // The original game writes this value twice and overwrites it
