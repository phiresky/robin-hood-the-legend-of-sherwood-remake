...
/// Read the complete player-character serialization grammar.
///
/// `read_human` must consume exactly the human-actor serialization at the
/// callback point. `read_sequence` must consume one sequence serialization
/// with pre-serialization disabled; it is called only when the preceding
...
impl<SequencePayload> LegacyPcPreHuman<SequencePayload> {
    fn read(
        ...
        reader.read_signature(
            ...
            "player-character actor fingerprint",
        )?;
...
/// Read only the bytes after NPC serialization in the soldier
/// serializer.
...
/// Read only the bytes after NPC serialization in Nottingham's
