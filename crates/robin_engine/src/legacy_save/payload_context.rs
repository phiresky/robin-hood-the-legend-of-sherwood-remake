...
//! The original game recovers them from the already initialized mission: script member
...
pub struct LegacyMissionPayloadMetadata {
    /// Original-game static-element creation count. Creation
    ...
    fn validate_dynamic_pc(
        ...
        if !self.is_dynamic(creation_order) || class != LegacyElementClass::ActorPc {
            ...
            return Err(reader.invalid_value(
                ...
                "a dynamic ActorPc supported by phase-one loading",
            ));
        }
        Ok(())
    }
...
impl LegacyPayloadDecodeContext for LegacyMissionPayloadDecodeContext<'_> {
    ...
    fn read_actor_script_members(
        ...
        if self.is_dynamic(creation_order) {
            ...
            // description, then actor restoration reads the
        ...
        // Actor restoration overwrites the script class from the save,
        // but an already initialized mission actor does not Bind that name
        // again. Script availability checks and member serialization consequently use
    ...
    fn read_local_ai(
        ...
        if self.is_dynamic(creation_order) {
            ...
            return Err(reader.invalid_value(
                ...
                "a static NPC; dynamic loading does not support NPCs",
            ));
        }
        let metadata = self.validate_identity(reader, creation_order, class)?;
    ...
    fn read_script_members(
        ...
        if self.is_dynamic(creation_order) {
            if class == LegacyElementClass::Scroll {
                // The original game constructs a dynamic scroll element with its
                // default constructor. Phase one does not instantiate a
                // script class, and scroll serialization writes no class name,
                // so no script instance or member bytes exist.
                return Ok(None);
            }
            let offset = reader.offset();
            return Err(reader.invalid_value(
        ...
        with_reader(&[], |reader| {
            ...
            assert!(
                context
                    ...
                    .unwrap_err()
                    .to_string()
                    .contains("dynamic loading does not support NPCs")
            );
            assert!(
                context
                    .read_actor_script_members(
