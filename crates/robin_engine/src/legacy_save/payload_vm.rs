//! Compiled-script-guided decoding of original-game v48 VM state.
//!
//! Saved VM state has no embedded schema. It follows
//! the bound SCB class's member list in declaration order. Ordinary
//! members are serialized as one opaque signed 32-bit value; native members delegate to
//! serializers registered by the engine for actors, scrolls, and
...
pub enum LegacyVmMemberKind {
    /// The original game writes a signed 32-bit value for every tag except the native type.
...
pub enum LegacyVmMemberValue {
    /// Exact little-endian bits from the original game's signed 32-bit slot.
    Raw32 {
        bits: u32,
    },
        ...
        match member.ty.native_type_name.as_str() {
            ...
            native_type => {
                return Err(reader.invalid_value(
                    ...
                    native_type,
                    "Actor, Scroll, or Location (the engine-registered serializers)",
                ));
            }
        }
    };
