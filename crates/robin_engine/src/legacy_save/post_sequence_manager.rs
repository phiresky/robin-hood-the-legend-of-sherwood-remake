//! Original-game v48 sequence-manager decoding.
//!
//! Manager-owned sequences use the registered sequence format: each
    ...
    pub fn read(
        ...
        reader.scope("sequence manager", |reader| {
            ...
            reader.read_signature(
                ...
                "sequence static-data fingerprint",
            )?;
            ...
            reader.read_signature(
                ...
                "sequence-element static-data fingerprint",
            )?;
    ...
    for deferred in deferred_elements {
        ...
        if !manager_element_ids.contains(&id) {
            return Err(reader.invalid_value(
                ...
                id,
                "an ID of a sequence element owned by this sequence manager",
            ));
        }
...
fn validate_fixups(
    ...
    for element in &sequence.elements {
        ...
        validate_optional_ref(
            ...
            fixups.mummy.0,
            sequence_ids,
            "an ID of a sequence registered while reading this sequence manager",
        )?;
        if let LegacyInlineSequenceElement::Movement(movement) = element {
