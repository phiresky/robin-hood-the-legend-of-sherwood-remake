...
pub enum LegacyMissingTopologyFact {
    /// Element creation order for each live static element.
    ///
    /// Entity slots preserve element order, but not hidden constructor draws
    /// from the creation counter (notably each engine-owned mobile master).
    ElementCreationOrders,
    /// Full fast-grid gate array, including byte-less jump gates.
    ///
    ...
    GridGateOrder,
    /// Sparse fast-grid sector array, including empty holes and the
...
///
/// Original-game master creation constructs thirty object masters
/// instances and the engine constructor creates one standalone projectile
/// trajectory. None enters the element collection, but all increment
/// global element creation counter.
...
pub struct LegacyStaticElementTopology {
    ...
    /// Original-game number of created static elements.
...
///
/// - Element construction assigns and increments the creation order.
/// - Master creation plus the projectile helper consume 31 orders.
/// - Every later mission-created element is inserted into
///   the element collection in construction order.
/// - Adding a mobile element inserts the already-constructed
...
fn build_original_static_element_sequence(
        ...
        && entity_assets.legacy_proto_element_chunk_order.is_empty()
    {
        return Err(missing_element_order(
            "prototype elements exist but their chunk order was not retained",
        ));
    }
    ...
    {
        return Err(missing_element_order(
            "mission elements exist but their chunk order was not retained",
        ));
    }

    // Beam population runs after the complete mission chunk loop.
    ...
    let patches = retained
        ...
        .map(|patch| {
            let handle = patch.fx_entity_handle.ok_or_else(|| {
                element_mismatch(format!(
                    "retained patch {} has no effect-element handle",
                    patch.patch_index
                ))
            })?;
            let slot =
...
/// Derive hiking paths in their exact stored order.
///
/// Required original-game behavior:
/// The original game's hiking-guide serialization walks paths
/// then waypoints without serializing either count. Waypoint serialization
/// serializes members only when global scripting and script-command mode are
/// both true.
...
/// Derive the mission-sized arrays consumed after titbit serialization.
///
