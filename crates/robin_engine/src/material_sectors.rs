...
/// One fast-grid material-sector insertion.
///
...
/// 0 and every other obstacle registers its own
/// list at that obstacle's layer
/// layer (`0xFFFF` for non-projection areas). A grid query therefore
...
pub struct MaterialSectors {
    ...
    /// Every sight-sector registration, in the original game's order:
    /// the default obstacle's sight list first, then
...
impl MaterialSectors {
    ...
    /// Append one sight-sector registration.
    pub fn register(
        &mut self,
    ...
    ///
    /// Mirrors the position interface's no-obstacle material arm
    /// It seeds the material with
    /// the default material, asks the grid for the SECTOR_SOUND sectors in
    /// the block at the current map position and layer, and takes the first one whose
