...
pub enum CustomDot {
    ...
    Animal = 666,
    // Horse marker value 777 is unsupported. No shipped
...
impl MinimapState {
    /// Restore only the minimap fields stored by the original game.
    ///
    ...
    pub fn is_over_widget(&self, screen_pos: ScreenPoint) -> bool {
        ...
        // matching the original game's strict interior check.
...
pub struct ElementDotInfo {
    ...
    /// Current relationship-aware classification. Present only when the
    /// original dot classifier consults an entity's allegiance.
    pub camp: Option<Camp>,
    /// Two-camp parity classification used when diplomacy visuals are off.
    pub legacy_camp: Option<Camp>,
}
...
fn classify_npc_soldier(info: &ElementDotInfo) -> Option<DotType> {
    ...
    let camp = info
        .camp
        .expect("ordinary non-VIP soldier minimap dot requires an allegiance classification");
    ...
    let is_enemy = camp == Camp::Hostile;
    if info.is_dead {
        Some(if is_enemy {
            DotType::DeadEnemy
        } else {
...
///
/// The original game's priority value packed an animal marker into bit 11; that
...
/// missions, so the animal bit is always 0 and unreachable.
