//! Runtime for the cart/chariot subset of mobile elements used by the shipped game
//! Robin Hood missions.
...
pub struct MobileElement {
    ...
    pub layer: u16,
    pub sector: u16,
    /// Current projection-area obstacle. Shipped records start empty and
...
impl MobileElement {
    pub fn from_raw(
        ...
        if path.waypoints.len() < 2 {
            return Err(format!(
                "mobile path {} has {} waypoints; movement requires at least two",
                raw.path_index,
                path.waypoints.len()
            ));
        }
    ...
    /// original game's movement-start update. `None` is the strict early return
    ...
    /// original game's goal and waypoint update. This must run only after
    ...
    fn execute_shipped_macro(
        ...
        // The original game consumes a random value even for a single 100% block.
...
mod tests {
    ...
    #[test]
    fn shipped_speed_acceleration_and_terminal_stop_match_game_behavior() {
