...
impl NativeContext<'_, '_> {
    pub(super) fn dispatch_actors(&mut self, native: NativeFn, stack: &mut NativeStack) -> i32 {
        ...
        match native {
            // --- entity type checks ---
            // Original-game current-actor lookup
            // returns the callback's script receiver verbatim.
            ThisActor => self.call_frame.script_this(),
            // Original-game behavior:
            // Scripted actor-count lookup returns
            // the script-element collection's size.
            ...
            IsActorObject | IsActorCharacter | IsActorPC | IsActorNPC | IsActorSoldier
            | IsActorCivilian | IsActorActive | IsActorRider | IsUnblipped => {
                let handle = stack.pop_i32();
                // IsActorActive and IsUnblipped historically diagnose null;
                // the other type predicates accept it as an absent object.
                if handle == 0 && !matches!(native, IsActorActive | IsUnblipped) {
                    return 0;
                }
                ...
                i32::from(match native {
                    IsActorObject => entity.is_object(),
                    IsActorCharacter => entity.is_actor(),
                    IsActorPC => entity.is_pc(),
                    IsActorNPC => entity.is_npc(),
                    IsActorSoldier => entity.is_soldier(),
                    IsActorCivilian => entity.is_civilian(),
                    IsActorActive => entity.is_active(),
                    IsActorRider => entity.soldier_data().is_some_and(|soldier| soldier.rider),
                    IsUnblipped => !entity.element_data().blipped,
                    _ => unreachable!("actor predicate dispatch is exhaustive"),
                })
            }
            ...
            SetActorPosture => {
                ...
                // follows the exact stop/broadcast/state/wait
            ...
            GetActorLocation => {
                ...
                match self.get_entity(actor) {
                    Some(entity) => {
                        ...
                        // The original game accepts every live element here. Some
                        // transient objects retain the raw no-layer sentinel;
                        // only a real sector pointer requires paired layer
                        // metadata in the computed position.
                        let meta = entity
                            .element_data()
                            .optional_layer()
                            .zip(sector)
                            .map(|(layer, sector)| (layer.get(), sector));
            ...
            IsInside => {
                ...
                // script-zone membership natives or on the
                    ...
                    && let Some(entity) = self.get_entity(actor)
                {
                    ...
                    // Script-sector containment compares its authored layer
                    // directly with the element layer. A
                    // projectile or other unplaced element may legitimately
                    // return the 0xffff sentinel; that simply differs from a
                    // normal script-zone layer and is not an invalid read.
                    let element_layer = ed
                        .optional_layer()
                        .map_or(u16::MAX, crate::position_interface::Layer::get);
                    if zone.layer != element_layer {
                        return 0;
                    }
            ...
            IsInsideBuilding => {
                ...
                if bld == 0 {
                    // No building specified: check if actor is inside any building
                    i32::from(
                        self.script_domains
                            .buildings
                            .actor_building
            ...
            UnBlip => {
                // Checks handle existence (warn + false on invalid
            ...
            GetCurrentAction => {
                // (1) Missing entity → warn + return 0
                // (2) Object → return the object's animation
                // (3) Non-actor → warn + return 0
            ...
            SetActorActionState => {
                // Requires an existing human (warn + early
            ...
            Sees => {
                ...
                let Some(npc_layer) = npc_entity.element_data().optional_layer() else {
                    return 0;
                };
                ...
                // Target side.
                let Some(tgt_layer) = target_entity.element_data().optional_layer() else {
                    return 0;
                };
            ...
            EnableViewCone => {
                // Requires an NPC (warn only, no early return)
                ...
                if self.ai_global().ezekiel_2517 {
                    ...
                    // damage on the target (requires a human).
            ...
            // --- action / property ---
            SetActionAvailable => {
                // Requires a PC, then validates `action ∈ [0, 5]`,
                // forwards an enable/disable message, and returns
                // true / false. The original-game message handlers do not
                // mutate the player-character disabled-actions table; real
            ...
            HasPCAction => {
                ...
                // Direct player-to-profile lookup (no raw-profile-index fallback for this native).
            ...
            HasAnyActionSelected => {
                // The script command checks the selected PC and the
                // process-global messenger action. A PC's remembered
