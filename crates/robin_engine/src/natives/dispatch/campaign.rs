...
impl NativeContext<'_, '_> {
    ...
    ///
    /// The original game's mission-team additions and removals pass the
    /// live player's description to the campaign. Character profiles are
        ...
        match self.get_entity(actor) {
            Some(entity) => {
                let Some(pc) = entity.pc_data() else {
                    script_error!(operation, "actor {actor} is not a PC");
                    return None;
                };
        ...
        match native {
            ...
            GetPCFromMissionTeam => {
                ...
                let Some(character_index) = self
                    ...
                    script_error!(native, "index {idx} is outside the current mission team");
                    return 0;
                };

                // Original-game mission-team lookup passes the selected
                // character description to engine lookup and returns that live
                ...
                // exact character description instantiated by this player.
            ...
            GetNumberOfBeamMes => {
                self.campaign.as_ref().map_or(5, |campaign| {
                    ...
                    if current_loc != Some(crate::profiles::MissionLocation::Sherwood) {
                        script_error!(native, "called from non-Sherwood mission");
                        return 0;
                    }
                    campaign
                        .next_mission_idx
            ...
            GetNumberOfActorsInSector => {
                ...
                if !self.is_script_sector_handle(loc) {
                    script_error!(native, "on non-sector handle {loc}");
                    return 0;
                }
                let Some(occupants) = self.zone_occupant_handles(loc) else {
                    script_error!(native, "missing zone for sector handle {loc}");
                    return 0;
                };
                ...
                match self.zone_occupant_handles(loc) {
                    Some(occ) => {
                        if idx >= 0 && (idx as usize) < occ.len() {
                            ...
                            script_error!(native, "index {idx} out of range (max={})", occ.len());
                            0
                        }
                    }
                    None => {
                ...
                if let Some(campaign) = self.campaign.as_mut() {
                    if let Some(pi) = profile_idx {
                        if let Some(char_idx) = campaign.get_character_by_profile(pi) {
                            ...
                            // the status name, which by then has
                            // either the localized peasant name
                            // from name generation or the
                        ...
                        script_error!(native, "cannot resolve actor {actor}");
                    }
                }
                0
            }
            ...
            SetExperiences => {
                ...
                // persistent copies. The player actor is constructed with
                // the player-character status record, and
                // Original-game capacity updates write through that pointer
                // the human actor status. Campaign serialization then writes
                ...
                if !matches!(self.get_entity(actor), Some(Entity::Pc(_))) {
                    script_error!(native, "passed non-PC actor {actor}");
                    return 0;
                }
                let profile_idx = self.resolve_profile(actor);
                if let Some(campaign) = self.campaign.as_mut()
            ...
            GetPCType => {
                ...
                if !self.actor_exists(actor) {
                    script_error!(native, "Trying to get the PC type of an invalid actor!");
                    return -1;
                }
                if !matches!(self.get_entity(actor), Some(Entity::Pc(_))) {
                    script_error!(native, "Trying to get the PC type of a non-PC!");
                    return -1;
                }
            ...
            IsPCSelected => {
                ...
                // infinite-loop.
                if !matches!(self.get_entity(actor), Some(Entity::Pc(_))) {
                    script_error!(native, "The Actor in IsPCSelected is invalid.");
                    return 1;
                }
            ...
            GetSelectedPC => {
                ...
                // before returning no value. Treat negative indices
                // as out-of-range too.
                let selected = self.selected_pc_handles();
                if idx < 0 || (idx as usize) >= selected.len() {
                    script_error!(
                        native,
                        "index {idx} out of range (count {})",
                        selected.len()
                    );
                    return 0;
                }
            ...
            ComputeLocationBetween => {
                ...
                // layer and sector. The result inherits the first operand's
                // layer/sector.
                ...
                if !self.is_script_point(loc_a) || !self.is_script_point(loc_b) {
                    script_error!(native, "non-point handle(s) {loc_a}, {loc_b}");
                    return 0;
                }
                        ...
                        || !crate::natives::script_sector_identities_match(sector_a, sector_b))
                {
                    script_error!(
                        native,
                        "locations span different layers/sectors (a={layer_sector_a:?}, b={layer_sector_b:?})"
                    );
                    return 0;
                }
                match (
                    ...
                    (Some(pos_a), Some(pos_b)) => {
                        ...
                        // Inherit layer/sector from the first operand. If it
