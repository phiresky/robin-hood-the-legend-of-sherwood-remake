...
impl NativeContext<'_, '_> {
    pub(super) fn dispatch_sequences(&mut self, native: NativeFn, stack: &mut NativeStack) -> i32 {
        ...
        match native {
            ...
            SetViewRadius => {
                // The original game narrows the script integer to 16 bits,
            ...
            RecordMoveIntoBuilding => {
                ...
                // (entry point, layer, sector) and finish by recording movement.
                let style = stack.pop_i32();
                let loc = stack.pop_i32();
                let actor = stack.pop_i32();
                if !self.is_actor_handle(actor) {
            ...
            RecordPlayAnimLoop => {
                let anim = stack.pop_i32();
                let actor = stack.pop_i32();
                // Uses the full handle-existence validator (no
            ...
            RecordSpeak => {
                ...
                let actor = stack.pop_i32();
                // Requires a human and bound-checks
            ...
            RecordTakeCorpse => {
                ...
                if let Some(corpse_entity) = self.get_entity(corpse) {
                    ...
                    // original action-distance calculation.
                    let Some(animation) =
