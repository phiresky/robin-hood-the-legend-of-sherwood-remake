...
impl NativeContext<'_, '_> {
    pub(super) fn dispatch_world(&mut self, native: NativeFn, stack: &mut NativeStack) -> i32 {
        ...
        match native {
            ...
            SetAnimationState => {
                // Rejects missing or non-FX entities with a warning
            ...
            LinkTargetToFX => {
                ...
                // setter has no visible effect in the shipped game,
                // but we retain the storage
                // so a future implementation of the highlight effect has
                // somewhere to read.
            ...
            AttachScrollToNPC => {
                // Four branches:
                //   1. missing entity or non-NPC -> warn + early return
                //   2. no scroll               -> attach nothing (detach)
                //   3. not an object, or object type is not scroll
                ...
                // Original-game scroll attachment updates the NPC's
