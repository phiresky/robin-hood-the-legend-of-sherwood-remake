...
/// sequence system is implemented.
#[derive(
    Debug,
    Clone,
    serde::Serialize,
...
pub struct Order {
    ...
    pub lock_ai: bool,
    /// Complete serialized order transition/flight frontier. These members
...
/// an `order_id` yet. AI code (`AiController::go_to`, `raise_shield`,
/// `point_to`, etc.) has no `EngineInner` reference and can't allocate
/// an id, so it pushes `AiOrderIntent`s onto `AiController.outbox.actor.orders`.
...
pub struct AiOrderIntent {
    ...
    /// movement construction. Distinct motion sectors can share one number;
    ...
    pub no_halt: bool,
    /// Original-game AI movement prepends a
    ...
    /// the movement's own sequence. The original game inserts it
    ...
    /// Non-sword movement issued while menacing carries `StopMenace` as the
...
impl AiOrderIntent {
    ...
    ///
    /// The original game constructs the movement sequence from
    /// the actor's map position, sector, and layer synchronously in
    /// an AI movement request; it has no later fallback that
    ...
    pub(crate) fn validate_queued_move_topology(&self) -> Result<(), String> {
        ...
        // slot derived from another actor's position), so it is stronger
