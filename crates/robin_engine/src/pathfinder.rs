...
///
/// The parity recorder takes this snapshot both when a request is accepted
/// and when its result is delivered. The two snapshots can legitimately
/// differ because speed and posture changes update a pending request.
...
/// is the original request's otherwise-unused raw sector value; `time` is
...
// path-request processing point. This type retains only the serializable A*
...
pub struct PathGraphNode {
    ...
    // Runtime queries copy these into their separate NodeSearchState workspace.
...
pub struct MotionObstacle {
    ...
    /// toggles the sector and all of its perimeter lines together.
    #[serde(default)]
...
impl PathFinder {
    ...
    /// obstacle sector has been registered, matching bulk state assignment's sector
...
impl PathFinderRuntime {
    ...
    fn find_path_nodes(&mut self, grid: &FastFindGrid, goal: MapPoint) -> Option<NodeIdx> {
                            ...
                            && new_dist < self.shortest_distance_found
                        {
                            ...
                            if !next.visited {
                                next.distance_to_goal =
                                    Self::estimate_distance(next_position, goal);
                            }
    ...
    fn pass_around_node(&self, link: LinkIdx, leave_places: u8, path: &mut Vec<MapPoint>) -> u8 {
        if leave_places == 0 {
            return 0;
        }
    ...
    /// do so.
    fn smooth_path(&self, grid: &FastFindGrid, path: &mut Vec<MapPoint>) {
        if path.len() <= 3 {
            return;
        }
...
mod tests {
    ...
    fn link_source_does_not_bypass_blocked_docking_corridor() {
        ...
        // x=60. The original game requires that corridor check even when it
