...
//! stats (score, ransom, difficulty, etc.). The manager owns only values,
//! collection operations, and archive validation. Platform persistence belongs
//! to the host; serialized directory metadata does not grant filesystem authority.
...
/// boundary. This keeps difficulty queries and the three-entry scroll
...
const PLAYER_PROFILE_LIMIT: usize = 10;
const PLAYER_PROFILE_NAME_BYTE_LIMIT: usize = 4 * 1024;
...
///
/// Persistence is provided by the host; this collection does not perform IO.
#[derive(
    Debug,
    Clone,
    Serialize,
...
pub struct PlayerProfileManager {
    ...
    /// Legacy archive metadata, not live storage authority.
...
/// Invalid persisted profile values, independent of any storage adapter.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, thiserror::Error)]
#[error("{message}")]
pub struct InvalidProfileArchive {
    message: String,
}

impl InvalidProfileArchive {
...
impl PlayerProfileManager {
    /// Create an empty collection retaining legacy directory metadata.
    ...
    pub fn validate_archive(&self) -> Result<(), InvalidProfileArchive> {
        if self.profiles.is_empty() || self.profiles.len() > PLAYER_PROFILE_LIMIT {
            return Err(InvalidProfileArchive::new(format!(
                "player-profile archive contains {} profiles; expected 1..={PLAYER_PROFILE_LIMIT}",
                self.profiles.len()
            )));
        }
        let active = self
        ...
        if active.is_none() {
            return Err(InvalidProfileArchive::new(format!(
                "invalid active player-profile index {:?}",
                self.active_index
            )));
        }
        let mut ids = std::collections::BTreeSet::new();
        for (index, profile) in self.profiles.iter().enumerate() {
            if profile.name.is_empty() || profile.name.len() > PLAYER_PROFILE_NAME_BYTE_LIMIT {
                return Err(InvalidProfileArchive::new(format!(
                    "player profile {index} name is {} bytes; expected 1..={PLAYER_PROFILE_NAME_BYTE_LIMIT}",
                    profile.name.len()
                )));
            }
            if !ids.insert(profile.id) {
                return Err(InvalidProfileArchive::new(format!(
                    "duplicate player profile id {}",
                    profile.id
                )));
            }
            if profile.active != (active == Some(index)) {
                return Err(InvalidProfileArchive::new(format!(
                    "player profile {index} active flag disagrees with active index"
                )));
            }
            profile.campaign_history.validate_schema().map_err(|version| {
                InvalidProfileArchive::new(format!(
        ...
        {
            return Err(InvalidProfileArchive::new(
                "player-profile next id would reuse an existing durable identity",
            ));
        }
        Ok(())
    }
    ...
    /// This is a pure collection operation; the host owns save-directory cleanup.
    ///
    /// # Panics
...
mod tests {
    ...
    #[test]
    fn archive_validation_rejects_reassigned_identity_and_active_mismatch() {
        let mut manager = PlayerProfileManager::new("/tmp/test_profiles".into());
        manager.create_profile("A".into(), DifficultyLevel::Easy);
        manager.create_profile("B".into(), DifficultyLevel::Medium);
        manager.set_active(0);
        manager.profiles[1].id = manager.profiles[0].id;
        assert!(
            manager
                .validate_archive()
                .unwrap_err()
        ...
        manager.profiles[1].id = 1;
        manager.profiles[1].active = true;
        assert!(
            manager
                .validate_archive()
                .unwrap_err()
                .to_string()
                .contains("active flag")
        );
    }
