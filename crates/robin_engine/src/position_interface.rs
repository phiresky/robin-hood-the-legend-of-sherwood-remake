...
//! repulsive-line / repulsive-point grid buckets are implemented via
...
impl PlaneZCoeffs {
    ...
    /// plane-height computation; points lying exactly on a sloped plane can
    ...
    pub fn from_plane_points(points: &[[f32; 3]; 3]) -> Self {
        ...
        // Preserve the plane initialization's floating-point operation sequence
...
pub struct SectorHandle {
    ...
    /// remain public-number based; original-game identity comparisons must opt in
    /// through [`Self::arena_index`]. Keeping the companion in this copyable
    /// value lets position-plus-vector copies retain provenance
...
/// Explicit identity view of a sector reference. Unlike `SectorHandle`'s
/// historical public-number equality, equality here includes the arena slot.
/// Number-only references remain valid for authored nonpolygon sectors and
/// compatibility inputs; the stored handle cannot prove which origin applies.
/// TODO(B6): distinguish those two origins at ingestion before requiring an
/// authored/nonpolygon variant in simulation APIs.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
pub enum SectorReference {
    Exact {
        number: crate::sector::SectorNumber,
        index: crate::fast_find_grid::SectorIndex,
    },
    NumberOnly(crate::sector::SectorNumber),
}

impl SectorHandle {
    #[inline]
    pub fn reference(self) -> SectorReference {
        match self.arena {
            Some(index) => SectorReference::Exact {
                number: self.public,
                index,
            },
            None => SectorReference::NumberOnly(self.public),
        }
    }
    ...
    /// Decode a nullable serialized sector-reference slot. This is intentionally
...
/// Keep the factor at each call site even though multiplication is a no-op,
/// so callers consistently pick up any future aspect-ratio adjustment.
pub const SWORDFIGHT_ASPECT_RATIO: f32 = 1.0;

/// Inverse sword-fight aspect ratio — `1.0` in the shipping game.
/// Sword combat does not apply isometric correction, so scaling Y by this
/// factor is a no-op.
...
/// Match the original game's vector-normalization operation order under the
...
pub struct PositionInterface {
    ...
    // The original game keeps posture in the position interface. ElementData remains
    ...
    // -- Material --
    // The original game serializes the enum as an unchecked unsigned 32-bit value. Projectile
...
/// position-interface serialization.
///
...
pub(crate) struct PositionInterfaceV48State {
    ...
    /// Raw unchecked material storage. Validated only by live material
    /// access, matching the original game's plain enum-byte copy.
    pub material: u32,
...
impl PositionInterface {
    ...
    /// Mirror original-game inline posture selection.
    ///
    ...
    /// Install the two posture slots written by the Original position-interface
    /// constructor. This is distinct from [`Self::set_posture`]: construction
    /// initializes both `mposture` and `mpostureOld` to the same value rather
    /// than recording a transition from an earlier posture.
    pub(crate) fn initialize_constructor_posture(&mut self, posture: crate::element::Posture) {
        self.saved_posture = posture;
        self.saved_old_posture = posture;
    }
    ...
    /// Return the exact cached original-game sprite position when one has been
    ...
    /// Install the exact integer sprite-space top-left used by original-game gameplay
    /// hotspot queries.
    #[inline]
    ...
    pub fn set_position(&mut self, pt: WorldPoint3D) {
        ...
        // Original-game 3D position selection makes
        // the supplied world point the sole authoritative projection.  In
        // particular, flight processing can install a 3D point while no
        // ground obstacle/plane is attached; a same-slot eye-point calculation
    ...
    /// projection valid, matching the original game's fully-computed position state.
    ///
    ...
    /// the delayed map position is applied by the next actor update.
    #[inline]
    ...
    /// Publish the projections produced by the original game's complete position calculation
    /// after a flight position update.
    ///
    /// The caller supplies the sprite-space point because it depends on the
    /// owning sprite's centre, which is deliberately not stored in the
    /// position interface itself.
    pub(crate) fn finish_flight_position_update(&mut self, sprite: MapPoint) {
        self.position_sprite = sprite;
        self.position_sprite_valid = true;
        self.computed_position = PositionComputed::ALL;
    }
    ...
    /// sprite takeoff preparation and retained by flight.
    pub(crate) fn set_flight_goal_and_increment(
        &mut self,
        goal: WorldPoint3D,
        increment: WorldVec3D,
        goal_sector: Option<SectorHandle>,
        goal_sector_index: Option<SectorIndex>,
    ) {
        self.goal = goal;
        self.increment = increment;
        self.computed_increment = IncrementComputed::INCREMENT;
        self.set_goal_sector_topology(goal_sector, goal_sector_index);
    }

    /// Publish the increment installed by the projectile update.
    /// Projectile flight has no position goal, so it must not use the actor
    /// flight helper above (which would fabricate one).
    pub(crate) fn set_projectile_increment(&mut self, increment: WorldVec3D) {
        self.increment = increment;
        self.computed_increment = IncrementComputed::INCREMENT;
    }
    // ====================================================================
    ...
    /// representation is valid. This matches zeroing the map increment,
    /// clears the increment, then marks every increment projection computed in the original game.
    ...
    ///
    /// Available implementation evidence never clears the deviation flag on this path, yet
    ...
    /// count-changing call happens between the two, so no direction-count
    ...
    /// the direction count is left untouched, mirroring how
    /// Disabling anti-collision clears only the latch, never the counter.
    ...
    pub fn update_forecasted_movement(&mut self, distance: f32, wait_time: u16) {
        ...
        let scaled_increment = inc.scale(distance);

        // The original game multiplies distance by the increment and divides by wait time.
        // The original game's 3D vector division evaluates that final division as
        // `(1 / k) * vect`, rounding the reciprocal to single precision before the
        // component multiplications. Direct component-wise
        // division can differ by one ULP.
        self.forecasted_movement = scaled_increment.scale(1.0 / wait);
    }
    ...
    /// normal new-order motion setup. Once they finish, stale
    ...
    // in sync. The original game's sprite position is reconstructed from element data at
    ...
    /// The original game stores this reference directly on the position interface and
    /// refreshes it only when sprite motion initializes a new
...
pub(crate) fn vector_to_sector_0_to_15_with_aspect(x: f32, y: f32, aspect_ratio: f32) -> i16 {
    ...
    // Preserve the 16-way vector-sector conversion's literal half-plane
...
pub fn vector_square_norm_iso(x: f32, y: f32) -> f32 {
    // Vector norm calculations divide by the supplied aspect ratio.
...
pub fn vector_normal_iso(x: f32, y: f32, direct: bool) -> [f32; 2] {
    // Vector-normal calculation divides by the aspect ratio here. Using
...
mod tests {
    ...
    #[test]
    fn explicit_sector_references_distinguish_duplicate_public_numbers() {
        let number_only = SectorHandle::new(23).unwrap();
        let first = number_only.with_arena_index(SectorIndex::new(4).unwrap());
        let second = number_only.with_arena_index(SectorIndex::new(9).unwrap());
        // Legacy number equality is unchanged for script-facing callers.
        assert_eq!(first, second);
        assert_ne!(first.reference(), second.reference());
        assert_ne!(first.reference(), number_only.reference());
        assert_eq!(
            number_only.reference(),
            SectorReference::NumberOnly(number_only.number())
        );
        let encoded = bitcode::encode(&first);
        let restored: SectorHandle = bitcode::decode(&encoded).unwrap();
        assert_eq!(restored.reference(), first.reference());
    }
    ...
    /// impossible — turn-vibration suppression needs `count <= -2` for the first
    ...
    #[test]
    fn forecast_division_uses_original_vector_reciprocal_rounding() {
        let mut pi = PositionInterface::new();
        pi.increment = WorldVec3D::new(f32::from_bits(0x3ed3_0132), 0.0, 0.0);

        pi.update_forecasted_movement(f32::from_bits(0x3f44_ec4f), 11);

        let forecast = pi.get_forecasted_movement();
        assert_eq!(forecast.x.to_bits(), 0x3cec_16d2);
        assert_ne!(
            forecast.x.to_bits(),
            (f32::from_bits(0x3ea2_4fb0) / 11.0_f32).to_bits(),
            "direct scalar division is the adjacent float, not Original's vector result"
        );
    }
