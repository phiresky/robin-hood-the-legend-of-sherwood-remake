...
impl MissionProfile {
    fn read_legacy_cpf(
        ...
        // structure has ten. This matches the original-game mission layout.
...
impl CharacterProfile {
    fn read_legacy_cpf(reader: &mut LegacyReader<'_>, index: u32) -> LegacyResult<Self> {
        ...
        // serialized. Note: the original game had an off-by-one
...
impl ProfileManager {
    ...
    fn read_all_legacy_cpf(reader: &mut LegacyReader<'_>) -> LegacyResult<Self> {
        // The original game stores this exact order. This is the
    ...
    /// campaign gang creation, native `GetNumberOfBeamMes`) silently
    ...
    pub fn import_beam_mes(&mut self, level_directory: &str) {
        self.import_beam_mes_with_files(
            level_directory,
            &crate::sbfile::SbFile::snapshot_legacy_file_system(),
        );
    }

    pub fn import_beam_mes_with_files(
        &mut self,
        level_directory: &str,
        ...
        for profile in self.missions.iter_mut() {
            ...
            match crate::level_data::scan_mission_for_beam_mes_with_files(&path, files) {
                Ok(scan) => {
