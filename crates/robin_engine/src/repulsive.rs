...
/// force assignment — see [`rhline::repulsive_set_force`]. The stored
...
impl RepulsivePoint {
    /// Build a `RepulsivePoint` with the given force parameters.
...
impl RepulsiveLine {
    /// Build a `RepulsiveLine` from endpoints and force parameters.
    pub fn new(a: MapPoint, b: MapPoint, radius: f32, action_radius_input: f32) -> Self {
        ...
        Self {
            ...
            force_a: fa,
            force_b: fb,
            // Repulsive lines start with a zero line type and
            // add LINE_REPULSIVE only. Unlike a repulsive point, a line is
...
mod tests {
    ...
    fn corpse_point_deviation_matches_frame_404_geometry() {
        ...
        // Original-game deviation calculation uses the stored total action
    ...
    fn line_is_deviating_between() {
        ...
        // one-sided like the original game's repulsive lines.
    ...
    fn line_deviation_uses_stored_total_action_radius() {
        // radius=2 and input action radius=5 stores an action radius of 7.
        // With actor radius 4, a destination ten units from the line is
        // inside the original game's threshold (4 + 7) but outside the old, incorrectly
        ...
        // single precision, so both components land one ULP away from the nearest
        // decimal literal.
