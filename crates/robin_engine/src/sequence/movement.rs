...
impl crate::engine::EngineInner {
    ...
    pub fn cancel_pending_move_commands(
        ...
        for &(seq_id, elem_idx) in &self.orders.sequence_manager.elements_to_go {
    ...
    /// Counterpart to [`SequenceManager::stop_pending_elements`] with a command
    ...
    pub fn stop_pending_elements_matching(
        ...
        let mut postponed = self
            ...
            .collect::<Vec<_>>();
        postponed.sort_by_key(|reference| {
            (
                self.orders
                    .sequence_manager
                    .sequences
                    .get_index_of(&reference.sequence_id),
                reference.element_index,
            )
        });
        for reference in postponed {
            self.stop_live_sequence_element(
                sim,
                assets,
                active_scripts,
