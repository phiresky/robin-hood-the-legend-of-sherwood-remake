...
/// Parse a menu sound bank into `(combined_id, filename)` entries.
/// Filenames are relative to the configured sound directory, not absolute
/// filesystem paths (limited to `WIDGET_NOISY_EVENT_COUNT` per group).
...
mod tests {
    ...
    #[test]
    fn menu_bank_sample_paths_are_sound_directory_relative() {
        let mut bank = b"FXBK".to_vec();
        bank.resize(12, 0);
        bank.extend_from_slice(&1u32.to_le_bytes());
        bank.extend_from_slice(&(SoundGroupType::MaterialGroup as u32).to_le_bytes());
        bank.extend_from_slice(&3u32.to_le_bytes());
        bank.extend_from_slice(&0u16.to_le_bytes());
        bank.extend_from_slice(&0u16.to_le_bytes());
        bank.extend_from_slice(&1u32.to_le_bytes());
        ...
        bank.extend_from_slice(b"click.wav");
        assert_eq!(
            parse_menu_bank(&bank).unwrap(),
            vec![(3 << 16, "Menu/click.wav".into())]
        );
    }
