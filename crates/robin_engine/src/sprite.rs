...
/// Order data the original game reads when a sprite first sees a motion order.
...
pub struct MotionOrderContext {
    ...
    /// The order antagonist, copied into the position interface when this
...
///
/// The original game seeds both fields when the order ID changes and
...
pub enum FrameProgression {
    ...
    /// progression. No live caller currently constructs this variant;
    /// it is kept here to preserve
...
/// use it as a deadline. Action initialization deliberately stores an
...
impl Sprite {
    ...
    pub fn apply_placement(
        ...
        // sector-reference slot. Preserve that exact arena object instead of
    ...
    pub fn num_frames_for_row(&self, row: u16) -> u16 {
        ...
        // Sprite frame-count lookup uses the delay-array length.
        // Some shipped rows retain additional paged-frame IDs after their
        // authored timing entries; those frames are resources, not playable
        // animation frames.
        scripts[row as usize].delays.len() as u16
    }
    ...
    /// falls. Original-game takeoff readiness uses the distinct exact
    /// helper below.
    ...
    /// Exact original-game animation duration: every authored frame costs
    /// its wait value plus the tick which advances past that wait.
    pub fn gameplay_time_for_anim(&self, action: OrderType) -> u16 {
        let Some(row) = self.row_for_action(action) else {
            return 0;
        };
    ...
    /// Exact original-game default-flight duration.
    ///
    /// The original game starts at one tick and adds the wait duration plus one for
    ...
    pub fn ready_for_takeoff_ticks_for_anim(&self, action: OrderType) -> u16 {
        ...
        let mut total: u32 = 1;
        let nonterminal_frames = script.delays.len().saturating_sub(1);
    ...
    /// `Entity::gameplay_sprite_position()` to produce the current map-space
    ...
    /// original-game action-distance parity lookup.
    ///
    /// The reference computes:
    /// the norm of animation hotspot plus sprite origin minus map position.
    /// Rust keeps the authoritative sprite anchor in [`Self::center`], so the
    /// original-game sprite-position term is reconstructed from
    /// `position_map - center`.
    ...
    fn apply_sprite_info(
        ...
        let path = scriptor.resolve_rhs_path(frame_kind, base_dir, filename, ambiance)?;
    ...
    /// Original-game modulated frame increment, used by moving cart sprites.
    ...
    pub fn perform_action(
        ...
        // The original game reports motion start for
        ...
        // Action done? With original-game start-tick timing above, newly
    ...
    pub fn perform_motion(
            ...
            && !self.current_conversion().is_empty()
        {
            ...
            if let Some(row) = self.row_for_action(resolved) {
                ...
                // Original returns before assigning
        ...
        // initialization, using the direction at that point. Mirror that
        ...
        // Action processing reports a natural animation loop as terminated.
        // Sprite motion only observes that loop for
        ...
        // Action processing deliberately leaves a new order on frame_count
        // 0xFFFF for its initial tick. Motion processing does not: the original game
        // initializes the order and then unconditionally calls
        // frame advancement in the same invocation, making frame-zero distance
        // available immediately. Keep the two entry points distinct.
        if starts_new_motion_order {
            // Motion processing does not inherit action processing's completion
            ...
            // that advance because action processing rewrites start to done
            // before returning.
            state = MotionState::Start;
    ...
    pub fn frames_from_now_till_action_done(&self) -> i16 {
        ...
        if self.action_done_frame >= num_frames {
            // Action-completion initialization uses the maximum 16-bit value when the current row
            // has no usable action-done marker. The
            // The original game walks beyond the delay array for that sentinel, but
        ...
        // (1) Remaining time in current frame. Match the original game's
        // signed expression: wait time minus frame count can be
        ...
        // The original game stores the running total and returns it directly.
    ...
    /// to accumulate a negative signed 16-bit value for S075 frame 731, but reproducing
    /// allocator contents would be undefined and non-portable. Preserve the
    /// authored meaning from action-done initialization instead, so a
...
mod tests {
    ...
    #[test]
    fn playable_row_length_comes_from_delays_not_paged_frame_ids() {
        ...
        let mut conversion = vec![UNMAPPED; OrderType::NonanimationEnd as usize];
        conversion[OrderType::WaitingUpright as usize] = 0;
        let mut sprite = Sprite::new(
            std::sync::Arc::new(vec![script]),
            std::sync::Arc::new(conversion),
        );
        let order_id = Some(std::num::NonZeroU32::new(1).unwrap());
        let sim = crate::sim_rng::test_context();

        assert_eq!(sprite.num_frames_for_row(0), 2);
        assert_eq!(
    ...
    fn test_frames_from_now_till_action_done() {
        ...
        // The original game performs signed subtraction here. This can go negative when
        ...
        // Action initialization stores 0xffff when a two-frame row has no
        // usable action-done marker. Callers such as sword-strike selection
    ...
    #[test]
    fn exact_arrival_does_not_publish_the_unexecuted_motion_order_id() {
        let mut sprite = make_test_sprite();
        let old_order_id = std::num::NonZeroU32::new(41).unwrap();
        let new_order_id = std::num::NonZeroU32::new(42).unwrap();
        sprite.last_processed_order_id = old_order_id.get();
        let destination = sprite.position_iface.map_position();
        ...
        assert_eq!(state, MotionState::Terminated);
        assert_eq!(distance, 0.0);
        assert_eq!(
            sprite.last_processed_order_id,
            old_order_id.get(),
            "Original returns before publishing the exact-arrival order identity"
        );
    }
    ...
    fn perform_action_start_tick_does_not_advance_frame() {
        ...
        assert_eq!(
            s.current_frame, 0,
            "the original game reports START without incrementing the new order"
        );
        assert_eq!(s.frame_count, 0xFFFF);
    }
