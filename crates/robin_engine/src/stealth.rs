...
/// Exact order body appended by original-game player translation for
/// `LEAVE_HELPING_CLIMB`.
...
// action initialization when the level data spawns a PC with the
// waiting-cape initial animation. Only the leave transition is
// reachable at runtime, through a posture transition.
...
/// Soldier-specific posture transition: a LeaningOut
...
pub fn eye_z_for_posture(posture: Posture, is_rider: bool) -> f32 {
    match posture {
        ...
        | Posture::Tied => 5.0,
        // The original game initializes the result to the current position, reports its
...
pub fn detection_z_for_posture(posture: Posture, is_rider: bool) -> f32 {
    match posture {
        ...
        | Posture::StuckUnderNet
        | Posture::Tied => 2.0,
        // Detection-point calculation initializes the output to world position.
...
pub fn detection_point_xy(ground: MapPoint, posture: Posture, direction: i16) -> MapPoint {
    ...
    // The original game obtains this detection vector through
    // Original-game direction calculation scales Y
...
///
/// Detection-point calculation seeds its output with the actor's stored 3D
...
/// position. This is the XY half of the original game's eye-point calculation.
pub fn eye_point_xy(
    ground: MapPoint,
    posture: Posture,
    direction: i16,
...
mod tests {
    ...
    fn eye_z_upright_vs_crouched() {
        ...
        // The original game's default branches leave the output at the current position.
