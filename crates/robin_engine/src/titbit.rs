...
//! (refresh, text rendering, sprite decompression) is handled separately
...
impl TitbitManager {
    ...
    /// in the original-game save, but keeps the already-loaded sprite metadata.
    ...
    pub fn prepare_refresh(&mut self, get_display_order: impl Fn(ElementHandle) -> Option<f32>) {
        ...
        // The original game's titbit ordering compares only display order,
...
/// Total preorder extending the original game's display-order comparison.
///
/// Numeric values retain the original game's comparison behavior, including equivalent
...
mod tests {
    ...
    #[test]
    fn native_bitcode_roundtrips_typed_titbit_absence() {
        let mut mgr = TitbitManager::new();
        mgr.add_titbit(
            pt3(10.0, 20.0, 5.0),
            1,
        ...
        let encoded = bitcode::encode(&mgr);
        let restored: TitbitManager = bitcode::decode(&encoded).unwrap();
