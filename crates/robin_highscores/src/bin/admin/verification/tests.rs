...
// Own live stores until every mutation/recovery phase finishes, exactly like
// the original coordinated scenario. Capabilities are not serialization inputs.
struct VerificationFixture {
    directory: tempfile::TempDir,
    release_manifest_path: PathBuf,
    release_identity: robin_highscores::backup::BackupReleaseIdentityV2,
    config: ServerConfig,
    restore_sources: std::collections::BTreeMap<PathBuf, PathBuf>,
...
impl VerificationFixture {
    ...
    async fn assert_historical_release_authority(&self) {
        let directory = &self.directory;
        let release_manifest_path = self.release_manifest_path.clone();
        let release_identity = self.release_identity.clone();
        let config = self.config.clone();
        let destination = self.destination.clone();
        let backup_id = destination.file_name().unwrap().to_str().unwrap();
    ...
    async fn assert_database_and_object_closure(&self) {
        let directory = &self.directory;
        let destination = self.destination.clone();
        let replay_digest = self.replay_digest;
