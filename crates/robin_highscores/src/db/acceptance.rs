...
impl Database {
    pub async fn accept_job(
        ...
        let expected_campaign_ordinal = match &signed_offer.starting_state {
            ...
            robin_run_protocol::InitialStateExpectationV1::CampaignContinuation {
                ...
                if campaign_chain_id.as_deref() != Some(chain_id.as_str())
                    || predecessor_run_id.as_deref() != Some(offered_predecessor.as_str())
                {
                    return Err(DbError::ResultInvariant(
                ...
                if owner_rows.len() != 1 {
                    return Err(DbError::Corrupt(
                        "campaign chain does not have exactly one live ordinal-zero owner"
                            .to_owned(),
                    ));
                }
