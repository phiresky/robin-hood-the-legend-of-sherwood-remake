...
async fn ensure_backup_ready(state: &AppState) -> Result<(), ApiError> {
...
async fn backup_age_ms(state: &AppState) -> Result<Option<u64>, ApiError> {
...
async fn backup_age_ms_with_active_release(
    state: &AppState,
...
async fn read_bounded_nofollow(path: &std::path::Path, maximum: u64) -> anyhow::Result<Vec<u8>> {
    #[cfg(target_os = "linux")]
    {
...
#[cfg(target_os = "linux")]
struct PinnedReadinessStatusParent {
...
#[cfg(target_os = "linux")]
fn pin_readiness_status_parent(
...
#[cfg(target_os = "linux")]
async fn read_bounded_from_pinned_status_parent<F>(
    configured_path: &std::path::Path,
    parent: &PinnedReadinessStatusParent,
    maximum: u64,
...
mod diagnostic_tests {
    ...
    async fn diagnostics_accept_retry_limit_and_protect_operator_access() {
        ...
        for uri in [
            "/api/v1/operator/diagnostics".to_owned(),
            format!("/api/v1/operator/diagnostics/{first_id}"),
        ] {
            let response = application
                .clone()
        ...
        let response = application
            .oneshot(
                Request::builder()
                    .method("DELETE")
                    .uri(format!("/api/v1/operator/diagnostics/{first_id}"))
                    .header("authorization", "Bearer 0123456789abcdef0123456789abcdef")
                    .body(Body::empty())
                    .unwrap(),
            )
        ...
        assert_eq!(response.status(), StatusCode::NO_CONTENT);
        assert!(matches!(
            database.diagnostic_report(&first_id).await,
            Err(crate::db::DbError::NotFound)
        ));
    }
}
