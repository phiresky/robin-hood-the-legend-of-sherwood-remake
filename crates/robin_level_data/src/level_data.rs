...
///
/// No shipped mission stream emits a script line. The field is
...
pub struct RawMobileElement {
    ...
    /// Legacy editor anchor serialized before the hook/barrel flags. The original-game
    ...
    /// Serialized by the editor but likewise unused by the original-game loader.
...
/// Source order of proto chunks which construct sectors or
/// gates in the original game. The v48 save stream omits the sizes of
...
pub enum ProtoGridChunk {
    ...
    Motion,
    Sight,
    /// Elevation lines share the original game's per-layer line-reference space.
    Bond,
    Material,
    Lift,
    Building,
...
/// On-disk order of mission chunks which extend the original game's grid arrays.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// Source order of mission chunks which construct game elements.
#[derive(
    Debug,
    Clone,
    Copy,
...
pub struct LoadedMission {
    ...
    /// Exact source order for chunks which append to the element collection.
    #[serde(default)]
...
pub struct TimedMissionDefinition {
    ...
    #[serde(default)]
    pub countdown: robin_engine_types::mission_environment::MissionCountdownMode,
}
...
pub struct AmbienceScheduleCue {
    ...
    pub at_seconds: u32,
    pub ambiance: robin_engine_types::mission_environment::Ambiance,
...
/// Prepare a level using only the caller's file authority.
pub fn load_level_with_files(
    mission_name: &str,
    proto_level_name: &str,
    level_directory: &str,
    is_beggar: &dyn Fn(u32) -> bool,
...
/// Read a sprite reference for
/// `RHFRAMEKIND_ANIMATION`.
///
/// Binary format:
/// - Frame metadata: string (frame_profile_name), string (profile_name).
///   For ANIMATION kind, no alternate profile is read.
/// - Position metadata: i16 x, i16 y, u16 elevation.
...
/// Read patches from the prototype stream.
///
...
/// Read a single patch from the prototype stream.
...
fn read_animals(reader: &mut ChunkReader, format: LevelFormat) -> Result<(), LevelError> {
    ...
    // Animal-actor mission initialization asserts false — no
...
pub fn scan_mission_for_beam_mes(path: &str) -> Result<MissionBeamMeScan, LevelError> {
    scan_mission_for_beam_mes_with_files(path, &SbFile::snapshot_legacy_file_system())
}

pub fn scan_mission_for_beam_mes_with_files(
    path: &str,
    files: &crate::sbfile::SbFileSystem,
...
fn read_targets(
    ...
    for _ in 0..count {
        ...
        // Script data loaded from the stream
...
/// Read a single mask from the prototype stream.
...
/// Read a single sight obstacle from the prototype stream.
fn read_one_sight_obstacle(reader: &mut ChunkReader) -> Result<RawSightObstacle, LevelError> {
    ...
    // stream alignment but discarded. The original game also reseeds the
...
/// Read a single jump line from the prototype stream.
...
/// Read a single door from the prototype stream.
...
fn read_lifts(reader: &mut ChunkReader, format: LevelFormat) -> Result<Vec<RawLift>, LevelError> {
    ...
    for _ in 0..count {
        ...
        // Lift-sector prototype initialization.
        let lift_type = reader.read_u8()?;

        // Clickable associated sector: parsed for stream
        // alignment but discarded — the original game's lift initialization
        // path that would register this polygon as a mouse-pick sector
        // has its associated-sector additions disabled
        // inactive in the original game, so the polygon carries no runtime
...
fn read_buildings(
    ...
    for _ in 0..count {
        ...
        if is_building {
            // Building sector: 16-bit door count followed by doors
            let num_doors = reader.read_u16()?;
            let mut doors = Vec::with_capacity(num_doors as usize);
            for _ in 0..num_doors {
                doors.push(read_one_door(reader, format)?);
...
/// Read the CART/TING chunk as a mobile mission element.
///
...
mod tests {
    ...
    fn hackable_descriptor_compiles_timed_and_ambience_authoring() {
        ...
        assert_eq!(
            timer.countdown,
            robin_engine_types::mission_environment::MissionCountdownMode::FinalOnly
        );
        assert_eq!(level.mission.ambience_schedule.len(), 2);
        assert_eq!(
            level.mission.ambience_schedule[0].ambiance,
            robin_engine_types::mission_environment::Ambiance::Night
        );
    }
    ...
    fn bundled_multi_team_missions_parse_and_cover_requested_rosters() {
        ...
        let cases = [
            ("MultiTeamThreeWay", 3, 0),
            ("MultiTeamTenWay", 10, 0),
            ("MultiTeamFourArmies", 48, 0),
            ("MultiTeamFourGrades", 48, 0),
            ("MultiTeamAllPcsCircle", 0, 10),
            ("MultiTeamArrowCrossfire", 32, 0),
            ("MultiTeamTwentyRobins", 20, 20),
            ("MultiTeamChampionsRetinues", 16, 4),
            ("MultiTeamAllVariantsWheel", 68, 0),
            ("MultiTeamRobinVsLittleJohn", 0, 2),
        ];
        for (mission, soldiers, pcs) in cases {
            let path = repo
                .join("mods")
                .join("multi-team-demos")
                .join("Data/Levels")
        ...
        let duel_path =
            repo.join("mods/multi-team-demos/Data/Levels/MultiTeamRobinVsLittleJohn.level.json");
        ...
        let pc_circle_path =
            repo.join("mods/multi-team-demos/Data/Levels/MultiTeamAllPcsCircle.level.json");
        let pc_circle =
        ...
        let robins_path =
            repo.join("mods/multi-team-demos/Data/Levels/MultiTeamTwentyRobins.level.json");
