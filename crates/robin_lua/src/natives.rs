...
use robin_engine::natives::{GameHost, NativeFn, NativeSignature};
...
pub fn register_natives(state: &mut MissionLuaState) -> mlua::Result<()> {
    ...
    //    its canonical name. The signature controls how Lua values
    //    are encoded into the engine's four-byte stack words and how
    //    its i32 result word is exposed to Lua. String-taking and
    //    table-returning natives are handled in Lua-only shims below
    //    — they bypass NativeStack.
