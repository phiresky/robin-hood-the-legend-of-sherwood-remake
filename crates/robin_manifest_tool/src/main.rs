    ...
    pub(super) fn main() -> Result<()> {
        match Cli::parse().command {
            ...
            Command::ValidateRelease { plan, release } => {
                let _ = (plan, release);
                bail!(
                    "historical V1 release validation is disabled because it reauthors V1 receipts"
                );
            }
        }
        Ok(())
...
fn main() -> anyhow::Result<()> {
    anyhow::bail!(
        "manifestctl deployment operations require Linux (procfs, openat2, renameat2 and flock)"
    )
}
