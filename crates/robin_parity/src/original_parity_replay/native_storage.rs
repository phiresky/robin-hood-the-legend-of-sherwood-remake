...
pub(super) fn reject_conversion_symlink(path: &Path) {
...
pub(super) fn lock_native_trace_generation(native_path: &Path) -> File {
...
pub(super) fn sync_directory(directory: &Path, operation: &str) {
    File::open(directory)
        .unwrap_or_else(|error| {
            panic!(
...
pub(super) fn native_reblock_temporary_prefix(native_path: &Path, binding: bool) -> String {
...
pub(super) fn cleanup_native_reblock_orphans(native_path: &Path) {
...
pub(super) fn native_reblock_canonical_path(native_path: &Path) -> PathBuf {
    let parent = native_path
        .parent()
        .expect("absolute native trace always has a parent directory")
        .canonicalize()
...
pub(super) fn native_reblock_file_identity(path: &Path) -> (u64, String, u64, u64) {
...
pub(super) fn native_reblock_semantic_identity(path: &Path) -> (u64, u64, String) {
...
pub(super) fn create_native_reblock_binding(native_path: &Path) -> NativeReblockBinding {
...
pub(super) fn write_native_reblock_binding(path: &Path, binding: &NativeReblockBinding) {
    let parent = path
        .parent()
...
pub(super) fn read_native_reblock_binding(path: &Path, native_path: &Path) -> NativeReblockBinding {
...
pub(super) fn requested_native_trace_path(trace_path: &Path) -> PathBuf {
...
pub(super) fn reblock_native_trace(trace_path: &Path, policy: NativeStoragePolicy) {
...
pub(super) fn validate_native_trace(trace_path: &Path) {
    let native_path = requested_native_trace_path(trace_path);
    let started = Instant::now();
...
pub(super) fn digest_and_validate_native_trace(path: &Path) -> (u64, sha2::digest::Output<Sha256>) {
...
pub(super) fn ensure_native_binary_trace(trace_path: &std::path::Path) -> PathBuf {
    if trace_path
        .as_os_str()
        .to_string_lossy()
        .ends_with(TRACE_NATIVE_SUFFIX)
...
pub(super) fn read_binary_trace_header(path: &std::path::Path) -> BinaryTraceHeaderV68 {
...
pub(super) fn bench_trace_encodings(trace_path: &Path) {
    #[derive(Default)]
...
pub(super) fn read_all_rng_draws(native_path: &std::path::Path) -> Vec<u32> {
    let mut reader = BinaryTraceReader::open(native_path);
    let header = reader.read_header();
