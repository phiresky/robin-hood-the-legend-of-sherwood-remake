...
/// to terminate amplification safely. See `crates/robin_highscores/README.md`.
...
pub struct ReplayAdmissionLimits {
    ...
    /// Fixed build identity field; the verifier policy signs this ceiling
    /// independently even though the canonical grammar requires 12 bytes.
    pub max_version_hash_bytes: usize,
    ...
    /// Mission records across the decoded campaign and every retained snapshot.
    pub max_campaign_missions: usize,
    /// Character descriptions across the campaign and retained snapshots.
    pub max_campaign_characters: usize,
    /// Aggregate entries in campaign identity/index/name collections,
    /// including recruited-character records retained in attempt history.
    pub max_campaign_collection_entries: usize,
    /// Production points across every sector and retained campaign layer.
    pub max_campaign_production_points: usize,
    /// Production occupants across every sector and retained campaign layer.
    pub max_campaign_production_occupants: usize,
    /// Immutable mission-attempt records across every retained campaign layer.
    pub max_campaign_history_attempts: usize,
    /// UTF-8 bytes in one campaign-owned character or peasant name.
    pub max_campaign_string_bytes: usize,
    /// Maximum nesting depth for pre-mission or practice-return snapshots.
    /// The current schema is nonrecursive, so public admission permits one.
    pub max_campaign_snapshot_depth: usize,
}
...
pub const DEFAULT_REPLAY_ADMISSION_LIMITS: ReplayAdmissionLimits = ReplayAdmissionLimits {
    ...
    max_version_hash_bytes: VERSION_HASH_BYTES,
    ...
    max_campaign_missions: 4_096,
    max_campaign_characters: 16_384,
    max_campaign_collection_entries: 65_536,
    max_campaign_production_points: 65_536,
    max_campaign_production_occupants: 16_384,
    max_campaign_history_attempts: 1_000_000,
    max_campaign_string_bytes: 16 * 1024,
    max_campaign_snapshot_depth: 1,
};

/// Larger isolated-worker lane for local/browser playback of current-schema
/// custom missions. Ranked/server admission deliberately continues to use
/// [`DEFAULT_REPLAY_ADMISSION_LIMITS`]; accepting executable packages there is
...
pub const LOCAL_CUSTOM_REPLAY_ADMISSION_LIMITS: ReplayAdmissionLimits = ReplayAdmissionLimits {
    ...
    max_typed_string_bytes: 16 * 1024,
    max_total_typed_string_bytes: 4 * 1024 * 1024,
    max_total_typed_work: 48_000_000,
    max_campaign_missions: 4_096,
    max_campaign_characters: 16_384,
    max_campaign_collection_entries: 65_536,
    max_campaign_production_points: 65_536,
...
pub enum ReplayLimitKind {
    ...
    CampaignMissions,
    CampaignCharacters,
    CampaignCollectionEntries,
    CampaignProductionPoints,
    CampaignProductionOccupants,
    CampaignHistoryAttempts,
    CampaignStringBytes,
    CampaignSnapshotDepth,
}
...
fn decode_compact_inner(
    ...
    let parse_limits = if let Some(limits) = limits {
        ...
        unbounded_limits = ReplayAdmissionLimits {
            ...
            max_version_hash_bytes: usize::MAX,
            ...
            max_campaign_missions: usize::MAX,
            max_campaign_characters: usize::MAX,
            max_campaign_collection_entries: usize::MAX,
            max_campaign_production_points: usize::MAX,
            max_campaign_production_occupants: usize::MAX,
            max_campaign_history_attempts: usize::MAX,
            max_campaign_string_bytes: usize::MAX,
            max_campaign_snapshot_depth: usize::MAX,
        };
        &unbounded_limits
    };
...
fn validate_file_for_admission(
    ...
    validate_campaign_specific_limits(&campaign, limits)?;
    ...
    // Every retained header graph is part of the same typed-work envelope.
    // In particular, a custom mission package may contain the largest byte
    // collection in a valid local replay; counting it here keeps the 64-MiB
    // local lane explicit without weakening the public 16-MiB lane.
    file.header
        .mission_assets
...
struct CampaignLimitBudget<'a> {
    ...
    production_points: usize,
    production_occupants: usize,
    history_attempts: usize,
}

impl<'a> CampaignLimitBudget<'a> {
    ...
    fn visit(&mut self, layer: &impl CampaignLimitLayer) -> Result<(), FormatError> {
        ...
        for character in layer.characters() {
            check_limit(
                ...
                self.limits.max_campaign_string_bytes,
            )?;
        }
        ...
        for entries in [
            ...
            Self::add(
                ...
                entries,
                ReplayLimitKind::CampaignCollectionEntries,
        ...
        for sector in layer.production_sectors() {
            Self::add(
                ...
                ReplayLimitKind::CampaignProductionPoints,
                self.limits.max_campaign_production_points,
            )?;
            Self::add(
                &mut self.production_occupants,
                sector.occupants.len(),
                ReplayLimitKind::CampaignProductionOccupants,
                self.limits.max_campaign_production_occupants,
            )?;
        }
        ...
        for mission in layer.missions() {
            ...
            Self::add(
                ...
                attempts.len(),
                ReplayLimitKind::CampaignHistoryAttempts,
                self.limits.max_campaign_history_attempts,
            )?;
            for attempt in attempts {
                if let Some(recruited) = &attempt.stats().recruited_characters {
                    Self::add(
                        &mut self.collection_entries,
                        recruited.len(),
                        ReplayLimitKind::CampaignCollectionEntries,
                    ...
                    for character in recruited {
                        check_limit(
                            ReplayLimitKind::CampaignStringBytes,
                            character.fallback.len(),
                            self.limits.max_campaign_string_bytes,
                        )?;
                    }
                }
...
/// Enforce the campaign-specific ceilings over the complete serialized graph,
/// not independently per layer. A replay cannot multiply every allowance by
/// copying the same collections into restart/practice snapshots.
fn validate_campaign_specific_limits(
    campaign: &Campaign,
    limits: &ReplayAdmissionLimits,
) -> Result<(), FormatError> {
    ...
    if let Some(snapshot) = campaign.pre_mission_snapshot.as_ref() {
        check_limit(
            ReplayLimitKind::CampaignSnapshotDepth,
            1,
        ...
        if let Some(practice) = snapshot.practice_return_snapshot.as_ref() {
            budget.visit(&practice.validation_view())?;
        }
    }
    if let Some(practice) = campaign.practice_return_snapshot.as_ref() {
        check_limit(
            ReplayLimitKind::CampaignSnapshotDepth,
            1,
            limits.max_campaign_snapshot_depth,
...
mod tests {
    ...
    fn sample_data() -> ReplayData {
        sample_file().try_into().expect("valid sample replay")
    }

    fn near_max_custom_file() -> ReplayFile {
        let mut file = sample_file();
        file.header.mission_id = "CustomMission".into();
        file.header.mission_assets = robin_engine::mission_assets::MissionAssetDescriptor::archive(
            "CustomMission",
            "CustomMap",
            "CustomMap",
            robin_engine::mission_assets::ArchiveMissionAssets {
                ...
                selected_rhm_entry: "nested/CustomMission.rhm".into(),
                shared_archive: None,
        ...
        .unwrap();
        // Use high-entropy printable source. Bitcode deliberately compresses
        // long runs, so a repeated byte would not exercise the public lane's
        // decompressed-artifact ceiling even at the package source limit.
        let mut state = 0x9e37_79b9_u32;
        let source = (0..16 * 1024 * 1024)
            .map(|_| {
                state ^= state << 13;
                state ^= state >> 17;
                state ^= state << 5;
        ...
        let mut package = robin_engine::spellforge::SpellforgePackage {
            ...
            entrypoint: "custommission.lua".into(),
            files: [("custommission.lua".into(), source)].into_iter().collect(),
            sha256: [0; 32],
        };
    ...
    #[test]
    fn local_custom_lane_fits_max_valid_spellforge_package_without_weakening_public_limits() {
        let file = near_max_custom_file();
        let raw = bitcode::encode(&file);
        assert!(raw.len() > DEFAULT_REPLAY_ADMISSION_LIMITS.max_decompressed_bytes);
    ...
    fn assert_campaign_limit_boundary(
        campaign: &robin_engine::campaign::Campaign,
        observed: usize,
        kind: ReplayLimitKind,
        set_limit: fn(&mut ReplayAdmissionLimits, usize),
    ) {
        assert!(observed > 0, "boundary fixture must have a one-over case");
        let mut limits = ReplayAdmissionLimits::default();
        set_limit(&mut limits, observed);
        validate_campaign_specific_limits(campaign, &limits)
            .expect("the exact campaign-specific boundary must be accepted");
        set_limit(&mut limits, observed - 1);
        assert_limit(
            validate_campaign_specific_limits(campaign, &limits)
                .expect_err("one campaign entry beyond the boundary must be rejected"),
            kind,
        );
    }

    fn campaign_with_native_attempt(
        recruited_names: impl IntoIterator<Item = String>,
    ...
    fn with_practice_return_snapshot(
        campaign: robin_engine::campaign::Campaign,
    ) -> robin_engine::campaign::Campaign {
        let practice = serde_json::to_value(&campaign).unwrap();
        let mut outer = serde_json::to_value(&campaign).unwrap();
        outer["practice_return_snapshot"] = practice;
        outer["history_replay_mission_idx"] = serde_json::json!(0);
    ...
    fn campaign_mission_and_character_limits_accept_boundary_and_reject_one_over() {
        ...
        assert_campaign_limit_boundary(
            ...
            2,
            ReplayLimitKind::CampaignCharacters,
            |limits, value| limits.max_campaign_characters = value,
        );
    }

    #[test]
    fn every_campaign_identity_collection_consumes_the_aggregate_limit() {
        let collection_limit = |limits: &mut ReplayAdmissionLimits, value| {
        ...
        let campaign = campaign_with_native_attempt(["a".into(), "b".into()]);
        assert_campaign_limit_boundary(
            &campaign,
            2,
            ReplayLimitKind::CampaignCollectionEntries,
    ...
    #[test]
    fn campaign_production_and_history_limits_accept_boundary_and_reject_one_over() {
        let mut points = robin_engine::campaign::Campaign::default();
        points.production_sectors[0].production_points =
            vec![robin_engine::sector_production::Point::default(); 2];
        assert_campaign_limit_boundary(
            &points,
            2,
            ReplayLimitKind::CampaignProductionPoints,
            |limits, value| limits.max_campaign_production_points = value,
        );

        let mut occupants = robin_engine::campaign::Campaign::default();
        occupants.production_sectors[0].occupants =
            vec![robin_engine::sector_production::Occupant::default(); 2];
        assert_campaign_limit_boundary(
            &occupants,
            2,
            ReplayLimitKind::CampaignProductionOccupants,
            |limits, value| limits.max_campaign_production_occupants = value,
        );

        let history = campaign_with_native_attempt(std::iter::empty::<String>());
        assert_campaign_limit_boundary(
            &history,
            1,
            ReplayLimitKind::CampaignHistoryAttempts,
            |limits, value| limits.max_campaign_history_attempts = value,
        );
    }

    #[test]
    fn every_campaign_name_source_has_an_exact_utf8_byte_limit() {
        let mut character = robin_engine::campaign::Campaign::default();
        let mut description = robin_engine::campaign::PcDescription::default();
        description.status.name = "éé".into();
        character.characters.push(description);
        assert_campaign_limit_boundary(
            &character,
            "éé".len(),
            ReplayLimitKind::CampaignStringBytes,
            |limits, value| limits.max_campaign_string_bytes = value,
        );

        let mut peasant = robin_engine::campaign::Campaign::default();
        peasant.peasant_names.push("éé".into());
        assert_campaign_limit_boundary(
            &peasant,
            "éé".len(),
            ReplayLimitKind::CampaignStringBytes,
        ...
        let recruited = campaign_with_native_attempt(["éé".into()]);
        assert_campaign_limit_boundary(
            &recruited,
            "éé".len(),
            ReplayLimitKind::CampaignStringBytes,
    ...
    #[test]
    fn campaign_snapshots_enforce_depth_and_aggregate_every_record_budget() {
        let mut campaign = campaign_with_native_attempt(std::iter::empty::<String>());
        campaign.characters.push(Default::default());
        campaign.peasant_names.push("name".into());
        ...
        assert_campaign_limit_boundary(
            ...
            1,
            ReplayLimitKind::CampaignSnapshotDepth,
            |limits, value| limits.max_campaign_snapshot_depth = value,
        );

        for (kind, set_limit) in [
            (
                ReplayLimitKind::CampaignMissions,
                (|limits: &mut ReplayAdmissionLimits, value| {
                    limits.max_campaign_missions = value;
                }) as fn(&mut ReplayAdmissionLimits, usize),
            ),
            (
                ReplayLimitKind::CampaignCharacters,
                |limits: &mut ReplayAdmissionLimits, value| {
                    limits.max_campaign_characters = value;
                },
            ),
            (
                ReplayLimitKind::CampaignCollectionEntries,
            ...
            (
                ReplayLimitKind::CampaignProductionPoints,
                |limits: &mut ReplayAdmissionLimits, value| {
                    limits.max_campaign_production_points = value;
                },
            ),
            (
                ReplayLimitKind::CampaignProductionOccupants,
                |limits: &mut ReplayAdmissionLimits, value| {
                    limits.max_campaign_production_occupants = value;
                },
            ),
            (
                ReplayLimitKind::CampaignHistoryAttempts,
                |limits: &mut ReplayAdmissionLimits, value| {
                    limits.max_campaign_history_attempts = value;
                },
            ),
        ] {
            assert_campaign_limit_boundary(&campaign, 2, kind, set_limit);
        }
    }

    #[test]
    fn practice_return_snapshot_consumes_depth_and_aggregate_budgets() {
        let mut campaign = robin_engine::campaign::Campaign::default();
        campaign
            .missions
            .push(robin_engine::mission::Mission::new());
        let campaign = with_practice_return_snapshot(campaign);
    ...
    fn contained_compact_decode_invokes_campaign_specific_limits() {
        ...
        let boundary = ReplayAdmissionLimits {
            max_campaign_missions: 2,
            ..Default::default()
        };
        decode_compact_bounded(&compact, &boundary)
            .expect("contained decode must accept the exact campaign mission boundary");

        let one_over = ReplayAdmissionLimits {
            max_campaign_missions: 1,
            ..Default::default()
        };
        assert_limit(
            decode_compact_bounded(&compact, &one_over)
                .expect_err("contained decode must enforce campaign-specific limits"),
            ReplayLimitKind::CampaignMissions,
        );
    }
    ...
    fn every_binary_expansion_stage_has_an_independent_limit() {
        ...
        for (limits, kind) in [
            ...
            (
                ReplayAdmissionLimits {
                    max_version_hash_bytes: VERSION_HASH_BYTES - 1,
                    ..Default::default()
                },
                ReplayLimitKind::VersionHashBytes,
            ),
