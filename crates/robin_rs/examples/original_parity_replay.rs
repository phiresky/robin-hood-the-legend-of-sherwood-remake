...
fn inject_schema_sixteen_delayed_drop_ale_routes(
    schema: u32,
    route_events: Option<&[TraceRouteConstructionEvent]>,
    ...
    for event in route_events.unwrap_or_default() {
        ...
        if !engine.has_pending_recorded_drop_ale_route(actor, destination) {
            continue;
        }
        ...
        let (goal_sector, goal_sector_index) =
            entity_map.translate_required_drop_ale_goal_sector(event.goal_sector);
        let goal_sector =
            robin_engine::position_interface::SectorHandle::new(u16::from(goal_sector))
                .expect("translated DropAle goal sector must be valid")
                .with_arena_index(goal_sector_index);
        assert!(engine.inject_recorded_drop_ale_route(
            actor,
            destination,
            goal_sector,
            event.goal_level,
            recorded_gate_path_from_event(event, entity_map),
        ));
    }
}
