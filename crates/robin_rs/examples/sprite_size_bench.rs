...
//!   • Background maps (`Data/Levels/*/<name>.map` — packed 16-bit pictures, bzip2 RGB565)
...
fn bench_anim_samples(
    ...
    for rhs_path in &rhs_files {
        let character = rhs_path
            ...
            .unwrap_or_default();
        let (_sig, profiles) =
            match SpriteScriptor::load_all_profiles_legacy(rhs_path.to_str().unwrap()) {
                Ok(p) => p,
                Err(e) => {
...
fn bench_whole_character(data_dir: &Path, characters: &[String]) -> Result<()> {
    ...
    for name in characters {
        ...
        let (_sig, profiles) = SpriteScriptor::load_all_profiles_legacy(rhs_path.to_str().unwrap())
