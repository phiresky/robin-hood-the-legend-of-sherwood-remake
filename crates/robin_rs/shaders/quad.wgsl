...
fn fs_colorize(in: VsOut) -> @location(0) vec4<f32> {
    ...
    // The original game pre-multiplies before the HSV conversion so the dim picks
...
// the overlay colour in sRGB, `tint.a` is alpha on the original game's 0..256 scale.
@fragment
