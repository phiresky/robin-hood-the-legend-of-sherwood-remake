...
impl AudioDurationCache {
    /// Empty, non-persistent authority used by official projection export.
    ///
    /// A release projection must be derived from the selected source closure,
    /// never from mutable per-user cache state. Ordinary mission startup may
    /// still use [`Self::load`] because cache hits are revalidated against the
    /// resolved file identity.
    pub fn fresh_for_official_projection() -> Self {
        Self::default()
    }
...
mod tests {
    ...
    #[test]
    fn ordinary_and_official_cache_paths_derive_identical_metadata() {
        let directory = tempfile::tempdir().unwrap();
        std::fs::write(directory.path().join("voice.wav"), b"identity bytes").unwrap();
        let resolver = SampleResolver::new(directory.path());
        let loader: Box<SampleLoader> =
            Box::new(|name| (name == "voice.wav").then(|| (b"identity bytes".to_vec(), 14, 1234)));

        let mut ordinary = AudioDurationCache::default();
        let ordinary_duration = ordinary.duration_ms(&resolver, "voice.wav", loader.as_ref());
        let mut official = AudioDurationCache::fresh_for_official_projection();
        let official_duration = official.duration_ms(&resolver, "voice.wav", loader.as_ref());

        assert_eq!(ordinary_duration, Some(1234));
        assert_eq!(official_duration, ordinary_duration);
