...
// Platform exactly-one-writer queue: a writer thread natively, page-thread
// microtasks in the browser. Both expose submit/take_completions/shutdown.
#[cfg(target_arch = "wasm32")]
mod browser;
#[cfg(not(target_arch = "wasm32"))]
mod native;
#[cfg(target_arch = "wasm32")]
use browser::AutosaveWriter;
#[cfg(not(target_arch = "wasm32"))]
use native::AutosaveWriter;
...
impl AutosaveCoordinator {
    ...
    pub(crate) fn poll(&mut self, manager: &mut SaveGameManager) -> Vec<AutosavePollResult> {
        ...
        for completion in self.writer.take_completions() {
            match completion {
...
mod tests {
    ...
    fn browser_save_store_open_uses_memory_and_propagates_autosave_errors() {
        ...
        storage.write_text(&key, "corrupt manifest").unwrap();
    ...
    fn populated_diplomacy_autosave_commits_payload_before_publishing_manifest() {
        ...
        let host = Host::scratch(1280.0, 720.0);
        let filename = "Autosave_1788174574_0002".to_owned();
        let payload = GameSaveFile::capture(&engine, &host, 1, "Mission".into());
        let mut metadata = published_autosave(filename.clone(), payload.header.timestamp_unix);
        ...
        let completion = write_job(&AutosaveJob {
            ...
            reason: AutosaveReason::Backgrounded,
        })
        .expect("commit populated autosave and publish its manifest");
        assert!(matches!(
            completion,
            AutosaveCompletion::Saved {
                filename: committed,
        ...
        let manifest = load_manifest(&save_directory)
            .unwrap()
            .expect("published autosave manifest");
        ...
        assert_eq!(
            restored
                .engine
                .relationship(Camp::Custom(2), Camp::Custom(7)),
