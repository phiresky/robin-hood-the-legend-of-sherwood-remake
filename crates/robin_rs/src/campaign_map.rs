...
struct CampaignMapAssets {
    scrollbar: [Option<MenuSurface>; 6],
...
pub(crate) struct CampaignMapModalState {
    ...
    registration: Option<crate::leaderboard_registration::RegistrationHandle>,
...
impl CampaignMapModalState {
    ...
    fn details_offset(&self) -> Option<usize> {
        self.details_open
            .then(|| self.scroll_views[TEXT_VIEW].offset())
    }

    fn sync_scroll_views(&mut self, reveal: bool) {
        let initialize = self.scroll_views[TREE_VIEW].visible_range().is_empty();
        let total_depths = self
            .graph
            .nodes
            .iter()
            .map(|n| n.depth + 1)
        ...
        if let Some(node) = self.graph.nodes.get(self.selected_progress) {
            ...
            if reveal || initialize {
                ...
                self.scroll_views[GALLERY_VIEW].reveal(self.selected_progress / 4);
            }
        }
    }

    fn move_progress_page(&mut self, direction: i32) {
        let id = if self.achievement_overview {
            AWARDS_VIEW
        ...
        let page = self.scroll_views[id].visible_count();
        self.scroll_views[id].scroll_by(direction as isize * page as isize);
    }
    ...
    fn scroll_details(&mut self, direction: i32, history: bool) {
        ...
        let id = if history { HISTORY_VIEW } else { TEXT_VIEW };
    ...
    fn submit_or_open_selected_play(&mut self) {
        let result = (|| -> Result<String, String> {
            ...
            let campaign_run_id = play
                .campaign_run_id
                .ok_or("Recording has no campaign identity")?;
            #[cfg(target_arch = "wasm32")]
            {
                let _ = campaign_run_id;
            ...
            #[cfg(not(target_arch = "wasm32"))]
            {
                ...
                self.registration =
                    robin_util::sync::lock(&self.recording_index.submissions).registration(path);
    ...
    #[cfg(not(target_arch = "wasm32"))]
    fn registration_open(&self) -> bool {
        self.registration
            .as_ref()
            .is_some_and(|handle| robin_util::sync::lock(handle).is_some())
    }
    ...
    fn registration_event(&mut self, event: &GameEvent, transform: MenuTransform) {
        ...
        if let Some(registration) = slot.as_mut() {
            registration.event(event, transform, (240, 220));
    ...
    fn draw_registration(&self, renderer: &mut Renderer, transform: MenuTransform) {
        if let Some(handle) = &self.registration
            && let Some(registration) = robin_util::sync::lock(handle).as_mut()
        {
            let font = self.assets.progress_font.as_ref().expect("campaign font");
            registration.draw(renderer, font, transform, (240, 220));
        }
    }
    ...
    fn poll_recording_index(&mut self) {
    ...
    fn handle_events(
        ...
        if !self.registration_open()
            && let Some(handle) =
                robin_util::sync::lock(&self.recording_index.submissions).active_registration()
        {
        ...
        for event in events {
            ...
            let flow = match event {
                GameEvent::Quit => EventFlow::Finish(CampaignMapChoice::Quit),
                GameEvent::KeyDown { keycode, .. } => self.handle_key_down(keycode, browsing),
                GameEvent::MouseWheel(_) => EventFlow::Continue,
                ...
                GameEvent::MouseDown(x, y, 1, clicks) => {
                    self.handle_mouse_click(x, y, clicks, transform, browsing)
                }
                _ => EventFlow::Continue,
            };
            match flow {
                EventFlow::Continue => {}
                EventFlow::Skip => continue,
                EventFlow::Finish(choice) => {
                    final_choice = Some(choice);
                    break;
                }
            }
            if let Some(choice) = self.finish_event(&event, previous_selection) {
                final_choice = Some(choice);
                break;
            }
        }
    ...
    /// Key arms keep the original guard order; only `KeyDown` reaches here.
    fn handle_key_down(&mut self, keycode: Keycode, browsing: bool) -> EventFlow {
        match keycode {
            Keycode::Char(b'r' | b'd' | b'b')
                if self.presentation != CampaignPresentationMode::ClassicMap =>
            {
            ...
            Keycode::Escape => return EventFlow::Finish(CampaignMapChoice::Quit),
            Keycode::Tab => {
                self.achievement_overview = false;
                self.details_open = false;
                self.selected_play = 0;
            ...
            Keycode::PageDown if self.presentation != CampaignPresentationMode::ClassicMap => {
                self.move_progress_page(1)
            }
            Keycode::PageUp if self.presentation != CampaignPresentationMode::ClassicMap => {
                self.move_progress_page(-1)
            }
            Keycode::Left | Keycode::Right
                if self.presentation == CampaignPresentationMode::ProgressTree =>
            {
                let dx = if keycode == Keycode::Left { -1 } else { 1 };
                self.selected_progress = tree_neighbor(&self.graph, self.selected_progress, dx, 0);
            }
            Keycode::Up
                if self.presentation == CampaignPresentationMode::ClassicMap
                    && !self.items.is_empty() =>
            ...
            Keycode::Up if self.presentation != CampaignPresentationMode::ClassicMap => {
                match self.presentation {
                    CampaignPresentationMode::ProgressTree => {
                        self.selected_progress =
            ...
            Keycode::Down if self.presentation != CampaignPresentationMode::ClassicMap => {
                match self.presentation {
                    CampaignPresentationMode::ProgressTree => {
                        self.selected_progress =
                            tree_neighbor(&self.graph, self.selected_progress, 0, 1)
            ...
            Keycode::Left if self.presentation == CampaignPresentationMode::SherwoodMuseum => {
                self.exhibit_grid.navigate(-1, 0);
                self.selected_progress = self.exhibit_grid.selected;
            }
            Keycode::Right if self.presentation == CampaignPresentationMode::SherwoodMuseum => {
                self.exhibit_grid.navigate(1, 0);
                self.selected_progress = self.exhibit_grid.selected;
            }
            Keycode::Return | Keycode::KpEnter | Keycode::Space
                if self.presentation == CampaignPresentationMode::ClassicMap
                    && !self.items.is_empty() =>
            {
                return EventFlow::Finish(CampaignMapChoice::SelectMission(
            ...
            Keycode::Return | Keycode::KpEnter | Keycode::Space
                if self.presentation == CampaignPresentationMode::ClassicMap
                    && self.items.is_empty() =>
            {
                return EventFlow::Finish(CampaignMapChoice::Quit);
            }
            Keycode::Return | Keycode::KpEnter | Keycode::Space
                if self.presentation != CampaignPresentationMode::ClassicMap
                    && !browsing
    ...
    fn handle_mouse_click(
        ...
        self.input.virt_y = vy as f32;
        if self.presentation != CampaignPresentationMode::ClassicMap
            && let Some(flow) = self.handle_progress_chrome_click(vx, vy, browsing)
        {
            return flow;
        }
        if self.presentation != CampaignPresentationMode::ClassicMap
            && !self.achievement_overview
            && !self.details_open
            && let Some(index) = progress_hit_test_scrolled(
                &self.graph,
                self.presentation,
                self.selected_progress,
                vx,
                vy,
                self.scroll_views[if self.presentation == CampaignPresentationMode::ProgressTree {
                    TREE_VIEW
                } else {
                    GALLERY_VIEW
                }]
            ...
            )
        {
            ...
            self.exhibit_grid = ExhibitGridNavigator::new(self.graph.nodes.len(), index);
            if clicks >= 2 {
                self.details_open = true;
                self.scroll_views[TEXT_VIEW].reset();
                self.scroll_views[HISTORY_VIEW].reset();
    ...
    /// Tabs, pager and launch button of the progress screens. `None` means
    /// the click missed every chrome region and falls through to node hits.
    fn handle_progress_chrome_click(
        &mut self,
        vx: i32,
        vy: i32,
        browsing: bool,
        ...
        if (76..116).contains(&vy) {
            return Some(self.handle_progress_tab_click(vx));
        }
        if (490..526).contains(&vy) && !self.achievement_overview && !self.details_open {
        ...
        {
            return Some(EventFlow::Finish(CampaignMapChoice::SelectMission(
                self.graph.nodes[self.selected_progress].mission_idx,
            )));
        }
        None
    ...
    fn handle_progress_tab_click(&mut self, vx: i32) -> EventFlow {
        match vx {
            32..=207 => {
            ...
            884..=991 => {
                if std::mem::take(&mut self.details_open) {
    ...
    /// Per-event tail: selection-change resets, scroll sync and the classic
    /// widget frame. Returns a choice when a classic widget was activated.
    fn finish_event(
        &mut self,
        event: &GameEvent,
        previous_selection: usize,
    ) -> Option<CampaignMapChoice> {
        if previous_selection != self.selected_progress && self.details_open {
            self.details_open = true;
            self.scroll_views[TEXT_VIEW].reset();
            self.selected_play = 0;
            self.scroll_views[HISTORY_VIEW].reset();
        ...
        if matches!(
            event,
            GameEvent::KeyDown {
                keycode: Keycode::Left | Keycode::Right | Keycode::Up | Keycode::Down,
                ..
            }
        ) {
            self.sync_scroll_views(true);
        ...
        if self.presentation == CampaignPresentationMode::ClassicMap {
            ...
            if let Some(id) = widget_bridge::find_activated(&events) {
                ...
                if let Some(item) = self.items.iter().find(|item| item.loc_idx as u32 == id) {
                    return Some(CampaignMapChoice::SelectMission(item.mission_idx));
                }
            }
        }
        None
...
/// Outcome of routing one campaign-screen event through a per-event handler.
enum EventFlow {
    /// Run the per-event tail (`finish_event`).
    Continue,
    /// Skip the per-event tail, like `continue` in the event loop.
    Skip,
    /// Close the screen with this choice.
    Finish(CampaignMapChoice),
}

// TODO: Move campaign-manager labels and status text into the localization catalog.
const TREE_VIEW: usize = 0;
const GALLERY_VIEW: usize = 1;
const AWARDS_VIEW: usize = 2;
const HISTORY_VIEW: usize = 3;
const TEXT_VIEW: usize = 4;

fn campaign_scroll_views(assets: &CampaignMapAssets) -> [ScrollView; 5] {
    // Geometry remains available to headless input tests without GPU assets.
    let width = assets.scrollbar[0].map_or(16, |s| s.width);
    let min_thumb = assets.scrollbar[3]
        .zip(assets.scrollbar[5])
        .map_or(16, |(a, b)| a.height + b.height);
    let make = |bounds, row, horizontal| {
        ScrollView::with_geometry(bounds, row, width, min_thumb, horizontal)
    };
    let mut views = [
        make([32, 176, 960, 310], 240, true),
        make([32, 176, 960, 300], 100, false),
        make([32, 176, 960, 472], 236, false),
        make([560, 260, 432, 360], 72, false),
        make([32, 254, 512, 408], 25, false),
    ];
    views[TEXT_VIEW].set_wheel_step(3);
    views
}
...
fn progress_node_rect(
    ...
    let offset = if presentation == CampaignPresentationMode::ProgressTree {
        graph.nodes[selected].depth.saturating_sub(1).min(
            graph
                .nodes
                .iter()
                .map(|n| n.depth)
        ...
        selected / PROGRESS_PAGE_SIZE * 3
    };
    progress_node_rect_scrolled(graph, presentation, selected, index, offset)
}
#[cfg(test)]
fn progress_hit_test(
    ...
    graph.nodes.iter().enumerate().find_map(|(i, _)| {
        let rect = progress_node_rect(graph, presentation, selected, i);
        let (rx, ry, w, h) = rect;
        (card_visible(rect) && (rx..rx + w).contains(&x) && (ry..ry + h).contains(&y)).then_some(i)
    })
}
...
fn progress_node_rect_scrolled(
    graph: &CampaignProgressGraph,
    presentation: CampaignPresentationMode,
    selected: usize,
    index: usize,
    ...
    let (column, row) = match presentation {
        ...
        CampaignPresentationMode::SherwoodMuseum => {
            let page_index = index as i32 - (offset * 4) as i32;
...
fn render_campaign_progress(
    ...
    let views = if let Some(views) = scroll_views {
        ...
        capture_views = campaign_scroll_views(assets);
        capture_views[TREE_VIEW]
        ...
        if let Some(node) = graph.nodes.get(selected) {
            capture_views[TREE_VIEW].reveal(node.depth);
            capture_views[GALLERY_VIEW].reveal(selected / 4);
    ...
    let card_offset = views[if presentation == CampaignPresentationMode::ProgressTree {
        TREE_VIEW
    } else {
        GALLERY_VIEW
    }]
...
fn render_progress_cards(
    ...
    if presentation == CampaignPresentationMode::ProgressTree {
        for (_, child, rect) in visible_progress_cards(graph, presentation, selected, card_offset) {
            ...
            for &parent in &child.prerequisite_nodes {
                let (px, py, pw, ph) =
                    progress_node_rect_scrolled(graph, presentation, selected, parent, card_offset);
    ...
    views[if gallery { GALLERY_VIEW } else { TREE_VIEW }].draw_skin(
        renderer,
        transform,
        &assets.scrollbar,
    );
...
fn render_mission_details(
    ...
    text_view.draw_skin(renderer, transform, &assets.scrollbar);
    progress_text(
        renderer,
        font,
        transform,
...
/// Borrowed inputs of the classic-map mission tooltip.
// Not serde: a per-frame view over live renderer-side state, never persisted.
struct TooltipView<'a> {
    campaign: &'a Campaign,
    profiles: &'a engine_profiles::ProfileManager,
    item: &'a CampaignMapItem,
    assets: &'a CampaignMapAssets,
    resources: Option<&'a IngameMenuResources>,
    input: &'a ModalInputState,
    show_achievement_badges: bool,
}

fn render_tooltip(renderer: &mut Renderer, transform: MenuTransform, view: TooltipView<'_>) {
    let TooltipView {
        campaign,
        profiles,
        item,
...
mod browser_tests {
    ...
    fn campaign_views_scroll_independently_and_hit_test_the_visible_cards() {
        ...
        state.graph.nodes = (0..40)
            ...
            .collect();
        let t = progress_transform(1024, 768);
        state.input.virt_x = 100.0;
        state.input.virt_y = 200.0;
        state.handle_events(vec![GameEvent::MouseWheel(-2)], t, true);
        assert_eq!(state.scroll_views[TREE_VIEW].offset(), 2);
        assert_eq!(state.selected_progress, 0);
        state.presentation = CampaignPresentationMode::SherwoodMuseum;
        state.handle_events(vec![GameEvent::MouseWheel(-2)], t, true);
        assert_eq!(state.scroll_views[GALLERY_VIEW].offset(), 2);
        assert_eq!(state.scroll_views[TREE_VIEW].offset(), 2);
        state.handle_events(vec![GameEvent::MouseDown(40, 185, 1, 1)], t, true);
        assert_eq!(state.selected_progress, 8);
        state.achievement_overview = true;
        state.handle_events(vec![GameEvent::MouseWheel(-100)], t, true);
        assert!(state.scroll_views[AWARDS_VIEW].offset() > 0);
        assert_eq!(state.selected_progress, 8);
        assert_eq!(state.scroll_views[GALLERY_VIEW].offset(), 2);
    }
    ...
    fn detail_columns_scroll_independently_and_clamp_to_content() {
        ...
        assert_eq!(state.details_offset(), Some(3));
        assert_eq!(state.scroll_views[HISTORY_VIEW].offset(), 0);
        state.input.virt_x = 600.0;
        state.handle_events(vec![GameEvent::MouseWheel(-2)], transform, true);
        assert_eq!(state.scroll_views[HISTORY_VIEW].offset(), 2);
        assert_eq!(state.details_offset(), Some(3));
        state.handle_events(vec![GameEvent::MouseDown(580, 265, 1, 1)], transform, true);
    ...
    fn mission_details_cannot_launch_a_mission_and_tabs_restore_navigation() {
        ...
        assert_eq!(state.details_offset(), Some(0));
        state.handle_events(vec![key(Keycode::Char(b'r'))], transform, false);
        assert_eq!(state.details_offset(), None);
    ...
    fn browser() -> CampaignMapModalState {
        ...
        CampaignMapModalState {
            items: Vec::new(),
            assets: CampaignMapAssets {
                progress_font: Some(Font::TrueType(crate::font::TrueTypeFont::from_parts(
                    &[0; 32],
                    15,
                    0,
                    0,
                ...
                ..CampaignMapAssets::default()
            },
            scroll_views: campaign_scroll_views(&CampaignMapAssets::default()),
            frame: FrameWnd::default(),
    ...
    fn username_modal_captures_editing_and_cancel_without_navigating() {
        let mut state = browser();
        state.registration = Some(std::sync::Arc::new(std::sync::Mutex::new(Some(
            crate::leaderboard_registration::Registration::awaiting_name(),
        ))));
        let transform = progress_transform(1024, 768);
        // Reopening the screen must recover a prompt owned by an upload worker.
        let path = std::path::PathBuf::from("pending-registration.rhrec.jsonl");
        {
            let mut submissions = robin_util::sync::lock(&state.recording_index.submissions);
            submissions.seed_capture_info(
                path.clone(),
                crate::leaderboard::history::ReplaySubmissionInfo {
                    message: "Submitting replay...".into(),
                    can_submit: false,
                    url: None,
                },
            );
            let handle = submissions.registration(&path).unwrap();
            *robin_util::sync::lock(&handle) =
                robin_util::sync::lock(state.registration.as_ref().unwrap()).take();
        }
        state.registration = None;
        state.handle_events(Vec::new(), transform, true);
        ...
        assert!(
            !robin_util::sync::lock(state.registration.as_ref().unwrap())
                .as_ref()
                .unwrap()
                .busy()
        );
...
mod capture_tests {
    ...
    fn capture_campaign_ui() {
        ...
        for (width, height) in [(1024, 768)] {
            ...
            let mut mission_renderer = Renderer::offscreen(gpu.clone(), 640, 480);
            mission_renderer.begin_gpu_frame_clear();
            mission_renderer.begin_ui_only_frame();
            ...
            registration.draw(
                &mut mission_renderer,
                resources.label_font_any().unwrap(),
                MenuTransform::centered(640, 480),
                (48, 88),
            );
            write_capture_png(
                &mut mission_renderer,
                &output.join("registration-mission-end-640x480.png"),
            );
        }
    }

    fn write_capture_json(output: &std::path::Path, name: &str, value: &impl serde::Serialize) {
        serde_json::to_writer_pretty(std::fs::File::create(output.join(name)).unwrap(), value)
    ...
    fn seed_capture_history(state: &mut CampaignMapModalState) {
    ...
    /// Longest detail text, first, fifth, middle, last and deepest-lane nodes.
    fn capture_selections(state: &CampaignMapModalState) -> [usize; 6] {
        [
            state
                .graph
                .nodes
    ...
    fn capture_progress_views(
        ...
        for (name, mode) in [
            ...
            state.registration = (name == "registration").then(|| {
                ...
                std::sync::Arc::new(std::sync::Mutex::new(Some(registration)))
            });
