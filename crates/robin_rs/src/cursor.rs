...
/// Mouse-cursor draw flags from the `CUR ` resource. The
/// draw mode occupies the low four bits: normal mode is a plain
/// source-transparent blit, while shadowed mode keys the shadow colour out and
/// alpha-blends it.
const CURSOR_DRAW_SHADOWED: u16 = 2;
const CURSOR_DRAW_MASK: u16 = 0xF;
...
/// Presentation-only treatment for the cursor's authored shadow pixels.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq)]
pub enum CursorShadowEffect {
    /// Ordinary opacity and optional bow-target tint.
    #[default]
    Normal,
    /// Original RGB565 recording feedback for Original's three-slot QA recorder.
    QuickActionRecording { tick: u32 },
}

const QUICK_ACTION_PULSE_PERIOD_TICKS: u32 = 20;
const QUICK_ACTION_PULSE_PEAK_TICK: u32 = QUICK_ACTION_PULSE_PERIOD_TICKS / 2;

impl CursorShadowEffect {
    /// Select the recording treatment only for the classic manual
    /// quick-action recorder. The caller supplies that recorder's global
    /// ownership state; automatic Shift-click queues never set it.
    fn for_quick_action_recording(
        enabled: bool,
        recording: bool,
        elapsed: std::time::Duration,
    ) -> Self {
        if enabled && recording {
            Self::QuickActionRecording {
                tick: ((elapsed.as_millis() / 40) % u128::from(QUICK_ACTION_PULSE_PERIOD_TICKS))
                    as u32,
            }
        } else {
            Self::Normal
...
pub struct CursorRenderer {
    ...
    /// Local presentation clock; never saved or tied to simulation/replay time.
    pulse_clock: web_time::Instant,
...
impl CursorRenderer {
    ...
    /// Recording state selects the effect; only the cursor owns its animation time.
    pub fn quick_action_recording_effect(
        &self,
        enabled: bool,
        recording: bool,
    ) -> CursorShadowEffect {
        CursorShadowEffect::for_quick_action_recording(
            enabled,
            recording,
            self.pulse_clock.elapsed(),
        )
    }
    ...
    pub fn load_cursor(
        ...
        let (hotspot, frame_length, shadowed) = match resource_manager.get_mouse_entry(cursor_id) {
            Ok(entry) => (
                entry.hotspot,
                entry.frame_length,
                entry.flags & CURSOR_DRAW_MASK == CURSOR_DRAW_SHADOWED,
            ),
            Err(e) => {
    ...
    pub fn render(
        ...
        self.render_with_effect(
            ...
            opacity,
            shadow_color,
            CursorShadowEffect::Normal,
        );
    }

    /// Render the cursor with an explicit presentation-only shadow effect.
    pub fn render_with_effect(
        ...
        let shadow_tint = cursor_shadow_tint(opacity, shadow_color, effect);
        // Recording intentionally remains visible when an action arm has
        // disabled the ordinary bow shadow by setting `opacity` to zero.
        // Original's pulse branch likewise ignored the normal shadow level.
        if shadow_tint[3] > 0.0
...
fn cursor_shadow_tint(opacity: u16, shadow_color: u16, effect: CursorShadowEffect) -> [f32; 4] {
    let normal_alpha = opacity.min(100) as f32 / 100.0;
    match effect {
        CursorShadowEffect::Normal => {
            let (r, g, b) = if shadow_color == 0 {
                (0, 0, 0)
            } else {
                rgb565_to_rgb8(shadow_color)
            };
            [
                r as f32 / 255.0,
                g as f32 / 255.0,
        ...
        CursorShadowEffect::QuickActionRecording { tick } => {
            let value = quick_action_pulse_value(tick);
            // SBThreadedInput::ChangeShadowIntoPulse's 16-bit formula.
            // Shadow-key pixels are replaced outright, even at the black trough.
            let packed =
                ((31 * value / 100) << 11) | ((60 * value / 100) << 5) | (19 * value / 100);
            let (r, g, b) = rgb565_to_rgb8(packed);
            [r as f32 / 255.0, g as f32 / 255.0, b as f32 / 255.0, 1.0]
        }
    }
}

fn quick_action_pulse_value(tick: u32) -> u16 {
    let phase = tick % QUICK_ACTION_PULSE_PERIOD_TICKS;
    let distance = phase.min(QUICK_ACTION_PULSE_PERIOD_TICKS - phase);
    (distance * 100 / QUICK_ACTION_PULSE_PEAK_TICK) as u16
}
...
mod tests {
    ...
    fn recording_pulse_uses_local_elapsed_time_and_wraps() {
        for (millis, value) in [(0, 0), (200, 50), (400, 100), (600, 50), (800, 0)] {
            ...
            let effect = CursorShadowEffect::for_quick_action_recording(true, true, elapsed);
            let CursorShadowEffect::QuickActionRecording { tick } = effect else {
                panic!("recording must select the pulse");
            };
            assert_eq!(quick_action_pulse_value(tick), value);
            assert_eq!(
                CursorShadowEffect::for_quick_action_recording(false, true, elapsed),
                CursorShadowEffect::Normal
            );
            assert_eq!(
                CursorShadowEffect::for_quick_action_recording(true, false, elapsed),
                CursorShadowEffect::Normal
            );
        }
    }
    ...
    #[test]
    fn normal_cursor_shadow_honors_rgb565_bow_tint() {
        assert_eq!(
            cursor_shadow_tint(75, 0xF800, CursorShadowEffect::Normal),
            [248.0 / 255.0, 0.0, 0.0, 0.75]
        );
        assert_eq!(
            cursor_shadow_tint(40, 0, CursorShadowEffect::Normal),
            [0.0, 0.0, 0.0, 0.4]
        );
    }

    #[test]
    fn recording_pulse_matches_original_rgb565_and_ignores_normal_shadow() {
        for (tick, rgb) in [
            (0, [0, 0, 0]),
            (5, [120, 120, 72]),
            (10, [248, 240, 152]),
            ...
            let effect = CursorShadowEffect::QuickActionRecording { tick };
            let expected = [
                rgb[0] as f32 / 255.0,
                rgb[1] as f32 / 255.0,
                rgb[2] as f32 / 255.0,
            ...
            assert_eq!(cursor_shadow_tint(40, 0xF800, effect), expected);
            assert_eq!(cursor_shadow_tint(0, 0, effect), expected);
        }
    }
