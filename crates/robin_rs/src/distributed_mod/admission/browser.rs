...
/// Browser store: the asynchronous IndexedDB cache. It is a process-global
/// database, so the application context is not consulted.
pub struct BrowserDistributedModStore;

impl DistributedModStore for BrowserDistributedModStore {
    // Browser transports deliver the offer asynchronously and IndexedDB writes
    // can stall (quota prompts, suspended tabs), so the browser admission uses
    // explicit wall-clock budgets. Native admission deliberately has none.
    const OFFER_WAIT: Option<std::time::Duration> = Some(std::time::Duration::from_secs(15));
    ...
    async fn append_chunk(
        ...
        distributed_mod_cache::append_chunk(
