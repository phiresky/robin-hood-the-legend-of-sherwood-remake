...
impl TrueTypeFont {
    ...
    pub fn load(sbf_path: &Path, files: &SbFileSystem) -> Self {
        ...
        let data = match files.read_all(&path) {
            ...
            Err(status) => {
                tracing::error!(
                    "robin_rs font: cannot read '{}' through the active asset lookup: file error {}",
                    sbf_path.display(),
                    status
                );
                return font;
        ...
        let resolved = font.find_and_load_ttf(sbf_path.parent(), files);
    ...
    fn find_and_load_ttf(&mut self, sbf_dir: Option<&Path>, files: &SbFileSystem) -> u32 {
...
mod tests {
    ...
    fn descriptor_and_face_use_the_same_preparation_reader() {
        let mut authored = make_test_font();
        authored.tt_name = [0; FONT_NAME_LEN];
        authored.tt_name[..12].copy_from_slice(b"PreparedFace");
        let descriptor = authored.to_sbf_bytes();
        let face = std::fs::read(find_arial()).unwrap();
        let make = |font_bytes: Vec<u8>| {
            let vfs = std::sync::Arc::new(robin_util::asset_fs::AssetVfs::new());
            vfs.install_preloaded_asset("isolated/fonts/font.tfn", descriptor.clone())
                .unwrap();
            vfs.install_preloaded_asset("isolated/fonts/PreparedFace.ttf", font_bytes)
                .unwrap();
            SbFileSystem::new(vfs)
        };
        let valid = make(face);
        let invalid = make(b"not a font".to_vec());
