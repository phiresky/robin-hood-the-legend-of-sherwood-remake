...
pub struct Game {
    ...
    /// Process-local immutable identity of the assets mounted for this
    /// mission. Saves and replays must clone this exact descriptor; they must
    /// never infer an archive source from mutable filesystem state later.
    mission_assets: Option<robin_engine::mission_assets::MissionAssetDescriptor>,
}
...
impl Game {
    ...
    pub(crate) fn set_mission_assets(
        &mut self,
        descriptor: robin_engine::mission_assets::MissionAssetDescriptor,
    ) -> Result<(), String> {
        descriptor.validate().map_err(|error| error.to_string())?;
        self.mission_assets = Some(descriptor);
        Ok(())
    }

    pub(crate) fn mission_assets(
        &self,
    ) -> Result<&robin_engine::mission_assets::MissionAssetDescriptor, String> {
        self.mission_assets
            .as_ref()
            .ok_or_else(|| "mission asset descriptor was not installed at bootstrap".to_owned())
    }
    ...
    pub fn process_operation(
        ...
        match self.operation.get_current() {
            ...
            GameCode::LevelSave => {
                callbacks.request_save(campaign, profiles);
                self.operation.set(GameCode::LevelInProgress);
                None
            }
            GameCode::LevelRestart => {
                callbacks.request_restart_snapshot(false);
                self.operation.set(GameCode::LevelInProgress);
                None
            }
            GameCode::Quit => {
            ...
            GameCode::LevelNext => {
                ...
                // The game loop applies these level-transition changes
    ...
    fn handle_level_load(
        ...
        if save_mission_id == current_mission_id {
            callbacks.request_load(save_mission_id);
            self.operation.set(GameCode::LevelInProgress);
    ...
    fn handle_quit(
        ...
        callbacks.request_continue_save(mission_id);
    ...
    fn handle_level_succeeded(
        ...
        callbacks.synchronize_profile_with_campaign(campaign, profiles);
        // Original-game main-loop behavior:
...
mod tests {
    ...
    impl GameCallbacks for StubCallbacks {
        fn request_save(&mut self, _: &Campaign, _: &engine_profiles::ProfileManager) {}
        fn request_load(&mut self, _: u32) {}
        fn request_restart_snapshot(&mut self, _: bool) {}
        fn request_continue_save(&mut self, _: u32) {}
        fn save_profiles(&mut self) {}
        fn synchronize_profile_with_campaign(
            &mut self,
            _: &Campaign,
