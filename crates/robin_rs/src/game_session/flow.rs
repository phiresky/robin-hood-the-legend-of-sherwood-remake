...
impl InteractiveMission {
    /// Own the loaded interactive mission until its existing exit path
    /// returns control to the session wrapper.
    pub(super) async fn run(
        ...
        loop {
            ...
            if !input_suppressed {
                let zoom_enable =
                    ZoomButtonEnable::from_engine(&manager.engine, &host.engine_display);
                ...
                for event in &events {
                    match *event {
                        GameEvent::MouseDown(mx, my, 1 /* left */, _) => {
                            let Some(btn) = hud.corner_layout.hit_test(mx, my, corner_enable)
                            else {
                                continue;
                            };
            ...
            let repeat_step_key = |held: bool, hit: bool, repeat_at_ms: &mut Option<u32>| -> bool {
                ...
                {
                    *repeat_at_ms =
                        Some(frame.started_at_ms.saturating_add(STEP_REPEAT_INTERVAL_MS));
                    return true;
                }
                false
            };
                        ...
                        GameAction::DisplayMenu => {
                            if ui.pause_menu.is_some() {
                                ...
                                if let Some(resources) = resources.menu.as_ref() {
                                    ui.pause_menu =
                                        Some(PauseMenu::new(resources, ui.restart_allowed));
                                } else {
                                ...
                                if ui.pause_menu.is_some() {
                                    ...
                                    callbacks
                                        .emit_app_effect(AppEffect::SetSoundMode(SoundMode::Menu));
                                }
                            }
                        }
                            ...
                            match action {
                                ...
                                GameAction::Teleport => {
                                    ...
                                    if let Some(mouse_map) =
                                        host.viewport.screen_to_map(mouse_screen)
                                    {
                                        if !manager
                                            .engine
            ...
            let mut paused =
                ui.pause_menu.is_some() || *manual_pause || mp_clock_pause || modal_pause;
