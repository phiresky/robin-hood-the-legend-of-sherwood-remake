    ...
    async fn run_frame(
        ...
        let control = match outcome {
            ...
            FrameSimulationOutcome::Present(handoff) => {
                ...
                // its live replay record have all been finalized.
                let MissionRuntime {
                    world,
                    timeline,
                    control,
...
fn plan_interactive_pacing(
    ...
    let target = if args.fast_forward {
        ...
        1
    } else if host.frontend.slow_motion {
...
async fn pace_interactive_frame(
    ...
    if remaining_wait_ms > 0 {
        ...
        // event loop. Otherwise repeated slow ticks monopolize one future
        // poll and starve input, audio completion, RPC and lifecycle events.
        // This is a no-op on the dedicated native game thread.
