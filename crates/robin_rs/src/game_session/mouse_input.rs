...
fn portrait_action_right_click(
    ...
    // The original radio widget sends UnSelect for a selected button. For an
...
pub(super) fn handle_pause_menu_events(
    ...
    let audio_backend: &mut Option<KiraAudioBackend> = &mut context.audio.backend;
    ...
    match btn {
        CornerButton::Clock => {
            ...
            if !engine.is_recording_macro() {
                ...
                let slot = choose_recording_place(engine, local_seat);
...
mod shift_planning_tests {
    ...
    fn touch_takeover_cancels_gesture_without_dispatching_world_release() {
        ...
        host.frontend
            .input
            .press_left_pointer(Default::default(), 2);
        host.frontend
            .input
            .start_multi_selection(Default::default());
        host.frontend.input.latch_selection_outline();
        host.frontend.input.ignore_mouse_event(true, true, true);
        let mut commands = FrameCommands::new();

        cancel_left_pointer(&mut host, &mut commands);

        assert!(
            commands.is_empty(),
            "touch takeover cannot box-select or click the world"
        );
        assert!(!host.frontend.input.left_mouse_down());
        assert!(!host.frontend.input.is_dragging());
        assert!(!host.frontend.input.multi_selection_active());
        assert!(!host.frontend.input.draw_multi_selection());
        assert!(!host.frontend.input.ignore_next_drag());
        assert!(!host.frontend.input.consume_suppressed_click());
        assert!(!host.frontend.input.release_left_pointer());
        host.frontend
            .input
            .press_left_pointer(Default::default(), 2);
        assert!(host.frontend.input.release_left_pointer());
