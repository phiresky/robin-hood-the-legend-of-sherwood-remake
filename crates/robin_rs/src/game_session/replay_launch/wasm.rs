...
pub(super) async fn resolve_non_built_in_mission_assets(
    ...
    match &descriptor.source {
        ...
        MissionAssetSource::BuiltIn => {
            Ok(crate::mission_asset_restore::resolve_built_in_mission_assets(descriptor, package)?)
        }
        MissionAssetSource::Archive(archive) => {
            ...
            let lease = crate::distributed_mod_cache::acquire(cache_identity.full_mod_sha256)
                .await
                .map_err(|error| {
                    MissionError::asset(format!("acquire browser replay mission cache: {error}"))
                })?
                .ok_or_else(|| {
                    MissionError::asset(format!(
            ...
            Ok(crate::mission_asset_restore::resolve_cached_mission_assets(
                descriptor,
                package,
                lease,
                application_context
