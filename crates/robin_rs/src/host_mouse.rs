...
pub fn update_pc_popup_information(
    ...
    let drag_active = host.frontend.input.multi_selection_active()
...
pub fn choose_mouse_pointer_for_no_action(
    ...
    if host.frontend.input.draw_multi_selection() {
    ...
    for &eid in draw_order_ids.iter().rev() {
        ...
        // returns the default cursor (the regular arrow stays put —
        // left-click switches selection); the only override is
        // short-leg cursor when the hovered PC is in the
        ...
        if use_ok {
            ...
            // unavailable-payment (beggar with insufficient ransom) and
            // unavailable-pickup (heavy corpse, unpickupable object) cursors —
    ...
    use robin_engine::resource_ids::*;
    {
        ...
        if !in_restricted {
            ...
            if let Some(target_id) = engine.find_focusable_entity(
                ...
                // Extract entity data before mutating host.frontend.input.
