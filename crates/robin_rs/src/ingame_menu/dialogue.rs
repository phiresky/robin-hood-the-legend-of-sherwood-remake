...
impl DialogueModalState {
    ...
    /// Speech and mouth animation are presentation, not replay completion
    /// authority. Advance completed voice sentences, but hold the final one
    /// until ModalBatch receives its recorded outcome.
    pub(crate) fn advance_replay_presentation(
        &mut self,
        sound: &mut SoundManager,
        sound_config: &robin_engine::sound_config::SoundConfig,
        audio: &mut Option<&mut dyn AudioBackend>,
        ...
        self.ensure_audio_started(sound, sound_config, audio, sound_enabled);
        if self.last_timer_tick.elapsed() < TIMER_INTERVAL || !sound_enabled {
            return;
        }
        self.last_timer_tick = Instant::now();
        if sound.is_dialog_finished() {
    ...
    pub(crate) fn finish_replay_audio(
        &mut self,
        sound: &mut SoundManager,
        sound_config: &robin_engine::sound_config::SoundConfig,
        audio: Option<&mut dyn AudioBackend>,
        ...
        if self.entered_dialogue {
            self.finish(sound, sound_config, audio, result);
        }
    }
...
fn draw_dialogue_body(
    ...
    let current_portrait = resources.portrait(renderer, portrait_fade.current, mouth_frame);
    let previous_portrait = if portrait_fade.is_fading() {
        resources.portrait(renderer, portrait_fade.previous, mouth_frame)
    } else {
        None
    };
...
mod tests {
    ...
    impl AudioBackend for ReplayAudioProbe {
        fn play_sound(&mut self, _: &str, _: bool) -> Option<i32> {
            panic!("unexpected channel playback")
        }
        fn play_sound_at(&mut self, _: &str, _: bool, _: f32) -> Option<i32> {
            panic!("unexpected channel playback")
        }
        fn halt_channel(&mut self, _: i32) {
            panic!("unexpected channel stop")
        }
        fn set_channel_volume(&mut self, _: i32, _: u16) {
            panic!("unexpected channel volume")
        }
        fn is_channel_playing(&self, _: i32) -> bool {
            panic!("unexpected channel query")
        }
        fn pause_channels(&mut self, _: i32) {
            panic!("unexpected channel pause")
        }
        fn resume_channels(&mut self, _: i32) {
            panic!("unexpected channel resume")
        }
        fn play_music(&mut self, path: &str, _: bool) -> bool {
            self.played.push(path.into());
            true
        }
        ...
        fn pause_music(&mut self) {
            panic!("unexpected music pause")
        }
        fn resume_music(&mut self) {
            panic!("unexpected music resume")
        }
        ...
        fn play_jingle(&mut self, _: &str) -> Option<i32> {
            panic!("unexpected jingle")
        }
        fn free_jingle(&mut self) {
            panic!("unexpected jingle cleanup")
        }
    ...
    fn replay_audio_fixture() -> DialogueModalState {
        DialogueModalState {
            sentences: ["first.wav", "second.wav"]
                .into_iter()
                .map(|path| DialogueSentence {
                    portrait_index: 0,
                    text: path.into(),
            ...
            tooltip: TooltipState::new(),
            transform: MenuTransform::centered(1024, 768),
            virt_x: 0,
            virt_y: 0,
            mouth_frame: 0,
            same_face_count: 0,
    ...
    #[test]
    fn replay_dialogue_plays_advances_holds_final_and_cleans_up_recorded_outcome() {
        let mut state = replay_audio_fixture();
        let mut backend = ReplayAudioProbe::default();
        let mut sound = SoundManager::new();
        sound.initialize(&mut backend, false).unwrap();
        let config = robin_engine::sound_config::SoundConfig::default();
        state.advance_replay_presentation(&mut sound, &config, &mut Some(&mut backend), true);
        assert_eq!(backend.played, ["first.wav"]);
        assert!(state.entered_dialogue);
        assert_eq!(serde_json::to_value(&sound).unwrap()["dialog_mode"], true);
        // A second render must not restart the same voice sample.
        state.advance_replay_presentation(&mut sound, &config, &mut Some(&mut backend), true);
        assert_eq!(backend.played.len(), 1);
        for expected_index in [1, 1] {
            backend.finished = true;
            sound.hourglass(
                &mut backend,
                &|_| None,
                &mut |_| 0,
                crate::sound::AlertStatus::Green,
                &robin_engine::sound_source::SoundSourceManager::new(),
                &mut Vec::new(),
            );
            state.last_timer_tick = Instant::now() - TIMER_INTERVAL;
            state.advance_replay_presentation(&mut sound, &config, &mut Some(&mut backend), true);
            assert_eq!(state.sentence_idx, expected_index);
            assert!(
                state.entered_dialogue,
                "final audio cannot complete the modal"
            );
        }
        assert_eq!(backend.played, ["first.wav", "second.wav"]);
        let stops_before = backend.halted;
        state.finish_replay_audio(
            &mut sound,
            &config,
            Some(&mut backend),
            DialogResult::Completed,
        );
        assert_eq!(backend.halted, stops_before + 1);
        assert!(!state.entered_dialogue);
        assert_eq!(serde_json::to_value(&sound).unwrap()["dialog_mode"], false);
    }

    #[test]
    fn immediately_recorded_dialogue_does_not_stop_unowned_audio() {
        let mut state = replay_audio_fixture();
        let mut sound = SoundManager::new();
        let mut backend = ReplayAudioProbe::default();
        sound.initialize(&mut backend, false).unwrap();
