...
// Rust-port strings use namespaced keys instead of pretending to be entries in
// the retail numeric table. This prevents unrelated port features from silently
// overwriting each other's fallback strings.
pub const MT_PORT_BTN_DIFFICULTY: &str = "difficulty.button";
pub const MT_PORT_STR_DIFFICULTY_LEGENDARY: &str = "difficulty.legendary";
pub const MT_PORT_STR_DIFFICULTY_CUSTOM: &str = "difficulty.custom";
pub const MT_PORT_TTL_ADVANCED_DIFFICULTY: &str = "difficulty.advanced.title";
pub const MT_PORT_STR_DIFFICULTY_HELP: &str = "difficulty.advanced.help";
pub const MT_PORT_DIFF_LEGACY_SCRIPT: &str = "difficulty.rule.script_compatibility";
pub const MT_PORT_DIFF_ENEMY_FIGHTING: &str = "difficulty.rule.enemy_fighting";
pub const MT_PORT_DIFF_ENEMY_SHOOTING: &str = "difficulty.rule.enemy_shooting";
pub const MT_PORT_DIFF_ENEMY_IQ: &str = "difficulty.rule.enemy_intelligence";
pub const MT_PORT_DIFF_ENEMY_HEALTH: &str = "difficulty.rule.enemy_health";
pub const MT_PORT_DIFF_REACTION_TIME: &str = "difficulty.rule.reaction_time";
pub const MT_PORT_DIFF_BLIP_RANGE: &str = "difficulty.rule.minimap_detection_range";
pub const MT_PORT_DIFF_CARNAGE: &str = "difficulty.rule.recruitment_carnage";
pub const MT_PORT_DIFF_SMALL_CAPACITY: &str = "difficulty.rule.small_item_capacity";
pub const MT_PORT_DIFF_LARGE_CAPACITY: &str = "difficulty.rule.large_item_capacity";
pub const MT_PORT_DIFF_AUTO_HEAL: &str = "difficulty.rule.auto_heal_interval";
pub const MT_PORT_DIFF_NET_PREVIEW: &str = "difficulty.rule.accurate_net_preview";
pub const MT_PORT_DIFF_FRIENDLY_FIRE: &str = "difficulty.rule.protect_allies_from_arrows";
pub const MT_PORT_DIFF_SPECIAL_DELAY: &str = "difficulty.rule.special_strike_delay";
pub const MT_PORT_DIFF_PUNCH: &str = "difficulty.rule.punch_concussion";
pub const MT_PORT_DIFF_GUARD_VIEW_DISTANCE: &str = "difficulty.rule.guard_view_distance";
pub const MT_PORT_DIFF_GUARD_VIEW_ANGLE: &str = "difficulty.rule.guard_view_angle";
pub const MT_PORT_DIFF_GUARD_NOISE_SENSITIVITY: &str = "difficulty.rule.guard_noise_sensitivity";
...
// time via tooltip-text assignment.
...
fn default_fallbacks() -> &'static HashMap<usize, &'static str> {
    static FALLBACK_MAP: std::sync::LazyLock<HashMap<usize, &'static str>> =
        std::sync::LazyLock::new(|| DEFAULT_FALLBACKS.iter().copied().collect());
    &FALLBACK_MAP
}

/// Hardcoded English strings matching `1033/Data/Interface/Start.sxt` from
/// the international release.  Used when no `.sxt` file is available so the
/// Rust port is still usable on developer machines without the localised
/// text bundle.  Keys are unique (`default_fallback_table_has_unique_keys`),
/// so the lookup map holds exactly these pairs.
static DEFAULT_FALLBACKS: &[(usize, &str)] = &[
...
fn port_fallback(key: &str) -> Option<&'static str> {
    Some(match key {
        MT_PORT_BTN_DIFFICULTY => "Difficulty",
        MT_PORT_STR_DIFFICULTY_LEGENDARY => "Legendary",
        MT_PORT_STR_DIFFICULTY_CUSTOM => "Custom",
        MT_PORT_TTL_ADVANCED_DIFFICULTY => "Advanced difficulty",
        MT_PORT_STR_DIFFICULTY_HELP => "Up/Down selects a rule. Left/Right changes it.",
        MT_PORT_DIFF_LEGACY_SCRIPT => "Script compatibility",
        MT_PORT_DIFF_ENEMY_FIGHTING => "Enemy fighting",
        MT_PORT_DIFF_ENEMY_SHOOTING => "Enemy shooting",
        MT_PORT_DIFF_ENEMY_IQ => "Enemy intelligence",
        MT_PORT_DIFF_ENEMY_HEALTH => "Enemy health",
        MT_PORT_DIFF_REACTION_TIME => "Reaction time",
        MT_PORT_DIFF_BLIP_RANGE => "Minimap detection range",
        MT_PORT_DIFF_CARNAGE => "Recruitment carnage",
        MT_PORT_DIFF_SMALL_CAPACITY => "Small item capacity",
        MT_PORT_DIFF_LARGE_CAPACITY => "Large item capacity",
        MT_PORT_DIFF_AUTO_HEAL => "Auto-heal interval",
        MT_PORT_DIFF_NET_PREVIEW => "Accurate net preview",
        MT_PORT_DIFF_FRIENDLY_FIRE => "Protect allies from arrows",
        MT_PORT_DIFF_SPECIAL_DELAY => "Special-strike delay",
        MT_PORT_DIFF_PUNCH => "Punch concussion",
        MT_PORT_DIFF_GUARD_VIEW_DISTANCE => "Guard view distance",
        MT_PORT_DIFF_GUARD_VIEW_ANGLE => "Guard view angle",
        MT_PORT_DIFF_GUARD_NOISE_SENSITIVITY => "Guard noise sensitivity",
        _ => return None,
    })
}
...
impl MenuFonts {
    pub fn load(files: &robin_engine::sbfile::SbFileSystem) -> Self {
        let config = match native_font::load_font_config(files) {
            Ok(c) => c,
            Err(e) => {
...
impl IngameMenuResources {
    ...
    pub fn new(
        ...
        let mut res = ResourceManager::with_files(files.clone());
        ...
        Self::from_manager(renderer, shipping, res, files)
    }
    ...
    pub fn from_manager(
        ...
        let ok_button = load_sprite_pack(&mut res, renderer, &mut owners, resource_ids::RHID_OK);
        let cancel_button =
            load_sprite_pack(&mut res, renderer, &mut owners, resource_ids::RHID_CANCEL);
        let radio = load_sprite_pack(&mut res, renderer, &mut owners, resource_ids::RHID_RADIO);
        let restart_button =
        ...
        let input_field = load_sprite_pack(
            &mut res,
            renderer,
            &mut owners,
            resource_ids::RHID_MENU_INPUT_FIELD,
        );
        let slider = load_sprite_pack(&mut res, renderer, &mut owners, resource_ids::RHID_SLIDER);
    ...
    pub(super) fn stub() -> Self {
        Self {
            ...
            ok_button: SpriteBank::default(),
            cancel_button: SpriteBank::default(),
            restart_button: SpriteBank::default(),
            load_button: SpriteBank::default(),
            parchment_huge: None,
            menu_bg_small: None,
            menu_bg: [None, None, None, None],
            input_field: SpriteBank::default(),
            radio: SpriteBank::default(),
...
mod tests {
    ...
    #[test]
    fn default_fallback_table_has_unique_keys() {
        let mut seen = std::collections::HashSet::new();
        for (key, _) in DEFAULT_FALLBACKS {
            assert!(seen.insert(*key), "duplicate fallback key {key}");
        }
        let map = default_fallbacks();
        assert_eq!(map.len(), DEFAULT_FALLBACKS.len());
        ...
        assert_eq!(map.get(&MT_STR_KEY_RESERVED), Some(&"<Reserved>"));
    }
    ...
    #[test]
    fn port_strings_do_not_collide_with_extended_resource_tables() {
        let mut strings = vec![String::new(); MT_STR_PRODUCTION_FORECAST_LINE + 1];
        ...
        assert_eq!(
            text.get(MT_STR_PRODUCTION_FORECAST_RATE),
            "Localized production-rate text"
        );
