...
pub(crate) fn listbox_scrollbar_thumb(
    ...
    let height = ((usable as u128 * visible.min(total) as u128 / total as u128) as usize)
    ...
    let top = if max_offset == 0 {
        ...
        ((travel as u128 * offset.min(max_offset) as u128 + max_offset as u128 / 2)
...
// `(msg_type → noisy-event)`. After input processing, the helper looks
...
/// Widget-noisy-event values. The original game lists activation
