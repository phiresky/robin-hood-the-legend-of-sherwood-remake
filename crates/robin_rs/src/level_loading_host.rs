...
fn finish_background_picture(
    ...
    for path in &depth_candidates {
        if !files
            .try_exists(path)
            .map_err(|error| format!("failed to probe occlusion depth '{path}': {error}"))?
        {
            continue;
        }
        let bytes = files
...
mod tests {
    ...
    #[test]
    #[cfg(not(target_arch = "wasm32"))]
    fn concurrent_terrain_jobs_retain_independent_preparation_readers() {
        let roots = [tempfile::tempdir().unwrap(), tempfile::tempdir().unwrap()];
        let readers: Vec<_> = roots
            .iter()
            .enumerate()
            .map(|(index, root)| {
                let directory = root.path().join("Levels/Day");
                std::fs::create_dir_all(&directory).unwrap();
                let mut bytes = Vec::new();
                {
                    let mut encoder = png::Encoder::new(&mut bytes, (index + 1) as u32, 1);
                    encoder.set_color(png::ColorType::Rgb);
                    encoder.set_depth(png::BitDepth::Eight);
                    let mut writer = encoder.write_header().unwrap();
                    let color = if index == 0 { [255, 0, 0] } else { [0, 255, 0] };
                    writer.write_image_data(&color.repeat(index + 1)).unwrap();
                }
            ...
            .collect();
        let jobs: Vec<_> = readers
            .iter()
            .enumerate()
            .map(|(index, files)| {
                assert_eq!(
                    probe_background_map_dims_with_files("shared", "Day", "Levels", None, files),
                    Some(((index + 1) as u16, 1))
                );
                PendingTerrainDecode::start_with_files(
                    "shared",
                    "Day",
                    "Levels",
                    None,
        ...
        for (index, job) in jobs.into_iter().enumerate() {
            let decoded = job.join_blocking();
            let background = decoded.background.unwrap().unwrap();
            let minimap = decoded.minimap.unwrap();
            let expected = vec![if index == 0 { 0xf800 } else { 0x07e0 }; index + 1];
            assert_eq!(
