//! Native attempt links: one JSON link per completed attempt, written beside
//! local recordings and backfilled by a cancellable background scan.
...
/// Native runtime ownership held by [`RecordingIndex`]; never serialized.
...
impl RecordingIndex {
    ...
    pub(crate) fn refresh_index(&self) -> Result<(), String> {
        ...
        let cancelled = self.runtime.cancelled.clone();
        scan.worker = Some(
            std::thread::Builder::new()
                .name("recording-index".into())
                .spawn(move || backfill(&directory, &cancelled).map_err(|error| error.to_string()))
    ...
    pub(crate) fn take_completion(&self) -> Option<Result<(), String>> {
        let mut scan = robin_util::sync::lock(&self.runtime.scan);
        if scan
            .worker
            .as_ref()
    ...
    pub(crate) fn shutdown(&self) -> Result<(), String> {
        ...
        let (mut scan, poisoned) = match self.runtime.scan.lock() {
            Ok(scan) => (scan, false),
            Err(error) => {
    ...
    pub(crate) fn recording_started(&self, path: &std::path::Path) {
        ...
        match path.canonicalize() {
            Ok(path) => *robin_util::sync::lock(&self.runtime.active_path) = Some(path),
            Err(error) => {
                *robin_util::sync::lock(&self.runtime.active_path) = None;
    ...
    pub(crate) fn recording_finished(&self, key: MissionAttemptKey, completed_at: Option<i64>) {
        ...
        let Some(path) = robin_util::sync::lock(&self.runtime.active_path).take() else {
...
mod tests {
    ...
    fn refresh_retains_running_and_completed_workers_until_observed() {
        ...
        robin_util::sync::lock(&index.runtime.scan).worker = Some(worker);
        index.refresh_index().unwrap();
        assert_eq!(
            robin_util::sync::lock(&index.runtime.scan)
                .worker
                .as_ref()
                .unwrap()
                .thread()
        ...
        release.send(()).unwrap();
        let deadline = std::time::Instant::now() + std::time::Duration::from_secs(5);
        while !robin_util::sync::lock(&index.runtime.scan)
            .worker
            .as_ref()
            .unwrap()
            .is_finished()
    ...
    fn worker_panic_is_observable_and_does_not_wedge_retry() {
        let dir = tempfile::tempdir().unwrap();
        let index = RecordingIndex::native(dir.path().join("attempts"));
        robin_util::sync::lock(&index.runtime.scan).worker =
    ...
    fn shutdown_and_drop_join_workers_and_prevent_new_scans() {
        ...
        let cancelled = index.runtime.cancelled.clone();
        robin_util::sync::lock(&index.runtime.scan).worker = Some(std::thread::spawn(move || {
        ...
        assert!(robin_util::sync::lock(&index.runtime.scan).worker.is_none());
