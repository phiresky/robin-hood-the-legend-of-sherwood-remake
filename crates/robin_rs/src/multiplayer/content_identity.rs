...
pub fn active_content_identity(
    ...
    if let Some(manifest_path) =
        files.resolve_data_path(&format!("Data/{WEB_CONTENT_MANIFEST_NAME}"))
    {
...
mod tests {
    ...
    use super::{
        ...
        WebContentEdition, WebContentFile, WebContentFileKind, WebContentManifest, hex_digest,
        source_content_identity, verify_web_content_package,
    };
    use sha2::{Digest as _, Sha256};
    ...
    fn package_manifest_verifies_every_declared_byte_and_no_extras() {
        ...
        let manifest = WebContentManifest {
            ...
            edition: WebContentEdition::Full,
        ...
        let mut demo = manifest;
        demo.edition = WebContentEdition::Demo;
        std::fs::write(
            &manifest_path,
            serde_json::to_vec(&demo).expect("Demo manifest JSON"),
        )
