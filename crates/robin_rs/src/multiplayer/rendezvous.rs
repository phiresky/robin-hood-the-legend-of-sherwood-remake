...
impl TopicRendezvous {
    ...
    fn unix_seconds_at(now: std::time::SystemTime) -> Result<u64> {
        Ok(now
    ...
    fn unix_seconds() -> Result<u64> {
        Self::unix_seconds_at(std::time::SystemTime::now())
    }

    fn unix_minute() -> Result<u64> {
        Ok(Self::unix_seconds()? / 60)
    }
    ...
    pub async fn bootstrap_ids(&self) -> Result<Vec<[u8; 32]>> {
    ...
    pub async fn announce(&self) -> Result<()> {
        ...
        let seq = i64::try_from(Self::unix_seconds()?)
            .context("Unix timestamp exceeds the BEP-44 sequence range")?;
        let key = self.slot_signing_key(minute);
        let salt = self.slot_salt(minute);
...
mod tests {
    ...
    #[test]
    fn pre_epoch_clock_is_an_error_instead_of_slot_zero() {
        let before_epoch = std::time::UNIX_EPOCH - std::time::Duration::from_secs(1);
        let error = TopicRendezvous::unix_seconds_at(before_epoch).unwrap_err();
