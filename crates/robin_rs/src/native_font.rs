...
pub fn load_font_by_name(
    ...
    if let Some(filename) = entry.native.as_deref() {
        match NativeFont::load(&format!("{FONT_PATH}{filename}"), files) {
    ...
    if let Some(filename) = entry.truetype.as_deref() {
        ...
        let resolved = PathBuf::from(&rel);
        let tt = TrueTypeFont::load(&resolved, files);
        if tt.is_valid() {
...
pub fn load_font_by_name_for_locale(
    ...
    load_font_by_name(config, name, files)
}
...
mod tests {
    ...
    fn font_config_and_native_font_reads_use_independent_preparations() {
        let make = |font_name: &str, tag: &[u8]| {
            let vfs = std::sync::Arc::new(robin_util::asset_fs::AssetVfs::new());
            vfs.install_preloaded_asset(
                "Data/Interface/Fonts/manager.cfg",
                format!("Default: {font_name}.sbf,").into_bytes(),
            )
            .unwrap();
            vfs.install_preloaded_asset("Data/Interface/Fonts/shared.sbf", tag.to_vec())
                .unwrap();
            SbFileSystem::new(vfs)
        };
        let first = make("first", b"FIRST!");
        let second = make("second", b"SECOND");
        for (files, name, tag) in [
            (&first, "first.sbf", "FIRST!"),
            (&second, "second.sbf", "SECOND"),
            (&first, "first.sbf", "FIRST!"),
        ] {
            assert_eq!(
                load_font_config(files).unwrap()["Default"]
                    .native
                    .as_deref(),
                Some(name)
            );
            let error = NativeFont::load("Data/Interface/Fonts/shared.sbf", files)
                .err()
                .expect("invalid fixture tag must fail");
    ...
    /// (file lookup resolves against the game datadir), so this test relies
    ...
    fn load_font_by_name_attempts_truetype_when_native_absent() {
        ...
        let err = match load_font_by_name(
            &cfg,
            "ListDefault",
