...
struct ProfilerEntry {
    name: String,
    /// Timestamp captured at profiling/frame start (milliseconds).
    time: u32,
    /// Accumulated time or externally-set value for the current frame.
    current_amount: u32,
    /// Tracks nested start/end pairs so only the outermost measures.
    recursion_depth: u16,
...
impl Profiler {
    ...
    pub fn log_end(&mut self, key: ProfilerKey) -> bool {
        ...
        if k < self.entries.len() {
            let entry = &mut self.entries[k];
            // Pre-decrement on `u16`: an unbalanced profiling end underflows to
