...
//!
//! `IntroScreen`, `OptionsScreen`, `GraphicsScreen`, `SoundsScreen` and
//! `LoadSaveScreen` are retained public reference models for the original
//! screens, not the production menu controllers. Their tests document that
//! reference behavior. Production options use `options_model` together with
//! `ingame_menu` and the cooperative mission UI; add product regression tests
//! to those controllers. Other types here still have production consumers.
//!
//! Screens covered:
...
pub enum GraphicsOption {
    ...
    QuickActionCursorPulse = 7,
}

/// Number of graphics option toggles.
pub const GRAPHICS_OPTION_COUNT: usize = 8;
...
impl GraphicsScreen {
    ...
    pub fn on_toggle(&mut self, option: GraphicsOption) {
        ...
        match option {
            ...
            GraphicsOption::QuickActionCursorPulse => {
                self.config.quick_action_cursor_pulse = self.option_toggles[idx];
            }
        }
    }
