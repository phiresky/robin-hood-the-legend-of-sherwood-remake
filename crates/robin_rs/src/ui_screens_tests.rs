...
fn graphics_screen_toggle() {
    ...
    assert!(screen.config.quick_action_cursor_pulse);
    screen.on_toggle(GraphicsOption::QuickActionCursorPulse);
    assert!(!screen.config.quick_action_cursor_pulse);
}

#[test]
fn graphics_cursor_pulse_ok_persists_and_cancel_restores() {
    let config = GraphicConfig::default();

    let mut accepted = GraphicsScreen::new(config.clone());
    accepted.on_toggle(GraphicsOption::QuickActionCursorPulse);
    accepted.on_ok();
    assert!(accepted.accepted);
    assert!(!accepted.config.quick_action_cursor_pulse);

    let mut cancelled = GraphicsScreen::new(config);
    cancelled.on_toggle(GraphicsOption::QuickActionCursorPulse);
    cancelled.on_cancel();
    assert!(!cancelled.accepted);
    assert!(cancelled.config.quick_action_cursor_pulse);
}
