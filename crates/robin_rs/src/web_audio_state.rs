...
impl PlaybackKind {
    pub(crate) fn for_category(category: crate::sound::PlaybackCategory, looping: bool) -> Self {
        use crate::sound::PlaybackCategory;
        match category {
            PlaybackCategory::Voice => Self::Voice,
            PlaybackCategory::Effect | PlaybackCategory::Ambience if looping => Self::Loop,
            PlaybackCategory::Effect | PlaybackCategory::Ambience => Self::Effect,
        }
    }

    /// Compatibility for legacy direct backend calls. SoundManager passes an
    /// explicit category so relative actor filenames never determine expiry.
...
impl PendingPlayback {
    pub(crate) fn new(
        ...
        Self {
            ...
            path,
            looping,
            fraction: crate::sound::playback_fraction(fraction),
...
mod tests {
    ...
    #[test]
    fn audio_relative_voice_asset_keeps_semantic_lifetime() {
        let kind = PlaybackKind::for_category(crate::sound::PlaybackCategory::Voice, false);
        let pending = PendingPlayback::new(
            1,
            1,
            kind,
            "Expressions/x.wav".into(),
            false,
            0.0,
            0.0,
            1.0,
        ...
        assert_eq!(
            PlaybackKind::for_category(crate::sound::PlaybackCategory::Effect, false),
            PlaybackKind::Effect
        );
        assert_eq!(
            PlaybackKind::for_category(crate::sound::PlaybackCategory::Ambience, false),
            PlaybackKind::Effect
        );
        assert_eq!(
            PlaybackKind::for_category(crate::sound::PlaybackCategory::Ambience, true),
            PlaybackKind::Loop
        );
    }
    ...
    #[test]
    fn independent_session_generations_do_not_cancel_each_other() {
        let mut first = PlaybackGeneration::default();
        let second = PlaybackGeneration::default();
        ...
        first.advance();
        assert_eq!(
            completion_decision(&pending, 1, second.current(), 0, true),
            CompletionDecision::Start
        );
        assert_eq!(
            completion_decision(&pending, 1, first.current(), 0, true),
            CompletionDecision::IgnoreStale
        );
