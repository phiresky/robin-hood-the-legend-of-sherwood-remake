...
fn retention_prunes_commands_to_the_oldest_replayable_checkpoint() {
    ...
    for frame in 10..=12 {
        ...
        assert!(history.commit_frame_input(commands(&format!("frame-{frame}"))));
    }
...
fn truncating_at_the_command_horizon_preserves_the_branch_checkpoint() {
    ...
    for frame in 10..=12 {
        engine.test_set_frame_counter(frame);
        history.begin_frame(frame, &engine);
        assert!(history.commit_frame_input(commands(&format!("old-{frame}"))));
    }
    ...
    assert!(history.commit_frame_input(commands("new-11")));
    assert_eq!(history.oldest_command_frame(), Some(11));
...
fn truncating_before_the_retained_horizon_is_a_no_op() {
    ...
    for frame in 20..=22 {
        engine.test_set_frame_counter(frame);
        history.begin_frame(frame, &engine);
        assert!(history.commit_frame_input(SimulationFrameInput::default()));
    }
...
fn periodic_history_starts_journaling_only_when_a_checkpoint_can_anchor_replay() {
    ...
    for frame in 3..5 {
        engine.test_set_frame_counter(frame);
        history.begin_frame(frame, &engine);
        assert!(!history.commit_frame_input(commands(&format!("unanchored-{frame}"))));
    }
    ...
    assert!(history.commit_frame_input(commands("anchored-5")));
...
fn an_empty_truncated_journal_keeps_its_branch_frame_until_clear() {
    ...
    journal.record_frame(40, SimulationFrameInput::default());
    journal.record_frame(41, SimulationFrameInput::default());
    ...
    journal.record_frame(100, SimulationFrameInput::default());
