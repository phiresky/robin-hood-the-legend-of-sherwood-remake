//! Duplicate-aware JSON admission shared by signed-document consumers.
//!
...
mod tests {
    ...
    fn rejects_trailing_values_and_unknown_members() {
        assert!(matches!(
            from_slice::<Fixture>(br#"{"outer":[]} {}"#),
            Err(StrictJsonError::Syntax(_))
        ));
        assert!(matches!(
            from_slice::<Fixture>(br#"{"outer":[],"future":true}"#),
            Err(StrictJsonError::Document(_))
        ));
    }

    #[test]
    fn preserves_numbers_null_and_array_values() {
        let bytes =
            br#"[null,true,false,-9223372036854775808,18446744073709551615,1.25,"text",{}]"#;
        assert_eq!(
            from_slice::<serde_json::Value>(bytes).unwrap(),
            serde_json::from_slice::<serde_json::Value>(bytes).unwrap()
        );
        assert!(matches!(
            from_slice::<Fixture>(br#"{"outer":[{"value":4294967296}]}"#),
            Err(StrictJsonError::Document(_))
        ));
        assert!(matches!(
            from_slice::<serde_json::Value>(b"1e9999"),
            Err(StrictJsonError::Syntax(_))
        ));
    }

    #[test]
    fn decoded_key_identity_rejects_escaped_duplicates_but_not_siblings() {
        assert!(matches!(
            from_slice::<serde_json::Value>(br#"{"a":1,"\u0061":2}"#),
            Err(StrictJsonError::DuplicateKey(key)) if key == "a"
        ));
        assert!(from_slice::<Fixture>(br#"{"outer":[{"value":1},{"value":2}]}"#).is_ok());
    }
}
