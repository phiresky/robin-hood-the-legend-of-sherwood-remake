...
//! persisted replay hashes and is a hash-schema compatibility change. Moving
//! code or changing documentation must preserve field and variant order.
//!
...
//!
//! Authoritative snapshot fields participate by default. Derived attachments
//! and transient presentation fields use Serde skips; serialized but unhashed
//! fields require an explicit `state_hash(skip)` and a documented reason at
//! the field. Changing those classifications changes the hash byte stream,
//! including skipped-field markers, and requires a replay schema decision.
//! Save layout, replay layout/hash schema, and executable build identity are
//! separate contracts; changing one does not automatically version the others.
