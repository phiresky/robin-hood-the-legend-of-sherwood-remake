...
#[test]
fn persisted_derive_schema_matches_independent_byte_fixtures() {
    // Literal little-endian contract bytes, independent of derive expansion.
    // The tuple variant index precedes fields, including SKIPFLD\0 in place
    // of the skipped value. Field and variant moves must preserve this.
    let bytes = [
        0, 0, 0, 0, 0, 0, 0, 0, // variant zero, u64
        1, 0, 0, 0, // a: u32
        b'S', b'K', b'I', b'P', b'F', b'L', b'D', 0, 3, 0, 0, 0, // c: u32
    ];
    assert_eq!(
        compute(&WithSkippedEnumFields::Tuple(1, 999, 3)),
        compute(&bytes)
    );
