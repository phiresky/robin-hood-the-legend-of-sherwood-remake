...
The shipped behavior was checked against the original game, not inferred from the
Rust port:

- The original game uses the shared `1.153` projectile range factor,
  base ranges of 300 for apple/purse/net/wasp and only 200 for stone, and the
  projectile masses/apex factors.
- Stone behavior applies the direct-hit path and falls back to the apple AI
  event for protected targets. The live direct-hit implementation uses damage
  10 and concussion 100; the unrelated stone-damage value of 30 is
  not the executed item path.
- Net behavior uses a strict squared-radius test `< 1600` (radius 40),
...
- Enemy AI accepts ale outdoors when the soldier profile's beer value is
  positive. Soldier behavior adds
  that authored value to blood alcohol on completion, clamped to 100.
- Wasp behavior uses 50 for initial victim acquisition, with distinct
  chase/charge/sting/forget distances and apple-scent preference. The accepted
  change touches acquisition only.
- Purse behavior emits five recoverable coins. That strong, flexible item
