...
| In progress | Soldier 104 Point/Wait completion state | At frame 199 Original exposes action state `Bored` after animation 283 terminates under `Wait`, while Rust exposes `Waiting` with displayed action `Pointing`. | Trace the Point sequence's completion and the replacement idle Wait ordering to determine which base-actor state side effect Rust applies too early or omits. |
