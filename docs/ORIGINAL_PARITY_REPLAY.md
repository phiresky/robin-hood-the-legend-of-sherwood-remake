...
| Recording required | Refused anonymous-archer bow click | The dominant member of the schema-12 `sound manager resolved exclamation 14 ... with no pending request` group. The random-input driver forwards `MSG_SELECT_ACTION(BOW)` (recorded) and then a raw left click at a visible enemy (not recorded). `ManageInputActionBow` sees an `AnonymousArcher` shooter, barks `HERO_UNABLE_TO_DO_SOMETHING`, and returns before `RecordInteraction`, so the refusal leaves no command in the trace at all — the same shape as an aim-only frame with no recorded commands. Rust already implements the refusal on the live input path, but the replay has no click to apply. | The recorder must emit the refused bow interaction (or the raw click) before the early return; existing traces cannot be repaired and need a fresh capture. |
