...
| In progress | Soldier 95 movement goal | At frame 169 Original soldier 95 has direction goal 11 and position `(2275.445, 1639.0577)`, while Rust has direction goal 6 and position `(2277.0676, 1639.3135)`. | Trace the preceding patrol/formation movement instruction and cached increment to find the first internal state difference before the visible step. |
