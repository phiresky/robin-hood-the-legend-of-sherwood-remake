...
The current verified clean prefix is through frame 46. At frame 47 soldier 113
has command `Turn` and AI substate 10 in Original while Rust has `Wait` and
substate 15. Trace the missing turn/AI-state boundary from the now-matching
frame-46 state. Increase the clean-prefix statement only after a normal
...
| In progress | Frame-50 RNG frontier | Original consumes global RNG draw 124 during frame 50 while Rust consumes none. | Identify the owning actor/macro operation from the structured frame dump and Original RNG callsite stream, then restore the missing deterministic draw at the same logical boundary. |
