...
| In progress | Soldier 104 wait-entry state | At frame 156 Original soldier 104 starts `TransitionWaitingUprightWaitingUprightBored` with action state `Bored`, while Rust exposes `Waiting` and still reports the prior Turning sprite action. | Trace the Wait sequence's initial-order promotion and base actor start/completion side effects around the scripted officer conversation. |
