...
| In progress | Boundary-vertex `LINE_CROSS` dispatch | A Windows Sherwood save retained correctly decoded script-zone occupants, but later zone-dependent RNG counts diverged after actors moved between production zones. The decoder and initial identity resolution matched Linux and Original. The dynamic crossing audit found that `RHElementActor::CheckForLineCrossing` calls `GetLines`, which keeps every segment intersection and only later removes a boundary when the actor's old position lies on its line. Rust queried with `RHLine::IsCrossed` semantics instead, whose additional determinant rejects movement passing exactly through a boundary endpoint. Rust also collapsed two patch edges at a shared vertex into one callback, unlike Original's ordered per-line dispatch. | Script, patch, and sound candidate queries now retain endpoint intersections. Their consumers apply only the source-defined old-position guard and stable distance ordering; patches execute once per crossed edge rather than once per owning polygon. Focused fast-grid coverage uses generic geometry that passes through a boundary vertex, with no save, actor, zone, or frame identity embedded in gameplay code. |
