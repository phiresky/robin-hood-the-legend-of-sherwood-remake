...
| In progress | Soldier 82 shadow-reaction movement | At frame 123 Original soldier 82 is `MoveWaiting` facing sector 5, while Rust is already `MoveOk` facing sector 8. | Trace the shadow-reaction `Face`/movement/path barrier ordering from the prior predetection transition and match the first state-changing Original boundary. |
