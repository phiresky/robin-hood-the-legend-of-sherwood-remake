...
| --- | --- | --- | --- |
| Done | Ability entry drops the move queued behind it | Entering listen or the helping-to-climb stance re-forwards `MSG_SELECT_ACTION` for the action that is already current once the entry transition reaches DONE. The engine handler runs the full action-selection path, whose group `Stop` at Normal priority walks the actor's postponed chain, so a move the player instructed while the ability was running is interrupted and never resumes. Rust kept the move postponed and resumed it when the ability terminated, starting a walk (or a leave-stance transition) where the Original stayed idle — the `state:actor.command:Wait->MoveOk` group. Confirmed by rebuilding the Original with `Stop`/`Postpone`/`SetState` tracing and replaying the recorded input seed: the postponed move is interrupted from `SelectAction` inside the PC's own `Hourglass`. | The listen entry handoff and a new helping-to-climb entry handoff both run the ported `RHEngine::SelectAction` (`set_pc_action_from_message`), matching the beggar entry that already did. |
| Done | Contest archer may not be moved | The archery-contest PC (posture `AnonymousArcher`) refuses every Move/Seek instruction: the element becomes `Impossible` and the hero barks `HERO_UNABLE_TO_DO_SOMETHING` instead of walking. Rust dispatched the move normally and never queued the bark, so the recorded speech resolution arrived with an empty pending FIFO. Random-input replays of the contest savegames hit this on every group move and on the seek of every target interaction. | Move/Seek dispatch now refuses the move for an `AnonymousArcher` PC, plays the refusal bark, and marks the element `Impossible`. Three pass-group controls still match every recorded frame. |
| Recapture required | Refused anonymous-archer bow click and refused locked-patch click | The whole schema-12 `sound manager resolved exclamation 14 ... with no pending request` group, in two shapes. The random-input driver forwards `MSG_SELECT_ACTION` (recorded) and then a raw left click (not recorded). `ManageInputActionBow` sees an `AnonymousArcher` shooter, and the map-click dispatch sees a locked selected patch; both bark `HERO_UNABLE_TO_DO_SOMETHING` and return before any recording call, so the refusal leaves no command in the trace at all — the same shape as an aim-only frame with no recorded commands. Rust implements both refusals on the live input path, but the replay has no click to apply. Rebuilding the recorder with a marker at each of the thirteen `HERO_UNABLE_TO_DO_SOMETHING` emitters and rerunning the group's input seeds confirms these two sites, and only these two, account for every member. | Schema 14 records both as `hero_refused_action` and the replay applies them. Existing schema-12 captures cannot be repaired in place and need a fresh capture. Splicing the schema-14 refusal commands back into nine schema-12 members at their recorded frames takes every one of them from a frame-390-to-486 panic to matching every recorded frame; the Leicester member clears its frame-26,616 panic and advances to an unrelated frame-26,688 RNG mismatch on `DrunkCombatFreeze`. Note that a schema-14 recapture of these savegames currently fails at the `ai.macro_cursor` divergence around frame 79 instead. |
...
| Done | Ordered non-elevation `LINE_CROSS` dispatch | A Windows Sherwood save retained correctly decoded script-zone occupants, but later zone-dependent RNG counts diverged after actors moved between production zones. The decoder and initial identity resolution matched Linux and Original. The dynamic crossing audit found that `RHElementActor::CheckForLineCrossing` calls `GetLines`, keeps every segment intersection, removes only a boundary containing the actor's old position, then stable-sorts one combined patch/script/sound list by distance. Rust used the stricter `RHLine::IsCrossed` predicate, collapsed two patch edges at a shared vertex, and dispatched all patches before all scripts before all sounds. | Actor crossing now retains endpoint intersections and builds one candidate stream in Original block/line discovery order. After the source-defined old-position guard and stable distance sort, every line executes its patch, script, and sound flags in that order; duplicate edges remain observable. Focused generic geometry covers all three kinds passing through one boundary vertex and verifies equal-distance discovery order without embedding save, actor, zone, or frame identities in gameplay code. |
...
| Done | Capture reproducibility | Two captures of one save at one seed did not agree. Audio drew from the same libc generator as the simulation, and which audio decisions a frame makes follows mixer and playback progress rather than gameplay, so those draws shifted every later simulation value. Opening the sound device also consumed draws inside the audio backend before recording started, which made the recorded stream depend on the host's audio driver: the same fixture recorded with and without a real device produced different traces. | Audio decisions (sound selection, speech variant, music loop, delayed-source timers) now come from a separate generator seeded from the parity seed and restarted with each session, and they are no longer written to the trace, so the recorded stream is what an audio-free run would produce. The recorded stream also reseeds the simulation generator at the boundary that defines index zero, making a capture depend only on the parity seed. Schema stays at 14: the `domains` field is unchanged and simply never carries `audio` in new captures, which the replay runner already filtered out. A 600-frame fixture now records byte-identical traces across repeated runs and across dummy and real audio devices. |
| Done | Stable macro cursor | NPC AI state serialized the macro cursor as `mpubMacroCommand - pWaypoint->pData`. That subtraction is only a position while the cursor still lies in the waypoint block it was authored against; advancing the patrol path or breaking a macro leaves the cursor on the previous waypoint's block, so the difference spanned two unrelated allocations and reported an address-layout artefact that changed with ASLR. | The Original reports the offset only while the cursor genuinely lies within the current waypoint's block, so the field is a stable index or absent. The replay runner withholds it on the same cases. Because the Rust cursor walks an owned copy of the block rather than the block itself, the runner records which waypoint that copy was taken from and reports the offset only while the path still stands on that waypoint. Comparing the copy's bytes against the current waypoint's data instead was not sufficient: authored macros are short and repeat, so a cursor left behind on an earlier waypoint could still byte-match the current one and be reported where the Original withholds it. |
...
| Done | Frame-355 retained lateral sweep | Original stores a lateral/circle strike's victim list and angles on the human rather than the interrupted sequence. PC 198's completed D action point is interrupted, but the shared victim FIFO survives; replacement strike E's `START` warning forecast rebases the shared initial/current/final angles to E, and its first `IN_PROGRESS` advances that retained geometry using E's direction, rotation, profile, and damage, hitting Soldier 82. | Generic actor-stop cleanup preserves human-owned sweep state. A replacement lateral strike with retained victims now mirrors the warning forecast's angle writes before its first in-progress tick, while fresh strikes still initialize their real victim list at `DONE`. Focused coverage interrupts D, verifies E's start rebases all three angles without replacing the FIFO, and proves E's first in-progress arc hits only the victim selected by E's semantics. |
...
rules. The completed straight hit uses that same collector and therefore also
bypasses the common predicate before registering `ReceiveSwordDamage`.
Separately, a PC's `WarnForStrike` guard uses the original definition of
