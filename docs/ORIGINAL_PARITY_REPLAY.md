...
| In progress | PC idle-transition action state | At frame 114 Original PC 198 still reports action state `Bored` while command 161 displays animation 3; Rust reports `Waiting` for its `TransitionWalkingUprightWaitingUpright` order. | Identify the Original command/animation arm and the precise base-vs-PC action-state side-effect ordering, then implement the general actor-family rule and add focused coverage. |
