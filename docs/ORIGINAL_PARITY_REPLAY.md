...
| Done | Stable macro cursor | NPC AI state serialized the macro cursor as `mpubMacroCommand - pWaypoint->pData`. That subtraction is only a position while the cursor still lies in the waypoint block it was authored against; advancing the patrol path or breaking a macro leaves the cursor on the previous waypoint's block, so the difference spanned two unrelated allocations and reported an address-layout artefact that changed with ASLR. | The Original reports the offset only while the cursor genuinely lies within the current waypoint's block, so the field is a stable index or absent. The replay runner withholds it on the same cases by requiring the retained opcode stream to be the current waypoint's macro data and the cursor to lie within it, rather than always reporting an offset into its own copy. |
