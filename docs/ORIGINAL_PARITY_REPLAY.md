...
| In progress | Frame-119 idle RNG burst | During original frame 119 Rust consumes thirteen extra `BoredAnimationChoice` draws and two `VipIdleRemark` draws where Original consumes only two draws total. | Identify the first actors and order lifecycle difference responsible for the synchronized idle-loop burst, then correct the general order-ID/progression or periodic-owner rule. |
