...
| --- | --- | --- | --- |
| PA-013 | High | incomplete | `09ec7c5fe` restores a live-size creation-ordered pass for bow release/projectiles, PC auto-heal, purses/coins, wasps, nets, fallback-timed melee completion, and abilities. `4c9b5715a` restores the mixed pre/post target position observed by EYES_FOLLOW. Subsequent slices restore creation-ordered melee, SEEK target sampling, detection, base Actor Execute/ActionChange, owner-local AI callbacks, stationary WAIT/Lift/WaitingSword work, and the static FX/Target/Scroll/proven Bonus-class virtual Hourglasses. Ordinary movement, `RunningUpright` rider GALOPP, and selected `RiderCharging` execute in the fused live Actor → Human → PC/NPC owner envelope. Static FX samples patch state live and synchronously closes `ApplyFinal`; Scroll calls its due VM before resetting and animating; concrete `RHElementBonus` storage animates before owner-local `RefreshDiscovered`. The exhaustive Rust `Entity::Bonus` mapping distinguishes Original Bonus, dropped `RHElementAle`, and `RHElementCape`, and rejects unevidenced object types. FrozenAll stops sprite frames without suppressing Scroll script work or Bonus discovery. `tick_zone_occupants` has no cited actor owner and remains an explicit separate boundary after the owner walk. Remaining debt is active strike/bow/ability ownership (including PC Listen/object reveal and Target Heard), unsupported rider/Execute arms, mobile master/child scheduling, projectile/net owners, and other unevidenced entity-kind Hourglasses (`RHengine.cpp:3715-3724,7909-7944`; `RHelementfx.cpp:794-833`; `RHelementfxmasked.cpp:128-149`; `RHelementtarget.cpp:858-865`; `RHElementBonus.cpp:513-519`; `RHElementScroll.cpp:119-149`; `RHElementAle.cpp:65-68`; `RHElementCape.cpp:45-49`). |
