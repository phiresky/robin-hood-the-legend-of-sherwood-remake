...
| NPC view, detection, timers, speech, patrol | `RHElementActorNPC::Hourglass` and AI subclasses | phase order verified by PA-016; followed-target and FRIEND-versus-synchronous-EVENT_VIEW creation boundaries fixed; synchronous pre-optical EVENT_HEAR, per-NPC EVENT_OUTOFVIEW, Royalist coordination, per-stimulus live contexts, and the broader NPC tail remain batched |
