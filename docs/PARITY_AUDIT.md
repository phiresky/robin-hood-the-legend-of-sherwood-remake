...
| --- | --- | --- | --- |
| PA-013 | High | incomplete | `09ec7c5fe` restores a live-size creation-ordered pass for bow release/projectiles, PC auto-heal, purses/coins, wasps, nets, fallback-timed melee completion, and abilities. Tests prove an earlier projectile precedes a later shooter, a newly appended arrow receives its second same-frame Hourglass, and appended coins/wasps run that frame. Movement, animation, NPC Hourglass work, and general melee progression remain globally batched, so the broader difference from `RHEngine::PerformHourglass`'s complete per-element creation-ordered virtual calls (`RHengine.cpp:3715-3724,7909-7944`) remains open pending a concrete ordering oracle. |
...
| Deterministic snapshots and replay | in progress, high risk | PA-033/PA-035 fixed; retain active-zoom rewind, fatal replay preload, and seed-priority regressions. |
