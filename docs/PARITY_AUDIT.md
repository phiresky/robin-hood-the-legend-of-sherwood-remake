...
| --- | --- | --- | --- |
| PA-013 | High | incomplete | The merged ordered mini-pass covers PC auto-heal versus existing projectiles, but movement, animation, NPC work, bow release, purses/coins, wasps, nets, melee, and abilities remain globally batched. Original `RHEngine::PerformHourglass` calls each complete virtual `Element::Hourglass` in creation order and rechecks the live array size (`RHengine.cpp`). A concrete remaining mismatch is that Rust bow release runs before every existing projectile regardless of creation order. |
| PA-025 | High | mismatch | Charly now uses a view cone, but transitions to `SeekingCharlyGoToOfficerSeen` before the officer's queued `Think` accepts the report. Original branches on the synchronous return and calls `ReturnToDuty` on refusal (`RHartificialmalignity.cpp`). The helper also lacks exact obstacle/light radius inputs and normal same-building/door gates. |
...
| PA-038 | High | mismatch | AI snapshots infer archer identity from positive normal-shot range instead of bow presence, and phalanx reconstruction substitutes the leftmost member's radius plus stale neighbor entries for each member's current 360-degree detection (`RHartificialmalignity.h`, `RHartificialmalignity.cpp`). |
| PA-039 | High | mismatch | Animated fatal pushes set `Posture::Dead` immediately and flight termination omits the original `ExecuteFallingPushed` start/end transitions. Original applies Flying/WaitingSword on start and DeadBack-or-Lying/WaitingSword on termination (`RHelementactorhuman.cpp`). |
