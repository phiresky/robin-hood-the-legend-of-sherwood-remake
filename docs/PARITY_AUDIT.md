...
| NPC view, detection, timers, speech, patrol | `RHElementActorNPC::Hourglass` and AI subclasses | phase order verified by PA-016; followed-target, FRIEND-versus-synchronous-EVENT_VIEW, synchronous pre-optical EVENT_HEAR, Lacklandist ordered multi-VIEW/OUTOFVIEW Enemy FIFO, per-stimulus live contexts and exact target inputs, OUTOFVIEW-before-later-detectable causality, and Royalist creation-order coordination with inline reveal and synchronous HeyFolks are fixed at the creation boundary; Royalist ordered multi-VIEW/OUTOFVIEW edges, Lacklandist scans while AI-locked, script-driven mid-FIFO detectable-list mutation, and the broader NPC tail remain batched or incomplete |
