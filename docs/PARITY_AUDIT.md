...
| NPC view, detection, timers, speech, patrol | `RHElementActorNPC::Hourglass` and AI subclasses | phase order verified by PA-016; followed-target, FRIEND-versus-synchronous-EVENT_VIEW, synchronous pre-optical EVENT_HEAR, ordered multi-VIEW/OUTOFVIEW Enemy FIFO, and OUTOFVIEW-before-later-detectable causality are fixed at the creation boundary; Royalist coordination, per-stimulus live contexts and target-specific primary inputs, and the broader NPC tail remain batched or incomplete |
