...
mixed civilian/Lacklandist-soldier optical walk and the split between projected
LOS points and Original world-horizontal distance points.
...
| NPC view, detection, timers, speech, patrol | `RHElementActorNPC::Hourglass` and AI subclasses | phase order verified by PA-016; followed-target, synchronous HEAR/VIEW/OUTOFVIEW ordering, both camps' Enemy FIFOs, per-stimulus live contexts, live per-Think Enemy-list reconstruction, creation-ordered NPC blip/reveal, lift approach data, lock semantics, and retained replay are fixed at the creation boundary. Mixed civilian/Lacklandist-soldier optical detection, cross-elevation distance coordinates, and the broader NPC tail remain incomplete. |
