...
post-callback retained animation. The coordinator still runs after the
batched animation phase instead of inside each actor's base Hourglass before
human/NPC tails; that exact placement remains PA-013 debt.
...
| Movement, animation, ActionChange, scroll Hourglass | actor/object virtual Hourglass and Execute methods | PA-013; EYES_FOLLOW and live SEEK-target mixed pre/post observations are fixed; ActionChange has live creation-order callback causality and post-callback retention, but its coordinator and animation remain batched outside the exact base-actor Hourglass position |
| NPC view, detection, timers, speech, patrol | `RHElementActorNPC::Hourglass` and AI subclasses | phase order verified by PA-016; followed-target, synchronous wake/recovery/blink/body-inform/view prelude, HEAR/VIEW/OUTOFVIEW ordering, the shared civilian/both-camp mixed Enemy walk, per-stimulus live contexts, live per-Think Enemy-list reconstruction, creation-ordered NPC blip/reveal, lift approach data, cross-elevation world-distance/projected-LOS coordinates, ambush/deafness, BUSY/ladder, wrapped phase, lock/timer/emoticon semantics, retained replay, and owner-local Enemy/Friendly `SetState` script callbacks are fixed at the creation boundary. Exact NPC animation/ActionChange and synchronous speech dispatch remain incomplete; the narrower `SetState` ordering debts listed above remain open. |
