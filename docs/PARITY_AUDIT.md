...
| --- | --- | --- | --- |
| PA-013 | High | incomplete | `09ec7c5fe` restores a live-size creation-ordered pass for bow release/projectiles, PC auto-heal, purses/coins, wasps, nets, fallback-timed melee completion, and abilities. `4c9b5715a` restores the mixed pre/post target position observed by EYES_FOLLOW. Subsequent slices restore creation-ordered melee, SEEK target sampling, detection, base Actor Execute/ActionChange, owner-local AI callbacks, and stationary WAIT/Lift/WaitingSword work. Ordinary movement, `RunningUpright` rider GALOPP, and selected `RiderCharging` now execute in the fused live Actor → Human → PC/NPC owner envelope. Each owner samples its current mutable order/target/geometry state at its legacy slot and closes anti-collision, crossings, door/seek/waypoint completion, synchronous ReachPoint/GALOPP Think, sword-movement checks, ActionChange, and derived tails before the next slot; only immutable RNG-free mobile geometry is prepared once. FrozenAll still freezes ordinary movement while executing RiderCharging initialization and polygon damage on the frozen sprite frame. Remaining debt is active strike/bow/ability ownership, non-NPC Listen/object discovery, unsupported Execute arms/entity kinds, and `tick_zone_occupants`: no cited Original actor-Execute owner was found for that Rust reconciliation pass, so it remains an explicit separate post-movement boundary (`RHengine.cpp:3715-3724,7909-7944`; `RHelementactor.cpp:534-728,1015-2740`; `RHelementactorhuman.cpp:277-324,3235-3560,9972-10097`; `RHelementactorpc.cpp:3616-3710,4950-4990,5481-5510`; `RHelementactorsoldier.cpp:1592-1608`). |
...
| Movement, animation, ActionChange, scroll Hourglass | actor/object virtual Hourglass and Execute methods | PA-013; EYES_FOLLOW and live SEEK-target mixed pre/post observations are fixed. The supported Soldier/Human/base Actor/PC-or-NPC envelope now shares each live legacy creation slot, and rider charge executes in its rider's movement slot. General movement, other rider actions, active combat, abilities, scrolls, and other entity kinds remain separate. |
