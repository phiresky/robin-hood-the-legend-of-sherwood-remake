...
- **NPCs:** The actor-owner envelope now nests the supported Soldier, Human,
  ...
  NPC tail (`original-code/RHelementactornpc.cpp:3495-3659`). `NpcOrders`
  remains a separate system boundary; PC Listen/object reveal and Target Heard
  are owner-local.
...
| PA-028 | Script `SendMessage` bypassed the original sequence element and dispatched after the script call. | `RHScript::SendMessage`, `RHScript.cpp`, launches `RHCOMMAND_SEND_MESSAGE`; `RHSequenceElementSendMessage::Go`, `RHsequenceelement.cpp`, performs delivery. | `618df2081` restores the real sequence launch and arbitration. PA-040 later closes `ProcessMessage`, termination and `Ready` successors synchronously at that registered action before the parent VM or older sibling continuation resumes. |
...
| PA-040 | Rust detached immediate and script callback work from the active sequence/VM continuation, allowing later registered siblings or the parent VM to run before target callbacks and callback-produced `Ready` successors. | `RHScript::SendMessage`, `RHElementActor::ExecuteImmediately`, `RHEngine::PerformExecuteCommand`, `RHSequenceElement::SetState`, and `RHSequence::Ready`/`NextSequenceElementsGo` invoke and close this work synchronously. | `cb0164e2b` and `375955d29` (merged by `9f41f202a`) use one ordered depth-first synchronous stream: callbacks run at the registered action, termination follows callback return, re-entrant WAIT/immediate successors precede older siblings, and the parent VM resumes last. |
| PA-041 | Door/lift Execute work collapsed `Done`/`Terminated` effects, suppressed required turns under FrozenAll, could run the second fast-climb motion after first-motion termination, snapped PassingDoor facing, and used noncanonical occupancy/reservation state. | PC UnlockingDoor/Trap, Actor ladder/wall Execute, PassingDoor, `RHGate`, and `RHSector` in the Original. | `a36401892` (merged by `d41c9a06a`) restores unlock flags at `Done` before `Terminated` advancement, per-tick `Turn` including FrozenAll, paired fast-climb motions with first-termination return, no PassingDoor facing snap, and canonical live building/lift occupancy and reservations with contextual invariants. |
| PA-042 | Specialized AI paths predicted cross-NPC `Think` results and continued callers before recipient filters/callbacks and live acceptance were known, so state, report and formation effects could use stale candidates or ordering. | Enemy/Friendly `SetState` invokes ordered state callbacks at the mutation boundary; `StartThink`/`EndThink` and Malignity alert/report/command-soldier handlers invoke recipient `Think` synchronously and branch on its live result. | `ee828e205` through `e9a854939` replace prediction with owner-local callback FIFOs and result-bearing continuations, re-resolve caller/recipient state after nested work, retain only accepted alert recipients, preserve inactive-but-conscious help eligibility, and close report/instruction/formation work before the next recipient or caller continuation. Focused refusal, mutation, FIFO, predicate and ordering tests cover the boundaries. |
...
| PA-043 | `Order::done` was read by the sequence-postpone race guard but never written, so an action reaching its done frame could be postponed instead of terminated. | `RHElementActor::Hourglass`, `RHelementactor.cpp`, sets `mpOrder->bDone` on `RHMOTION_DONE`; `RHSequenceElement::Postpone`, `RHsequenceelement.cpp`, terminates instead of postponing when the last order is done. | `444b04122` centralizes the writer through `Sprite::last_motion_state`: after all actor sprite owners have run, the engine marks the matching current order done and the existing postpone guard observes it. Focused tests cover both matching-order propagation and the termination race. |
...
| --- | --- | --- | --- |
| PA-044 | medium | mismatch | Shield danger-point layer plumbing collapses valid layer `0` into “unspecified”: `dispatch_raise_shield` stores `danger_layer.unwrap_or(0)`, and `sync_danger_point_titbits` falls back to the PC's layer whenever the stored value is `0`. The Original reads `RHFIELD_SHIELD_DANGER_POINT_LAYER` as a real layer for every nonzero danger point and uses `-1` only when the point itself is zero (`RHElementActorPC::Execute`, `RHelementactorpc.cpp`). Preserve presence separately, preferably as `Option<u16>`, and test explicit layer 0, an absent AI-issued layer, and a nonzero selected layer. |
...
2026-07-21 static-owner fusion: non-mobile FX, Target, Scroll, concrete
Original Bonus pickups, dropped Ale, and Cape now execute their virtual
Hourglass at the live legacy slot. The walk retains holes, rereads live length,
and admits callback-spawned later slots. Projectile/Net animation and mobile
child scheduling remain in their existing lanes. PC Listen/object reveal and
Target Heard remain explicitly outside this slice.
...
At that point it did not fuse the then-remaining movement/combat/NPC envelopes
or complete the broader PA-013 Actor Hourglass ownership work described above;
later owner-envelope and movement slices closed those specific boundaries.
...
action-arm remainder, and PA-042 closes the specialized AI-state callback and
result-continuation audit.
...
`ActiveAbility` executes, so stale or latent state neither runs nor suppresses
a generic selected order. The entry latch is retained for the whole owner slot,
deferring any successor exposed by terminal callbacks. One-shot effects,
including Carry's posture/link change, occur once at `Done`, while ownership
and the current order remain installed until `Terminated`; exact-owner aborts
clear ability/phase state and take the Original impossible path. Drop teardown,
climb teardown, and Strangle retaliation/damage retain their terminal
boundaries. FrozenAll
blocks sprite advancement (including throwable completion) but preserves
Listen countdown, Whistle wait, and selected beggar bidding; fried PCs skip the
actor envelope and `execution_frozen` suppresses active Execute and beggar work.

Listen and ReceivePurse use the Original's real three-order elements rather
than private phase IDs. Listen decrements from 25 on its first owner invocation
and fires on the 25th, then performs one captured-length, live-resolution scan
in creation order. Reveal and Heard callbacks occur synchronously in that
mixed order; callback-appended entities are excluded. Distance is strict 3D
`distance squared < 750 squared` with no layer gate. Heard requires LISTEN and
enabled scripts, retains LISTEN while scripts are disabled, clears it before
callback, and treats a missing required VM/script target as an invariant
failure. ReceivePurse reveals at `WaitingWithPurse` termination before
advancing. A LeaveListen issued during the non-interruptable EnterListen chain
is postponed; Enter naturally owns Listening and its exit, then the restamped
Leave becomes Impossible without rebinding ability identity. Beggar
`Turn -> PerformAction -> Start -> Bid -> InProgress` is
selected and advanced per owner, so earlier spawned coins are visible to later
slots. Every throwable receives its explicit primer before insertion and
exactly one additional advancement at its appended live creation slot in the
...
work closed it. PA-041 closes door/lift animation callbacks; other specialized
animation callback families remain separate audit topics.
...
prelude; its base-Human animation and `ActionChange` interleaving remains a
separate wake-sequence audit. PA-042 closes AI callback/result placement, not
this sequence-launch position. The eager posture/action/eye writes are gone.
...
pure-Rust observations outside the audited return/continuation boundaries
remain separate work. PA-042 closes specialized state callbacks and
result-bearing cross-NPC `Think` continuations, including live caller
re-resolution; the later exhaustive owner coordinator closed PA-013.
...
| Enemy detection and state machine | callback ordering verified, high risk | PA-025/PA-042 retain exact detection, synchronous live results, accepted-only alert formation, report ordering and inactive-but-conscious help eligibility; continue non-callback state semantics state by state. |
...
| Movement, paths, doors, lifts | callback ordering verified, high risk | Retain PA-014/PA-022/PA-030/PA-041 plus creation-ordered SEEK, motion-order cache, transition-prefix, unlock `Done`/`Terminated`, FrozenAll climb-turn, fast-climb early-termination, PassingDoor, occupancy and reservation regressions; continue non-callback movement/door parity. |
| Script natives and callbacks | callback ordering verified, high risk | Retain PA-021/PA-023/PA-024/PA-028/PA-040 exact-position, depth-first successor and parent-VM regressions; continue native-specific semantics. |
| Sequence manager and messages | in progress, high risk | PA-032/PA-040 fixed; retain WAIT launch-return, registered-order, depth-first `Ready`, condolence, SendMessage and parent-VM ordering tests. |
