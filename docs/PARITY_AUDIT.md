...
| --- | --- | --- | --- |
| PA-013 | High | incomplete | `09ec7c5fe` restores a live-size creation-ordered pass for bow release/projectiles, PC auto-heal, purses/coins, wasps, nets, fallback-timed melee completion, and abilities. `4c9b5715a` restores the mixed pre/post target position observed by EYES_FOLLOW. Subsequent slices restore creation-ordered straight and non-straight melee, SEEK target sampling, acoustic and optical detection, full detectable FIFO handling, live per-Think inputs, Royalist/civilian detection, base Actor Execute/ActionChange, owner-local AI state callbacks, stationary WAIT/Lift/WaitingSword work, and bonus-owned `RefreshDiscovered`. The fused owner envelope closes synchronous recursive work before the next slot; PC noise and mutable detection metadata are sampled at the consuming slot, and RNG-bearing forecasts are not built globally. Rider-charge execution now also runs in its rider's movement slot: only the live `RiderCharging` order initializes it, geometry and the actual `TransitionCharging` frame are sampled with Original pre-Turn timing, candidate-order damage is synchronous, FrozenAll retains its polygon/damage quirk, and the same surviving order is rewritten with a fresh ID on the actual last frame. General movement, active strike/bow/ability ownership (including PC Listen and Target Heard), other rider execution, and remaining entity-kind Hourglass boundaries remain PA-013 debt (`RHengine.cpp:3715-3724,7909-7944`; `RHelementactorsoldier.cpp:2573-2605`; `RHelementactorhuman.cpp:277-324`; `RHelementactor.cpp:534-728`; `RHelementactorpc.cpp:1904-1947`; `RHelementactornpc.cpp:3495-3659`; `RHartificialintelligence.cpp:7172-7259,7272-7305`). |
