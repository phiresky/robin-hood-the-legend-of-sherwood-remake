...
| --- | --- | --- | --- |
| PA-013 | High | incomplete | `09ec7c5fe` restores a live-size creation-ordered pass for bow release/projectiles, PC auto-heal, purses/coins, wasps, nets, fallback-timed melee completion, and abilities. `4c9b5715a` restores the mixed pre/post target position observed by EYES_FOLLOW. Subsequent slices restore creation-ordered melee, SEEK target sampling, detection, base Actor Execute/ActionChange, owner-local AI callbacks, stationary WAIT/Lift/WaitingSword work, and bonus-owned `RefreshDiscovered`. Ordinary movement, active straight/non-straight/completion/sweep melee, `RunningUpright` rider GALOPP, and selected `RiderCharging` now execute in the fused live Actor → Human → PC/NPC owner envelope. The coordinator snapshots and revalidates the exact selected sequence/element/order identity; background movement or melee cannot execute through a higher selected arm or suppress that arm's generic Execute. Melee victim damage, callbacks, completion, and RNG close before base wait modifiers/completion, ActionChange, and derived tails. Strike `WaitingSword` state and `WarnForStrike` Think/RNG begin only on the live sprite `MotionState::Start`; the warning forecast uses the Original collector, including principal-opponent-and-range-only straight/assault admission and the deliberately looser active-human lateral predicate. FrozenAll makes melee PerformAction effectively return InProgress without touching sprite, strike/sweep/fallback/completion, or order state, while the non-sprite Actor envelope continues. Each owner otherwise closes anti-collision, crossings, door/seek/waypoint completion, the real exact-once ReachPoint condolation stack, GALOPP Think, sword and shoulder-carry checks, ActionChange, and derived tails before the next slot. Per-actor execution freeze suppresses Human Execute arms. `tick_zone_occupants` has no cited actor owner and remains an explicit separate boundary after the owner walk. Remaining debt is bow/ability ownership (including PC Listen/object reveal and Target Heard), unsupported rider/Execute arms, and remaining entity-kind Hourglass owners (`RHengine.cpp:3715-3724,7909-7944`; `RHelementactor.cpp:534-728,1015-2740`; `RHelementactorhuman.cpp:277-324,3235-3560,9972-10097`; `RHelementactorpc.cpp:3616-3710,4950-4990,5481-5510`; `RHelementactorsoldier.cpp:1592-1608`; `RHElementBonus.cpp:513-519,583-625`). |
...
active strike and bow execution, ability ownership, remaining rider execution, non-NPC
...
| Movement, animation, ActionChange, scroll Hourglass | actor/object virtual Hourglass and Execute methods | PA-013; EYES_FOLLOW and live SEEK-target mixed pre/post observations are fixed. The supported Soldier/Human/base Actor/PC-or-NPC envelope now shares each live legacy creation slot, including ordinary movement, rider GALOPP, and rider charge. Unsupported rider actions, active combat, abilities, scrolls, and other entity kinds remain separate. |
...
| Projectiles, melee, and abilities | per-type virtual Hourglass/Execute methods | live creation order, spawn-frame inclusion, straight/assault causality, non-straight phase timing, synchronous melee victim ordering, and owner-local rider-charge damage are verified by PA-013 regressions; remaining rider actions and other combat maintenance remain batched |
