...
broader inactive Hourglass behavior. Creation-slot integration of the
currently batched NPC blip pass also remains open.
...
| NPC view, detection, timers, speech, patrol | `RHElementActorNPC::Hourglass` and AI subclasses | phase order verified by PA-016; followed-target, FRIEND-versus-synchronous-EVENT_VIEW, synchronous pre-optical EVENT_HEAR, both camps' ordered multi-VIEW/OUTOFVIEW Enemy FIFOs, per-stimulus live contexts and exact target inputs, OUTOFVIEW-before-later-detectable causality, Royalist creation-order coordination with inline reveal and synchronous HeyFolks, locked Lacklandist scans, static-versus-AILOCK freeze semantics, and retained VIEW/OUTOFVIEW replay are fixed at the creation boundary; inactive-inside-building viewers, script-driven mid-FIFO detectable-list mutation, and the broader NPC tail remain batched or incomplete |
