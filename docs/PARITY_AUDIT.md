...
phase, and timer/deadline overflow. `SetState` script notifications and `Say`
sound dispatch remain deferred global passes; the Original performs them
synchronously, so they are explicit future re-entrant PA-013 debt rather than
