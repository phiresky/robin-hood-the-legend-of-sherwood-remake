...
- #70 completed: synchronous owner dispatch now propagates the original Actor::Instruct pointer-change barrier explicitly. Terminal `AssertPosition` and already-satisfied swordfight entries skip the shared epilogue, while accepted work publishes `mpOrder` and stamps motion `InProgress` even when an empty translated order terminates immediately afterward; focused synchronous and manager-phase regressions pass.
