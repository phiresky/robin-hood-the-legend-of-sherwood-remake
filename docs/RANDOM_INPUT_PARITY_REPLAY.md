...
| `SuN1Sh1nE/Profile_004/Savegame_002/replay-003`, 35715 -> 35716 | Soldier 217, `REMARK_WARCRY` (9) | Special-strike boundary, not a PC-expression boundary. Soldier 217 finishes `ReceiveSwordDamage` in raw substate 161 (`AttackingSwordfightParade`), Original's synchronous `EVENT_AFTER_COMBAT_INJURY` reconsideration selects `SwordstrikeThrustH`, and the C++ thrust C/F/G/H/I branch emits the warcry before launching its wait/strike sequence. Rust has no pending request when that line resolves; retain this with the open special-strike entry/selection family rather than changing hero speech. |
