...
seed/config. Replays require campaign, seed, and config in their version-9
header; network version 15 announces mission identity with the same seed and
...
| `LevelBonusInitialFrame` | 2 | `engine/level_loading/entities.rs`: mission bonus spawn; `legacy_save/adopt_dynamic_elements.rs`: v48 phase-one bonus reconstruction | `[0,row_frame_count)` once per successfully loaded bonus; one raw constructor draw per reconstructed bonus before phase two restores its saved sprite frame | `RHElementBonus::Initialize`, `RHElementBonus.cpp:683`; `RHSprite::ForceRandomSpriteFrame` | Mission entity ingestion and v48 load reconstruction | E |
...
| `RescuePcFirstName` | 1 | `engine/level_loading/entities.rs`: rescue-PC construction | `[0,22)` once per non-VIP naming attempt, before surname | `RHPCStatus::GenerateName` / `ChooseRandomMenuText(g_cuMT_STR_FIRSTNAME_00, 22)`, `RHPCStatus.cpp:185-256` | PRIS entity construction | E |
| `RescuePcSurname` | 1 | `engine/level_loading/entities.rs`: rescue-PC construction | `[0,22)` once per non-VIP naming attempt | `RHPCStatus::GenerateName` / `ChooseRandomMenuText(g_cuMT_STR_SURNAME_00, 22)`, `RHPCStatus.cpp:185-256` | PRIS entity construction; accepted pairs are registered immediately, collisions retry up to ten pairs, and VIP fixed names consume no draws | E |
...
| `AuxiliaryRngSite::DelayedSoundTimer` | 1 | `engine/tick.rs`: delayed-source completion | Draw one delay step when a delayed audio sample finishes. | `RHSound::StopSoundSource`; the original shares process RNG with audio, but audio timing must not perturb Rust gameplay RNG. | Sound-source completion at the start of a simulation frame | The ephemeral RNG is seeded from completion frame, source ID, and source index. The resulting timer is serialized and state-hashed. |
