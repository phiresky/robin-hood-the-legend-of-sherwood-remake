...
browser peer to the protocol-27 predictive multiplayer session through
...
- The canonical public artifact is an `rhmp3-...` ticket with schema 3. The
  final integration uses gameplay wire protocol 27; invitation and content
...
- Native peers retain native iroh QUIC, direct connectivity, and relay
  fallback. Both paths use the same ALPN, protocol-27 messages, class-tagged
...
1. Deploy the stable game shell, versioned `/wasm/<12-char-commit>/` artifact,
   ...
   27, ticket schema 3, and multiplayer content schema 2.
...
- Browser transport is relay-WebSocket only. Browser direct UDP/hole punching
  is not implemented.
- Builds must match the full engine commit and protocol 27. There is no legacy
