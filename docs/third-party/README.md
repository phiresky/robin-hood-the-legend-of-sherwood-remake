...
Keep one record per distinct article or guide. Repeated links, platform-specific GameFAQs mirrors, tracking parameters, and translated storefront URLs do not necessarily represent independent sources. Preserve original-language titles or terms when useful, but write new summaries rather than copying the source text. Record author, publication or revision date when verified, access date, scope inspected, and an original-source link. For inaccessible material, retain the link and a concrete TODO instead of inventing a summary.
...
- [Video lead](reference/video-walkthrough.md): watch and annotate edition, difficulty, mission coverage, and timestamps before using as visual evidence.
...
The following entries record findings and access limitations at each pass; later entries supersede resolved leads.
...
#### Additional walkthrough, checkpoint, and campaign research

Added seven further non-Steam records, bringing the collection to 121. New material covers an independently authored Czech guide, an annotated savegame catalogue, castle-recapture branches, Italian prerelease coverage, a July 2002 publicity item, the 2012 GOG launch announcement, and a Mac/Porting Kit completion-loop report.

The [GameSurf preview](history/gamesurf-preview.md) and [Siege of Lincoln discussion](guides/gog-castle-recapture.md) resolve leads listed in the preceding pass. The [checkpoint catalogue](reference/nicouzouf-savegames.md) specifies English, medium difficulty, and version 1.1, but its files were not downloaded. The [Czech walkthrough](guides/cheater-miky-czech.md) is attributed to Michal “Miky” Deus and is distinct from the previously collected Roman Ficnar guide.
