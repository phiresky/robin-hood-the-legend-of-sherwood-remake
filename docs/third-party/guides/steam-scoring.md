# OzzyRockerPL — scoring system

- Original source: [OzzyRockerPL — scoring system](https://steamcommunity.com/sharedfiles/filedetails/?id=2829972690)
- Author / publication: OzzyRockerPL / Steam Community
- Language / date: English; published and updated 2022-07-09
- Access: Full guide page retrieved directly
- Checked: 2026-09-09
- Retrieved: 2026-09-09
- Archived copy: none found in the Wayback Machine (checked 2026-09-09)
- Format: header notes, original summary, then complete factual notes and both tables (no redistribution grant on Steam guides, so not transcribed)

Reported mission-end points:

| Event | Points |
| --- | ---: |
| Main mission completed | 1,000 |
| Ambush shield obtained | 1,000 |
| Ground/coffer money, per £100 | 10 |
| Soldier killed | 50 |
| Soldier still unconscious or bound at completion | 70 |
| Royal collectible | 1,000 |
| Guy or the Sheriff defeated | 100 |

The author reports no points for elapsed time, looted personal money, pre-existing corpses, recovered prisoners, netted/trapped enemies, or the Silver Arrow. Allied kills count.

Includes seventeen mission discussions, conditional Free Lincoln, and a score table. It warns about two swordsmen breaking The Letter's conversation. Pre-existing corpses reportedly affect spared lives despite awarding no points.

**Limitations:** community findings; maxima, rounding, and event classification are unverified.

## Detailed notes

Guide metadata at retrieval: v1.0, posted 9 Jul 2022 1:37am and updated 1:50am the same day; categories Achievements, Gameplay Basics, Maps or Levels, Secrets, Walkthroughs; 644 unique visitors, 4 favourites, 2 comments. Sections: Introduction, Points, Overview, Table with maximum scores, Links.

**Introduction.** Mission-end score is shown but never explained in game; most players watch only the spared-lives ratio. The author invites corrections and better scores.

**Points** (the table in the summary above is the complete list). Additional statements: quest completion points apply only to the 17 main missions; shields count only in Sherwood ambushes; personal money is ignored probably to stop purse-throw exploits; knocked-out or tied soldiers count only if still restrained at mission end; soldiers dead before the mission starts (Pillaging, The Black Castle, The March on York) give nothing; enemies in Stuteley's net or in ambush traps (nets, pits) give nothing; the Silver Arrow is a tournament prize, not a royal item; losing Merry Men or allied soldiers costs nothing; kills by allied soldiers do count.

**Per-mission notes.**
1. Finding Godwin: Robin alone, so nobody can be tied; on Hard soldiers wake quickly, but recovery time depends on remaining health, so weaken them, retreat, return and punch; some soldiers change position or behaviour after the first encounter; Robin can leave and re-enter the castle over the east wall, which enemies cannot climb.
2. Rescuing Stuteley: the inner-castle gate stays closed. Show yourself to a pro-sheriff civilian so they alarm the officer through the north tower; after things calm down a crossbowman relocates above the gate where Robin can shoot him. Suggested order: shoot the officer near the gate, then the second crossbowman when he comes into bow range, wait, then throw a purse at the four "lancers" (the manual's term; the author calls them recruits) and shoot the winner. Knights are immune to arrows but can be engaged in melee through the gate bars; the author judges that not worth the reloads.
3. Scarlet Night: nothing special.
4. Confessions of an Outlaw: seven bodyguards at the party in the sheriff's chamber cannot be attacked.
5. The Prince and the Outlaw: twelve soldiers cannot be attacked (per the summary's soldier count).
6. Pillaging: seven pre-placed corpses give no points but count against the spared ratio.
7. The Evening Visitor: the five soldiers in the western tower belong to the sheriff, not Ranulph, so killing them does not fail the mission; knocking out scores more anyway.
8. The Godfather in Prison: two crossbowmen and a halberdier near Guy of Gisbourne can be shot with Quick Actions.
9. The Lock-up and the Friar: one unreachable soldier.
10. The Silver Arrow: five unreachable soldiers.
11. A Wedding and a Funeral: three unreachable, including one in the far north-east.
12. The Escape: nothing special.
13. The Letter: do not attack the two swordsmen near a tower in the citadel (they can be lured out, for example by whistling); doing so breaks the game after the villains' conversation. Plus one unreachable soldier in the far north-east.
14. Free Lincoln: only offered if Lincoln is lost.
15. The Black Castle: twenty-one pre-placed corpses (no points, count against ratio); one unreachable soldier.
16. The March on York: six pre-placed corpses; one unreachable soldier in the far north-east.
17. Last Challenge: a swordsman becomes an ally if you talk to a beggar and then a man in the church vestry; to score him, confront him before talking about him. Four other soldiers unreachable.

**Maximum-score table** (author's results; "Other" lists score-bearing extras):

| Mission | Location | Coffers | Killed | Tied | Other | Score |
| --- | --- | ---: | ---: | ---: | --- | ---: |
| Finding Godwin | Lincoln | £1500 | 0 | 38 | | 3810 |
| Rescuing Stuteley | Nottingham | £1100 | 4 | 81 | | 6980 (max 7270) |
| Scarlet Night | Leicester | £2000 | 0 | 74 | | 6380 |
| Confessions of an Outlaw | Nottingham | £3900 | 0 | 104 | | 8670 |
| The Prince and the Outlaw | Derby | £2500 | 0 | 84 | Sceptre | 8130 |
| Pillaging | Sherwood | £100 | 0 | 53 | | 4720 |
| The Evening Visitor | Leicester | £1500 | 0 | 56 | Doomsday Book | 6070 |
| The Godfather in Prison | Lincoln | £5000 | 3 | 68 | | 6410 |
| The Lock-up and the Friar | Derby | £3500 | 0 | 85 | Coronation Spoon | 8300 |
| The Silver Arrow | Nottingham | £8500 | 0 | 103 | Sword of State | 10060 |
| A Wedding and a Funeral | York | £6000 | 1 | 125 | Ampulla, Guy of Gisbourne | 11500 |
| The Escape | Nottingham | £7500 | 0 | 117 | King Richard's Sigil | 10940 |
| The Letter | York | £8600 | 1 | 102 | Crown | 10050 |
| Free Lincoln (conditional) | Lincoln | £2000 | 0 | 90 | | 7500 |
| The Black Castle | Derby | £1500 | 1 | 66 | | 5820 |
| The March on York | York | £1000 | 0 | 126 | | 9920 |
| Last Challenge | Nottingham | £1100 | 1 | 154 | Sheriff of Nottingham | 12040 |

**Links given:** the archived official site (web.archive.org capture of robinhood-game.com from 2007-02-17) and Carter's GameFAQs walkthrough.

**Comments.** Asked whether all coffers in Rescuing Stuteley can be collected, the author (23 May 2023) lists £100 in the church vestry, £500 near the tax collector in the south and £500 on a roof in the north; the last appears after reading the scroll obtained by paying the beggar at least six times.

## Converted text from the original HTML



### guides__steam-scoring.html

_Source: `originals/guides__steam-scoring.html`._

[ Sign in ](https://steamcommunity.com/login/home/?goto=sharedfiles%2Ffiledetails%2F%3Fid%3D2829972690) [ Store ](https://store.steampowered.com/)

[ Home ](https://store.steampowered.com/) [ Discovery Queue ](https://store.steampowered.com/explore/) [ Wishlist ](https://store.steampowered.com/wishlist/) [ Points Shop ](https://store.steampowered.com/points/shop/) [ News ](https://store.steampowered.com/news/) [ Charts ](https://store.steampowered.com/stats/)

[ Community ](https://steamcommunity.com/)

[ Home ](https://steamcommunity.com/) [ Discussions ](https://steamcommunity.com/discussions/) [ Workshop ](https://steamcommunity.com/workshop/) [ Market ](https://steamcommunity.com/market/) [ Broadcasts ](https://steamcommunity.com/?subsection=broadcasts)

[ About ](https://store.steampowered.com/about/) [ Support ](https://help.steampowered.com/en/)

Change language 

[Get the Steam Mobile App](https://store.steampowered.com/mobile)

View desktop website 

© Valve Corporation. All rights reserved. All trademarks are property of their respective owners in the US and other countries.  [Privacy Policy](https://store.steampowered.com/privacy_agreement/)  |  [Legal](http://www.valvesoftware.com/legal.htm)  |  [Accessibility](https://help.steampowered.com/faqs/view/10BB-D27A-6378-4436)  |  [Steam Subscriber Agreement](https://store.steampowered.com/subscriber_agreement/)  |  [Refunds](https://store.steampowered.com/steam_refunds/)  |  [Cookies](https://store.steampowered.com/account/cookiepreferences/)

[ ](https://store.steampowered.com/)

[ ](https://store.steampowered.com/)

[ STORE ](https://store.steampowered.com/)

[ Home ](https://store.steampowered.com/) [ Discovery Queue ](https://store.steampowered.com/explore/) [ Wishlist ](https://store.steampowered.com/wishlist/) [ Points Shop ](https://store.steampowered.com/points/shop/) [ News ](https://store.steampowered.com/news/) [ Charts ](https://store.steampowered.com/stats/)

[ COMMUNITY ](https://steamcommunity.com/)

[ Home ](https://steamcommunity.com/) [ Discussions ](https://steamcommunity.com/discussions/) [ Workshop ](https://steamcommunity.com/workshop/) [ Market ](https://steamcommunity.com/market/) [ Broadcasts ](https://steamcommunity.com/?subsection=broadcasts)

[ About ](https://store.steampowered.com/about/) [ SUPPORT ](https://help.steampowered.com/en/)

[ Install Steam  ](https://store.steampowered.com/about/) [sign in](https://steamcommunity.com/login/home/?goto=sharedfiles%2Ffiledetails%2F%3Fid%3D2829972690)  |  language

[ 简体中文 (Simplified Chinese) ](?l=schinese&id=2829972690) [ 繁體中文 (Traditional Chinese) ](?l=tchinese&id=2829972690) [ 日本語 (Japanese) ](?l=japanese&id=2829972690) [ 한국어 (Korean) ](?l=koreana&id=2829972690) [ ไทย (Thai) ](?l=thai&id=2829972690) [ Bahasa Indonesia (Indonesian) ](?l=indonesian&id=2829972690) [ Bahasa Melayu (Malay) BETA ](?l=malay&id=2829972690) [ Български (Bulgarian) ](?l=bulgarian&id=2829972690) [ Čeština (Czech) ](?l=czech&id=2829972690) [ Dansk (Danish) ](?l=danish&id=2829972690) [ Deutsch (German) ](?l=german&id=2829972690) [ Español - España (Spanish - Spain) ](?l=spanish&id=2829972690) [ Español - Latinoamérica (Spanish - Latin America) ](?l=latam&id=2829972690) [ Ελληνικά (Greek) ](?l=greek&id=2829972690) [ Français (French) ](?l=french&id=2829972690) [ Italiano (Italian) ](?l=italian&id=2829972690) [ Magyar (Hungarian) ](?l=hungarian&id=2829972690) [ Nederlands (Dutch) ](?l=dutch&id=2829972690) [ Norsk (Norwegian) ](?l=norwegian&id=2829972690) [ Polski (Polish) ](?l=polish&id=2829972690) [ Português (Portuguese - Portugal) ](?l=portuguese&id=2829972690) [ Português - Brasil (Portuguese - Brazil) ](?l=brazilian&id=2829972690) [ Română (Romanian) ](?l=romanian&id=2829972690) [ Русский (Russian) ](?l=russian&id=2829972690) [ Suomi (Finnish) ](?l=finnish&id=2829972690) [ Svenska (Swedish) ](?l=swedish&id=2829972690) [ Türkçe (Turkish) ](?l=turkish&id=2829972690) [ Tiếng Việt (Vietnamese) ](?l=vietnamese&id=2829972690) [ Українська (Ukrainian) ](?l=ukrainian&id=2829972690) [Report a translation problem](https://www.valvesoftware.com/contact?contact-person=Translation%20Team%20Feedback)

[ Store Page ](https://store.steampowered.com/app/46560?snr=2_100100_100101_100106_apphubheader)

Robin Hood

[All](https://steamcommunity.com/app/46560) [Discussions](https://steamcommunity.com/app/46560/discussions/) [Screenshots](https://steamcommunity.com/app/46560/screenshots/) [Artwork](https://steamcommunity.com/app/46560/images/) [Broadcasts](https://steamcommunity.com/app/46560/broadcasts/) [Videos](https://steamcommunity.com/app/46560/videos/) [News](https://steamcommunity.com/app/46560/allnews/) [Guides](https://steamcommunity.com/app/46560/guides/) [Reviews](https://steamcommunity.com/app/46560/reviews/)

All  Discussions  Screenshots  Artwork  Broadcasts  Videos  News  Guides  Reviews 

### Robin Hood

[ Store Page ](https://store.steampowered.com/app/46560)

[Robin Hood](https://steamcommunity.com/app/46560) > [Guides](https://steamcommunity.com/app/46560/guides/) > [OzzyRockerPL's Guides](https://steamcommunity.com/profiles/76561199229889570/myworkshopfiles/?section=guides&appid=46560)

This item has been removed from the community because it violates Steam Community & Content Guidelines. It is only visible to you. If you believe your item has been removed by mistake, please contact [Steam Support](https://help.steampowered.com/en/wizard/HelpWithSteamIssue/?issueid=415). 

This item is incompatible with Robin Hood. Please see the [instructions page](https://steamcommunity.com) for reasons why this item might not work within Robin Hood. 

Not enough ratings 

[EN] Scoring system in Robin Hood: Legend of Sherwood

By OzzyRockerPL

How points are calculated in Robin Hood: Legend of Sherwood.

1

1

   

Award

Favorite

Favorited

Unfavorite

Share

This item has been added to your [Favorites](https://steamcommunity.com/my/myworkshopfiles/?section=guides&browsefilter=myfavorites).

Created by

[](https://steamcommunity.com/profiles/76561199229889570)

OzzyRockerPL  
Offline 

Category: [Achievements](https://steamcommunity.com/app/46560/guides/?browsesort=trend&filetype=11&requiredtags%5B%5D=Achievements), [Gameplay Basics](https://steamcommunity.com/app/46560/guides/?browsesort=trend&filetype=11&requiredtags%5B%5D=Gameplay+Basics), [Maps or Levels](https://steamcommunity.com/app/46560/guides/?browsesort=trend&filetype=11&requiredtags%5B%5D=Maps+or+Levels), [Secrets](https://steamcommunity.com/app/46560/guides/?browsesort=trend&filetype=11&requiredtags%5B%5D=Secrets), [Walkthroughs](https://steamcommunity.com/app/46560/guides/?browsesort=trend&filetype=11&requiredtags%5B%5D=Walkthroughs)

Languages: [English](https://steamcommunity.com/app/46560/guides/?browsesort=trend&filetype=11&requiredtags%5B%5D=English)

Posted 

Updated 

9 Jul, 2022 @ 1:37am

9 Jul, 2022 @ 1:50am

644 | Unique Visitors  
---|---  
4 | Current Favorites  
  
Guide Index

Overview

Introduction 

Points 

Overview 

Table with maximum scores 

Links 

Comments

Introduction 

At the end of every mission there is an information about score (points gained) for that quest - but there is no information what it depends on. Most players concentrate on spared lives ratio during their games because its mechanics is clear and obvious. Now it's time to break down rating system in Robin Hood: Legend of Sherwood and put new challenges for the players!  
  
This guide was made to the best of my knowledge and ability, but some small details may still be there to discover. Leave a comment if you manage to find something more or beat any score listed in the table below!  
  


Points 

**Points are awarded as follows:**  


  * +1000 for quest completion (only in main missions - there are 17 in total)  

  * +1000 for every Shield (only during ambush missions in Sherwood forest)  

  * +10 for every 100£ collected "from the coffers" (which means picked up from the ground)  

  * +50 for every killed soldier  

  * +70 for every knocked out or tied up soldier (at the moment of ending the mission)  

  * +1000 for every royal item (there are 7 in total)  

  * +100 for defeating Guy of Gisbourne or Sheriff of Nottingham

  
**What doesn't affect score:**  


  * time  

  * money collected on characters (probably because otherwise player could throw purses with gold to enemy soldiers and afterwards collect it back, increasing score in the process)  

  * soldiers that were knocked out or tied up, but woke up or were freed before mission end  

  * soldiers that were killed before start of the mission (missions Pillaging, The Black Castle, The March on York)  

  * soldiers caught in Stuteley's net  

  * soldiers caught in traps like nets or ditches during ambushes  

  * Silver Arrow (it's not a royal item, just a prize for a tournament win)  

  * losing Merry Men or allied soldiers

  
**Additional notes:**  


  * enemies killed by allied soldiers (during ambushes or assaults) also count for the score



Overview 

Short description of all 17 main missions focused on getting highest score possible.  


  1. **Finding Godwin**  
First mission is very tricky and requires the most detailed planning of all missions, because player's only character is Robin. That means you can't tie enemies up. On Hard difficulty, enemy soldiers wake up quickly after being knocked out. However, it's possible to have all soldiers on the map knocked out when mission ends. What helps is that amount of time soldiers need to recover depends on their health (hit points). Player needs to weaken them first, run away, return and punch them. Another tricky part is that some soldiers change their location or behaviour after first encounter. Fortunately, Robin can leave and enter castle any time he wishes by climbing the wall in east part of the map - something that enemies can't do.  
  

  2. **Rescuing Stuteley**  
There is an issue with closed gate to the inner castle. After clearing accessible parts of the map player should show up in front of one of the civilians, which support Sheriff of Nottingham. They can run into inner castle by north tower and alarm the officer. Purpose for this is that after enemies calm down, one of the crossbowman will relocate and take position right above gate, where he can be shot down by Robin. My suggestion for other soldiers is as follows: stand with Robin near the gate and shoot down officer so that second crossbowman can see that. When he enters Robin's bow range, shoot him as well. Now step back and wait until enemies calm down. When this happens, four "lancers" (according to game instruction, I call them recruits) stand close to each other. Throw a purse with Robin and let them fight for it, then shoot down the unlucky winner of the brawl. Taking down officer first was necessary, because he prevents recruits from fighting for the money.  
This is not a perfect instruction and doesn't lead to maximum score, but there's a good chance it can be done in first attempt. Other idea is simply shooting down all soldiers (except knights, which are invulnerable to arrows). Theoretically, player can knock out all soldiers behind gate (even knights), because you can get into melee fight with them when they are close to gate's steel bars - but in my opinion amount of time and number of save/reloads to pull that off are not worth it.  
  

  3. **Scarlet Night**  
Nothing special in that mission.  
  

  4. **Confessions of an Outlaw**  
Seven soldiers, which are bodyguards on a party in Sheriff's chamber can't be assaulted.  
  

  5. **The Prince and the Outlaw**  
There are twelve soldiers, that can't be assaulted (according to total number of soldiers, that is shown in mission summary).  
  

  6. **Pillaging**  
Points are not awarded for seven dead bodies that are lying around at the start of the mission. They do count against spared lives ratio however.  
  

  7. **The Evening Visitor**  
It may feel that you can't kill a single soldier in this mission, but you actually can - five soldiers in the western tower are Sheriff's men, not Ranulph's - therefore eliminating them doesn't fail the mission. It's still better to knock them out as it gives more points.  
  

  8. **The Godfather in Prison**  
Three soldiers - two crossbowmen and a halberdier - that are near Guy of Gisbourne can be shot down by Robin using _Quick Actions_.  
  

  9. **The Lock-up and the Friar**  
There is one soldier, that can't be assaulted (according to total number of soldiers, that is shown in mission summary).  
  

  10. **The Silver Arrow**  
There are five soldiers, that can't be assaulted (according to total number of soldiers, that is shown in mission summary).  
  

  11. **A Wedding and a Funeral**  
There are three soldiers (including the one in far north east) that can't be assaulted (according to total number of soldiers, that is shown in mission summary).  
  

  12. **The Escape**  
Nothing special in that mission.  
  

  13. **The Letter**  
Be careful with bug in this mission! You mustn't assault two swordsmen near a tower in the citadel. It is possible to lure them out (by whistling for example). Doing that causes the game to bug after hearing the conversation between main villains. Besides those two, there is a soldier in far north east that is unreachable - to make total of three soldiers that a player can't do anything to.  
  

  14. **Free Lincoln**  
This mission will be available only if a player loses Lincoln.  
  

  15. **The Black Castle**  
Points are not awarded for twenty one dead bodies that are lying around at the start of the mission. They do count against spared lives ratio however. There is also one soldier, that can't be assaulted (according to total number of soldiers, that is shown in mission summary).  
  

  16. **The March on York**  
Points are not awarded for six dead bodies that are lying around at the start of the mission. They do count against spared lives ratio however. There is also a soldier in far north east that is unreachable.  
  

  17. **Last Challenge**  
There is a swordsman that can become an ally if a player talks with one of the beggars and then with a man in a church's vestry. This eliminates the possibility of assaulting him, so in order to get points for that soldier player must not talk about him before he is confronted. Apart from him, there are four soldiers, that can't be assaulted (according to total number of soldiers, that is shown in mission summary).



Table with maximum scores 

Mission

Location

Coffers

Killed

Tied

Other

Score

Finding Godwin

Lincoln

1500£

0

38

3810

Rescuing Stuteley

Nottingham

1100£

4

81

6980 (max 7270)

Scarlet Night

Leicester

2000£

0

74

6380

Confessions of an Outlaw

Nottingham

3900£

0

104

8670

The Prince and the Outlaw

Derby

2500£

0

84

Sceptre

8130

Pillaging

Sherwood

100£

0

53

4720

The Evening Visitor

Leicester

1500£

0

56

The Doomsday Book

6070

The Godfather in Prison

Lincoln

5000£

3

68

6410

The Lock-up and the Friar

Derby

3500£

0

85

The Coronation Spoon

8300

The Silver Arrow

Nottingham

8500£

0

103

The Sword of State

10060

A Wedding and a Funeral

York

6000£

1

125

The Ampulla  
Guy of Gisbourne

11500

The Escape

Nottingham

7500£

0

117

King Richard's Sigil

10940

The Letter

York

8600£

1

102

The Crown

10050

Free Lincoln *

Lincoln

2000£

0

90

7500

The Black Castle

Derby

1500£

1

66

5820

The March on York

York

1000£

0

126

9920

Last Challenge

Nottingham

1100£

1

154

Sheriff of Nottingham

12040

Links 

  * [Official website (archived)](https://steamcommunity.com/linkfilter/?u=https%3A%2F%2Fweb.archive.org%2Fweb%2F20070217140808%2Fhttp%3A%2F%2Fwww.robinhood-game.com%3A80%2F)[web.archive.org]  

  * [Walkthrough by swcarter from May 3, 2003](https://steamcommunity.com/linkfilter/?u=https%3A%2F%2Fgamefaqs.gamespot.com%2Fpc%2F562021-robin-hood-the-legend-of-sherwood%2Ffaqs%2F23124)[gamefaqs.gamespot.com]

  
Guide release: v1.0 on July 9, 2022.  


2 Comments 

[<](javascript:void\(0\);) [>](javascript:void\(0\);)

[ ](https://steamcommunity.com/profiles/76561199229889570)

[ OzzyRockerPL](https://steamcommunity.com/profiles/76561199229889570)  [author]

23 May, 2023 @ 9:50am 

Yes, in Rescuing Stuteley there is 100£ in the church's vestry, 500£ near the tax collector in the south and 500£ on the roof of one of the buildings in the north. To get the last one you need to give money to the beggar at least six times to get the scroll from him, cash appears after you read the scroll. 

[ ](https://steamcommunity.com/id/ddacjj)

[ deamonpog](https://steamcommunity.com/id/ddacjj)

5 Apr, 2023 @ 12:03am 

Have you ever been able to collect all coins from coffers in the Rescuing Stutely mission? 

[<](javascript:void\(0\);) [>](javascript:void\(0\);)

Share to your Steam activity feed

[]( "Share on Facebook")

[]( "Share on Twitter")

[]( "Share on Reddit")

Link: 

You need to sign in or create an account to do that.

[Sign In](https://steamcommunity.com/login/home/?goto=sharedfiles%2Ffiledetails%2F%3Fid%3D2829972690%26insideModal%3D0%26requirelogin%3D1) [Create an Account](https://store.steampowered.com/join) Cancel

[Update](javascript:UpdateKVTagsSingle\(\);)
