...
parser.add_argument('--hq', action='store_true', help='1920x1280, 60 fps, 24-second slower turntable')
parser.add_argument('--split', action='store_true', help='Original baseline on left, refined scene on right')
parser.add_argument('--output-dir', type=Path, help='Alternate pass/output directory alongside turntable-original')
...
if args.hq and not args.split:
    parser.error('--hq requires --split because HQ border passes contain only their comparison half')
OUT = Path(__file__).resolve().parents[3] / 'level-editor/work/sherwood-refinement' / ('turntable-hq' if args.hq else 'turntable-fast' if args.fast else 'turntable')
if args.output_dir:
    OUT = args.output_dir.resolve()
W, H = (1920, 1280) if args.hq else (960, 640) if args.fast else (1440, 1080)
...
font = ImageFont.load_default(size=44 if args.hq else 22)
small = ImageFont.load_default(size=32 if args.hq else 16)
...
for output_frame, frame in enumerate(FRAMES, 1):
    ...
    draw.text((35, 88 if args.hq else 62), label, font=small, fill=(151, 174, 181))
    if args.split:
        draw.line((W//2, 0, W//2, H), fill=(235, 237, 235), width=2)
        draw.text((W//2+35, 29), 'AFTER / Refined', font=font, fill=(225, 231, 231))
        draw.text((W//2+35, 88 if args.hq else 62), label, font=small, fill=(151, 174, 181))
    ...
    for directory in [OUT, BASELINE] if args.split else [OUT]:
        for name in modes:
