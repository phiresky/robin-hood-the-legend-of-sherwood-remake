...
// index.json is read-modify-write; serialize updates so parallel
// reconstruction workers don't lose entries
let indexLock: Promise<void> = Promise.resolve();

function updateIndex(desc: AssetDescriptor): Promise<void> {
  const run = indexLock.then(() => updateIndexUnlocked(desc));
  indexLock = run.catch(() => undefined);
  return run;
}

async function updateIndexUnlocked(desc: AssetDescriptor) {
