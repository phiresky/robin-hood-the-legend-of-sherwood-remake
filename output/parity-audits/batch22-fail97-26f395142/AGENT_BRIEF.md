...
## Authoritative data
Runner `7cdfa0799`, release profile, sha256 `a25bd45910d7470e…`. **36 traces remain.**
`/home/phire/robinhood/output/parity-audits/batch23-7cdfa0799/`
- `bundles/<your bundle>.txt` — your traces, grouped
- `remaining-36.snapshot` — the full remaining set
...
rest is a good outcome; a forced single story is not. **25 of the 30 remaining groups are singletons, and the largest group is 3** — expect exactly one
cause per trace, and prioritise breadth of attribution over depth on any single member.

## TWO RECURRING BUG CLASSES — check these FIRST on any member
1. **AI-resolved position used where the Original uses the raw element position (6 sites found).**
   `RHArtificialIntelligence::Position()` SNAPS to a gate endpoint while an `ActiveDoorPass` exists. Any
   C++ site reading `GetPosition()` / `GetPositionGround()` / `GetPositionMap()` must use the raw value —
   `FighterSnapshot::raw_position`, `ctx.self_body_position_world`, `view.detection_position_world`.
   Already fixed at: `NumberOfNearbyArchersWhoNeedProtection`, `enemy_is_behind_me`, the 3D `Face` origin,
   `GetNearestFighter` ranking, `GetNearestDoor` sector/layer maluses, and the beggar approach distance.
   Still unaudited: the `ask_obstacle` pre-flight (`engine/movement.rs:5853-5875`) passes
   `pi.map_position()` where the Original passes `Point(mpMe)` = the SNAPPING `Position()`; and
   `point_to`'s origin.
2. **Invented invariants / shortcuts Rust has and the Original does not (4 sites found).**
   The Original often only `assert`s in debug builds, or has no check at all. Already removed: the
   jump-line normal flip, the pass-door `sector_out` test (which PANICKED), the door-callback sector
   assert, and an entire shortcut in `find_path_into_door` that suppressed the Original's multi-gate
   detours. If Rust has a guard, an early return, or an optimisation, verify the Original has it too.
...
`remaining-36.snapshot`). **Cap controls at ~10 targeted traces** — identify which passing traces actually
