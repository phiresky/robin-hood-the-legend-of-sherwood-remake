...
the saves named individually by Rust tests. The (since removed) schema16
corpus ladder, distributed capture, and onward controller passed the entire
`reference-saves/` directory to the Original recorder.
`scripts/capture_parity_subset.sh` still uses it as its default source.
