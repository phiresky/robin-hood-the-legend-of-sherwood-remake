...
#   CAPTURE_JOBS=10  ZSTD_THREADS=1  COMPRESS=1  HEADFUL=0  FORCE=0
...
if [[ "$compress" == 1 ]] && ! command -v zstd >/dev/null; then
    printf 'error: COMPRESS=1 requires zstd on PATH\n' >&2
    exit 2
fi
    ...
    then
        ...
        if (( ${#captured_traces[@]} == 0 )); then
            ...
            if (( capture_status == 0 )) && [[ "$compress" == 1 ]]; then
                for captured_trace in "${captured_traces[@]}"; do
                    if ! zstd -10 --long=31 "-T$zstd_threads" -q --rm -- "$captured_trace"; then
                        capture_status=65
                        failure_reason="unable to compress trace: $captured_trace"
                        break
                    fi
                done
            fi
