...
save_group_cap=${DISTRIBUTED_REPLAY_SAVE_GROUP_CAP:-1}
...
[[ "$lease_seconds" =~ ^[0-9]+$ && "$heartbeat_seconds" =~ ^[0-9]+$ \
    && "$timeout_seconds" =~ ^[0-9]+$ && "$save_group_cap" =~ ^[0-9]+$ ]] \
    || { printf 'invalid timing or save-group cap value\n' >&2; exit 2; }
...
(( save_group_cap > 0 )) || { printf 'save-group cap must be positive\n' >&2; exit 2; }
...
# Legacy interactive traces can retain process-local NPC state across a loaded
# save. The parity runner reconstructs that state from the immediately
# preceding session's terminal frame when the adjacent native trace is present.
preceding_session_logical() {
    local logical=$1 session
    [[ "$logical" =~ ^(.+)-session-([0-9]{4})\.jsonl\.zst$ ]] || return 1
    session=$((10#${BASH_REMATCH[2]}))
    (( session > 1 )) || return 1
    printf '%s-session-%04d.jsonl.zst\n' "${BASH_REMATCH[1]}" "$((session - 1))"
}
...
publish_result() {
    ...
    [[ -z "$marker_path" ]] || marker_sha=$(sha256_file "$marker_path" 2>/dev/null || printf missing)
    {
        ...
        printf 'COMMAND=original_parity_replay.remote --no-auto-dump NATIVE_TRACE\n'
...
')
    ...
    existing_key=${fields[5]}
    [[ -n "$logical" && "$logical" != /* && "$logical" != *..* && "$logical" != *$'\n'* \
        && "$native_expected" =~ ^[0-9a-f]{64}$ \
        && ( -z "$existing_key" || "$existing_key" =~ ^[0-9a-f]{64}$ ) ]] \
    ...
    if [[ "$mode" == local ]]; then
        ...
        predecessor_logical=
        if predecessor_logical=$(preceding_session_logical "$logical"); then
            predecessor_native="$claim_dir/root/$predecessor_logical.parity.bitcode.zst"
            mkdir -p -- "${predecessor_native%/*}"
            predecessor_status=0
            fetch_remote_file "$predecessor_logical" .parity.bitcode.zst \
                "$predecessor_native" || predecessor_status=$?
            case $predecessor_status in
                0) ;;
                # A predecessor is optional: fresh sessions and incomplete
                # historical captures legitimately have no adjacent trace.
                4) rm -f -- "$predecessor_native" ;;
                *) : >"$lost_flag" ;;
            esac
        fi
        logical_run="$native"
    ...
    elif ! python3 "$result_validator" --allow-legacy --trace "$logical_run" "$private_log"; then result_status=integrity-eof-marker
