...
verify_result() {
    ...
    python3 "$result_validator" --allow-legacy "$result/log" || return 1
...
run_one() {
    ...
    elif ! python3 "$result_validator" --allow-legacy --trace "$logical" "$private_log"; then
...
data_dir=$(realpath -e -- "${PARITY_EOF_DATADIR:-$workspace/datadirs/fullgame_linux}") \
