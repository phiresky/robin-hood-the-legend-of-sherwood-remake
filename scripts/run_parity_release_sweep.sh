...
status_is_exact_eof() {
    ...
    [[ "$value" == 0 ]] || return 1
    python3 "$script_dir/parity_result.py" --allow-legacy "$log"
}
...
for ((index = shard; index < ${#traces[@]}; index += shards)); do
    ...
    # Absolute paths can exceed NAME_MAX after slash flattening. Hash the
    # canonical identity, and retain the readable identity beside the lock.
    trace_identity=$(realpath -m -- "$trace")
    trace_lock_key=$(printf '%s' "$trace_identity" | sha256sum)
    trace_lock_key=${trace_lock_key%% *}
    exec {trace_lock_fd}>"$audit_dir/.trace-locks/$trace_lock_key.lock" || exit 2
    flock "$trace_lock_fd" || exit 2
    printf '%s\n' "$trace_identity" >"$audit_dir/.trace-locks/$trace_lock_key.trace" || exit 2
    ...
    if (( runner_command_status == 0 )); then
        if python3 "$script_dir/parity_result.py" --trace "$trace" "$run_log"; then
            runner_status=0
        else
