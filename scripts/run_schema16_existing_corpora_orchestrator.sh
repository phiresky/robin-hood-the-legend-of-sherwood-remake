...
# Historical wrapper: frozen deployment identities are separate from reusable
# admission/supervision code. All hashes remain bound into immutable provenance.
campaign_script_dir=$(dirname -- "$(realpath -- "${BASH_SOURCE[0]}")")
campaign_manifest=${PARITY_CAMPAIGN_MANIFEST:-$campaign_script_dir/parity-campaigns/schema16-20260824.json}
campaign_assignments=$(python3 "$campaign_script_dir/parity_campaign.py" "$campaign_manifest" --profile existing_corpora) || exit 2
