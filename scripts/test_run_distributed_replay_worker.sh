...
printf '%s\n' \
    ...
    '  const char *native_suffix=".parity.bitcode.zst";' \
    '  if (getenv("FAKE_RUNNER_REQUIRE_NATIVE") && (argc < 2 || strlen(argv[argc-1]) < strlen(native_suffix) || strcmp(argv[argc-1] + strlen(argv[argc-1]) - strlen(native_suffix), native_suffix) != 0)) return 70;' \
...
setup_case exact
FAKE_RUNNER_MODE=exact FAKE_RUNNER_REQUIRE_NATIVE=1 run_worker exact
python3 - "$database" <<'PY'
import sqlite3,sys
...
# A retry identity with already-attested exact evidence is completed directly
# from the claim payload, without staging or executing the replay again.
python3 - "$database" <<'PY'
import sqlite3,sys
c=sqlite3.connect(sys.argv[1])
with c:
    c.execute("""insert into work_items(
        ...
        select ?,operation,replay_id,corpus_id,save_group,stripe_key,runner_id,
               conversion_protocol,target_encoding,source_sha256,priority
        from work_items limit 1""", ("e" * 64,))
PY
rm -- "$remote_root/$logical.parity.bitcode.zst"
FAKE_RUNNER_MODE=fail run_worker exact-reuse
python3 - "$database" <<'PY'
import sqlite3,sys
c=sqlite3.connect(sys.argv[1])
...
setup_case adjacent_session
old_logical=$logical
logical="$corpus/traces/save/replay-001-session-0002.jsonl.zst"
mv -- "$remote_root/$old_logical.parity.bitcode.zst" \
    "$remote_root/$logical.parity.bitcode.zst"
python3 - "$database" "$old_logical" "$logical" <<'PY'
import sqlite3,sys
c=sqlite3.connect(sys.argv[1])
with c:
    c.execute("update replays set logical_path=? where logical_path=?",(sys.argv[3],sys.argv[2]))
PY
predecessor="$corpus/traces/save/replay-001-session-0001.jsonl.zst"
printf 'preceding-native\nRHPRTRACEFOOTER!12345678901234567890' \
    >"$remote_root/$predecessor.parity.bitcode.zst"
FAKE_RUNNER_MODE=exact FAKE_RUNNER_REQUIRE_NATIVE=1 run_worker adjacent-session
[[ $(find "$local_root/.agent-debug/distributed-replay-worker" -type f \
    -path "*/root/$predecessor.parity.bitcode.zst" | wc -l) == 1 ]]

setup_case missing_adjacent_session
old_logical=$logical
logical="$corpus/traces/save/replay-001-session-0002.jsonl.zst"
mv -- "$remote_root/$old_logical.parity.bitcode.zst" \
    "$remote_root/$logical.parity.bitcode.zst"
...
with c:
    c.execute("update replays set logical_path=? where logical_path=?",(sys.argv[3],sys.argv[2]))
PY
FAKE_RUNNER_MODE=exact FAKE_RUNNER_REQUIRE_NATIVE=1 run_worker missing-adjacent-session
python3 - "$database" <<'PY'
import sqlite3,sys
c=sqlite3.connect(sys.argv[1])
assert c.execute("select outcome from replay_runs").fetchone()[0] == "exact_eof"
...
FAKE_RUNNER_MODE=exact FAKE_RUNNER_REQUIRE_NATIVE=1 run_remote_worker exact
python3 - "$database" <<'PY'
import sqlite3,sys
c=sqlite3.connect(sys.argv[1])
assert c.execute("select count(*) from work_completions").fetchone()[0] == 1
...
DISTRIBUTED_REPLAY_SAVE_GROUP_CAP=2 FAKE_RUNNER_MODE=exact run_worker lease-peer
[[ ! -e "$remote_audit/STOP.env" ]]
python3 - "$database" <<'PY'
import sqlite3,sys
c=sqlite3.connect(sys.argv[1])
