...
print('ROBIN_PARITY_RESULT ' + json.dumps(dict(
    result_version=1, trace_path=str(Path(sys.argv[1]).resolve()),
    native_trace_sha256='a'*64, executable_path=str(Path(sys.argv[2]).resolve()),
    executable_sha256='b'*64, expected_frames=2, processed_frames=2,
    expected_final_frame=2, final_frame=2, terminator_validated=True,
...
# Deep worktree/corpus paths must not become oversized lock basenames.
long_corpus="$test_root/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa/bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb/cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc"
long_audit="$test_root/long-path"
mkdir -p "$long_corpus" "$long_audit"
printf 'pass\n' >"$long_corpus/01-pass.jsonl.zst"
printf '%s\n' "$long_corpus/01-pass.jsonl.zst" >"$long_audit/traces.snapshot"
env FAKE_RUNNER_INVOCATIONS="$invocation_log" PARITY_SWEEP_SLOT_DIR="$slots" \
    scripts/run_parity_release_sweep.sh "$long_corpus" "$long_audit" "$runner" 0 1
[[ "$(<"$long_audit/status/01-pass.jsonl.zst.status")" == 0 ]]
lock_hash=$(printf '%s' "$long_corpus/01-pass.jsonl.zst" | sha256sum)
lock_hash=${lock_hash%% *}
[[ -f "$long_audit/.trace-locks/$lock_hash.lock" ]]
[[ "$(<"$long_audit/.trace-locks/$lock_hash.trace")" == "$long_corpus/01-pass.jsonl.zst" ]]
