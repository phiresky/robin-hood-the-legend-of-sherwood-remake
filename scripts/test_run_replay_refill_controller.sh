...
Path(__file__).with_name("enqueue-called").touch()
print('{}')
PY
...
rm -f "$test_root/workspace/scripts/enqueue-called"
python3 - "$test_root/state.sqlite3" <<'PY'
import sqlite3,sys
db=sqlite3.connect(sys.argv[1])
db.executescript("""
insert into replays values(1,1);
insert into work_items values(1,'replay',1,1);
""")
db.commit()
PY
set +e
...
[[ $status == 124 ]]
[[ ! -e "$test_root/workspace/scripts/enqueue-called" ]]
grep -q 'heartbeat target=1 desired=1 pending=1' \
    "$test_root/audit/refill-controller.log"
