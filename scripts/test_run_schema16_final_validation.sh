...
# Keep the fixture longer than integration worktree paths: flattened absolute
# lock names previously crossed NAME_MAX only after integration.
test_root=$(mktemp -d "$repository/.codex-tmp/schema16-final-validation-long-integration-worktree-regression-test.XXXXXX")
locked_pid=
cleanup() {
...
sparse_lock_key=$(printf '%s' "$(realpath -m -- "$sparse_pass")" | sha256sum)
sparse_lock_key=${sparse_lock_key%% *}
exec {sparse_lock_fd}>"$sparse_audit/.trace-locks/$sparse_lock_key.lock"
