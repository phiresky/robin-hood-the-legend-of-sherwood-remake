...
mkdir -p -- "$workspace/scripts" "$workspace/original-game/scripts" "$fake_bin"
...
# This regression verifies supervision and identity checks, not Original
# capture; a full proprietary checkout is not a fixture prerequisite.
...
python3 "$repository/scripts/parity_campaign.py" \
    "$repository/scripts/parity-campaigns/schema16-20260824.json" \
    --freeze-workspace "$workspace" >"$test_root/campaign.json"
export PARITY_CAMPAIGN_MANIFEST="$test_root/campaign.json"
