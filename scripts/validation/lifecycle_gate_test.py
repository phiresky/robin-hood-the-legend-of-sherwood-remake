...
class LifecycleGateTests(unittest.TestCase):
    ...
    def test_native_keeps_frozen_binary_when_build_tree_changes_between_phases(self):
        ...
        self.assertNotEqual(summary["source_initial"], summary["source_final"])
    ...
    def test_native_records_deleted_build_tree_without_reopening_it(self):
        ...
        with patch.dict(os.environ, self.env), patch.object(gate, "executable"), \
            ...
            gate.native(self.evidence, summary)
        self.assertEqual(summary["source_final"]["error_type"], "FileNotFoundError")
        self.assertEqual(len(summary["checks"]), 4)

    def test_native_rejects_changed_retained_game_after_scenario(self):
        def mutate(argv, **kwargs):
            self.fake_native_run(argv, **kwargs)
            binary = Path(argv[argv.index("--binary") + 1])
            binary.chmod(0o755)
            binary.write_bytes(b"changed retained executable")
        with patch.dict(os.environ, self.env), patch.object(gate, "executable"), \
                patch.object(gate, "run", side_effect=mutate) as run:
    ...
    def test_native_rejects_game_replaced_while_admitting_binary(self):
        original = gate.snapshot_client
        def replace(source, evidence, summary):
            self.binary.write_bytes(b"replacement during admission")
            return original(source, evidence, summary)
        summary = {}
        with patch.dict(os.environ, self.env), patch.object(gate, "executable"), \
                patch.object(gate, "snapshot_client", side_effect=replace), \
                patch.object(gate, "run") as run:
            with self.assertRaisesRegex(RuntimeError, "changed while admitting"):
                gate.native(self.evidence, summary)
        run.assert_not_called()
        self.assertIn("source_final", summary)
