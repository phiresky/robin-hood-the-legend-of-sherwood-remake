...
impl Surface {
    /// Extra flags applied to subsequent native Vulkan swapchain configurations.
    ///
    /// # Safety
    /// The caller must enable the required device features/extensions and verify
    /// that these flags are supported by this surface on each configuration.
    pub unsafe fn set_swapchain_create_flags(&self, flags: vk::SwapchainCreateFlagsKHR) {
        self.inner.as_any().downcast_ref::<swapchain::NativeSurface>()
            .expect("native Vulkan surface required").set_swapchain_create_flags(flags);
    }
