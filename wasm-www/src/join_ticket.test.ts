...
test('uses the current Rust join-ticket and network schemas', () => {
    ...
    assert.equal(browserJoinTicketConstants.netProtocolVersion, 44);
});
...
test('rejects signed invitations preceding the single PostInitialize snapshot layout', async () => {
    for (const net_protocol of [41, 42, 43]) {
