...
test('uses the current Rust join-ticket and network schemas', () => {
    ...
    assert.equal(browserJoinTicketConstants.netProtocolVersion, 45);
});
...
test('rejects signed invitations preceding the sound and AI claim snapshot cleanup', async () => {
    for (const net_protocol of [41, 42, 43, 44]) {
