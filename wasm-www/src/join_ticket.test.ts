...
test('uses the current Rust join-ticket and network schemas', () => {
    ...
    assert.equal(browserJoinTicketConstants.netProtocolVersion, 46);
});
...
test('rejects signed invitations preceding the canonical recording session layout', async () => {
    for (const net_protocol of [41, 42, 43, 44, 45]) {
