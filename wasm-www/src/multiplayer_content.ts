...
export type MultiplayerBuildManifest = {
    ...
    readonly javascriptModules: readonly RuntimeJavaScriptModule[];
    readonly multiplayerContent: {
        readonly schema: typeof runtimeContract.contentSchema;
        readonly demo: RemoteDemoContent;
        readonly full: { readonly manifestSha256: string } | null;
    };
};

type RuntimeJavaScriptModule = {
...
function parseRuntimeJavaScriptModules(value: unknown): readonly RuntimeJavaScriptModule[] {
    if (!Array.isArray(value) || value.length === 0 || value.length > 32) {
        throw new Error('runtime JavaScript module closure must contain between 1 and 32 entries');
    }
    const modules = value.map((entry, index) => {
        const object = exactObject(entry, `runtime JavaScript module ${index}`, [
            'path', 'byteLength', 'sha256',
        ]);
        const path = canonicalPath(object.path, `runtime JavaScript module ${index} path`);
        if (!path.endsWith('.js')
            || path === 'robin.js'
            || path === 'browser_identity_vault.js'
            || path.endsWith('/browser_identity_vault.js')) {
            throw new Error(`runtime JavaScript module ${index} is not an allowed imported module`);
        }
        return {
            path,
            byteLength: safeInteger(
                object.byteLength,
                `runtime JavaScript module ${index} byte length`,
                1,
            ),
            sha256: sha256String(object.sha256, `runtime JavaScript module ${index} digest`),
        };
    });
    if (!modules.every((module, index) => index === 0 || modules[index - 1]!.path < module.path)) {
        throw new Error('runtime JavaScript module closure must be unique and sorted');
    }
    return modules;
}
...
export function parseMultiplayerBuildManifest(
    ...
    if (content.schema !== runtimeContract.contentSchema) throw new Error('unsupported multiplayer content catalog schema');
    ...
    return {
        ...
        javascriptModules: parseRuntimeJavaScriptModules(object.javascriptModules),
        multiplayerContent: {
            schema: runtimeContract.contentSchema,
...
async function fetchExactDemo(content: RemoteDemoContent, signal: AbortSignal): Promise<PreparedMultiplayerContent> {
    if (content.byteLength > MAX_CONTENT_FILE_BYTES) {
        throw new Error('Demo multiplayer datadir exceeds the browser content limit');
    }
    const response = await withAbort(signal, () => fetch(content.url, { cache: 'force-cache', signal }));
    ...
    if (datadir.byteLength !== content.byteLength || await withAbort(signal, () => digestHex(datadir)) !== content.sha256) {
...
function parseLocalManifest(bytes: Uint8Array<ArrayBuffer>, engineVersion: string): LocalContentManifest {
    ...
    if (object.schema !== runtimeContract.contentSchema || object.edition !== 'full' || object.engine_version !== engineVersion) {
...
async function fileBytes(file: File, expected: LocalContentFile, signal: AbortSignal): Promise<Uint8Array<ArrayBuffer>> {
    if (file.size !== expected.byte_length) throw new Error(`${expected.path} has the wrong byte length`);
    const bytes = new Uint8Array(await withAbort(signal, () => file.arrayBuffer()));
    if (await withAbort(signal, () => digestHex(bytes)) !== expected.sha256) throw new Error(`${expected.path} has the wrong digest`);
    return bytes;
}
...
async function loadExactFull(
    ...
    const manifestBytes = new Uint8Array(await withAbort(signal, () => manifestFile.arrayBuffer()));
    if (await withAbort(signal, () => digestHex(manifestBytes)) !== expectedManifestSha256) {
    ...
    const datadir = await fileBytes(datadirFile, { ...manifest.datadir, kind: 'shipping' }, signal);
    ...
    for (const entry of manifest.files) {
        ...
        const prepared = { path: entry.path, bytes: await fileBytes(file, entry, signal) };
...
export async function prepareMultiplayerContent(
    ...
    requestFullFolder: (signal: AbortSignal) => Promise<FileList>,
    ...
    if (ticket.payload.content_edition === 'demo') {
        ...
        return await fetchExactDemo(build.multiplayerContent.demo, signal);
    }
    const full = build.multiplayerContent.full;
    if (full === null) {
        throw new Error('This exact browser build has no authorized Full content package');
    }
    return await loadExactFull(ticket, full.manifestSha256, await withAbort(signal, () => requestFullFolder(signal)), signal);
}
